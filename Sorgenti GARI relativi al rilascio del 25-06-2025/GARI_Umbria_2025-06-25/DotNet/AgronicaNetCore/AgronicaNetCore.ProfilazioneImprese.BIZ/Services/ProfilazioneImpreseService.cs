...
public class ProfilazioneImpreseService : BaseServiceProfilazioneImpreseBIZ, IProfilazioneImpreseService
{
        ...
        int vegCod, bool leggiSoloNoteConLavorazioneNullSeLavCodZero, AgronicaCoreParametri objParams)
    {
        ...
        try
        {
            dt = await _profilazioneImprese.LeggiProfilazioneImpreseAsync(piva, idProfiloDati, codiceChiave, idGruppo, "",
                lavCod,
                vegCod, leggiSoloNoteConLavorazioneNullSeLavCodZero, objParams);
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> SalvaProfilazioneAsync(SalvaProfilazione_In body, AgronicaCoreParametri objParams)
    {
        var val2Save = "";
        ...
        var imprese = await _profilazioneImprese.LeggiProfilazioneImpreseAsync(body.Piva, 0, body.Lavorazione.ToString(),
            "macXlav", "", body.Lavorazione, body.Specie, true, objParams);
    ...
    public async Task<bool> SalvaNoteAsync(SalvaNote_In body, AgronicaCoreParametri objParams)
    {
        ...
        AgronicaCoreParametri objParams)
    {
        try
        {
            ...
            var imprese = await _profilazioneImprese.LeggiProfilazioneImpreseAsync(piva, 0, codiceChiave, idGruppo,
                    "", lavCod, vegCod, true, objParams);
            if (imprese.Rows.Count == 0)
            {
                var idProfiloDati = await _sequence.NuovoId_TabellaAsync("Profilazione_Dati", 0, 2000000000, objParams);
                await _profilazioneImprese.ScriviAsync(piva, idProfiloDati, codiceChiave, quesito, idGruppo,
                    valoreSalvato, validitaInizio, validitaFine, lavCod, vegCod, objParams);
            }
            else
            {
                ...
                await _profilazioneImprese.ModificaAsync(quesito, valoreSalvato, piva, idProfiloDati, codiceChiave,
                    idGruppo, validitaInizio, validitaFine, lavCod, vegCod, objParams);
            }
        ...
        AgronicaCoreParametri objParams)
    {
        ...
        var dataset = await ReadMacchineAndContattiAsync(piva, vegCodInput, objParams);
        ...
        foreach (DataTable table in dataset.Tables)
        {
            foreach (DataRow row in table.Rows)
            {
                ...
                if (existingRows.Length == 0)
                {
                    ...
                    newRow["lav_des"] =
                        await _operazione.LavorazioneDesFromLavorazioneCodAsync((int)row["lav_cod"], objParams); // Operazioni_R
                    newRow["veg_cod"] = row["veg_cod"];

                    if (int.TryParse(row["Veg_cod"].ToString(), out var vegCodParsed) && vegCodParsed != 0)
                    {
                        newRow["Veg_des"] = await _specieVegetali.VegDesFromVegCodAsync(vegCodParsed, objParams);
                    }
                    else
                    {
        ...
        AgronicaCoreParametri objParams)
    {
        try
        {
            ...
            // 'cancello
            await _profilazioneImprese.CancellaTutteLavorazioniAsync(piva, "macXlav", vegCod, objParams);

            // 'ciclo per i vari lav cod
            var dtLav = await _operazione.LeggiAsync(0, "C", "GruppoOperazioni.GRU_COD,Operazioni.LAV_DES", objParams);
    ...
    public async Task<bool> AggiornaOreMinutiAsync(AggiornaOreMinuti_In body, AgronicaCoreParametri objParams)
    {
        bool result;
        try
        {
            var dt = await _profilazioneImprese.LeggiProfilazioneImpreseAsync(body.Piva, 0, "", "macXlav", "",
                body.LavCod, body.VegCod, false, objParams);
            ...
            result = await ScriviInserisceOAggiornaAsync(body.Piva, body.LavCod.ToString(), "macXlav",
                ...
                DateTime.Parse(CostantiPersonalizzate.AGRODATAINIZIO),
                DateTime.Parse(CostantiPersonalizzate.AGRODATAFINE), body.LavCod, body.VegCod, objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
    ...
    private async Task<DataSet> ReadMacchineAndContattiAsync(string piva, int vegCod, AgronicaCoreParametri objParams)
    {
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            for (var c = 1; c < dati.Length; c++)
            {
                ...
                {
                    await ProcessaMacchineAsync(codici, dtMacchine, idProfiloDati, lavCod, oremin, dr, piva, objParams);
                }
                else if (dati[c].StartsWith("cod_cont"))
                {
                    await ProcessaContattiAsync(codici, dtContatti, idProfiloDati, oremin, dr, lavCod, objParams);
                }
            }
        }
    ...
    private async Task ProcessaContattiAsync(string[] codici, DataTable dtContatti, object idProfiloDati, string[] oremin,
        DataRow dr, int lavCod, AgronicaCoreParametri objParams)
    {
        foreach (var codice in codici)
        {
            ...
            var dtDes = await _contatti.LeggiFromCodRisUmAsync(drC["cod_risum"].ToString()!, objParams);
    ...
    private async Task ProcessaMacchineAsync(string[] codici, DataTable dtMacchine, object idProfiloDati, int lavCod,
        string[] oremin, DataRow dr, string piva, AgronicaCoreParametri objParams)
    {

        var dtCentriVisibili = await _utentiVisibilitaAppoggio.ReadAsync((int)TipiEnumerativi.Enum_TipoEntita.Centro, objParams, piva: piva);
        ...
        foreach (var codice in codici)
        {
            ...
            drM["mac_des"] = await _parcoMacchine.LeggiDesFromMacCodAsync(piva, (int)drM["mac_cod"], dtCentriVisibili, objParams);
