...
namespace AgronicaNetCore.UtilityDB.BIZ.Services.Agro_Sequenze
{
    public class Agro_SequenceService : BaseServiceUtilityDBBiz, IAgro_SequenceService
    {
        ...
        /// <returns></returns>
        public async Task<(int, int)> Calcola_BaseCodeAsync(AgronicaCoreParametri objParametri_Server, AgronicaCoreParametri objParametri_Utenti)
        {
            (int, int) ret = (0,0);
            try
            {
                var serialGias = await _progGiasDal.LeggiProgressivoGIASAsync(objParametri_Utenti);
                if (serialGias <= 0) throw new Exception("No Gias serial.");
                ret = _sequenceDal.Calcola_BaseCode(serialGias, objParametri_Server);
            }
            catch (Exception)
            {
                ret = (0, 0);
