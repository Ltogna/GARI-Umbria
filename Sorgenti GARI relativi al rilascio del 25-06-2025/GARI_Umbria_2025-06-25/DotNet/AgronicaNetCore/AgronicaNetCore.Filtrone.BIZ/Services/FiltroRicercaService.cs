...
namespace AgronicaNetCore.FiltroRicerca.BIZ.Services
{
    public class FiltroRicercaService : BaseServiceFiltroRicercaBIZ, IFiltroRicercaService
    {
        ...
        public async Task<CriteriRicerca_OUT> GetResultAsync(CriteriRicerca_IN criteriRicerca_IN,
                                                             ...
                                                             AgronicaCoreParametri objP_Super_Server)
        {
            ...
            try
            {
                ...
                criteriRicerca.CheckVisibilitaAziende = !criteriRicerca_IN.OnLoad && (await _utentiVisibilitaAppoggio.ReadAsync((int)Enum_TipoEntita.Impresa, objP_Server))!.Rows.Count > 0;
                criteriRicerca.CheckVisibilitaCentriAziendali = !criteriRicerca_IN.OnLoad && (await _utentiVisibilitaAppoggio.ReadAsync((int)Enum_TipoEntita.Centro, objP_Server))!.Rows.Count > 0; ;

                criteriRicerca.SQLCompatibility = !criteriRicerca_IN.OnLoad && await _utilityDB.Read_SQL_Compatibility_LevelAsync(objP_Server) >= 130;
                criteriRicerca.USE_FORCE_LEGACY_CARDINALITY_ESTIMATION = !criteriRicerca_IN.OnLoad && await _utilityDB.Read_SQL_Version_YearAsync(objP_Server) >= 2014;
                criteriRicerca.FattoreConversione = criteriRicerca_IN.OnLoad ? 0 : await GetFattoreConversioneAsync(objP_Server, objP_Utenti);
                criteriRicerca.selectTopRows = criteriRicerca_IN.OnLoad ? 0 : await GetNumeroMassimoRigheEstraibiliFiltroRicerca(objP_Server, objP_Super_Server);

                if (!criteriRicerca_IN.OnLoad)
                {
                    await VerificaApplicaVisibilitaSpecieAsync(criteriRicerca_IN.TipoMostra, criteriRicerca, objP_Utenti);
                    await VerificaApplicaVisibilitaOperazioniAsync(criteriRicerca_IN.TipoMostra, criteriRicerca, objP_Server, objP_Utenti);
                }

                string utenti_DB_name = _utilityDB.GetDBName(objP_Utenti);
                criteriRicerca_OUT = await _filtroRicerca.GetResultAsync(criteriRicerca, configuratore, utenti_DB_name, objP_Server, objP_Utenti);
        ...
        private async Task VerificaApplicaVisibilitaSpecieAsync(int TipoMostra, CriteriRicercaExtended criteriRicerca, AgronicaCoreParametri objP_Utenti)
        {
            ...
            {
                DataTable utentiImpostazioniMonoDt_GruppiVegetali = await _utentiImpostazioniFiltroMono.LeggiAsync(TipiEnumerativi.Enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_GRUPPI_VEGETALI, 0, objP_Utenti);
                bool CheckVisibilitaGruppiVegetali = utentiImpostazioniMonoDt_GruppiVegetali.Rows.Count > 0;

                if (CheckVisibilitaGruppiVegetali)
                {
                    DataTable VisibilitaGruppiVegetaliDT = await _specieVegetali.GruppoVegetale_GestioneFiltroUtente_LeggiAsync(0, objP_Utenti);
                    ...
                    DataTable utentiImpostazioniMonoDt_SpecieVegetali = await _utentiImpostazioniFiltroMono.LeggiAsync(TipiEnumerativi.Enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_SPECIE_VEGETALI, 0, objP_Utenti);
                    bool CheckVisibilitaSpecieVegetali = utentiImpostazioniMonoDt_SpecieVegetali.Rows.Count > 0;

                    if (CheckVisibilitaSpecieVegetali)
                    {
                        DataTable VisibilitaSpecieDT = (await _specieVegetali.SpecieVegetali_GestioneFiltroUtente_LeggiAsync(new LeggiSpecieVegetali_IN(), utentiImpostazioniMonoDt_GruppiVegetali, objP_Utenti)).DataTable;
                        ...
                        if (VisibilitaSpecieDT.Rows.Count > 0)
                        {
                            ...
                            DataTable utentiImpostazioniMonoDt_Varieta = await _utentiImpostazioniFiltroMono.LeggiAsync(TipiEnumerativi.Enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_VARIETA, 0, objP_Utenti);
                            bool CheckVisibilitaVarieta = utentiImpostazioniMonoDt_Varieta.Rows.Count > 0;

                            if (CheckVisibilitaVarieta)
                            {
                                DataTable VisibilitaVarietaDT = await _specieVegetali.Cultivar_GestioneFiltroUtente_LeggiAsync(new Cultivar_GestioneFiltroUtente_Leggi_IN(), utentiImpostazioniMonoDt_GruppiVegetali, objP_Utenti);
        ...
        private async Task VerificaApplicaVisibilitaOperazioniAsync(int TipoMostra, CriteriRicercaExtended criteriRicerca, AgronicaCoreParametri objP_Server, AgronicaCoreParametri objP_Utenti)
        {
            ...
            DataTable utentiImpostazioniDt_GruppoOperazioni = await _utentiImpostazioni.Read_JoinWithFiltroMonoAsync(TipiEnumerativi.Enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_GRUPPI_OPERAZIONI, 1, objP_Utenti);
            ...
            if (CheckVisibilitaGruppoOperazioni)
            {
                ...
                if (gruppiGestitiDaImpostazioniUtente.Count > 0)
                {
                    ...
                    DataTable utentiImpostazioniDt_Operazioni = await _utentiImpostazioni.Read_JoinWithFiltroMonoAsync(TipiEnumerativi.Enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_LAVORAZIONI, 1, objP_Utenti);
                    ...
                    {
                        DataTable VisibilitaOperazioniDT = (await _operazioni.Operazioni_GestioneFiltroUtente_LeggiAsync(new LeggiOperazioni_IN { GruppoOperazioni = GruppoOperazioniAmmissibili }, utentiImpostazioniDt_GruppoOperazioni, objP_Server, objP_Utenti)).DataTable;
        ...
        private async Task<decimal> GetFattoreConversioneAsync(AgronicaCoreParametri objP_Server, AgronicaCoreParametri objP_Utenti)
        {
            int udm_cod = 0;
            decimal fattoreConversione = 1;

            DataTable DtUtentiImp = (await _utentiImpostazioni.ReadAsync((int)Enum_Impostazioni_Utenti.Visualizza_UdmAggiuntiva_xSup, 2, objP_Utenti))!;
            ...
            if (udm_cod != 0)
            {
                DataTable dtfattoreConversione = await _unitaMisuraConversione.ReadAsync(new UnitaMisuraConversione_IN { Udm_Cod_Da = (int)Enum_UnitaMisura.Ettaro, Udm_Cod_A = udm_cod }, objP_Server);
        ...
        private async Task<int> GetNumeroMassimoRigheEstraibiliFiltroRicerca(AgronicaCoreParametri objP_Server, AgronicaCoreParametri objP_Super_Server)
        {
            string valore = (await _securityLayer.LeggiConfigurazioneSitiScalareAsync("NumeroMassimoRigheEstraibiliFiltroRicerca", objP_Server, objP_Super_Server))!;
        ...
        private static void PulisciKendoColumns(ref CriteriRicerca_OUT criteriRicerca_OUT)
        {
            ...
            {
                criteriRicerca_OUT.kendoColumns.Remove(kCol);
            }

            //Ordino le colonne default
            criteriRicerca_OUT.kendoColumns = criteriRicerca_OUT.kendoColumns.OrderBy(k => k.ColumnOrder).ToList();
        }
            ...
            foreach (DataColumn col in criteriRicerca_OUT.result.Columns)
            {
                switch (col.ColumnName.ToLowerInvariant())
                {
                    ...
                    case "sa_cod":
                        ...
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Sa_Cod", Title = "Sa_Cod", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "campo_cod":
                        ...
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Campo_Cod", Title = "Campo_Cod", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "appezza":
                        ...
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Appezza", Title = "Appezza", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "id_reg":
                        ...
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Id_Reg", Title = "Id_Reg", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "progetto_cod":
                        if (configuratore.isMostraEsercizi || configuratore.isMostraImpianti || configuratore.isMostraMovimenti)
                        {
                            ...
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Progetto_Cod", Title = "Progetto_Cod", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "fabbricato_cod":
                        ...
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Fabbricato_Cod", Title = "Fabbricato_Cod", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "veg_cod":
                        if (configuratore.isMostraImpianti || configuratore.isMostraEsercizi || configuratore.isMostraMovimenti)
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Veg_Cod", Title = "Veg_Cod", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "id_agenda":
                        ...
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Id_Agenda", Title = "Id_Agenda", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                    case "id_particella":
                    ...
                    case "id_budget":
                        ...
                        {
                            pk.Add(col);
                            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "Id_Budget", Title = "Id_Budgetv", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                        }
                        break;
                }
            }
        ...
        private static void AggiungiColonnaChiave(int TipoMostra, ref CriteriRicerca_OUT criteriRicerca_OUT)
        {
            ...
            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "chiave", Title = "chiave", TranslateTitle = false, DataType = "string", ColumnOrder = -1, Display = false, Hidden = true });
            ...
            criteriRicerca_OUT.kendoColumns.Add(new KendoColumn { Field = "chiave_GIAS", Title = "Chiave GIAS", TranslateTitle = false, DataType = "string", ColumnOrder = -1, Display = true, Hidden = true });
