...
namespace AgronicaNetCore.Anagrafe.DAL.DataLayer.Imprese_Impostazioni
{
    public class Imprese_Impostazioni : DAL_Base, IImprese_Impostazioni
    {
            ...
            AgronicaCoreParametri objParametri_Server)
        {
            ...
            valImpostazione = await LeggiScalareCentroAziendaSuperUser_ElemCod(piva, sa_cod_lst[0], impostazioneCod, tipoProdotto, valoreDefault, objParametri_Utenti, objParametri_Server);

            foreach (var sa_cod in sa_cod_lst.Skip(1))
            {
                var valImpostazioneCorrente = await LeggiScalareCentroAziendaSuperUser_ElemCod(piva, sa_cod, impostazioneCod, tipoProdotto, valoreDefault, objParametri_Utenti, objParametri_Server);
        ...
        private async Task<string> LeggiScalareCentroAziendaSuperUser_ElemCod(string piva, int sa_cod, Enum_Impostazioni_Utenti impostazioneCod, int tipoProdotto, string valoreDefault, AgronicaCoreParametri objParametri_Utenti, AgronicaCoreParametri objParametri_Server)
        {
            string valImpostazione = await LeggiScalareCentroAziendaSuperUser(piva, sa_cod, impostazioneCod, valoreDefault, objParametri_Utenti, objParametri_Server);
        ...
        private async Task<string> LeggiScalareCentroAziendaSuperUser(string piva, int sa_cod, Enum_Impostazioni_Utenti impostazioneCod, string valoreDefault, AgronicaCoreParametri objParametri_Utenti, AgronicaCoreParametri objParametri_Server)
        {

            // TODO_DT: da capire come modificare quando arriverà la tabella nuova delle impostazioni con default e "solo azienda"
            string valImpostazione = valoreDefault;

            var strImpostazioni = await LeggiScalare_Centro_Azienda_SuperUser(piva, sa_cod, impostazioneCod, objParametri_Utenti, objParametri_Server);
        ...
        private async Task<string> LeggiScalare_Centro_Azienda_SuperUser(string piva, int sa_cod, Enum_Impostazioni_Utenti impostazione_cod, AgronicaCoreParametri objParametri_Utenti, AgronicaCoreParametri objParametri_Server)
        {
            ...
            try
            {
                strImpostazione_Valore = await LeggiScalare_Centro_Azienda(piva, sa_cod, (int) impostazione_cod, objParametri_Server);

                if (string.IsNullOrEmpty(strImpostazione_Valore))
                {
                    var tuttiICentri = 0;
                    strImpostazione_Valore = await LeggiScalare_Centro_Azienda(piva, tuttiICentri, (int) impostazione_cod, objParametri_Server);
                }

                if (string.IsNullOrEmpty(strImpostazione_Valore))
                {
                    strImpostazione_Valore = await _utentiImpostazioni.ImpostazioneValore1_from_ImpostazioneCod(impostazione_cod, 2, objParametri_Utenti);
                }
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objParametri_Server, ex);
        ...
        private async Task<string> LeggiScalare_Centro_Azienda(string piva, int sa_cod, int impostazione_cod, AgronicaCoreParametri objParametri_Server)
        {
            ...
            try
            {
                ...
                parSql.Add("@Piva_SuperUser", objParametri_Server.PivaSuperUser);
                ...
                // --------------------------------------------------------------------------
                dt = await GetDataProvider(objParametri_Server).ExecuteReadAsync(stb.ToString(), parSql);
            }
            // --------------------------------------------------------------------------
