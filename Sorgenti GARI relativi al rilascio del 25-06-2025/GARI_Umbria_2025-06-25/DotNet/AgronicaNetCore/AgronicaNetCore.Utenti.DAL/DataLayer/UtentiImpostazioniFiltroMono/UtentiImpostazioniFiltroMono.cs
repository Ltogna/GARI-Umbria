...
namespace AgronicaNetCore.Utenti.DAL.DataLayer.UtentiImpostazioniFiltroMono
{
    public class UtentiImpostazioniFiltroMono : BaseDALUtenti, IUtentiImpostazioniFiltroMono
    {
        ...
        public async Task<DataTable> LeggiAsync(TipiEnumerativi.Enum_Impostazioni_Utenti Impostazione_Cod, int ID_0, AgronicaCoreParametri objParametriUtente)
        {
            ...
            if (!string.IsNullOrEmpty(objParametriUtente.PivaSuperUser))
            {
            ...
            if (!string.IsNullOrEmpty(objParametriUtente.UtenteUsername))
            {
            ...
            try
            {
                result = await GetDataProvider(objParametriUtente).ExecuteReadAsync(stbQuery.ToString(), parametriSql);
            }
            catch (Exception ex)
            {
