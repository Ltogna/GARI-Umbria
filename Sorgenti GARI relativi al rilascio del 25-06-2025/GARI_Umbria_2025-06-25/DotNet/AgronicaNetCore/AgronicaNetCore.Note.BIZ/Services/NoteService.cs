...
public class NoteService : BaseServiceNoteBIZ, INoteService
{
    ...
    public async Task<DataTable> LeggiNoteInterventoUtilizzoAsync(int notaUtilizzoCod, AgronicaCoreParametri objParams)
    {
        DataTable dt;
        try
        {
            dt = await _noteDal.LeggiNoteInterventoUtilizzoAsync(notaUtilizzoCod, objParams);
        }
        catch (Exception ex)
        {
        ...
        string filtroAggiuntivo, AgronicaCoreParametri objParams)
    {
        ...
        try
        {
            dt = await _noteDal.LeggiProfilazioneNoteAsync(notaUtilizzoCod, notaGruppoCod, soloNoteConUtilizzo,
                Visibilita.Visibili, filtroAggiuntivo, objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
        ...
        AgronicaCoreParametri objParams)
    {
        ...
        var dtNote = await LeggiProfilazioneNoteAsync(piva, "", inputLavCod, inputVegCod, false, false, objParams);
        foreach (DataRow drDati in dtNote.Rows)
        {
            ...
            if (drRes.Length == 0)
            {
                ...
                drModIns["notautilizzo_des"] =
                    await _noteDal.LeggiNotaUtilizzoDesFromNotaUtilizzoCodAsync((int)drDati["notautilizzo_cod"], objParams);

                drModIns["Lav_cod"] = drDati["Lav_cod"];
                if (int.TryParse(drDati["Lav_cod"].ToString(), out var dbLavCod) && dbLavCod != 0)
                {
                    drModIns["Lav_des"] =
                        await _operazioneDal.LavorazioneDesFromLavorazioneCodAsync((int)drDati["Lav_cod"], objParams);
                }
                else
                {
                ...
                if (int.TryParse(drDati["Veg_cod"].ToString(), out var dbVegCod) && dbVegCod != 0)
                {
                    ...
                    drModIns["Veg_des"] = await _specieVegetaliDal.VegDesFromVegCodAsync(dbVegCod, objParams);
                }
                else
                {
    ...
    public async Task<DataTable> LeggiGruppoNoteAsync(AgronicaCoreParametri objParams)
    {
        DataTable dt;
        try
        {
            dt = await _noteDal.LeggiGruppoNoteAsync(0, "", "NotaGruppo_Cod ASC", objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
        ...
        AgronicaCoreParametri objParams)
    {
        ...
        var dt = await _profilazioneImpreseDal.LeggiProfilazioneImpreseAsync(piva, 0, codiceChiave, "note", filtro, lavCod, 0,
            leggiSoloNoteConLavorazioneNullSeLavCodZero, objParams);
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            foreach (var dato in dati)
            {
                ...
                foreach (var codice in codici)
                {
                    ...
                    drNota["nota_des"] = await _noteDal.LeggiNotaDesFromNotaCodAsync(intCod, objParams);
    ...
    public async Task<bool> SalvaGruppoNoteAsync(SalvaGruppoNote_In body, AgronicaCoreParametri objParams)
    {
        bool result;
        try
        {
            ...
            if (id == 0)
            {
                id = await _noteDal.NoteInterventoGruppiNewIdAsync(objParams);
            }
    ...
    public async Task<bool> AggiornaGruppoNoteAsync(SalvaGruppoNote_In body, AgronicaCoreParametri objParams)
    {
        ...
        try
        {
            result = await _noteDal.AggiornaGruppoNoteAsync(body.NotaGruppoCod, body.NotaGruppoDes,
                DateTime.Parse(CostantiPersonalizzate.AGRODATAINIZIO),
                DateTime.Parse(CostantiPersonalizzate.AGRODATAFINE), true, objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
    ...
    public async Task<bool> CancellaGruppoNoteAsync(int gruppoNoteCod, AgronicaCoreParametri objParams)
    {
        if (gruppoNoteCod <= 0)
        {
            return false;
        ...
        try
        {
            result = await _noteDal.CancellaGruppoNoteAsync(gruppoNoteCod, objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
        ...
        AgronicaCoreParametri objParams)
    {
        ...
        try
        {
            dt = await _noteDal.LeggiProfilazioneNoteAsync(notaUtilizzoCod, notaGruppoCod, false, Visibilita.Tutte, "", objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
    ...
    public async Task<bool> AggiungiNotaAsync(string notaDes, int notaGruppoCod, AgronicaCoreParametri objParams)
    {
        bool result;
        try
        {
            var notaCod = await _noteDal.NoteNewIdAsync(objParams);
        ...
        AgronicaCoreParametri objParams)
    {
        ...
        try
        {
            result = await _noteDal.LeggiNoteInterventoUtilizzoGruppiAsync(notaGruppoCod, notaUtilizzoCod, objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
    ...
    public async Task<bool> SalvaGruppoNoteCompletoAsync(SalvaGruppoNoteCompleto_In body, AgronicaCoreParametri objParams)
    {
        try
        {
            await OpenConnectionAsync(objParams);
            if (!await _noteDal.CancellaGruppoNoteUtilizzoAsync(body.NotaGruppoCod, 0, objParams))
            {
            ...
            if (!await _noteDal.ModificaVisibilitaGruppoAsync(body.NotaGruppoCod, body.Visible, objParams))
            {
            ...
            {
                if (!await _noteDal.ModificaVisibilitaNotaAsync(nota.NotaCod, nota.Visible, objParams))
                {
    ...
    public async Task<bool> CancellaNotaAsync(int notaCod, AgronicaCoreParametri objParams)
    {
        if (notaCod <= 0)
        {
            return false;
        ...
        try
        {
            result = await _noteDal.CancellaNotaAsync(notaCod, objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
