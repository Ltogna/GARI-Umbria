...
public class ProfilazioneImprese : BaseDALProfilazioneImprese, IProfilazioneImprese
{
        ...
        AgronicaCoreParametri objParams)
    {
        ...
        if (objParams.FlagCancellazioneLogica == AgronicaCoreParametri.enumCancellazioneLogica.CancellazioneLogica)
        {
            ...
            expandoObj.TryAdd("@usernameOperazione", objParams.UsernameOperazione);
            expandoObj.TryAdd("@piva", piva);
            expandoObj.TryAdd("@idGruppo", idGruppo);
        }
        else
    ...
    public async Task<bool> CancellaTutteLavorazioniAsync(string piva, string idGruppo, int vegCod, AgronicaCoreParametri objParams)
    {
        var builder = new StringBuilder();
        var expandoObj = new ExpandoObject();
        if (objParams.FlagCancellazioneLogica == AgronicaCoreParametri.enumCancellazioneLogica.CancellazioneLogica)
    ...
    )
    {
        ...
        try
        {
            return await GetDataProvider(objParams).ExecuteReadAsync(builder.ToString(), sqlParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
