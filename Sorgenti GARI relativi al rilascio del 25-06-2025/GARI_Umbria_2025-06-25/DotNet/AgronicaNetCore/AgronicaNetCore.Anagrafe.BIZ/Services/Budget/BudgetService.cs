...
namespace AgronicaNetCore.Anagrafe.BIZ.Services.Budget
{
    public class BudgetService : BaseServiceAnagrafeBIZ, IBudgetService
    {
        ...
        public async Task<DataTable> LeggiTestateBudgetAsync(LeggiBudgetTestate leggiBudgetTestate , AgronicaCoreParametri objParametri)
        {
            DataTable dt;
            try
            {
                dt = await _budgetDAL.LeggiTestateBudgetAsync(leggiBudgetTestate.Piva, leggiBudgetTestate.Id_Budget, leggiBudgetTestate.IncludiPubblici,  objParametri);
            }
            catch (Exception ex)
            {
