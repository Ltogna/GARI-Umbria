...
namespace AgronicaNetCore.Anagrafe.BIZ.Services.Imprese
{
    public class ImpresaService : BaseServiceAnagrafeBIZ, IImpresaService
    {
        ...
        public async Task<DataTable> TestClausolaINAsync(List<string> elencoPiva, List<int> elencoVegCod, int varieta, AgronicaCoreParametri objParametri)
        {
            try
            {
                ...
                var dt = await impresaDal.LeggiClausolaInAsync(elencoPiva, elencoVegCod,varieta, objParametri);
                using (LogContext.PushProperty("LogPath", Path.Combine(objParametri.LogDirectory, Path.GetFileNameWithoutExtension(objParametri.LogFileName))))
                {
        ...
        public async Task<DataTable> Leggi2Async(AgronicaCoreParametri objParametri, string partiIva)
        {
            try
            {
                ...
                var dt = await impresaDal.LeggiAsync(partiIva, objParametri);
                using (LogContext.PushProperty("LogPath", Path.Combine(objParametri.LogDirectory, Path.GetFileNameWithoutExtension(objParametri.LogFileName))))
                {
                    _logger.LogInformation("questo è un log di prova");
                }
        ...
        public async Task<AgronicaCoreModelsSTD.anagrafiche.Impresa?> LeggiImpresaAsync(AgronicaCoreParametri objParametri, string partitaIva)
        {
            try
            {
                ...
                var dt = await impresaDal.LeggiAsync(partitaIva, objParametri);
        ...
        public async Task<DataTable> LeggiImpreseAsync(string piva, AgronicaCoreParametri objParametriServer)
        {
            DataTable result;
            try
            {
        ...
        public async Task<DataTable> LeggiPadriAsync(AgronicaCoreParametri objParametri)
        {
            DataTable dt;

            try
            {
                var dtImpreseVisibili = await _utentiVisibilitaAppoggio.ReadAsync((int)Enum_TipoEntita.Impresa, objParametri);
                var impresaDal = _serviceProvider.GetRequiredService<IImpresa>();
                dt = await impresaDal.LeggiPadriAsync(dtImpreseVisibili, objParametri);
            }
            catch (Exception ex)
            {
                using (LogContext.PushProperty("LogPath", Path.Combine(objParametri.LogDirectory, Path.GetFileNameWithoutExtension(objParametri.LogFileName))))
