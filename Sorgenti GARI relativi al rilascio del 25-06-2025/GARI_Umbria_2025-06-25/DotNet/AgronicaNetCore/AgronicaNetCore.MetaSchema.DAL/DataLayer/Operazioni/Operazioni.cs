...
namespace AgronicaNetCore.MetaSchema.DAL.DataLayer.Operazioni
{
    public class Operazioni : BaseDALMetaschema, IOperazioni
    {
        ...
        public async Task<DtConVisibilita_OUT> Operazioni_GestioneFiltroUtente_LeggiAsync(LeggiOperazioni_IN leggiOperazioni, DataTable utentiImpostazioniDt, AgronicaCoreParametri objParametriServer, AgronicaCoreParametri objParametriUtente)
        {
            ...
            {
                result.DataTable = await LeggiOperazioniApplicandoLimitiDiVisibilitaAsync(utentiImpostazioniDt, leggiOperazioni, objParametriUtente);
            }
            else
            {
        ...
        public async Task<DataTable> LeggiOperazioniPerTipoAsync(string tipoGruppoOperazione, AgronicaCoreParametri objParametri)
        {
            ...
            sqlParams.Add("@linguaCod", objParametri.Lingua_Cod);
            sqlParams.Add("@dtFine", objParametri.FinestraTemporaleFine);
            sqlParams.Add("@dtInizio", objParametri.FinestraTemporaleInizio);
        ...
        private async Task<DataTable> LeggiOperazioniApplicandoLimitiDiVisibilitaAsync(DataTable utentiImpostazioniLivelloGruCodDt,
            LeggiOperazioni_IN leggiOperazioni, AgronicaCoreParametri objParametriUtente)
        {
            DataTable result = new DataTable();

            var applicaFiltroSuPivaSuperUser = !string.IsNullOrEmpty(objParametriUtente.PivaSuperUser);
            var applicaFiltroSuUtenteUsername = !string.IsNullOrEmpty(objParametriUtente.UtenteUsername);
            ...
            foreach (DataRow row in utentiImpostazioniLivelloGruCodDt.Rows)
            {
                ...
                parametriSql.Add("@dtInizio", objParametriUtente.FinestraTemporaleInizio);
                parametriSql.Add("@dtFine", objParametriUtente.FinestraTemporaleFine);


                if (applicaFiltroSuPivaSuperUser)
                    parametriSql.Add("@pivaSuperUser", objParametriUtente.PivaSuperUser);

                if (applicaFiltroSuUtenteUsername)
                    parametriSql.Add("@username", objParametriUtente.UtenteUsername);
                ...
                try
                {
                    intermediateResult = await GetDataProvider(objParametriUtente).ExecuteReadAsync(stbQuery.ToString(), parametriSql, parametriSqlIn);
                }
                catch (Exception ex)
                {
                    LogError(ex.Message, objParametriUtente, ex);
                    throw;
                }
        ...
        private async Task<DataTable> LeggiAsync(LeggiOperazioni_IN leggiOperazioni, AgronicaCoreParametri objParametri)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable result;
