...
namespace AgronicaNetCore.MetaSchema.DAL.DataLayer.SpecieVegetali
{
    public class SpecieVegetali : BaseDALMetaschema, ISpecieVegetali
    {
        ...
        public async Task<DtConVisibilita_OUT> SpecieVegetali_GestioneFiltroUtente_LeggiAsync(LeggiSpecieVegetali_IN leggiSpecie_IN, DataTable utentiImpostazioniMonoDt, AgronicaCoreParametri objParametri)
        {
            ...
            if (tipoQuery == TipoQuerySpecieVegetali.EseguiQueryConGruCod)
            {
                ...
                parametriSql.Add("@utenteUserName", objParametri.UtenteUsername);
            ...
            if (queryCreata)
            {
                try
                {
                    result.DataTable = await GetDataProvider(objParametri).ExecuteReadAsync(stbQuery.ToString(), parametriSql);
                }
                catch (Exception ex)
                {
        ...
        /// <returns></returns>
        public async Task<DataTable> Cultivar_GestioneFiltroUtente_LeggiAsync(Cultivar_GestioneFiltroUtente_Leggi_IN leggiCultivar_IN, DataTable utentiImpostazioniMonoDt, AgronicaCoreParametri objParametri)
        {
            var gruCodList = new List<int>();
            TipoQueryCultivar tipoQuery = await DeterminaIlTipoDiQueryPerLaLetturaDelleVarietaAsync(leggiCultivar_IN, gruCodList, utentiImpostazioniMonoDt, objParametri);

            DataTable result;

            if (tipoQuery == TipoQueryCultivar.UtenteConVisibilitaCompleta)
            {
                result = await LeggiVarieta_ConVisibilitaCompletaAsync(leggiCultivar_IN, objParametri);
            }
            else if (tipoQuery == TipoQueryCultivar.QueryUnionAll)
            {
                result = await EseguiQueryUnionAllPerLetturaVarietaAsync(leggiCultivar_IN, gruCodList, objParametri);
            }
            else if (tipoQuery == TipoQueryCultivar.LeggiVarietaDaListaDiVegCod_SenzaControlloVisibilitaSpecie)
            {
                result = await LeggiVarietaDaListaDiVegCodAsync(leggiCultivar_IN.Veg_Cod_List, objParametri, leggiCultivar_IN.Cul_Cod, leggiCultivar_IN.Cerca_CulDes);
            }
            else
        ...
        public async Task<DataTable> GruppoVegetale_GestioneFiltroUtente_LeggiAsync(int gru_cod, AgronicaCoreParametri objParametri)
        {
            ...
            parametriSql.Add("@pivaSuperUser", objParametri.PivaSuperUser);
            parametriSql.Add("@usernameOperazione", objParametri.UtenteUsername);
        ...
        public async Task<DataTable> LeggiGruppiVarietaliAsync(LeggiGruppiVarietali_IN leggiGruppiVarietali_IN, AgronicaCoreParametri objParametri)
        {
            ...
            try
            {
                var provider = GetDataProvider(objParametri);
        ...
        public async Task<string> VegDesFromVegCodAsync(int vegCod, AgronicaCoreParametri objParams)
        {
            var stbQuery = new StringBuilder();
            ...
            try
            {
                var datatable = await GetDataProvider(objParams).ExecuteReadAsync(stbQuery.ToString(), sqlParams);
        ...
        public async Task<DataTable> LeggiVarietaFilteredAsync(string piva, int culCod, int vegCod, DataTable dtCentriVisibili, AgronicaCoreParametri objParams)
        {
            var stbQuery = new StringBuilder();
            var sqlParams = new Dictionary<string, object>();
        ...
        public async Task<DataTable> LeggiCompletaAsync(int vegCod, int gruCod, AgronicaCoreParametri objParams)
        {
            var stbQuery = new StringBuilder();
            var sqlParams = new Dictionary<string, object>();
        ...
        /// <param name="objParametri"></param>
        /// <returns></returns>
        private async Task<TipoQueryCultivar> DeterminaIlTipoDiQueryPerLaLetturaDelleVarietaAsync(Cultivar_GestioneFiltroUtente_Leggi_IN leggiCultivar_IN, List<int> gruCodList, DataTable utentiImpostazioniMonoDt, AgronicaCoreParametri objParametri)
        {
        ...
        /// <returns></returns>
        private async Task<DataTable> LeggiVarieta_ConVisibilitaCompletaAsync(Cultivar_GestioneFiltroUtente_Leggi_IN leggiCultivar_IN, AgronicaCoreParametri objParametri, int gruCod = 0)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable result;
        ...
        /// <param name="objParametri"></param>
        /// <returns></returns>
        private async Task<DataTable> EseguiQueryUnionAllPerLetturaVarietaAsync(Cultivar_GestioneFiltroUtente_Leggi_IN leggiCultivar_IN, List<int> gruCodList, AgronicaCoreParametri objParametri)
        {
            ...
            foreach (var gruCodElement in gruCodList)
            {
                if (gruCodElement != 0)
                {
                    ...
                    var filtriALivelloDiSpeciePerGruCod = await FiltriALivelloDiSpeciePerGruCodAsync(gruCodElement, objParametri);
                    if (filtriALivelloDiSpeciePerGruCod)
                    {
                        var vegCodVisibiliPerGruCod = await GetVegCodVisibiliPerGruCodAsync(gruCodElement, objParametri);
                        if (leggiCultivar_IN.Veg_Cod_List.Any())
                        {
                            ...
                            {
                                risultatoIntermedio = await LeggiVarietaDaListaDiVegCodAsync(vegCodDaUsarePerIlFiltro, objParametri, leggiCultivar_IN.Cul_Cod, leggiCultivar_IN.Cerca_CulDes);
                            }
                            else
                            {
                                var dummyVegCodList = new List<int>()
                            {
                                ...
                                risultatoIntermedio = await LeggiVarietaDaListaDiVegCodAsync(dummyVegCodList, objParametri, leggiCultivar_IN.Cul_Cod, leggiCultivar_IN.Cerca_CulDes);
                                risultatoIntermedio.Clear();
                            }
                        }
                        else //leggi le varietà per tutte le specie visibili per questo gruCod
                        {
                            risultatoIntermedio = await LeggiVarietaDaListaDiVegCodAsync(vegCodVisibiliPerGruCod, objParametri, leggiCultivar_IN.Cul_Cod, leggiCultivar_IN.Cerca_CulDes);
                        }

                    }
                    else //posso leggere tutte le varietà per questo gru cod
                    {
                        risultatoIntermedio = await LeggiVarieta_ConVisibilitaCompletaAsync(leggiCultivar_IN, objParametri, gruCodElement);
                    }
        ...
        /// <param name="leggiCultivar_IN"></param>
        /// <returns></returns>
        private async Task<DataTable> LeggiVarietaDaListaDiVegCodAsync(List<int> vegCodList, AgronicaCoreParametri objParametri, int Cul_Cod, string Cerca_CulDes)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable risultatoFinale = new DataTable();
            ...
            foreach (var vegCod in vegCodList)
            {
                ...
                try
                {
                    risultatoIntermedio = await GetDataProvider(objParametri).ExecuteReadAsync(stbQuery.ToString(), parametriSql);
                }
                catch (Exception ex)
                {
                    LogError(ex.Message, objParametri, ex);
        ...
        /// <param name="objParametri"></param>
        /// <returns></returns>
        private async Task<bool> FiltriALivelloDiSpeciePerGruCodAsync(int gruCod, AgronicaCoreParametri objParametri)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable result;
        ...
        /// <param name="objParametri"></param>
        /// <returns></returns>
        private async Task<List<int>> GetVegCodVisibiliPerGruCodAsync(int gruCod, AgronicaCoreParametri objParametri)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable result;
