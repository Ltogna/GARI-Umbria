...
namespace AgronicaNetCore.MetaSchema.DAL.DataLayer.DivisioniAmministrative
{
    public class DivisioniAmministrative : BaseDALMetaschema, IDivisioniAmministrative
    {
        ...
        public async Task<DataTable> LeggiRegioniAsync(LeggiRegioni_IN leggiRegioniIN, AgronicaCoreParametri objParametri)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable result;
        ...
        public async Task<DataTable> LeggiProvinceAsync(LeggiProvince_IN leggiProvinceIN, AgronicaCoreParametri objParametri)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable result;
        ...
        public async Task<DataTable> LeggiComuniAsync(LeggiComuni_IN istatParametriIN, AgronicaCoreParametri objParametri)
        {
            var stbQuery = new StringBuilder();
            var parametriSql = new Dictionary<string, object>();
            DataTable result;

            var filtraPerPROV = istatParametriIN.PROV_List.Any();
            var filtraPerCOM_LOCALITA = !string.IsNullOrEmpty(istatParametriIN.COM_LOCALITA);
            var filtraPerCOM_PROVINCIA = !string.IsNullOrEmpty(istatParametriIN.COM_PROVINCIA);
            var filtraPerSIGLA_PROVINCIA = !string.IsNullOrEmpty(istatParametriIN.SIGLA_PROVINCIA);
            var filtraPerCAP = !string.IsNullOrEmpty(istatParametriIN.CAP);
            var filtraPerREG = !string.IsNullOrEmpty(istatParametriIN.REG);
            var filtraPerStrProvincia = !string.IsNullOrEmpty(istatParametriIN.Filtro_StrProvincia);
            var filtraPerStrComune = !string.IsNullOrEmpty(istatParametriIN.Filtro_StrComune);
            ...
            if (filtraPerPROV)
            {
                ...
                for (int i = 0; i < istatParametriIN.PROV_List.Count; i++)
                {
                    stbQuery.Append($"@PROV{i}");

                    if (i != istatParametriIN.PROV_List.Count - 1)
                    {
                        //aggiungiamo la virgola per il prossimo parametro
                        stbQuery.Append(",");
                    }
            ...
            if (filtraPerPROV)
            {
                for (int i = 0; i < istatParametriIN.PROV_List.Count; i++)
                {
                    istatParametriIN.PROV_List[i] = istatParametriIN.PROV_List[i].Trim();
                    if (istatParametriIN.PROV_List[i].Length < 3)
                    {
                        if (istatParametriIN.PROV_List[i].Length == 1)
                            istatParametriIN.PROV_List[i] = "00" + istatParametriIN.PROV_List[i];
                        else if (istatParametriIN.PROV_List[i].Length == 2)
                            istatParametriIN.PROV_List[i] = "0" + istatParametriIN.PROV_List[i];
                    }
                    parametriSql.Add($"@PROV{i}", istatParametriIN.PROV_List[i]);
                }
            }

            if (filtraPerCOM_LOCALITA)
                parametriSql.Add("@COM_LOCALITA", istatParametriIN.COM_LOCALITA);

            if (filtraPerCOM_PROVINCIA)
                parametriSql.Add("@COM_PROVINCIA", istatParametriIN.COM_PROVINCIA);

            if (filtraPerSIGLA_PROVINCIA)
                parametriSql.Add("@SIGLA_PROVINCIA", istatParametriIN.SIGLA_PROVINCIA);

            if (filtraPerCAP)
                parametriSql.Add("@CAP", istatParametriIN.CAP);

            if (filtraPerREG)
                parametriSql.Add("@REG", istatParametriIN.REG);

            if (filtraPerStrProvincia)
                parametriSql.Add("@filtroStrProvincia", istatParametriIN.Filtro_StrProvincia);

            if (filtraPerStrComune)
                parametriSql.Add("@filtroStrComune", istatParametriIN.Filtro_StrComune);
