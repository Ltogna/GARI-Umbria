...
public class ParcoMacchine : BaseDALMetaschema, IParcoMacchine
{
    ...
    public async Task<string?> LeggiDesFromMacCodAsync(string piva, int macCod, DataTable dtCentriVisibili, AgronicaCoreParametri objParams)
    {
        var dt = await LeggiParcoMacchinexSuperUserAsync(piva, macCod, 0, true, dtCentriVisibili, objParams);
        if (dt.Rows.Count <= 0)
        {
            return null;
        }
                                                                   ...
                                                                   AgronicaCoreParametri objParams)
    {
        ...
        sqlParams.Add("@pivaSuperUser", objParams.PivaSuperUser.Trim());
