...
public interface IParcoMacchine
{
    ...
    Task<string?> LeggiDesFromMacCodAsync(string piva, int i, DataTable dtCentriVisibili, AgronicaCoreParametri objParams);

    public Task<DataTable> LeggiParcoMacchinexSuperUserAsync(string piva, int macCod, int macCodOrigine, bool ancheImportati,
                                                             string filtroAggiuntivo, string orderBy, DataTable dtCentriVisibili, AgronicaCoreParametri objParams);
}