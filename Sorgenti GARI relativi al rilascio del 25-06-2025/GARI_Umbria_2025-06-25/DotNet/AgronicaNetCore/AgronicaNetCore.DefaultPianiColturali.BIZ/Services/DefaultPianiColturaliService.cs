...
public class DefaultPianiColturaliService : BaseServiceDefaultPianiColturaliBIZ, IDefaultPianiColturaliService
{
        ...
        AgronicaCoreParametri objParams)
    {
        ...
        var dt = await _specieVegetali.LeggiCompletaAsync(vegCod, 0, objParams);
        if (dt.Rows.Count <= 0)
        {
            return result;
        }
        ...
        result.NCicli = await _specieVegetaliDefault.CicliCulturaliAsync(piva, true, vegCod,
            ...
            DateTime.Parse(CostantiPersonalizzate.AGRODATAFINE), "", objParams);
        if (result.NCicli == 0)
        {
            result.NCicli = 1;
        }
    ...
    public async Task<DataTable> LeggiDefaultGeneraleSpecieAsync(string piva, AgronicaCoreParametri objParams)
    {
        try
        {
            ...
            var dtCarica = await _specieVegetaliDefault.LeggiAsync(piva, 0, 0, 0, 0,
                DateTime.Parse(CostantiPersonalizzate.AGRODATAINIZIO),
                DateTime.Parse(CostantiPersonalizzate.AGRODATAFINE), filter, objParams);
    ...
    public async Task<bool> ScriviSpecieVegetaleAsync(SpecieVegetali_In body, AgronicaCoreParametri objParams)
    {
        try
        {
            ...
            await OpenConnectionAsync(objParams);
            foreach (var data in body.Data)
            {
                ...
                await _specieVegetaliDefault.CancellaAsync(body.Piva, data.VegCod, 0,
                    ...
                    " , " + (int)TipiEnumerativi.EnumCodiciDefault.SogliaMinima +
                    " , " + (int)TipiEnumerativi.EnumCodiciDefault.ResaStabilimento +
                    " , " + (int)TipiEnumerativi.EnumCodiciDefault.PesoSgocciolato + ")", objParams);


                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, 0,
                    (int)TipiEnumerativi.EnumCodiciDefault.TipoMaturazione,
                    data.TipoMaturazione.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, 0,
                    (int)TipiEnumerativi.EnumCodiciDefault.SogliaMinima,
                    data.SogliaMinima.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, 0,
                    (int)TipiEnumerativi.EnumCodiciDefault.ResaStabilimento,
                    data.ResaStabilimento.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, 0,
                    (int)TipiEnumerativi.EnumCodiciDefault.PesoSgocciolato,
                    data.PesoSgocciolato.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);
            }
        }
        catch (Exception ex)
        {
            CloseTransaction(objParams, true);
            LogError(ex.Message, objParams, ex);
            throw;
        }
        finally
        {
            CloseConnection(objParams);
        }

        return true;
    }

    public async Task<bool> CancellaSpecieVegetaleAsync(string piva, int vegCod, AgronicaCoreParametri objParams)
    {
        try
        {
    ...
    public async Task<DataTable> LeggiDefaultGeneraliAsync(string piva, AgronicaCoreParametri objParams)
    {
        try
        {
            var result = await GeneraDtDefaultGeneraleAsync(piva, objParams);
            return result;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> ScriviDefaultDateInizialiAsync(DefaultGeneraliColtura_In body,
        AgronicaCoreParametri objParams)
    {
        try
        {
    ...
    public async Task<bool> ScriviDefaultGeneraliAsync(DefaultGenerali_In body, AgronicaCoreParametri objParams)
    {
        try
        {
            var dtInitio = DateTime.Parse(CostantiPersonalizzate.AGRODATAINIZIO);
            ...
            foreach (var data in body.Data)
            {
                ...
                await _specieVegetaliDefault.CancellaAsync(body.Piva, data.VegCod, data.CulCod,
                    ...
                    " , " + (int)TipiEnumerativi.EnumCodiciDefault.DosiHaUdm + ")", objParams);


                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.UnitaCalore,
                    data.UnitaCalore.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Gg,
                    data.Gg.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Resa,
                    data.Resa.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.DosiHa,
                    data.DosiHa.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.DosiHaUdm,
                    data.DosiHaUdm.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Cal1,
                    data.Cal1.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Cal2,
                    data.Cal2.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Cal3,
                    data.Cal3.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Cal4,
                    data.Cal4.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Cal5,
                    data.Cal5.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);

                await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                    (int)TipiEnumerativi.EnumCodiciDefault.Cal6,
                    data.Cal6.ToString(CultureInfo.InvariantCulture), 0, dtInitio, dtFine, objParams);
            }
        }
        catch (Exception ex)
        {
        ...
        AgronicaCoreParametri objParams)
    {
        try
        {
            await _specieVegetaliDefault.CancellaAsync(piva, vegCod, culCod,
                ...
                " , " + (int)TipiEnumerativi.EnumCodiciDefault.DosiHa +
                " , " + (int)TipiEnumerativi.EnumCodiciDefault.DosiHaUdm +
                " , " + (int)TipiEnumerativi.EnumCodiciDefault.UnitaCalore + ")", objParams);
        }
        catch (Exception ex)
        {
            LogError(ex.Message, objParams, ex);
    ...
    public async Task<bool> SalvaDefaultDistintaDiProduzioneAsync(DistintaProduzione_In body, AgronicaCoreParametri objParams)
    {
        try
        {
            var dtInitio = DateTime.Parse(CostantiPersonalizzate.AGRODATAINIZIO);
            ...
            if (!await _specieVegetaliDefault.CancellaAsync(body.Piva, body.VegCod, -1, "", objParams))
            {
                ...
                foreach (var value in data.Values)
                {
                    if (!string.IsNullOrEmpty(value.DataFioritura))
                    {
                        await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                            ...
                            value.DataFioritura, value.NCiclo, dtInitio, dtFine, objParams);
                    }
                    ...
                    if (!string.IsNullOrEmpty(value.DataSemina))
                    {
                        await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                            ...
                            value.DataSemina, value.NCiclo, dtInitio, dtFine, objParams);
                    }
                    ...
                    if (!string.IsNullOrEmpty(value.DataRaccolta))
                    {
                        await ScriviSpecieDefaultOrThrowAsync(body.Piva, data.VegCod, data.CulCod,
                            ...
                            value.DataRaccolta, value.NCiclo, dtInitio, dtFine, objParams);
                    }
                }
            }
        }
    ...
    private async Task ScriviSpecieDefaultOrThrowAsync(string piva, int vegCod, int culCod, int codice, string valore, int nCiclo,
        DateTime dtInitio, DateTime dtFine, AgronicaCoreParametri objParams)
    {
        var id = await _sequenceDal.NuovoId_TabellaAsync("SpecieVegetali_Default", 0, 2000000000, objParams);
        var ok = await _specieVegetaliDefault.ScriviAsync(id, piva, vegCod, culCod, codice,
            valore, nCiclo, dtInitio, dtFine, objParams);
        if (!ok)
        {
    ...
    private async Task<DataTable> GeneraDtDefaultGeneraleAsync(string piva, AgronicaCoreParametri objParams)
    {
        var result = new DataTable();
        result.Columns.Add(new DataColumn("Veg_Cod", typeof(int)));
        result.Columns.Add(new DataColumn("Cul_Cod", typeof(int)));
        ...
        var dtCarica = await _specieVegetaliDefault.LeggiAsync(piva, 0, 0, 0, 0,
            ...
            " SpecieVegetali_Default.Cul_Cod <> 0 ", objParams);
        if (dtCarica.Rows.Count <= 0)
        {
            return result;
        }
