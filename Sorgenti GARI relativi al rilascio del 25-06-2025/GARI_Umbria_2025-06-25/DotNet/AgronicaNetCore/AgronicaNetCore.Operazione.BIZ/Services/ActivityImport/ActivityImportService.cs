...
namespace AgronicaNetCore.Operazione.BIZ.Services.ActivityImport
{
    public class ActivityImportService : BaseServiceOperazioneBIZ, IActivityImportService
    {
        ...
        private async Task BloccaAppezzamenti(IEnumerable<Appezzamento.PK> appezzamenti, AgronicaCoreParametri objParametri)
        {
            try
            {
                await OpenConnectionAsync(objParametri);
                foreach (var item in appezzamenti)
                {
                    await _appezzamenti.BloccaSbloccaAsync(true, item, objParametri);
                }
            }
            catch (Exception ex)
            {
        ...
        )
        {
            ...
            try
            {
                idAgenda = await _agenda.ScriviAttivitaAgendaAsync((activity, agenda), objParametriServer);
                if (operationData.flag_blocco_impianti)
                {
                    ...
                    await BloccaAppezzamenti(app, objParametriServer);
                }
        ...
        private async Task<bool> WriteAgronicaLogInvioChiamateAgendaAsync(WriteAgronicaLogInvioChiamate writeAgronicaLogInvioChiamate, WriteAgronicaLogInvioAgenda writeAgronicaLogInvioAgenda, AgronicaCoreParametri objParametri)
        {
            bool result = false;
            try
            {
                ...
                result = await _logInvioChiamate.WriteAsync(writeAgronicaLogInvioChiamate, objParametri);

                if (result && writeAgronicaLogInvioChiamate.ID > 0)
                {
                    writeAgronicaLogInvioAgenda.ID_Log_Invio = writeAgronicaLogInvioChiamate.ID;
                    result = await _logInvioAgenda.WriteAsync(writeAgronicaLogInvioAgenda, objParametri);
                }
