...
namespace AgronicaNetCore.Operazione.BIZ.Services.ActivityImport.Mapper
{
    public abstract class ActivityImportMapperTemplate<T>  : BaseServiceOperazioneBIZ, IActivityImportMapper<T> where T : IActivityImportData
    {
        ...
        protected T _base;
        protected AgronicaCoreParametri _server;
        ...
        public async Task<Attivita> MapActivity(AgronicaCoreParametri server)
        {
            return await MapActivity("", DEFAULT_RACCOGLITORE, server);
        }

        public async Task<Attivita> MapActivity(string piva, int raccoglitore, AgronicaCoreParametri server)
        {
            _server = server;
