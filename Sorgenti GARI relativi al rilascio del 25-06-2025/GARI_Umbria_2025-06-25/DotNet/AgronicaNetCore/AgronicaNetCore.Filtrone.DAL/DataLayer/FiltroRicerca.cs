...
namespace AgronicaNetCore.FiltroRicerca.DAL.DataLayer.FiltroRicerca
{
    ...
    public class FiltroRicerca : BaseDALFiltroRicerca, IFiltroRicerca
    {
        ...
        public async Task<CriteriRicerca_OUT> GetResultAsync(CriteriRicercaExtended criteriRicerca, ConfiguratoreFiltroRicerca configuratore, string utenti_DB_name, AgronicaCoreParametri objP_Server, AgronicaCoreParametri objP_Utenti)
        {
            ...
            result = await BuildQueryAsync(kendoColumns, parSql, parSqlIn, stbFiltri, objP_Server, objP_Utenti);

            return new() { result = result, kendoColumns = kendoColumns };
        }
        ...
        private async Task<DataTable> BuildQueryAsync(List<KendoColumn> kendoColumns,
                                                      ...
                                                      AgronicaCoreParametri objP_Utenti)

        {

            var stbQuery = new StringBuilder();
            var strTTVisibilitaAppoggio = new StringBuilder();
            ...
            stbQuery.AppendLine(GetUtenti_TT(kendoColumns, objP_Utenti).ToString());

            //Le imprese base vengono sempre estratte
            stbQuery.AppendLine(await GetAziendeBase_TTAsync(kendoColumns, stbFiltri, parSql, objP_Server, objP_Utenti));

            if (_configuratore.AziendeDettagli)
            {
                stbQuery.AppendLine(GetAziendeDettagli_TT(ref kendoColumns, stbFiltri, objP_Server));
            }

            if (_configuratore.joinCentro)
            {
                stbQuery.AppendLine(await GetCentriAziendaliBase_TTAsync(kendoColumns, stbFiltri, parSql, objP_Server, objP_Utenti));

                if (_configuratore.CentriAziendaliDettagli)
                {
                    stbQuery.AppendLine(GetCentriAziendaliDettagli_TT(ref kendoColumns, stbFiltri, objP_Server));
                }
            }

            if (_configuratore.joinCampo)
            {
                stbQuery.AppendLine(await GetCampiBase_TTAsync(kendoColumns, stbFiltri, objP_Server, objP_Utenti));

                if (_configuratore.CampiDettagli)
                {
                    stbQuery.AppendLine(GetCampiDettagli_TT(ref kendoColumns, stbFiltri, objP_Server));
                }
            }

            if (_configuratore.joinAppezzamento)
            {
                stbQuery.AppendLine(await GetAppezzamentiBase_TTAsync(kendoColumns, stbFiltri, objP_Server, objP_Utenti));

                if (_configuratore.AppezzamentiDettagli)
                {
                    stbQuery.AppendLine(GetAppezzamentiDettagli_TT(ref kendoColumns, stbFiltri, objP_Server));
                }
            }

            if (_configuratore.joinImpianto)
            {
                stbQuery.AppendLine(await GetImpiantiBase_TTAsync(kendoColumns, stbFiltri, objP_Server, objP_Utenti));

                if (_configuratore.ImpiantiDettagli)
                {
                    stbQuery.AppendLine(GetImpiantiDettagli_TT(ref kendoColumns, stbFiltri, objP_Server));
                }
            }

            if (_configuratore.joinEsercizio)
            {
                stbQuery.AppendLine(await GetEserciziBase_TTAsync(kendoColumns, stbFiltri, objP_Server, objP_Utenti));

                if (_configuratore.EserciziDettagli)
                {
                    stbQuery.AppendLine(GetEserciziDettagli_TT(ref kendoColumns, stbFiltri, objP_Server));
                }
            }

            if (_configuratore.joinFabbricato)
            {
                stbQuery.AppendLine(await GetFabbricatiBase_TTAsync(kendoColumns, stbFiltri, objP_Server, objP_Utenti));

                if (_configuratore.FabbricatiDettagli)
                {
                    stbQuery.AppendLine(GetFabbricatiDettagli_TT(ref kendoColumns, stbFiltri, objP_Server));
                }
            }

            if (_configuratore.isMostraMovimenti)
            {
                stbQuery.AppendLine(GetMovimentiBase_TT(ref kendoColumns, stbFiltri, objP_Utenti));
            }
            ...
            return await GetDataProvider(objP_Server).ExecuteReadAsync(stbQuery.ToString(), parSql, parSqlIn);
        }
                                                          ...
                                                          AgronicaCoreParametri objP_Utenti)
        {
            ...
            kendoColumns.Add(new KendoColumn { Field = "Piva", Title = "PartitaIvaAbbr", DataType = "string", ColumnOrder = SetColumnOrder(ref columnOrderIndex) });
            ...
            if (_checkVisibilitaAziende)
                stbQuery.AppendLine(GetUtentiVisibilitaAppoggio_TT((int)Enum_TipoEntita.Impresa, stbJoinVisibilitaAppoggio, ref parSql, objP_Server).ToString());
            ...
            if (_configuratore.CaricaImpreseReferenti)
                stbQuery.AppendLine("       , ImpreseReferenti, piva_padri ");
            ...
            await GetBaseColumnsAsync(stbQuery, stbJoinUtenti, Enum_Tabelle.Aziende, kendoColumns, !_configuratore.isMostraAziende, objP_Server);
                                             ...
                                             AgronicaCoreParametri objP_Server)
        {
            ...
            GetCodici(Enum_Tabelle.Aziende, _configuratore.CodiciAzienda, ref strSqlTTCodici, ref stbSelectCodici, ref stbJoinCodici, ref kendoColumns, objP_Server);
                                                                  ...
                                                                  AgronicaCoreParametri objP_Utenti)
        {
            ...
            if (_checkVisibilitaCentriAziendali)
                stbQuery.AppendLine(GetUtentiVisibilitaAppoggio_TT((int)Enum_TipoEntita.Centro, stbJoinVisibilitaAppoggio, ref parSql, objP_Server).ToString());
            ...
            if (_configuratore.needColumnsCentriAziendali)
            {
                ...
                await GetBaseColumnsAsync(stbQuery, stbJoinUtenti, Enum_Tabelle.CentriAziendali, kendoColumns, !_configuratore.isMostraCentriAziendali, objP_Server);
                                                     ...
                                                     AgronicaCoreParametri objP_Server)
        {
            ...
            GetCodici(Enum_Tabelle.CentriAziendali, _configuratore.CodiciCentri, ref strSqlTTCodici, ref stbSelectCodici, ref stbJoinCodici, ref kendoColumns, objP_Server);
                                                        ...
                                                        AgronicaCoreParametri objP_Utenti)
        {
            ...
            if (_configuratore.needColumnsCampi)
            {
                ...
                await GetBaseColumnsAsync(stbQuery, stbJoinUtenti, Enum_Tabelle.Campi, kendoColumns, !_configuratore.isMostraCampi, objP_Server);
                                           ...
                                           AgronicaCoreParametri objP_Server)
        {
            ...
            GetCodici(Enum_Tabelle.Campi, _configuratore.CodiciCampi, ref strSqlTTCodici, ref stbSelectCodici, ref stbJoinCodici, ref kendoColumns, objP_Server);
                                                               ...
                                                               AgronicaCoreParametri objP_Utenti)
        {
            ...
            if (_configuratore.needColumnsAppezzamenti)
            {
                ...
                stbQuery.AppendLine("         WHEN '1' THEN 'Integrato' ");
                stbQuery.AppendLine("         WHEN '2' THEN 'In Conversione' ");
                stbQuery.AppendLine("         WHEN '3' THEN 'Biologico' ");
                ...
                stbQuery.AppendLine("   , CASE WHEN ISNULL(Appezzamento.Blk_Flag, 0) = -1 THEN 'Sì' ELSE 'No' END AS Blk_Flag ");
                ...
                if (_configuratore.CaricaDatiCatastaliAppezzamento)
                {
                    ...
                    stbQuery.AppendLine("   , CASE WHEN AppezzamentiZVN_TT.ZVN IS NULL THEN 'No' ELSE 'Sì' END AS ZVN ");

                }

                await GetBaseColumnsAsync(stbQuery, stbJoinUtenti, Enum_Tabelle.Appezzamenti, kendoColumns, !_configuratore.isMostraAppezzamenti, objP_Server);
                                                  ...
                                                  AgronicaCoreParametri objP_Server)
        {
            ...
            GetCodici(Enum_Tabelle.Appezzamenti, _configuratore.CodiciAppezzamenti, ref strSqlTTCodici, ref stbSelectCodici, ref stbJoinCodici, ref kendoColumns, objP_Server);
                                                           ...
                                                           AgronicaCoreParametri objP_Utenti)
        {
            ...
            if (_configuratore.needColumnsImpianti)
            {
                ...
                //Colonne non visibili all'utente
                kendoColumns.Add(new KendoColumn { Field = "Cul_Cod", Title = "Cul_Cod", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
                kendoColumns.Add(new KendoColumn { Field = "Veg_Cod", Title = "Veg_Cod", DataType = "string", ColumnOrder = columnOrder_stdGIAS, Display = false });
            ...
            if (_configuratore.needColumnsImpianti)
            {
                ...
                switch (_configuratore.IncludiEscludiDestinazioniUso)
                {
                    case (int)Enum_FiltroDestinazioneUso_FiltroRicerca.SoloDestinazioniUso:
                        ...
                        stbQuery.AppendLine("       , ISNULL(DestinazioniUso_TT.Descrizione, 'Terreno Nudo')  AS Utilizzo ");
                    ...
                    default:
                        ...
                        stbQuery.AppendLine("           THEN ISNULL(DestinazioniUso_TT.Descrizione, 'Terreno Nudo') ");
                ...
                await GetBaseColumnsAsync(stbQuery, stbJoinUtenti, Enum_Tabelle.Impianti, kendoColumns, !_configuratore.isMostraImpianti, objP_Server);
                                              ...
                                              AgronicaCoreParametri objP_Server)
        {
            ...
            GetCodici(Enum_Tabelle.Impianti, _configuratore.CodiciImpianti, ref strSqlTTCodici, ref stbSelectCodici, ref stbJoinCodici, ref kendoColumns, objP_Server);
                                                           ...
                                                           AgronicaCoreParametri objP_Utenti)
        {
            ...
            if (_configuratore.needColumnsEsercizi)
            {
                ...
                stbQuery.AppendLine("   , CASE WHEN Imprese_Progetti.Validita_Fine <= GETDATE() THEN 'Sì' ELSE 'No' END AS EsercizioChiuso ");
                ...
                stbQuery.AppendLine("                           THEN 'Bio' ELSE Regolamenti.Reg_Des ");
                stbQuery.AppendLine("                  END ");
                ...
                if (_configuratore.isMostraEsercizi)
                {
                    await GetBaseColumnsAsync(stbQuery, stbJoinUtenti, Enum_Tabelle.Esercizi, kendoColumns, !(_configuratore.isMostraEsercizi && !_configuratore.isMostraPianoColturale), objP_Server);
                                              ...
                                              AgronicaCoreParametri objP_Server)
        {
            ...
            GetCodici(Enum_Tabelle.Esercizi, _configuratore.CodiciEsercizi, ref strSqlTTCodici, ref stbSelectCodici, ref stbJoinCodici, ref kendoColumns, objP_Server);
                                                             ...
                                                             AgronicaCoreParametri objP_Utenti)
        {
            ...
            if (_configuratore.needColumnsFabbricati)
            {
                ...
                await GetBaseColumnsAsync(stbQuery, stbJoinUtenti, Enum_Tabelle.Fabbricati, kendoColumns, !_configuratore.isMostraFabbricati, objP_Server);
                                                ...
                                                AgronicaCoreParametri objP_Server)
        {
            ...
            GetCodici(Enum_Tabelle.Fabbricati, _configuratore.CodiciFabbricati, ref strSqlTTCodici, ref stbSelectCodici, ref stbJoinCodici, ref kendoColumns, objP_Server);
                                           ...
                                           AgronicaCoreParametri objP_Utenti)
        {
            ...
            if (_configuratore.isMostraMovimenti)
            {
                ...
                stbQuery.AppendLine("   , CASE WHEN ISNULL(Agenda.Blocco_Flag, 0) = 0 THEN 'No' ELSE 'Sì' END AS OperazioneBloccata ");
        ...
        private string GetMovimenti_TT(StbFiltri stbFiltri)
        {
            ...
            {
                stbQuery.AppendLine(" WHERE 1 = 1 ");
                stbQuery.AppendLine($" AND Mov_Destinazioni.Tipo_Destinazione = {(int)TIPO_DESTINAZIONE.TIPO_DESTINAZIONE_IMPIANTO} ");
            }
        ...
        private string GetUtentiVisibilitaAppoggio_TT(int TipoEntita, StringBuilder stbJoin, ref Dictionary<string, object> parSql, AgronicaCoreParametri objP_Server)
        {
            ...
            parSql.Add(paramID, objP_Server.UtenteUsername);
        ...
        private void GetCodici(string TableName, List<CodiceAnagrafeBase> CodiciAnagrafe, ref StringBuilder stbSqlTTCodici, ref StringBuilder stbSqlSelectCodici, ref StringBuilder stbSqlJoinCodici, ref List<KendoColumn> kendoColumns, AgronicaCoreParametri objP_Server)
        {
            var prefix = "";
        ...
        private void GetCodici_CentroAziendale(int Id_Cod, string joinName, ref bool isTT, ref bool isBooleanField, ref string ambiguousCodeTag, ref StringBuilder stbSqlTTCodici)
        {
            ...
            switch (Id_Cod)
            {
                ...
                case (int)Enum_CodiciAnagrafe.TipoAttivita:
                    ...
                    stbSqlTTCodici.AppendLine("               WHEN 'PV' THEN 'Produzione vegetale'");
                    stbSqlTTCodici.AppendLine("               WHEN 'PZ' THEN 'Produzione zootecnica'");
                    stbSqlTTCodici.AppendLine("               WHEN 'PVZ' THEN 'Produzione vegetale e zootecnica'");
                    stbSqlTTCodici.AppendLine("               WHEN 'TPV' THEN 'Preparazione vegetale'");
                    stbSqlTTCodici.AppendLine("               WHEN 'TPZ' THEN 'Preparazione zootecnica'");
                    stbSqlTTCodici.AppendLine("               WHEN 'TPVZ' THEN 'Preparazione vegetale e zootecnica'");
                    stbSqlTTCodici.AppendLine("               WHEN 'I' THEN 'Importazione'");
                    stbSqlTTCodici.AppendLine("               WHEN 'RS' THEN 'Raccolta spontanea'");
                    stbSqlTTCodici.AppendLine("               WHEN 'P/TP' THEN 'Produzione / Preparazione'");
                    stbSqlTTCodici.AppendLine("               WHEN 'TP/I' THEN 'Preparazione / Importazione'");
                    stbSqlTTCodici.AppendLine("               WHEN '@' THEN 'Altro'");
        ...
        private void GetCodici_Appezzamento(int Id_Cod, string joinName, ref bool isTT, ref bool isBooleanField, ref string ambiguousCodeTag, ref StringBuilder stbSqlTTCodici)
        {
            ...
            switch (Id_Cod)
            {
                ...
                case (int)Enum_CodiciAnagrafe.Low_ILUC:
                    ...
                    stbSqlTTCodici.AppendLine("        , CASE tabellaCodice.val_cod ");
                    stbSqlTTCodici.AppendLine("            WHEN '1' THEN 'Sì' ");
                    stbSqlTTCodici.AppendLine("            ELSE 'No' ");
        ...
        private void GetCodici_ImpiantoEsercizio(int Id_Cod, string joinName, ref bool isTT, ref bool isBooleanField, ref string ambiguousCodeTag, ref StringBuilder stbSqlTTCodici)
        {
            ...
            switch (Id_Cod)
            {
                ...
                case (int)Enum_CodiciAnagrafe.Impianto_Ibrido:
                    ...
                    stbSqlTTCodici.AppendLine("        , CASE tabellaCodice.val_cod ");
                    stbSqlTTCodici.AppendLine("            WHEN '1' THEN 'Sì' ");
                    stbSqlTTCodici.AppendLine("            WHEN '0' THEN 'No' ");
        ...
        private void GetCodici_Fabbricato(int Id_Cod, string joinName, ref bool isTT, ref bool isBooleanField, ref string ambiguousCodeTag, ref StringBuilder stbSqlTTCodici)
        {
            ...
            switch (Id_Cod)
            {
                case (int)Enum_CodiciAnagrafe.Fabbricato_Forno_Combustibile:
                    ...
                    stbSqlTTCodici.AppendLine("           , CASE WHEN tabellaCodice.Val_Cod IN ('Metano ','METANO') THEN 'Metano' ");
                    stbSqlTTCodici.AppendLine("                   WHEN tabellaCodice.Val_Cod IN ('GPL ','gpl') THEN 'GPL' ");
                    stbSqlTTCodici.AppendLine("                  WHEN tabellaCodice.Val_Cod IN ('Gasolio ','GASOLIO') THEN 'Gasolio' ");
                    stbSqlTTCodici.AppendLine("                  WHEN tabellaCodice.Val_Cod IN ('Cippato ','cippato') THEN 'Cippato' ");
                ...
                case (int)Enum_CodiciAnagrafe.Fabbricato_Forno_Fiamma:
                    ...
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Sa_Cod AS Sa_Cod ");
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Fabbricato_Cod AS Fabbricato_Cod ");
                    stbSqlTTCodici.AppendLine("           , CASE WHEN tabellaCodice.Val_Cod IN ('indir','INDIRE','Indiretta','Indiretto') THEN 'Indiretta' ");
                    stbSqlTTCodici.AppendLine("            ELSE tabellaCodice.Val_Cod END AS Val_Cod ");
                    stbSqlTTCodici.AppendLine($" INTO #{joinName} ");
                    stbSqlTTCodici.AppendLine("   FROM Fabbricati_Codici tabellaCodice ");
                    stbSqlTTCodici.AppendLine(joinTabellaBase);
                ...
                case (int)Enum_CodiciAnagrafe.Fabbricato_Forno_Cantiere:
                    ...
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Sa_Cod AS Sa_Cod ");
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Fabbricato_Cod AS Fabbricato_Cod ");
                    stbSqlTTCodici.AppendLine("           , CASE WHEN tabellaCodice.Val_Cod IN ('cassoni ','Cassoni','CASSONI') THEN 'Cassoni' ");
                    stbSqlTTCodici.AppendLine("            ELSE tabellaCodice.Val_Cod END AS Val_Cod ");
                    stbSqlTTCodici.AppendLine($" INTO #{joinName} ");
                    stbSqlTTCodici.AppendLine("   FROM Fabbricati_Codici tabellaCodice ");
                    stbSqlTTCodici.AppendLine(joinTabellaBase);
                ...
                case (int)Enum_CodiciAnagrafe.Fabbricato_Forno_Umidificazione:
                    ...
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Sa_Cod AS Sa_Cod ");
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Fabbricato_Cod AS Fabbricato_Cod ");
                    stbSqlTTCodici.AppendLine("           , CASE WHEN tabellaCodice.Val_Cod IN ('Acqua ','acqua','ACQUA') THEN 'Acqua' ");
                    stbSqlTTCodici.AppendLine("            ELSE tabellaCodice.Val_Cod END AS Val_Cod ");
                    stbSqlTTCodici.AppendLine($" INTO #{joinName} ");
                ...
                case (int)Enum_CodiciAnagrafe.Fabbricato_Forno_Tipo:
                    ...
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Sa_Cod AS Sa_Cod ");
                    stbSqlTTCodici.AppendLine("           , tabellaCodice.Fabbricato_Cod AS Fabbricato_Cod ");
                    stbSqlTTCodici.AppendLine("           , CASE WHEN tabellaCodice.Val_Cod IN ('DE CLOE','DE CLOET') THEN 'De Cloet' ");
                    stbSqlTTCodici.AppendLine("                  WHEN tabellaCodice.Val_Cod IN ('EuropeTob','Europetop') THEN 'EuropeTob' ");
                    stbSqlTTCodici.AppendLine("                 WHEN tabellaCodice.Val_Cod IN ('TAB','Tab') THEN 'Tab' ");
                    stbSqlTTCodici.AppendLine("                 WHEN tabellaCodice.Val_Cod IN ('MENCAGLI','Mencagli') THEN 'Mencagli' ");
                    stbSqlTTCodici.AppendLine("                 WHEN tabellaCodice.Val_Cod IN ('GOME','Gome') THEN 'Gome' ");
                    stbSqlTTCodici.AppendLine("                 WHEN tabellaCodice.Val_Cod IN ('GODIOLI','Godioli') THEN 'Godioli' ");
                    stbSqlTTCodici.AppendLine("                 WHEN tabellaCodice.Val_Cod IN ('CALDERINI','Calderini') THEN 'Calderini' ");
                    stbSqlTTCodici.AppendLine("            ELSE tabellaCodice.Val_Cod END AS Val_Cod ");
                    stbSqlTTCodici.AppendLine($" INTO #{joinName} ");
                    stbSqlTTCodici.AppendLine("   FROM Fabbricati_Codici tabellaCodice ");
                    stbSqlTTCodici.AppendLine(joinTabellaBase);
        ...
        private async Task GetBaseColumnsAsync(StringBuilder stbQueryBase, StringBuilder stbJoinUtenti, string TableName, List<KendoColumn> kendoColumns, bool skipColonneStdGias, AgronicaCoreParametri objP_Server)
        {
            ...
            DataTable result = await GetDataProvider(objP_Server).ExecuteReadAsync(stbQuery.ToString(), parSql);
                        ...
                        )
                    {
                        ...
                        if (Enum_DTColumns.Skip_or_ApplyFormatDate.Contains(column.ToLowerInvariant()))
                        {
                            if (column.ToLowerInvariant().Contains("data"))
                            {
                                ...
                                //Cambio nome colonna
                                strSelect += TableName + "." + columnOriginal + " AS " + column + " ";
                            }
                            else
                            {
                                if (column.ToLowerInvariant().Contains("fine"))
                                    //Per i tipi data faccio un cast
                                    strSelect += " ISNULL(" + TableName + "." + column + ", CAST('2100-12-31' AS Date)) AS " + column + " ";
                                else
                                    strSelect += " ISNULL(" + TableName + "." + column + ", CAST('1900-01-01' AS Date)) AS " + column + " ";
                            }
                        ...
                        else
                        {
                            if (dataType == "boolean")
                            {
                                strSelect += $" CASE WHEN {TableName}.{column} = 1 THEN 'Si'";
                                strSelect += $"      WHEN {TableName}.{column} = 0 THEN 'NO'";
        ...
        private string GetUtenti_TT(List<KendoColumn> kendoColumns, AgronicaCoreParametri objP_Utenti)
        {
            if (!_configuratore.isMostraPianoColturale)
        ...
        private string GetZVNAppezzamenti_TT()
        {
            ...
            stbQuery.AppendLine("   , ZonexParticelle.Zona_Cod AS ZVN ");
        ...
        private void AggiungiFiltriAziende(FiltriAziende filtriAziende, Dictionary<string, object> parSql, Dictionary<string, Dictionary<Type, List<object>>> parSqlIn, StbFiltri stbFiltri)
        {
            ...
            // IMPRESE REFERENTI
            if (filtriAziende.ImpreseReferenti != null && filtriAziende.ImpreseReferenti.Count > 0)
            {
            ...
            // ZONE
            if (filtriAziende.Zone != null && filtriAziende.Zone.Count > 0)
            {
        ...
        private void AggiungiFiltriPianoColturale(FiltriPianoColturale filtriPianoColturale, Dictionary<string, object> parSql, Dictionary<string, Dictionary<Type, List<object>>> parSqlIn, StbFiltri stbFiltri)
        {
            ...
            // GRUPPO VEGETALE
            if (filtriPianoColturale.GruppoVegetale != null && filtriPianoColturale.GruppoVegetale.Count > 0)
            {
            ...
            // SPECIE
            if (filtriPianoColturale.Specie != null && filtriPianoColturale.Specie.Count > 0)
            {
            ...
            // TIPOLOGIA VARIETALE
            if (filtriPianoColturale.TipologiaVarietale != null && filtriPianoColturale.TipologiaVarietale.Count > 0)
            {
            ...
            // VARIETA'
            if (filtriPianoColturale.Varieta != null && filtriPianoColturale.Varieta.Count > 0)
            {
            ...
            switch (filtriPianoColturale.FiltroDestinazioneUso)
            {
                case (int)Enum_FiltroDestinazioneUso_FiltroRicerca.Tutto:
                    if (strFiltroColturale != "" && filtriPianoColturale.DestinazioniUso.Count > 0)
                    {
                ...
                case (int)Enum_FiltroDestinazioneUso_FiltroRicerca.SoloDestinazioniUso:
                    ...
                    if (filtriPianoColturale.DestinazioniUso != null && filtriPianoColturale.DestinazioniUso.Count > 0)
                    {
            ...
            // Contributi ACA
            if (filtriPianoColturale.ContributiACA != null && filtriPianoColturale.ContributiACA.Count > 0)
            {
        ...
        private void AggiungiFiltriCampi(FiltriCampi filtriCampi, Dictionary<string, object> parSql, Dictionary<string, Dictionary<Type, List<object>>> parSqlIn, StbFiltri stbFiltri)
        {
            ...
            // SPECIE
            if (filtriCampi.Specie != null && filtriCampi.Specie.Count > 0)
            {
        ...
        private void AggiungiFiltriTemporali(FiltriTemporali filtriTemporali, FiltriMovimenti filtriMovimenti, Dictionary<string, object> parSql, Dictionary<string, Dictionary<Type, List<object>>> parSqlIn, StbFiltri stbFiltri)
        {
            ...
            if (filtriTemporali != null && filtriTemporali.FiltriData != null && filtriTemporali.FiltriData.Count > 0)
            {
                ...
                if (!OuterFilter)
                {
                    ...
                    {
                        if (filtro.ColonnaData != (int)Enum_ColonnaData_FiltroRicerca.DataCreazioneAnagrafica)
                        {
                            switch (filtro.Entita)
                            {
                                case (int)Enum_Entita_FiltroRicerca.Esercizio:
                                    ...
                                    FiltroTemporale dummy1 = new()
                                    {
                                        ...
                                        ColonnaData = filtro.ColonnaData,
                                        TipoConfronto = filtro.TipoConfronto,
            ...
            if (!OuterFilter && filtriMovimenti != null && filtriMovimenti.DataMovimento != null)
            {
        ...
        private void AggiungiFiltriServizi(FiltriServizi filtriServizi, Dictionary<string, object> parSql, Dictionary<string, Dictionary<Type, List<object>>> parSqlIn, StbFiltri stbFiltri)
        {
            ...
            // STATO PRATICA
            if (filtriServizi.StatiPratica != null && filtriServizi.StatiPratica.Count > 0)
            {
        ...
        private void AggiungiFiltriMovimenti(FiltriMovimenti filtriMovimenti, CriteriRicercaExtended criteriRicerca, Dictionary<string, object> parSql, Dictionary<string, Dictionary<Type, List<object>>> parSqlIn, StbFiltri stbFiltri)
        {
            ...
            if (filtriMovimenti.FiltroOperazioni > 0)
            {
                ...
                if (filtriMovimenti.GruppoOperazioni != null && filtriMovimenti.GruppoOperazioni.Count > 0)
                {
                ...
                if (filtriMovimenti.Operazioni != null && filtriMovimenti.Operazioni.Count > 0)
                {
                                                           ...
                                                           int entitaDestinazione)
        {
            ...
            else if (Province != null && Province.Count > 0)
            {
            ...
            else if (Regioni != null && Regioni.Count > 0)
            {
            ...
            else if (Stati != null && Stati.Count > 0)
            {
