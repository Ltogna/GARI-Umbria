...
namespace AgronicaNetCore.Operazione.DAL.DataLayer.Movimenti
{
	...
	public Movimenti(IServiceProvider provider, IStringLocalizer<Messages> localizer, bool securityBypass = false) : base(provider,localizer, securityBypass)
        ...
        public async Task<bool> ExistAsync(int Id_Mov, AgronicaCoreParametri objP)
        {
            if (Id_Mov == 0)
        ...
        public async Task<bool> CreateAsync(WriteMovimenti dtoMovimenti, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> UpdateAsync(WriteMovimenti dtoMovimenti, AgronicaCoreParametri objP)
        {
        ...
        public async Task<int> ScriviModificaAsync(WriteMovimenti dtoMovimenti, AgronicaCoreParametri objP)
        {
            try
            {
                ...
                if (dtoMovimenti.Id_Mov == 0)
                {
                    ...
                    isNew = !await ExistAsync(dtoMovimenti.Id_Mov, objP);

                if (isNew)
                    await CreateAsync(dtoMovimenti, objP);
                else
                    await UpdateAsync(dtoMovimenti, objP);
        ...
        public async Task<bool> EliminaAsync(WriteMovimenti dtoMovimenti, AgronicaCoreParametri objP)
        {
            try
            {
                return await DeleteAsync(dtoMovimenti.Piva, dtoMovimenti.Id_Agenda, dtoMovimenti.Id_Mov, objP);
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objP, ex);
