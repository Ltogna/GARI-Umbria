...
namespace AgronicaNetCore.Operazione.DAL.DataLayer.Movimenti_Dettagli
{
	...
	public MovimentiDettagli(IServiceProvider provider, IStringLocalizer<Messages> localizer, bool securityBypass = false) : base(provider,localizer, securityBypass)
        ...
        public async Task<DataTable> ReadAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, AgronicaCoreParametri objP, FiltroAggiuntivo? xFiltroAggiuntivo = null)
        {
        ...
        public async Task<bool> ExistAsync(int Id_Mov_Det, AgronicaCoreParametri objP)
        {
            if (Id_Mov_Det == 0)
        ...
        public async Task<bool> CreateAsync(WriteMovDettagli dtoMovDettagli, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> UpdateAsync(WriteMovDettagli dtoMovDettagli, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> DeleteAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, AgronicaCoreParametri objP)
        {
            if (Id_Mov_Det == 0)
                throw new Exception("Id_Mov_Det non valorizzato.");
        ...
        public async Task<int> ScriviModificaAsync(WriteMovDettagli dtoMovDettagli, AgronicaCoreParametri objP)
        {
            try
            {
                ...
                if (dtoMovDettagli.Id_Mov_Det == 0)
                {
                    ...
                    isNew = !await ExistAsync(dtoMovDettagli.Id_Mov_Det, objP);

                if (isNew)
                    await CreateAsync(dtoMovDettagli, objP);
                else
                    await UpdateAsync(dtoMovDettagli, objP);
        ...
        public async Task<bool> EliminaAsync(WriteMovDettagli dtoMovDettagli, AgronicaCoreParametri objP)
        {
            try
            {
                return await DeleteAsync(dtoMovDettagli.Piva, dtoMovDettagli.Id_Agenda, dtoMovDettagli.Id_Mov, dtoMovDettagli.Id_Mov_Det, objP);
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objP, ex);
            ...
            using (TransactionScope ts = new(objP.objTransazione != null ? TransactionScopeOption.Required : TransactionScopeOption.RequiresNew, new TimeSpan(0, 10, 0), TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    foreach (var dto in MovDettagli)
                        await DeleteAsync(dto.Piva, dto.Id_Agenda, dto.Id_Mov, dto.Id_Mov_Det, objP);
