...
namespace AgronicaNetCore.Operazione.DAL.DataLayer.Movimenti_Zoo
{
    public class Movimenti_Zoo : BaseDALOperazione, IMovimenti_Zoo
    {
        ...
        public async Task<DataTable> ReadAsync(string Piva, int Id_Agenda, int Id_Mov, AgronicaCoreParametri objP, FiltroAggiuntivo? xFiltroAggiuntivo = null)
        {
        ...
        public async Task<bool> ExistAsync(string Piva, int Id_Agenda, int Id_Mov, AgronicaCoreParametri objP)
        {
            if (string.IsNullOrEmpty(Piva))
        ...
        public async Task<bool> CreateAsync(WriteMovimentiZoo dtoMovZoo, AgronicaCoreParametri objP)
        {
            ...
            stbQuery.AppendLine("INSERT INTO Movimenti_Zoo (Piva, Sa_Cod, Id_Agenda, Id_Mov, ")
                ...
                .AppendLine("    Pres_Numero, PresRiga_Numero, Tipo_Calcolo_Peso, Kg_Partenza, Coeff_Calo_Peso, Stato_Trattamento, Tipo_Trattamento, Kg_Aggiuntivi, ")
            ...
            stbQuery.AppendLine("VALUES (@piva, @saCod, @idAgenda, @idMov, ")
                ...
                .AppendLine("    @presNum, @presRNum, @tipoCalcPeso, @kgPart, @coeffCaloP, @statoTratt, @tipoTratt, @kgAgg, ")
        ...
        public async Task<bool> UpdateAsync(WriteMovimentiZoo dtoMovZoo, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> DeleteAsync(string Piva, int Id_Agenda, int Id_Mov, AgronicaCoreParametri objP)
        {
            if (string.IsNullOrEmpty(Piva))
                throw new Exception("Piva non valorizzato.");
            if (Id_Agenda == 0)
        ...
        public async Task<bool> ScriviModificaAsync(WriteMovimentiZoo dtoMovZoo, AgronicaCoreParametri objP)
        {
            try
            {
                bool isNew = !await ExistAsync(dtoMovZoo.Piva, dtoMovZoo.Id_Agenda, dtoMovZoo.Id_Mov, objP);

                if (isNew)
                    await CreateAsync(dtoMovZoo, objP);
                else
                    await UpdateAsync(dtoMovZoo, objP);
        ...
        public async Task<bool> EliminaAsync(WriteMovimentiZoo dtoMovZoo, AgronicaCoreParametri objP)
        {
            try
            {
                return await DeleteAsync(dtoMovZoo.Piva, dtoMovZoo.Id_Agenda, dtoMovZoo.Id_Mov, objP);
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objP, ex);
            ...
            using (TransactionScope ts = new(objP.objTransazione != null ? TransactionScopeOption.Required : TransactionScopeOption.RequiresNew, new TimeSpan(0, 10, 0), TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    foreach (var dto in MovZoo)
                        await DeleteAsync(dto.Piva, dto.Id_Agenda, dto.Id_Mov, objP);
                    ts.Complete();
                }
                catch (Exception ex)
                {
