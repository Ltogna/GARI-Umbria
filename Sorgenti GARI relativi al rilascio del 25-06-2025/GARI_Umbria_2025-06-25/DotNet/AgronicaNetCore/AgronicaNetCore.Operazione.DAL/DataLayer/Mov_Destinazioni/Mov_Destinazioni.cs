...
namespace AgronicaNetCore.Operazione.DAL.DataLayer.Mov_Destinazioni
{
    public class Mov_Destinazioni : BaseDALOperazione, IMov_Destinazioni
    {
        ...
        public async Task<DataTable> ReadAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, int Id_Dest, AgronicaCoreParametri objP, FiltroAggiuntivo? xFiltroAggiuntivo = null)
        {
        ...
        public async Task<bool> ExistAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, int Id_Destinazione, AgronicaCoreParametri objP)
        {
            if (string.IsNullOrEmpty(Piva))
        ...
        public async Task<bool> CreateAsync(WriteMovDestinazioni dtoMovDestinazioni, AgronicaCoreParametri objP)
        {
            ...
            sqlFields.Add(("Username_Creazione", "@userOp", objP.UsernameOperazione));
            sqlFields.Add(("Username_Modifica", "@userOp", objP.UsernameOperazione));
        ...
        public async Task<bool> UpdateAsync(WriteMovDestinazioni dtoMovDestinazioni, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> DeleteAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, int Id_Dest, AgronicaCoreParametri objP)
        {
            if (Id_Dest == 0)
                throw new Exception("Id_Destinazione non valorizzato.");
        ...
        public async Task<int> ScriviModificaAsync(WriteMovDestinazioni dtoMovDest, AgronicaCoreParametri objP)
        {
            try
            {
                bool isNew =
                    !await ExistAsync(dtoMovDest.Piva, dtoMovDest.Id_Agenda, dtoMovDest.Id_Mov, dtoMovDest.Id_Mov_Det, dtoMovDest.Id_Destinazione, objP);

                if (isNew)
                    await CreateAsync(dtoMovDest, objP);
                else
                    await UpdateAsync(dtoMovDest, objP);
        ...
        public async Task<bool> EliminaAsync(WriteMovDestinazioni dtoMovDest, AgronicaCoreParametri objP)
        {
            try
            {
                return await DeleteAsync(dtoMovDest.Piva, dtoMovDest.Id_Agenda, dtoMovDest.Id_Mov, dtoMovDest.Id_Mov_Det, dtoMovDest.Id_Destinazione, objP);
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objP, ex);
        ...
        public async Task<bool> EliminaAsync(List<WriteMovDestinazioni> MovDestinazioni, AgronicaCoreParametri objP)
        {
            using (TransactionScope ts = new(objP.objTransazione != null ? TransactionScopeOption.Required : TransactionScopeOption.RequiresNew, new TimeSpan(0, 10, 0), TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    foreach (var dto in MovDestinazioni)
                        await DeleteAsync(dto.Piva, dto.Id_Agenda, dto.Id_Mov, dto.Id_Mov_Det, dto.Id_Destinazione, objP);
