...
namespace AgronicaNetCore.Operazione.DAL.DataLayer.Mov_Dettaglio_Tecnico_Extra
{
	...
	public Mov_Dettaglio_Tecnico_Extra(IServiceProvider provider, IStringLocalizer<Messages> localizer, bool securityBypass = false) : base(provider,localizer, securityBypass)
        ...
        public async Task<DataTable> ReadAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, int Id_Reg_Det, AgronicaCoreParametri objP, FiltroAggiuntivo? xFiltroAggiuntivo = null)
        {
        ...
        public async Task<bool> ExistAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, int Id_Reg_Det, AgronicaCoreParametri objP)
        {
            if (string.IsNullOrEmpty(Piva))
        ...
        public async Task<bool> CreateAsync(WriteMovDettTecnicoExtra dtoMovDettTecnEx, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> UpdateAsync(WriteMovDettTecnicoExtra dtoMovDettTecnEx, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> DeleteAsync(string Piva, int Id_Agenda, int Id_Mov, int Id_Mov_Det, int Id_Reg_Det, AgronicaCoreParametri objP)
        {
            if (Id_Reg_Det == 0)
                throw new Exception("Id_Reg_Dettaglio non valorizzato.");
        ...
        public async Task<int> ScriviModificaAsync(WriteMovDettTecnicoExtra dtoMovDettTecExtra, AgronicaCoreParametri objP)
        {
            try
            {
                bool isNew = false;
                ...
                {
                    dtoMovDettTecExtra.Id_Reg_Det = await _sequenceDal.NuovoId_TabellaAsync("movimenti_dettagli_tecnici_extra", 0, 2000000000, objP);
                    isNew = true;
                }
                else
                    isNew = !await ExistAsync(dtoMovDettTecExtra.Piva, dtoMovDettTecExtra.Id_Agenda, dtoMovDettTecExtra.Id_Mov, dtoMovDettTecExtra.Id_Mov_Det, dtoMovDettTecExtra.Id_Reg_Det, objP);

                if (isNew)
                    await CreateAsync(dtoMovDettTecExtra, objP);
                else
                    await UpdateAsync(dtoMovDettTecExtra, objP);
        ...
        public async Task<bool> EliminaAsync(WriteMovDettTecnicoExtra dtoMovDettTecExtra, AgronicaCoreParametri objP)
        {
            try
            {
                return await DeleteAsync(dtoMovDettTecExtra.Piva, dtoMovDettTecExtra.Id_Agenda, dtoMovDettTecExtra.Id_Mov, dtoMovDettTecExtra.Id_Mov_Det, dtoMovDettTecExtra.Id_Reg_Det, objP);
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objP, ex);
        ...
        public async Task<bool> EliminaAsync(List<WriteMovDettTecnicoExtra> MovDettTecExtra, AgronicaCoreParametri objP)
        {
            using (TransactionScope ts = new(objP.objTransazione != null ? TransactionScopeOption.Required : TransactionScopeOption.RequiresNew, new TimeSpan(0, 10, 0), TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    foreach (var dto in MovDettTecExtra)
                        await DeleteAsync(dto.Piva, dto.Id_Agenda, dto.Id_Mov, dto.Id_Mov_Det, dto.Id_Reg_Det, objP);
