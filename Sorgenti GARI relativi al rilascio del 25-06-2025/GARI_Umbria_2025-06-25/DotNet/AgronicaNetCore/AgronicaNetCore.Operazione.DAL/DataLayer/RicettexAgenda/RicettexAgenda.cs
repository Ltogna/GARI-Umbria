...
namespace AgronicaNetCore.Operazione.DAL.DataLayer.RicettexAgenda
{
    public class RicettexAgenda : DAL_Base, IRicettexAgenda
    {
        ...
        public async Task<DataTable> ReadAsync(int Ricetta_Cod, int Id_Agenda, AgronicaCoreParametri objP, int RicettaOp_Cod = 0)
        {
        ...
        public async Task<bool> ExistAsync(int Ricetta_Cod, int Id_Agenda, AgronicaCoreParametri objP, int RicettaOp_Cod = 0)
        {
            if (Ricetta_Cod == 0)
        ...
        public async Task<bool> CreateAsync(WriteRicettexAgenda dtoRxA, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> UpdateAsync(WriteRicettexAgenda dtoRxA, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> DeleteAsync(int Ricetta_Cod, int Id_Agenda, AgronicaCoreParametri objP, int RicettaOp_Cod = 0)
        {
            if (Ricetta_Cod == 0)
                throw new Exception("Ricetta_Cod non valorizzato.");            
            if (Id_Agenda == 0)
        ...
        public async Task<bool> ScriviModificaAsync(WriteRicettexAgenda dtoRicxAg, AgronicaCoreParametri objP)
        {
            try
            {
                bool isNew = !await ExistAsync(dtoRicxAg.Ricetta_Cod, dtoRicxAg.Id_Agenda, objP);

                if (isNew)
                    await CreateAsync(dtoRicxAg, objP);
                else
                    await UpdateAsync(dtoRicxAg, objP);
        ...
        public async Task<bool> EliminaAsync(WriteRicettexAgenda dtoRicxAg, AgronicaCoreParametri objP)
        {
            try
            {
                return await DeleteAsync(dtoRicxAg.Ricetta_Cod, dtoRicxAg.Id_Agenda, objP);
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objP, ex);
        ...
        public async Task<bool> EliminaAsync(List<WriteRicettexAgenda> RicettexAgenda, AgronicaCoreParametri objP)
        {
            using (TransactionScope ts = new(objP.objTransazione != null ? TransactionScopeOption.Required : TransactionScopeOption.RequiresNew, new TimeSpan(0, 10, 0), TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    ...
                    {
                        await DeleteAsync(dto.Ricetta_Cod, dto.Id_Agenda, objP);
                    }
                    ts.Complete();
                }
                catch (Exception ex)
