...
public class OperazioneCausale : BaseDALOperazione, IOperazioneCausale
{
    ...
    public async Task<DataTable> LeggiAsync(int lavCod, DateTime? data, AgronicaCoreParametri objParams)
    {
    ...
    public async Task<DataTable> Leggi_CausaleDes_From_CausaleId_DALAsync(int id, AgronicaCoreParametri objParams)
    {
        var stbQuery = new StringBuilder();
        var sqlParams = new Dictionary<string, object>();
    ...
    public async Task<bool> ModificaAsync(int id, string causale, int lavCod, int inviato, DateTime validitaInizio, DateTime validitaFine, AgronicaCoreParametri objParams)
    {
    ...
    public async Task<bool> CancellaAsync(int id, int lavCod, AgronicaCoreParametri objParams)
    {
        ...
        try
        {
            dt = await GetDataProvider(objParams).ExecuteReadAsync(stbQuery.ToString(), sqlParams);
        }
        catch
            (Exception ex)
        {
        ...
        else
        {
            ...
            try
            {
                return await GetDataProvider(objParams).Execute_WriteAsync(sqlDeleteString, expandoObj);
            }
            catch
                (Exception ex)
            {
