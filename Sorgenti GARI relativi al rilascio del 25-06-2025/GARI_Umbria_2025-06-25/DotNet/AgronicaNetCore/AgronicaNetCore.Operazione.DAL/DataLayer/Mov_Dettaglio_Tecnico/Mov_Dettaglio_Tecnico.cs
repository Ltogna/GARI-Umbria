...
namespace AgronicaNetCore.Operazione.DAL.DataLayer.Mov_Dettaglio_Tecnico
{
	...
	public Mov_Dettaglio_Tecnico(IServiceProvider provider, IStringLocalizer<Messages> localizer, bool securityBypass = false) : base(provider,localizer, securityBypass)
        ...
        private WriteMovDettTecnico Valorizza(WriteMovDettTecnico dtoMovDettTec)
        {
            ...
            if (dtoMovDettTec.Validita_Fine > adFine)
        ...
        public async Task<bool> CreateAsync(WriteMovDettTecnico dtoMovDettTec, AgronicaCoreParametri objP)
        {
        ...
        public async Task<bool> UpdateAsync(WriteMovDettTecnico dtoMovDettTec, AgronicaCoreParametri objP)
        {
        ...
        public async Task<int> ScriviModificaAsync(WriteMovDettTecnico dtoMovDettTec, AgronicaCoreParametri objP)
        {
            try
            {
                ...
                if (dtoMovDettTec.Id_Reg_Dettaglio == 0)
                {
                    dtoMovDettTec.Id_Reg_Dettaglio = await _sequenceDal.NuovoId_TabellaAsync("movimenti_dettagli_tecnici", 0, 2000000000, objP);
                    isNew = true;
                }
                else
                    isNew = !await ExistAsync(dtoMovDettTec.Piva, dtoMovDettTec.Id_Agenda, dtoMovDettTec.Id_Mov, dtoMovDettTec.Id_Mov_Det, dtoMovDettTec.Id_Reg_Dettaglio, objP);

                if (isNew)
                    await CreateAsync(dtoMovDettTec, objP);
                else
                    await UpdateAsync(dtoMovDettTec, objP);
        ...
        public async Task<bool> EliminaAsync(WriteMovDettTecnico dtoMovDettTec, AgronicaCoreParametri objP)
        {
            try
            {
                return await DeleteAsync(dtoMovDettTec.Piva, dtoMovDettTec.Id_Agenda, dtoMovDettTec.Id_Mov, dtoMovDettTec.Id_Mov_Det, dtoMovDettTec.Id_Reg_Dettaglio, objP);
            }
            catch (Exception ex)
            {
                LogError(ex.Message, objP, ex);
            ...
            using (TransactionScope ts = new(objP.objTransazione != null ? TransactionScopeOption.Required : TransactionScopeOption.RequiresNew, new TimeSpan(0, 10, 0), TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    foreach (var dto in MovDettTec)
                        await DeleteAsync(dto.Piva, dto.Id_Agenda, dto.Id_Mov, dto.Id_Mov_Det, dto.Id_Reg_Dettaglio, objP);
