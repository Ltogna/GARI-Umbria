...
namespace AgronicaNetCore.Widgets.BIZ.Services.WidgetStatistiche
{
    public class WidgetDocumentaleService : BaseServiceWidgetsBIZ, IWidgetDocumentaleService
    {
        ...
        public async Task<DataTable?> GetDocumentRecapAsync(DateTime timeStart, AgronicaCoreParametri objParametriServer, AgronicaCoreParametri objParametriUser)
        {
            DataTable? res = null;
            try
            {
                ...
                DataTable dt = (await _utentiImpostazioni.ReadAsync((int)Enum_Impostazioni_Utenti.SUPERUSER_Documentale_GestioneWorkFlow, 2, objParametriUser))!;
                bool useWorkflow = dt.Rows.Count > 0;

                res = await _widgetsDocumentale.GetDocumentRecapAsync(timeStart, useWorkflow, userVisibility, objParametriServer, objParametriUser);
