...
public class GruppiOperazioneService : BaseServiceMetaschemaBIZ, IGruppiOperazioneService
{
    ...
    public async Task<DataTable> GruppiOperazione_LeggiAsync(AgronicaCoreParametri objParametriServer,
        AgronicaCoreParametri objParametriUtente)
    {
        DataTable dt;
        try
        {
            dt = await _gruppiOperazioneDal.GruppiOperazione_LeggiAsync(objParametriServer, objParametriUtente);
        }
        catch (Exception ex)
        {
