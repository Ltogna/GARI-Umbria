...
namespace AgronicaNetCore.MetaSchema.BIZ.Services.Operazioni
{
    public class OperazioniService : BaseServiceMetaschemaBIZ, IOperazioniService
    {
        ...
        public async Task<DtConVisibilita_OUT> Operazioni_GestioneFiltroUtente_LeggiAsync(LeggiOperazioni_IN leggiOperazioni, AgronicaCoreParametri objParametriServer, AgronicaCoreParametri objParametriUtente)
        {
            DtConVisibilita_OUT dtConVisibilita;
            try
            {
                var utentiImpostazioniMonoDt = await _utentiImpostazioniMono.LeggiAsync(TipiEnumerativi.Enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_GRUPPI_OPERAZIONI, 0, objParametriUtente);
                dtConVisibilita = await _operazioni.Operazioni_GestioneFiltroUtente_LeggiAsync(leggiOperazioni, utentiImpostazioniMonoDt, objParametriServer, objParametriUtente);
            }
            catch (Exception ex)
            {
