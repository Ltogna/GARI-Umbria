...
public class ParcoMacchineService : BaseServiceMetaschemaBIZ, IParcoMacchineService
{
    ...
    public async Task<DataTable> ParcoMacchine_LeggiAsync(string piva, AgronicaCoreParametri objParametriServer,
    ...
    private async Task<string> GetVisibilityFilterAsync(string piva, AgronicaCoreParametri objParams)
    {
        var strSql = new StringBuilder();
        strSql.AppendLine(" ( (Parco_Macchine.Sa_Cod = -1) ");

        var filtroCentri = "";
        var dtCentriVisibili = await _utentiVisibilitaAppoggioDal.ReadAsync((int)TipiEnumerativi.Enum_TipoEntita.Centro, objParams, piva: piva);
