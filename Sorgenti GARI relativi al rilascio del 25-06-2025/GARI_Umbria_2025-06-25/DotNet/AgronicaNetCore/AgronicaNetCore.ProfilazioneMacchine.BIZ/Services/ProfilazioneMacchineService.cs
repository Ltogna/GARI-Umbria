...
public class ProfilazioneMacchineService : BaseServiceProfilazioneMacchineBIZ, IProfilazioneMacchineService
{
    ...
    public async Task<DataTable> LeggiAsync(int idProfilazione, AgronicaCoreParametri objParams)
    {
        ...
        try
        {
            ...
            await Carica_MacchineAsync(result, idProfilazione, strMacCod, objParams);
            return result;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> UpdateCaratteristicheMacchinaAsync(CaratteristicheMacchina_In body, AgronicaCoreParametri objParams)
    {
        try
        {
            ...
            foreach (var caratteristica in body.Caratteristiche)
            {
                ...
                if (primoRecord)
                {
                    await _profilazioneMacchineDal.CancellaAsync(body.IdProfilazione,
                        caratteristica.MacCod, 0, "", objParams);
                    primoRecord = false;
                }
    ...
    private async Task Carica_MacchineAsync(DataTable result, int idProfiloDati, string strMacCods, AgronicaCoreParametri objParams)
    {
        // leggo ottengo i macchinari
        var dtCentriVisibili = await _utentiVisibilitaAppoggioDAL.ReadAsync((int)TipiEnumerativi.Enum_TipoEntita.Centro,  objParams, piva: objParams.PivaSuperUser);
        var dtM = await _parcoMacchineDal.LeggiParcoMacchinexSuperUserAsync(objParams.PivaSuperUser, 0, 0, true, $" Parco_Macchine.Mac_Cod IN ({strMacCods}) ", "mac_des", dtCentriVisibili, objParams);
        ...
        var dtMp = await _profilazioneMacchineDal.LeggiAsync(idProfiloDati, 0, 0,
            $" Mac_Cod IN ({strMacCods}) ", "", objParams);
        ...
        if (strClassCode != "")
        {
            dtCar = await _macchineCaratteristicheDal.LeggiAsync(0, "", $" CLASS_CODE IN ({strClassCode[..^1]}) ", "", objParams);
        }
        ...
        for (var i = 0; i < dtM.Rows.Count; i++)
        {
            ...
            {
                var dtDitta = await _ditteDal.LeggiAsync(dittaCod, "", "", "", objParams);
            ...
            {
                var udm = await _unitaMisuraDal.LeggiAsync(potenzaUdmCod, 0, "", "", objParams);
            ...
            if (drCar is { Length: > 0 })
            {
                foreach (var row in drCar)
                {
                    ...
                    {
                        var drMp = dtMp.Select("PivaSuperUser='" + objParams.PivaSuperUser + "'" +
