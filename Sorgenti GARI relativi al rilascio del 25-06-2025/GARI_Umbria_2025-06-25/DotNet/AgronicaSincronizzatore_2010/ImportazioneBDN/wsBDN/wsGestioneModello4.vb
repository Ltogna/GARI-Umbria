...
Public Class wsGestioneModello4
    ...
    Public Function deleteCapoModello(ByVal P_LISTA_PRENOT_ID As Integer,
        ...
        Dim dtDeleteCapoModello As New DataTable

        Try
            Dim xml_CancellaCapoModello As New dsINS_DEL_CAPO_MODELLO_IUS()
            xml_CancellaCapoModello.InitVars()
            xml_CancellaCapoModello.EnforceConstraints = True

            xml_CancellaCapoModello.PARAMETERS_LIST.InitVars()
            xml_CancellaCapoModello.PARAMETERS_LIST.AddPARAMETERS_LISTRow(P_LISTA_PRENOT_ID, P_PRENOTAZIONE_ID, P_CODICE_CAPO,
                                                                          P_DT_USCITA, P_FLAG_ESAMINATO)


            'genera la stringa dall'xml
            Dim strXml_CancellaCapoModello As String = xml_CancellaCapoModello.GetXml()

            'gestione xml
            Dim docXmlSwap As New XmlDocument
            docXmlSwap.LoadXml(strXml_CancellaCapoModello)
            ...
            'dopo i vari aggiustamenti al dannato xml, lo salva in stringa
            Dim finalXml_CancellaCapoModello As String = docXmlSwap.InnerXml

            'genera la stringa con Escape dalla stringa dell'xml
            Dim esStrXml_CancellaCapoModello As String = System.Security.SecurityElement.Escape(finalXml_CancellaCapoModello)

            Dim reqInsert As XmlNode = SoapRequestV2(esStrXml_CancellaCapoModello, Reflection.MethodBase.GetCurrentMethod().Name)

            dtDeleteCapoModello = MyBase.generateDTfromXml(reqInsert)
