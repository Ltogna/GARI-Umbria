...
Public MustInherit Class WSBDN
    ...
    Protected Sub New(objParametri_Server As AgronicaCoreParametri, objParametri_Utenti As AgronicaCoreParametri, token As String, ruolo As String, valore_ruolo_codice As String)
        ...
        If (username = "" Or password = "") AndAlso token = "" Then
            Throw New GiasException("Credenziali servizio Integrazione BDN non configurate correttamente")
        End If

        If token <> "" And Not serviceEnpoint.EndsWith("pub/") Then
            serviceEnpoint &= "pub/"
        End If
...
#End If
        ...
        'System.Net.ServicePointManager.SecurityProtocol = SecurityProtocolType.SystemDefault Or SecurityProtocolType.Tls11 Or SecurityProtocolType.Tls12 Or SecurityProtocolType.Tls13
        'System.Net.ServicePointManager.ServerCertificateValidationCallback = AddressOf AcceptAllCertifications
...
#End If
    ...
    Protected Function generateDTfromXml(xml As XmlNode) As DataTable
        ...
        If child_nodes IsNot Nothing AndAlso child_nodes.Count > 0 Then
            ...
            If errorDesNode IsNot Nothing AndAlso errorDesNode.InnerText <> "" And Not errorDesNode.InnerText.Contains("TOKEN SCADUTO") Then
                Throw New BDNException(errorDesNode.InnerText)
            End If

            If errorDesNode IsNot Nothing AndAlso errorDesNode.InnerText <> "" And errorDesNode.InnerText.Contains("TOKEN SCADUTO") Then
    ...
    Protected Function getStrDataFromDate(data As Date) As String
        Dim strData = ""
        If data = AgronicaCoreDataProvider.CostantiPersonalizzate.AGRODATAINIZIO Then
            Return strData
        End If
        If data = AgronicaCoreDataProvider.CostantiPersonalizzate.AGRODATAFINE Then
            Return strData
        End If
    ...
    Private Function getSoapBodyV2(ByVal soapAction As String,
    ...
    Protected Function SoapRequest(soapAction As String, params As Dictionary(Of String, String), Optional ByRef cacheable As Boolean = False) As XmlNode
        ...
        If tokenHandler.BDNtoken <> "" Then
            SOAPAutenticazione = "<SOAPAutenticazione xmlns=""http://bdr.izs.it/webservices"">
                                                          ...
                                                          <token>" & tokenHandler.BDNtoken & "</token>
                                  </SOAPAutenticazione>"
        Else
            SOAPAutenticazione = "<SOAPAutenticazione xmlns=""http://bdr.izs.it/webservices"">
