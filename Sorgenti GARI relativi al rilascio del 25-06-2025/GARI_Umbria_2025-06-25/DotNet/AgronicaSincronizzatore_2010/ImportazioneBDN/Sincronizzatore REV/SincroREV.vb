...
Public Class SincroREV
	...
	Public Function Sincronizza_REV_VetInfo(ByVal Piva As String,
		...
		resp.NumeroTrattamentiSincronizzati = 0
		resp.NumeroTrattamentiEliminati = 0
		resp.NumeroProtocolliSincronizzati = 0
		resp.NumeroRicetteVeterinarieSincronizzate = 0
		resp.NumeroIndicazioniTerapeuticheSincronizzate = 0
		...
		Try
			...
			If Chk_Sincronizza_Protocolli Then
				Dim Protocolli = Sincronizza_Prescrizioni(Piva, Sa_Cod, Sta_Num, aziendaCodice, propIdFiscale, idLogSincro, AGRODATAINIZIO, AGRODATAFINE, "PROTOCOLLO")
				resp.NumeroProtocolliSincronizzati = Protocolli.Count
			End If

			If Chk_Sincronizza_Indicazioni_Terapeutiche Then
				Dim Indicazioni_Terapeutiche = Sincronizza_Prescrizioni(Piva, Sa_Cod, Sta_Num, aziendaCodice, propIdFiscale, idLogSincro, Data_Inizio, Data_Fine, "PRESTRATSCORTA")
				resp.NumeroIndicazioniTerapeuticheSincronizzate = Indicazioni_Terapeutiche.Count
			End If

			If Chk_Sincronizza_Ricette_Veterinarie Then
				Dim Ricette_Veterinarie = Sincronizza_Prescrizioni(Piva, Sa_Cod, Sta_Num, aziendaCodice, propIdFiscale, idLogSincro, Data_Inizio, Data_Fine, "PRESVET")
				resp.NumeroRicetteVeterinarieSincronizzate = Ricette_Veterinarie.Count
			End If


			If Chk_Sincronizza_Trattamenti Then
				Sincronizza_Trattamenti(Piva, Sa_Cod, Sta_Num, aziendaCodice, propIdFiscale, idLogSincro, Data_Inizio, Data_Fine, resp)
			End If
	...
	Private Function Sincronizza_Trattamenti(ByVal Piva As String,
											 ...
											 ByVal Data_Inizio As Date,
											 ByVal Data_Fine As Date,
											 ByRef resp As Sincronizza_REV_VetInfo_Response)
		...
		If Not IsNothing(lista_RegistroTratt) AndAlso lista_RegistroTratt.Count > 0 Then
			...
			For Each tratt In lista_RegistroTratt
				...
				'If listaTrattamenti.Contains(trattNumero) Or listaTrattamenti.Contains(presNumero) Then
				'    Dim a = 0
				'Else
				'    Continue For
				'End If
						...
						For Each prodPres In lista_ElencoProdPres
							...
							Try
								...
								Dim sincronizzato = Sincronizza_Somministrazioni(Piva, Sa_Cod, Sta_Num, prodPres.presrigaNumero, trattNumero,
																				 ...
																				 presDtEmissione, capiNonTrovati, capiTrovati)

								If sincronizzato Then
									resp.NumeroTrattamentiSincronizzati += 1
								End If
	...
	Private Function Sincronizza_Somministrazioni(ByVal Piva As String,
												  ...
												  ByRef capiTrovati As Integer) As Boolean
				...
				If Not uguali Then
					...
					For Each scaricoSomm In lista_ElencoScarichiSomm
						...
						If True Then
							...
							Dim Magazzino_Cod As Integer = obj_Fabbricati_R.LeggiMagazzino(Piva, Sa_Cod, objP_Server)
							obj_MagazzinoMovimentazione.Magazzino.primaryKey = New anagrafiche.Fabbricato.PK()
							obj_MagazzinoMovimentazione.Magazzino.primaryKey.codice = Magazzino_Cod
							obj_MagazzinoMovimentazione.Magazzino.primaryKey.centroAziendalePK = New anagrafiche.CentroAziendale.PK(Sa_Cod, Piva)

							obj_MagazzinoMovimentazione.Magazzino.tipo = CostantiPersonalizzate.MAGAZZINO

							'se è presente un lotto o una giacenza del farmaco,
							'sincronizza la somministrazione ed esegue lo scarico
							Dim objGiacienzeFarmaco_R As New AgronicaCoreStampeDAL.Magazzino
							Dim dtGiacenzeFarmaco As DataTable = objGiacienzeFarmaco_R.SchedaGiacenzeMagazzino(Date.Now,
																											   Piva,
																											   0, 0,
																											   ...
																											   False, False,
																											   "", False,
																											   True, CStr(scaricoSomm.regscoProdottoAic))

							If Not IsNothing(dtGiacenzeFarmaco) AndAlso dtGiacenzeFarmaco.Rows.Count > 0 Then
								dtGiacenzeFarmaco.Select("WHERE Giacenza >= " &
														 CStr(obj_SommScarico_Capo.quantitaTotaleReale))

								If dtGiacenzeFarmaco.Rows.Count > 0 Then
									Dim lotto As String = dtGiacenzeFarmaco(dtGiacenzeFarmaco.Rows.Count - 1)("Lotto")
									obj_MagazzinoMovimentazione.Lotto = lotto
								Else
									obj_MagazzinoMovimentazione.Lotto = ""
								End If
								obj_SommScarico_Magazzino.MagazziniMovimentazioni.Add(obj_MagazzinoMovimentazione)

								objAttivita.risorse.Add(obj_SommScarico_Magazzino)
							End If
						End If
					Next
									...
									If sospensione.tempososTipoAlimentoCodice = "CARNE" Then
										If tempoSospensioneCarne = 0 Then
											...
											_tempoSospensioneCarne = New TempiSospensione() With {
												...
												.tempoSospensione = sospensione.tempososValore
																 }
										End If
					...
					For Each capo In lista_ElencoCapiSomm
						...
						If capoPresente = False Then
							...
							Dim dtGiacenzeStallaGiornoSuccessivo As DataTable = obj_ZooAnimali_R.Leggi_Giacenze(Piva, Sa_Cod, Sta_Num,
																					0, 0, dataGiacenza,
																					objP_Server)
...
#Region "Prescrizioni"
	...
	Private Function CreateRicetteZooFromPrescrizione(ByVal Piva As String,
		...
		Try
			...
			For Each riga In pres.RighePrescrizione
				...
				End If
				rigaPresToRicettaZooAgenda(riga, ricettaZooAg)
	...
	Private Function Sincronizza_Prescrizioni_Old(ByVal Piva As String,
		...
		For Each pres In listaPrescrizioni
			Dim r = CreateRicetteZooFromPrescrizione(Piva, Sa_Cod, Sta_Num, pres)
	...
	Private Function Sincronizza_Prescrizioni(ByVal Piva As String,
											  ...
											  ByVal Data_Inizio As Date,
											  ByVal Data_Fine As Date,
											  ByVal TipoPrescrizione As String) As List(Of Zoo.Prescrizione)
		' Inizio sincro prescrizioni
		objLog.Scrivi_LOG(objP_Server, MethodBase.GetCurrentMethod().Name,
...
#Region "Scrittura/Modifica Trattamenti"
	...
	''' <returns></returns>
	Private Function Handle_ExistingIndicazioneTerapeutica(ByVal tratt As Trattamento_ToSend) As CreateTrattamentoFromProtocollo_Response
		Dim indTerap = (From r In GiasContext.Ricette
						Join rxa In GiasContext.RicettexAgenda On rxa.Ricetta_Cod Equals r.Ricetta_Cod
						Join mdett In GiasContext.Movimenti_dettagli On mdett.Id_Agenda Equals rxa.Id_Agenda
						Join rzxa In GiasContext.Ricette_ZooxAgenda On rzxa.Id_Agenda Equals rxa.Id_Agenda
						Join rz In GiasContext.Ricette_Zoo On rz.IdRicetta Equals rzxa.Id_Ricetta
						Where Not String.IsNullOrEmpty(mdett.Rif_Esterno) And r.Ricetta_Cod = tratt.IdRicetta
						Select rz.Numero).FirstOrDefault()

		Return New CreateTrattamentoFromProtocollo_Response(Not String.IsNullOrEmpty(indTerap), presNumero:=indTerap)
	End Function
	...
	''' <param name="tratt"></param>
	Private Function SetScortaTrattamentoDaProtocollo(ByRef indTerap As Zoo.Prescrizione,
		...
		Dim rigaPres = indTerap.RighePrescrizione.FirstOrDefault(Function(riga) riga.PresrigaProdottoFamigliaAic = famigliaAic)

		If rigaPres IsNot Nothing Then
			...
			Dim scortaProd_daPrescrizione = ws.Leggi_ScortaProdotto_perSomministrazione({rigaPres.PresrigaNumero}.ToList, {filterAic}.ToList)
			If scortaProd_daPrescrizione IsNot Nothing AndAlso scortaProd_daPrescrizione.Count > 0 Then
	...
	Private Sub LinkTrattamentoToIndicazioneTerapeutica(ByVal indTerap As Zoo.Prescrizione,
		...
		If indTerap.RighePrescrizione.Any(Function(riga) riga.PresrigaProdottoFamigliaAic = famigliaAic) Then
			Dim rigaPres = indTerap.RighePrescrizione.Find(Function(riga) riga.PresrigaProdottoFamigliaAic = famigliaAic)
			tratt.PresRigaNumero = rigaPres.PresrigaNumero

			Dim movZoo = GiasContext.Movimenti_Zoo.Where(Function(mz) mz.Id_Agenda = idAgenda).FirstOrDefault

			' Da rimuovere, da ora scrivera' sempre il movimenti_zoo
			'If movZoo Is Nothing Then
			'	Dim mov = GiasContext.Movimenti.Where(Function(m) m.Id_Agenda = idAgenda AndAlso m.Cau_Mov = CAU_TRATTAMENTO_ZOO).FirstOrDefault
			'	If mov IsNot Nothing Then
			'		movZoo = New Movimenti_Zoo With {
			'			.Piva = mov.PIVA,
			'			.Sa_Cod = mov.Sa_Cod,
			'			.Id_Agenda = idAgenda,
			'			.Id_Mov = mov.Id_Mov,
			'			.Pres_Numero = indTerap.PresNumero,
			'			.PresRiga_Numero = rigaPres.PresrigaNumero,
			'			.Tipo_Trattamento = tratt.Tipo,
			'			.Stato_Trattamento = tratt.Stato,
			'			.Username_Creazione = mov.Username_Creazione,
			'			.Username_Modifica = mov.Username_Modifica
			'		}
			'		GiasContext.Movimenti_Zoo.Add(movZoo)
			'	End If
			'Else
			'End If
			...
			If movZoo IsNot Nothing Then
				...
				movZoo.PresRiga_Numero = rigaPres.PresrigaNumero
			...
			' Linka i record di Agenda del Trattamento creato da Protocollo alla nuova Indicazione Terapeutica generata
			...
			Dim rzxaProt = GiasContext.Ricette_ZooxAgenda.Where(Function(rzxa) rzxa.Id_Agenda = idAgenda).FirstOrDefault
			Dim idRicetta = rzxaProt.Id_Ricetta
			Dim idRigaRicetta = rzxaProt.Id_RigaRicetta

			Dim ricetteZoo_W As New AgronicaCoreContabBIZ.Ricette_Zoo_W
			Dim rzIndTerap = GiasContext.Ricette_Zoo.Where(Function(rz) rz.IdRicetta = idRicetta).FirstOrDefault
			...
			Dim rzaIndTerap = GiasContext.Ricette_Zoo_Agenda.Where(Function(rza) rza.IdRicetta = idRicetta AndAlso rza.IdAgenda = idRigaRicetta).FirstOrDefault
			Dim ricetteZooAg_W As New AgronicaCoreContabBIZ.Ricette_Zoo_Agenda_W
			rzaIndTerap.Numero = rigaPres.PresrigaNumero
	...
	Private Function SendTrattamento_Protocollo(ByRef tratt As Trattamento_ToSend) As Boolean
		...
		Try
			' OLD Verifica se esiste un'Indicazione Terapeutica creata da un altro Trattamento appartenente alla Ricetta corrente
			Dim respIndTerap = New CreateTrattamentoFromProtocollo_Response(False, "") '= Handle_ExistingIndicazioneTerapeutica(tratt)

			Dim newIndTerap = False
			If String.IsNullOrEmpty(tratt.PresNumero) OrElse String.IsNullOrEmpty(tratt.PresRigaNumero) Then
				...
				respIndTerap = ws.Insert_Trattamento_Protocollo_Terapeutico(protNumero, listCapiProtocollo)

				newIndTerap = True
			Else
				respIndTerap = New CreateTrattamentoFromProtocollo_Response(True, tratt.PresNumero, {tratt.PresRigaNumero}.ToList)
			End If
			...
			If respIndTerap.result Then
				...
				If newIndTerap Then LinkTrattamentoToIndicazioneTerapeutica(indTerap, tratt)
				...
				If String.IsNullOrEmpty(tratt.RegScoNumero) Then
					hasScorta = SetScortaTrattamentoDaProtocollo(indTerap, tratt)
				End If
	...
	Private Function SendSomministrazione_InTrattamento(ByRef tratt As Trattamento_ToSend) As Boolean
		...
		Try
			...
			If Not tratt.IsSync Then
				Dim respInsSomm =
					ws.Insert_Somministrazione_In_Trattamento(tratt.PresRigaNumero, tratt.DataInizio,
															  {New Tuple(Of String, Double)(tratt.RegScoNumero, tratt.Quantitativo)}.ToList)
				...
				Else
					tratt.msgErrore =
						If(String.IsNullOrEmpty(tratt.msgErrore), $"Errore inserimento Somministrazione da Prescrizione: {respInsSomm.errore}", tratt.msgErrore)
	...
	Private Function CloseTrattamento(ByRef tratt As Trattamento_ToSend) As Boolean
		...
		If tratt.ToClose Then
			...
			Dim isClosed As Boolean = IsTrattamentoClosed(tratt)
			...
			If Not isClosed Then
				Try
					...
					If {enum_TipoPrescrizione.Da_Protocollo, enum_TipoPrescrizione.Protocollo_Terapeutico, enum_TipoPrescrizione.Da_Protocollo_GIAS}.Contains(tratt.Tipo) Then
						...
						respClose = ws.Close_Trattamento_Protocollo_Terapeutico(presNumero, trattFine, trattStato, trattNote)

					Else
						' Chiude tutte le Somministrazioni legate al Trattamento appena chiuso 
						respClose = ws.Close_Trattamento_Prescrizione(trattNumero, trattFine, trattStato, trattNote)
	...
	''' <returns></returns>
	Public Function ScriviModifica_Trattamenti_VetInfo(ByVal listTrattamenti As List(Of JObject))
		Dim listTrattamenti_ToSend = listTrattamenti.Select(Function(row) New Trattamento_ToSend(row)).ToList

		For Each tratt In listTrattamenti_ToSend
			Dim isSync As Boolean = tratt.IsSync

			If Not isSync Then
				Select Case tratt.Tipo
					Case enum_TipoPrescrizione.Veterinaria, enum_TipoPrescrizione.Da_Protocollo, enum_TipoPrescrizione.Indicazione_Terapeutica
						isSync = SendSomministrazione_InTrattamento(tratt)

					Case enum_TipoPrescrizione.Protocollo_Terapeutico, enum_TipoPrescrizione.Da_Protocollo_GIAS
						isSync = SendTrattamento_Protocollo(tratt)
			...
			If isSync Then Dim closed = CloseTrattamento(tratt)

		Next

		Dim r As New RispostaStandard With {
			.RispostaStringa = GetRespSincro(listTrattamenti_ToSend),
			.RispostaOK = True
		}
		...
		Try
			...
			Else
				...
				If msg <> "" Then
					...
					note = msg
				Else
					If datiSincronizzati.NumeroTrattamentiSincronizzati <> 0 Or datiSincronizzati.NumeroTrattamentiEliminati <> 0 Then
						note &= " Trattamenti sincronizzati: " & datiSincronizzati.NumeroTrattamentiSincronizzati & ". Trattamenti eliminati:" & datiSincronizzati.NumeroTrattamentiEliminati & "."
					End If

					If datiSincronizzati.NumeroProtocolliSincronizzati <> 0 Then
						note &= " Protocolli sincronizzati: " & datiSincronizzati.NumeroProtocolliSincronizzati & ". "
					End If

					If datiSincronizzati.NumeroIndicazioniTerapeuticheSincronizzate <> 0 Then
						note &= " Indicazioni Terapeutiche sincronizzate: " & datiSincronizzati.NumeroIndicazioniTerapeuticheSincronizzate & ". "
					End If

					If datiSincronizzati.NumeroRicetteVeterinarieSincronizzate <> 0 Then
						note &= " Ricette sincronizzate: " & datiSincronizzati.NumeroRicetteVeterinarieSincronizzate & ". "
					End If
...
#End Region
	...
	Public Class Trattamento_ToSend
		...
		Private Function getAicFarmaco(proCod) As String
			Dim aic As String = ""
...
Public Class Sincronizza_REV_VetInfo_Response
	Public NumeroTrattamentiSincronizzati As Integer
	Public NumeroTrattamentiEliminati As Integer

	Public NumeroProtocolliSincronizzati As Integer

	Public NumeroIndicazioniTerapeuticheSincronizzate As Integer

	Public NumeroRicetteVeterinarieSincronizzate As Integer
