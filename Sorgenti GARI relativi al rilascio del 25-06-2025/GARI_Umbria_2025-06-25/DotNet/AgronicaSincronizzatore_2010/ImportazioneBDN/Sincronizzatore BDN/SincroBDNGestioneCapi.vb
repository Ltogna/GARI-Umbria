...
Public Class SincroBDNGestioneCapi
			...
			For Each capoAnimale As DataRow In dtAnimaleDB.Rows
				...
				Try
					...
					ElseIf Not IsDBNull(capoAnimale("Modello4_Ingresso_Numero")) AndAlso
						...
						Dim P_AZIENDA_ORIGINE_CODICE As String = dtAnimaleBDN(0)("AZIENDA_CODICE")
						Dim P_ALLEVAMENTO_ORIGINE_IDFISCALE As String = ""
				...
				If aggiornaDetentoreProprietario Then
					...
					Dim capo_animale_db = (From z In GiasContext.Zoo_Animali Where z.Matricola = Matricola And z.Cod_Progetto = Cod_Animale).FirstOrDefault
	...
	''' <returns></returns>
	Public Function ModificaCapiBDN(ByVal chiave As String) As String
	...
	''' <param name="chiave">Piva_SaCod_CodAnimale</param>
	''' <returns></returns>
	Public Function EliminaCapiBDN(ByVal chiave As String) As String
		Dim chiaveArr() As String = JsonConvert.DeserializeObject(Of String())(chiave)
			...
			If Not IsNothing(risposta) AndAlso risposta.Tables.Count > 0 Then
				...
				If idUscita <> 0 Then
					...
					Dim movDettaglioScarico = (From g In GiasContext.Movimenti_dettagli
											   Where g.PIVA = PivaScarico And g.Id_Agenda = ID_AgendaScarico And
											   g.Id_Mov = ID_MovScarico And g.Id_Mov_Det = ID_Mov_DetScarico).FirstOrDefault
		...
		For Each idAgenda In Operazioni_Agenda
			...
			Try
				...
				If listaMatricoleCapi.Count > 0 Then
					...
					Dim P_VET_AZIENDALE As String = "" 'nome e cognome del veterinario aziendale
					Dim P_INDIRIZZO As String = "" 'indirizzo del veterinario aziendale
					Dim P_TELEFONO As String = "" 'telefono del veterinario aziendale
					Dim P_ISTAT As String = "" 'codice istat comune del veterinario aziendale
					Dim P_SIGLA As String = "" 'sigla provincia del veterinario aziendale
					Dim P_NUM_ISCR_ALBO As String = "" 'num di iscrizione dell'albo del veterinario aziendale
					...
					If FlagTipoTrasporto = "S" Or FlagTipoTrasporto = "N" Then
					...
					If Not IsNothing(dtModelloInserito) AndAlso dtModelloInserito.Rows.Count > 0 Then
						...
						If Not IsNothing(dtModello4) AndAlso dtModello4.Rows.Count > 0 Then
							...
							objModelli4.giorniValdita = IIf(dtModello4.Columns.Contains("GIORNI_VALIDITA"), dtModello4.Rows(0).Item("GIORNI_VALIDITA"), 0)
							...
							Dim dataConferma As Date = IIf(dtModello4.Columns.Contains("DT_CONFERMA_ASL"), dtModello4.Rows(0).Item("DT_CONFERMA_ASL"), AGRODATAINIZIO)
	...
	''' <summary>
	''' TODO, non ancora implementata
	''' </summary>
	''' <param name="Piva"></param>
	''' <param name="Sa_Cod"></param>
	''' <param name="Operazioni_Agenda"></param>
	''' <returns></returns>
	'Public Function ModificaDati_Modello4(ByVal Piva As String,
	'								 ByVal Sa_Cod As Integer,
	'								 ByVal Operazioni_Agenda As List(Of Integer)) As List(Of RegistraModello4Response)
	'	Dim listaModelli4Response As New List(Of RegistraModello4Response)
	...
	Public Function downloadModelli4Selezionati(ByVal chiave As List(Of ChiaveCaricoScaricoCapi),
		...
		For Each animale In chiave
			...
			Try
				...
				If prenotazioneId <> "" And Not prenotazioneId_Scaricati.Contains(prenotazioneId) Then
	...
	Public Function gestioneDownloadAllegato(ByVal pathsPdf As List(Of String),
		...
		Dim fileManager As New FileManager(pathTemp, "")

		Dim rec As New Report_Excel_Costi
		...
		ElseIf pathsPdf.Count > 1 Then
			...
			fileManager.aggiungiPiuFileAZip(pathsPdf)
			fileManager.salvaFileZip(pathFileZip)
			pathResp = pathFileZip

			Estensione = ".zip"

			rec.eliminaFiles(pathsPdf)
		End If
		rec = Nothing
