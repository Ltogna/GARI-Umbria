...
#End Region
	...
	Public Sub New(objParametri_Server As AgronicaCoreParametri,
		...
		Dim sequenze As New AgronicaCoreDataProvider.Agro_Sequenze
		If Username = "" Then
		...
		Me.logDirectory = Me.objParametri_Server.LogDirectory & "\BDN\"
		Me.logFileName = "chiamateVetInfo.txt"
	...
	Protected Function callVetInfo(ByVal uri As String,
								   ...
								   Optional number As Integer = 0) As String
	...
	Protected Function callVetInfoWrite(ByVal uri As String,
		...
		objLog.Scrivi_LOG(objParametri_Server, MethodBase.GetCurrentMethod().Name,
		...
		Dim responseStrRestShapr = client.chiamaWS_RestShapr(content, "", complete_uri,
															 ...
															 "", customHeader, UseNewtonsoftJson:=True)
		responseStr = responseStrRestShapr.Content

		objLog.Scrivi_LOG(objParametri_Server, MethodBase.GetCurrentMethod().Name,
								  "Risposta:" & vbCrLf & responseStr, CustomLOGParams:=customLOGParams)
...
#Region "Chiamate Lettura"
	...
	Public Function RegistroPrescrizioni(ByVal Azienda_Codice As String,
		...
		If paginations Is Nothing And filters Is Nothing Then
			getAllData = True
		End If
		...
		If IsNothing(objResp_RegistroTratt.errors) OrElse objResp_RegistroTratt.errors.Count = 0 Then
			...
			If Not IsNothing(objResp_RegistroTratt.data) AndAlso objResp_RegistroTratt.data.Count > 0 Then
				objResp_RegistroTratt.data.Sort(Function(tratt1, tratt2)
													Return tratt1.tratDtInizio < tratt2.tratDtInizio
												End Function)
	...
	''' <returns></returns>
	Public Function Ricava_FarmaciInMagazzino(ByVal presrigaNumero As String,
	...
	''' <param name="paginations"></param>
	''' <returns></returns>
	Public Function RegistroDisponibilitaMedicinali(ByVal regscoAziendaCodice As String,
													ByVal regscoProprietarioIdFiscale As String,
													Optional ByVal filters As List(Of VetInfoModel.VetInfoFilter) = Nothing,
...
#Region "Chiamate Scrittura"
	...
	''' <returns></returns>
	Public Function Insert_Trattamento_Protocollo_Terapeutico(ByVal Pres_Numero As String,
		...
		If objResp.data.Count = 1 Then Return New CreateTrattamentoFromProtocollo_Response(True, objResp.data(0).presNumero, objResp.data(0).presrigaNumeri, objResp.data(0).presrigaProdottiFamiglieAic)

		Return New CreateTrattamentoFromProtocollo_Response(False, objResp.errors(0).field & " - " & objResp.errors(0).message)
	...
	''' <param name="Note"></param>
	''' <returns></returns>
	Public Function Insert_Trattamento_Protocollo_Semplificato(ByVal Pres_Numero As String,
	...
	''' <returns></returns>
	Public Function Insert_Somministrazione_In_Trattamento(ByVal PresRiga_Numero As String,
														   ByVal Somm_Data As Date,
														   ByVal RegSco_Scarichi As List(Of Tuple(Of String, Double)),
														   Optional ByVal Somm_Note As String = "") As InsertTrattamento_Response
		Dim dataToSend As New Insert_Somministrazione(PresRiga_Numero, Somm_Data, RegSco_Scarichi, Note:=Somm_Note)
		Dim strToSend As String = JsonConvert.SerializeObject(dataToSend, formatting:=Newtonsoft.Json.Formatting.None)
		Dim jobjToSend = JsonConvert.DeserializeObject(strToSend)
		...
		If objResp.data Is Nothing Then Return New InsertTrattamento_Response(False, objResp.errors(0).field & " - " & objResp.errors(0).message)

		If objResp.data.Count = 1 Then Return New InsertTrattamento_Response(True, objResp.data(0).tratNumero, objResp.data(0).somNumero)

		Return New InsertTrattamento_Response(False, objResp.errors(0).field & " - " & objResp.errors(0).message)
	...
	Public Function Close_Trattamento_Protocollo_Terapeutico(ByVal Pres_Numero As String,
		...
		If objResp Is Nothing Then Return New CloseTrattamento_Response(False, "Errore non specificato durante la chiusura del trattamento.")

		If objResp.data.Count > 0 Then Return New CloseTrattamento_Response(True)

		Return New CloseTrattamento_Response(False, objResp.errors(0).field & " - " & objResp.errors(0).message)

	End Function

	Public Function Close_Trattamento_Prescrizione(ByVal Tratt_Numero As String,
												   ByVal Tratt_Fine As String,
												   ByVal Tratt_Stato As String,
												   ByVal Note As String) As CloseTrattamento_Response
