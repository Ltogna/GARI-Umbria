...
Public Class Impresa_Progetti_R
    ...
    Public Function Leggi_x_anagraficaNG(ByVal Piva As String,
        ...
        Dim versioneSql As Integer = VersioneSqlServer_anno(objParametri)
        ...
        Try
            ...
            If sql2017 Then
                ...
                StrSQL.AppendLine("    SELECT Piva, Sa_Cod, Appezza, STRING_AGG(ZVN, ',') AS ZVN FROM (")
            ...
            StrSQL.AppendLine("    SELECT *")
            StrSQL.AppendLine("    FROM Reg_Impianti_Codici (nolock)")
            StrSQL.AppendLine("    WHERE 1 = 1")
            If Piva <> "" Then
            ...
            StrSQL.AppendLine("    SELECT *")
            StrSQL.AppendLine("    FROM Appezzamento_Codici (nolock)")
            StrSQL.AppendLine("    WHERE 1 = 1")
            If Piva <> "" Then
                StrSQL.AppendLine("        AND Piva = '" & Agro_SQL_SaveText(Piva) & "'")
            End If
            ...
            If readLinkedMachines Then
                StrSQL.AppendLine(", #AxP AS (")
                StrSQL.AppendLine("    SELECT axp.*, pm.Mac_Des")
                StrSQL.AppendLine("    FROM AppezzamentiXParcoMacchine axp")
                StrSQL.AppendLine("        JOIN Parco_Macchine pm ON pm.Mac_Cod = axp.Mac_Cod")
                StrSQL.AppendLine(")")
            End If

            StrSQL.AppendLine(", #PxACA AS (")
            StrSQL.AppendLine("    SELECT pxc.*, c.ContributoDes")
            StrSQL.AppendLine("    FROM Imprese_ProgettiXContributi pxc")
            StrSQL.AppendLine("        JOIN Imprese_Progetti p ON p.Progetto_Cod = pxc.ProgettoCod")
            StrSQL.AppendLine("            AND p.Piva = pxc.Piva")
            ...
            If leggiDatiRibaltamento Then
                StrSQL.AppendLine("    , ISNULL((SELECT ''''+ ba.Nome_Budget + ''' - ' +  CONVERT(VARCHAR(30), Data_Ribaltamento, 103)")
                StrSQL.AppendLine("        FROM Ribaltamento_Imprese_Progetti rip 		   ")
                StrSQL.AppendLine("        JOIN Budget_Testata ba on ba.Id_Budget = rip.Budget_Id_Testata ")
                StrSQL.AppendLine("        WHERE rip.Reale_Piva = Imprese_Progetti.Piva ")
                StrSQL.AppendLine("        AND rip.Reale_Sa_Cod = Imprese_Progetti.Sa_Cod ")
                StrSQL.AppendLine("        AND rip.Reale_Appezza= Imprese_Progetti.Appezza ")
                StrSQL.AppendLine("        AND rip.Reale_Id_Reg= Imprese_Progetti.Id_Reg ")
                StrSQL.AppendLine("        AND rip.Reale_Progetto_Cod = Imprese_Progetti.Progetto_Cod")
            ...
            If readLinkedMachines Then
                ...
                StrSQL.AppendLine("	       (")
                StrSQL.AppendLine("	           SELECT '  ' + axp.Mac_Des +")
                StrSQL.AppendLine("                (")
                StrSQL.AppendLine("                    CASE ")
                StrSQL.AppendLine("                        WHEN CAST(axp.Validita_Inizio AS DATE) = '1900-01-01' ")
                StrSQL.AppendLine("                        THEN ''")
                StrSQL.AppendLine("                        ELSE ' : ' + CONVERT(VARCHAR(10), Validita_Inizio, 121)")
                StrSQL.AppendLine("                    END +")
                StrSQL.AppendLine("                    CASE ")
                StrSQL.AppendLine("                        WHEN CAST(axp.Validita_Fine AS DATE) = '2100-12-31' ")
                StrSQL.AppendLine("                        THEN ''")
                StrSQL.AppendLine("                        ELSE ' - '  + CONVERT(VARCHAR(10), validita_fine, 121)")
                StrSQL.AppendLine("                    END")
                StrSQL.AppendLine("                )")

                StrSQL.AppendLine("	           FROM #AxP axp")
                StrSQL.AppendLine("	           WHERE axp.Piva = Imprese_Progetti.Piva")
                StrSQL.AppendLine("	               AND axp.Sa_Cod = Imprese_Progetti.Sa_Cod")
                StrSQL.AppendLine("	               AND axp.Appezza = Imprese_Progetti.Appezza")
                StrSQL.AppendLine("	           FOR XML PATH(''), TYPE")
                StrSQL.AppendLine("	       ).value('.', 'nvarchar(max)'), 1, 1, ''")
                StrSQL.AppendLine("	   ), '') AS LinkedMachines")
            End If

            StrSQL.AppendLine("	   , COALESCE(STUFF (")
            StrSQL.AppendLine("	       (")
            StrSQL.AppendLine("	           SELECT '  ' + pxa.ContributoDes")
            StrSQL.AppendLine("	           FROM #PxACA pxa")
            StrSQL.AppendLine("	           WHERE pxa.Piva = Imprese_Progetti.Piva")
            StrSQL.AppendLine("	               AND pxa.ProgettoCod = Imprese_Progetti.Progetto_Cod")
            StrSQL.AppendLine("	           FOR XML PATH(''), TYPE")
            StrSQL.AppendLine("	       ).value('.', 'nvarchar(max)'), 1, 1, ''")
            StrSQL.AppendLine("	   ), '') AS LinkedACAContributes")
            ...
            StrSQL.AppendLine("    , COALESCE(a.LIMO, 0) AS Slit")
...
#Region "Modifica Multipla Piano Colturale"
    ...
    Public Function Leggi_per_ModificaMultipla(listChiavi As List(Of (String, Integer, Integer, Integer, Integer)),
        ...
        Try
            ...
            StrSQL.AppendLine($"     , ISNULL(MetodoProduzione.Val_Cod, '{CInt(enum_MetodoProduzione.Convenzionale)}') AS MetodoProduzione ")
