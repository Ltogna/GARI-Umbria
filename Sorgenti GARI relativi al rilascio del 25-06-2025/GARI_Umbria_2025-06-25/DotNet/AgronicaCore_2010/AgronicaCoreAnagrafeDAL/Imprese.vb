...
Public Class Imprese_Read
    ...
    Public Function DatiIntestazioneImpresa(ByVal Piva As String,
        ...
        Try
            ...
            StrSql = ""
            StrSql = "  SELECT       Impresa.PIVA, Impresa.rag_soc, ISNULL(Impresa.Forma_Giuridica, 0) Forma_Giuridica_Cod, ISNULL(FormeGiuridiche.FG_Des, '') AS Forma_Giuridica_Des, Impresa.TipoImpresaGerarchia, Impresa.Validita_Inizio, Impresa.Validita_Fine,        "
    ...
    Public Function Leggi_x_anagrafica(ByVal Piva As String,
        ...
        Try
            ...
            StrSQL.Length = 0
            StrSQL.AppendLine(" SELECT distinct i.piva as chiave, i.piva,  i.rag_soc, i.Data_Creazione, i.Data_Modifica, i.Validita_Inizio, i.Validita_Fine, ")
    ...
    Public Function Leggi_x_anagraficaVisibilita_Utente_NG(
                             ByVal Piva As String,
                                 ...
                                 ByRef objParametri_Utenti As AgronicaCoreParametri,
                                 ByVal Filtro_Visibilita_Utente As Boolean,
                                 Optional ByRef leggiSuperfici As Boolean = False
                                 ) As DataTable
        ...
        Try
            ...
            If leggiSuperfici Then
                ...
                StrSQL.AppendLine("  	SELECT  Piva, SUM(Sup_Condotta) as Sup_Catastale  ")
                StrSQL.AppendLine("  	FROM    ImpreseXParticelle (NOLOCK) ")
                StrSQL.AppendLine("      WHERE ImpreseXParticelle.Validita_inizio <=  " & Agro_SQL_SaveDate(objParametri_Server.FinestraTemporaleFine) & "   ")
                StrSQL.AppendLine("      AND   ImpreseXParticelle.Validita_Fine >=  " & Agro_SQL_SaveDate(objParametri_Server.FinestraTemporaleInizio) & "   ")
                If Piva <> "" Then
                ...
                StrSQL.AppendLine("      WHERE Appezzamento.Validita_inizio <=  " & Agro_SQL_SaveDate(objParametri_Server.FinestraTemporaleFine) & "   ")
                StrSQL.AppendLine("      AND   Appezzamento.Validita_Fine >=  " & Agro_SQL_SaveDate(objParametri_Server.FinestraTemporaleInizio) & "   ")
                If Piva <> "" Then
                    StrSQL.AppendLine("      AND   Appezzamento.Piva = '" & Agro_SQL_SaveText(Piva) & "'   ")
                End If
                StrSQL.AppendLine("  	GROUP BY Appezzamento.PIVA  ")
                StrSQL.AppendLine("  )  ")
            End If
            StrSQL.AppendLine(" SELECT distinct ISNULL(padre.piva, '') + '_' + i.piva as chiave, i.piva,  i.rag_soc, i.Data_Creazione, i.Data_Modifica, i.Validita_Inizio, i.Validita_Fine, ")
            StrSQL.AppendLine(" ind_des, frz_des, com_des, pro_cod, ind.Cap,  ")
            StrSQL.AppendLine(" CASE Stato WHEN '' THEN 'IT' WHEN 'ITALIA' THEN 'IT' ELSE stato END as Stato_Cod,  ")
            StrSQL.AppendLine(" ISNULL(ACCDAA_ANAG_T005_TabellaCodiciPaesiTerziISO3166.Descrizione, 'Italia') as Stato, ")
            StrSQL.AppendLine(" ind_des + ' ' + ind.Cap + ' ' + com_des + ' ' + pro_cod + ' ' + Stato AS Indirizzo, ii.Tipo_Indirizzo,    ")
            ...
            StrSQL.AppendLine(" IIF(coalesce(cuaaPadre.val_cod, '') = '', padre.Rag_Soc, padre.Rag_Soc + ' (' + coalesce(cuaaPadre.val_cod, '') + ')') as Rag_Soc_Padre, ")
            StrSQL.AppendLine(" GerarchiaImprese.LibroSoci_Codice as codice_iscrizione_libro_soci, ")
            StrSQL.AppendLine(" GerarchiaImprese.LibroSoci_DataIscrizione as data_iscrizione_libro_soci, ")
            ...
            StrSQL.AppendLine(" FROM Imprese  (nolock)  i ")
            If (leggiSuperfici) Then
            ...
            End If
            StrSQL.AppendLine(" LEFT JOIN GerarchiaImprese  (nolock)  ON i.Piva = GerarchiaImprese.Figlio ")
            StrSQL.AppendLine(" LEFT JOIN Imprese  (nolock)  padre ON GerarchiaImprese.Padre = padre.PIVA ")

            StrSQL.AppendLine(" LEFT JOIN Imprese_Codici  (nolock) cuaa ON i.piva = cuaa.PIVA AND Cuaa.id_cod = 1010 ")
            StrSQL.AppendLine(" LEFT JOIN Imprese_Codici  (nolock) socio ON i.piva = socio.PIVA AND socio.id_cod = 1033 ")
            StrSQL.AppendLine(" LEFT JOIN Imprese_Codici  (nolock) contrattoProduzione ON i.piva = contrattoProduzione.PIVA AND contrattoProduzione.id_cod = 1324 ")
            StrSQL.AppendLine(" LEFT JOIN Imprese_Codici  (nolock) cuaaPadre ON padre.piva = cuaaPadre.PIVA AND cuaaPadre.id_cod = 1010 ")
            'StrSQL.AppendLine(" LEFT JOIN Imprese_Codici  (nolock) tecnico ON i.Piva = tecnico.Piva AND tecnico.id_cod = 1088 ")
            'StrSQL.AppendLine(" LEFT JOIN Contatti  (nolock) ON tecnico.Val_Cod = Contatti.Cod_Contatto ")
            'StrSQL.AppendLine(" LEFT JOIN Contatti  (nolock) fiscale ON i.PIVA = fiscale.Cod_Contatto ")
            StrSQL.AppendLine(" LEFT JOIN Utenti  (nolock) creazione ON i.Username_Creazione = creazione.CODICE_FISCALE ")
            StrSQL.AppendLine(" LEFT JOIN Utenti  (nolock) modifica ON i.Username_Modifica = modifica.CODICE_FISCALE ")

            StrSQL.AppendLine(" JOIN ImpresexIndirizzi  (nolock)  ii ON ii.piva = i.piva ")
            StrSQL.AppendLine(" JOIN Indirizzi  (nolock) ind ON ind.cod_indirizzo = ii.cod_indirizzo  ")
            StrSQL.AppendLine(" JOIN ISTAT  (nolock) ON ISTAT.PROV = ind.pro_cod_istat AND ISTAT.COM = ind.com_cod_istat  ")
            StrSQL.AppendLine(" LEFT JOIN Lista_Province (nolock)  ON ind.pro_cod_istat = Lista_Province.PROV ")
            StrSQL.AppendLine(" LEFT JOIN ACCDAA_ANAG_T005_TabellaCodiciPaesiTerziISO3166  (nolock) ON ACCDAA_ANAG_T005_TabellaCodiciPaesiTerziISO3166.Codice = ind.Stato ")
            StrSQL.AppendLine(" LEFT JOIN Gruppi_Raccolta (nolock) ON i.GruppoRaccolta_Cod = Gruppi_Raccolta.GruppoRaccolta_Cod ")
            If Filtro_Visibilita_Utente Then
                StrSQL.AppendLine(" LEFT JOIN Utenti_Visibilita_Appoggio (NOLOCK) On i.Piva = Utenti_Visibilita_Appoggio.Piva AND Utenti_Visibilita_Appoggio.Entita_Cod=1 AND Utenti_Visibilita_Appoggio.Username = " & Agro_SQL_SaveText_NULL(objParametri_Server.UtenteUsername) & " ")
            End If
    ...
    Public Function RecuperaDatiImpresa_From_Piva(ByVal Piva As String,
        ...
        Try
            ...
            StrSQL.Length = 0
            StrSQL.AppendLine(" SELECT Imprese.PIVA, Imprese.rag_soc, Imprese.TipoImpresaGerarchia,  ")
