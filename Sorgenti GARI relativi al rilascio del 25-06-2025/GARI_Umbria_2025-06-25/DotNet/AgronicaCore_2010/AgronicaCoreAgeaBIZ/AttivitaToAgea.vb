...
Public Class AttivitaToAgea
    ...
    Public Function MapAttivitatoAgea(ByVal Piva As String, ByVal CUAA As String, ByVal Anno As Integer, ByVal Rag_Soc As String,
        ...
        Try
            ...
            DT_Lavorazioni = objMetaschemaDAL.Leggi(0, Sistema_Cod_Agea, "", 0, AGRODATAINIZIO, AGRODATAFINE, "", "", ObjParametri_Server)
            ...
            DT_Appezzamenti_Impianti = objAnagrafeDAL.Leggi_Join_Appezzamento_Esercizio(Piva, Validita_Inizio, Validita_Fine, "", "", ObjParametri_Server)

            Dim Dict_Attivita As New Dictionary(Of String, Attivita)

            Dim xFiltroAggiuntivo As String = "Agenda.Lav_Cod IN (" & String.Join(",", List_Lav_Cod) & ") AND Agenda.Validita_Inizio <= " & Agro_SQL_SaveDate(Validita_Fine, False) & " AND Agenda.Validita_Inizio >= " & Agro_SQL_SaveDate(Validita_Inizio, False)
            ...
            If Not IsNothing(DT_Agende) AndAlso DT_Agende.Rows.Count > 0 Then
                For Each Dr In DT_Agende.Rows
                    If Not Dict_Attivita.ContainsKey(Create_operationId(Dr("Piva"), Dr("Id_Agenda"))) Then
                        ...
                        If Not IsNothing(agenda) AndAlso agenda.Id_Agenda > 0 Then
                            ...
                            If Not IsNothing(attivita) Then
                                Dict_Attivita.Add(Create_operationId(Dr("Piva"), Dr("Id_Agenda")), attivita)
                            End If
            ...
            bundle = MapBundle(Dict_Attivita, Piva, CUAA, Anno, Rag_Soc)
    ...
    Private Function MapSupply(ByVal Dict_Attivita As Dictionary(Of String, Attivita), ByVal Piva As String, ByVal Anno As Integer, ByVal Rag_Soc As String) As Supply
        ...
        If checkBundle.ContinueExport() Then
            ...
            Dim tp_plots_Rilievi_FF As New List(Of Tuple(Of DataRow, Plot, EsercizioCDC))

            Dim List_Distinct_Impianti As New List(Of EsercizioCDC)
            ...
            For index As Integer = 0 To Dict_Attivita.Values.Count - 1
                ...
                If IsNothing(DT_Avversita) AndAlso List_Lav_Cod_With_Adversity.Contains(Lav_Cod) Then
                    ...
                    DT_Avversita = objMetaschemaDAL.Leggi3(0, 0, 0, 0, AGRODATAINIZIO, AGRODATAFINE, enumSelezioneVariabile.Selezione_TabellaCompleta,
                                                                                "", "", ObjParametri_Server)
                End If
                ...
                Dim tp_plots As New List(Of Tuple(Of DataRow, Plot, EsercizioCDC))
                ...
                If Not IsNothing(attivita.centriDiCosto) AndAlso attivita.centriDiCosto.Count > 0 Then
                    ...
                    For Each centroCdC In attivita.centriDiCosto
                        If centroCdC.classType = ClassType.EsercizioCDC Then
                            ...
                            Dim plot = MapPlot(esercizioCdC, Dr)

                            Dim tp_plot = Tuple.Create(Dr, plot, esercizioCdC)
                            ...
                            If List_Distinct_Impianti.FindIndex(Function(imp) imp.esercizio.codice = esercizioCdC.esercizio.codice AndAlso
                                                                            imp.esercizio.impiantoPK.codice = esercizioCdC.esercizio.impiantoPK.codice AndAlso
                                                                            imp.esercizio.impiantoPK.appezzamentoPK.codice = esercizioCdC.esercizio.impiantoPK.appezzamentoPK.codice AndAlso
                                                                             imp.esercizio.impiantoPK.appezzamentoPK.centroAziendalePK.codice = esercizioCdC.esercizio.impiantoPK.appezzamentoPK.centroAziendalePK.codice AndAlso
                                                                             imp.esercizio.impiantoPK.appezzamentoPK.centroAziendalePK.partitaIva = esercizioCdC.esercizio.impiantoPK.appezzamentoPK.centroAziendalePK.partitaIva) = -1 Then

                                List_Distinct_Impianti.Add(esercizioCdC)

                                checkBundle.CheckPlot(plot)
                            ...
                            If Not IsNothing(attivita.risorse) AndAlso attivita.risorse.Count > 0 Then
                                ...
                                Select Case Lav_Cod
                                    Case LAVCOD_TRATTAMENTO_ANTIPARASSITARIO,
                                        ...
                                        supply.phytochemicalTreatments.AddRange(MapPhytochemicalTreatments(operationId, operationDesLib, tp_plot.Item2, tp_plot.Item1, tp_plot.Item3,
                                    ...
                                    Case LAVCOD_DISTRIBUZIONE_CONCIME, LAVCOD_SARCHIATURA_CONCIMAZIONE, LAVCOD_CONCIMAZIONE_FOGLIARE, LAVCOD_FERTIRRIGAZIONE, LAVCOD_TRATTAMENTO_ANTIBUTTERATURA
                                        supply.chemicalFertilizations.AddRange(MapChemicalFertilizations(operationId, operationDesLib, tp_plot.Item2, tp_plot.Item1, tp_plot.Item3,
                                                                                                         attivita.inizio, attivita.oraInizio, attivita.risorse, attivita.modalitaApplicazione))

                                    Case LAVCOD_DISTRIBUZIONE_AMMENDANTI
                                        supply.organicFertilizations.AddRange(MapOrganicFertilizations(operationId, operationDesLib, tp_plot.Item2, tp_plot.Item1, tp_plot.Item3, attivita.inizio, attivita.oraInizio, attivita.risorse, attivita.modalitaApplicazione))
                                End Select
                            End If
    ...
    Private Function MapPhytochemicalTreatment(ByVal operationId As String, ByVal operationDesLib As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow,
                                               ByVal esercizioCDC As EsercizioCDC, ByVal anno As Integer, ByVal data_operazione As Date,
        ...
        Dim obj_Qta_Prodotto_X_Impianto = GetProductAndWaterQuantityFromPlot(operationId, risorsaDettaglioTrattamento, Dr_Appezzamento_Impianto, esercizioCDC, risorsaAcqua)
        ...
        If Not IsNothing(DT_Avversita) Then
            If Not IsNothing(risorsaDettaglioTrattamento.avversitaGruppo) Then

                Select Case risorsaDettaglioTrattamento.avversitaGruppo.classType
                    Case ClassType.Avversita

                        Dim Dr = DT_Avversita.Select("Av_Cod = " & risorsaDettaglioTrattamento.avversitaGruppo.codice)
                    ...
                    Case ClassType.GruppoAvversita
                        ...
                        Dim Dr = DT_Avversita.Select("Av_Gru = " & risorsaDettaglioTrattamento.avversitaGruppo.codice)
        ...
        phytochemicalTreatment.adversityName = adversityName
    ...
    Private Function MapPhytochemicalTreatments(ByVal operationId As String, ByVal operationDesLib As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow,
                                                ByVal esercizioCDC As EsercizioCDC, anno As Integer, ByVal data_operazione As Date, ByVal ora_operazione As Date,
        ...
        If Not IsNothing(risorseDettaglioTrattamento) AndAlso risorseDettaglioTrattamento.Count > 0 Then
            ...
            For Each risorsaDettaglioTrattamento In risorseDettaglioTrattamento
                ...
                If checkBundle.CheckIfProductIsExportable(risorsaDettaglioTrattamento) Then
                    phytochemicalTreatments.Add(MapPhytochemicalTreatment(operationId, operationDesLib, plot, Dr_Appezzamento_Impianto, esercizioCDC, anno,
    ...
    Private Function MapGrowthStages(ByVal Piva As String, ByVal tp_plots_Rilievi_FF As List(Of Tuple(Of DataRow, Plot, EsercizioCDC)), ByVal IDTestataTemp As Integer, ByVal objParametri_Server As AgronicaCoreParametri) As List(Of GrowthStage)
        ...
        If Not IsNothing(List_Validita) AndAlso List_Validita.Count > 0 Then
            For Each validita_rilievi_FF In List_Validita
                Dim index = tp_plots_Rilievi_FF.FindIndex(Function(t_p) t_p.Item1("Piva") = validita_rilievi_FF.Piva AndAlso t_p.Item1("Sa_Cod") = validita_rilievi_FF.Sa_Cod AndAlso
                                                  t_p.Item1("Appezza") = validita_rilievi_FF.Appezza AndAlso t_p.Item1("ID_REG") = validita_rilievi_FF.Id_Destinazione)

                If index > -1 Then
                    growthStages.Add(MapGrowthStage(tp_plots_Rilievi_FF(index).Item2, validita_rilievi_FF))
                End If
            Next
        End If
    ...
    Private Function MapPlots(ByVal List_EserciziCdC As List(Of EsercizioCDC)) As List(Of Plot)

        Dim plots As New List(Of Plot)

        For Each esercizioCdC In List_EserciziCdC
            plots.Add(MapPlot(esercizioCdC, Nothing))
        Next
    ...
    Private Function MapPlot(ByVal esercizioCdC As EsercizioCDC, ByRef Dr_Appezzamento_Impianto As DataRow) As Plot

        Dim plot As Plot = Nothing

        Dim plot_key As Plot_Key = MapPlot_Key(esercizioCdC, Dr_Appezzamento_Impianto)
    ...
    Private Function MapPlot_Key(ByVal esercizioCdC As EsercizioCDC, ByRef Dr_Appezzamento_Impianto As DataRow) As Plot_Key
        ...
        Dim Piva As String = esercizioCdC.esercizio.impiantoPK.appezzamentoPK.centroAziendalePK.partitaIva

        Dim Sa_Cod As Integer = esercizioCdC.esercizio.impiantoPK.appezzamentoPK.centroAziendalePK.codice

        Dim Appezza As Integer = esercizioCdC.esercizio.impiantoPK.appezzamentoPK.codice

        Dim Id_Reg As Integer = esercizioCdC.esercizio.impiantoPK.codice

        Dim Dr = DT_Appezzamenti_Impianti.Select("Piva = '" & Piva & "' And Sa_Cod = " & Sa_Cod & " And Appezza = " & Appezza & " And Id_Reg = " & Id_Reg)

        If Not IsNothing(Dr) AndAlso Dr.Length = 1 Then
            Dr_Appezzamento_Impianto = Dr(0)
            plotId = IIf(IsDBNull(Dr(0)("Agea_identificativoAppezzamento")), "", Dr(0)("Agea_identificativoAppezzamento"))
            islandId = IIf(IsDBNull(Dr(0)("Agea_identificativoIsola")), "", Dr(0)("Agea_identificativoIsola"))
            pcgId = IIf(IsDBNull(Dr(0)("Agea_identificativoPianoColtivazione")), "", Dr(0)("Agea_identificativoPianoColtivazione"))
            plantationId = IIf(IsDBNull(Dr(0)("Agea_idColt")), "", Dr(0)("Agea_idColt"))
            codiBarrScheVali = IIf(IsDBNull(Dr(0)("Agea_codiBarrScheVali")), "", Dr(0)("Agea_codiBarrScheVali"))
        End If
    ...
    Private Function MapIrrigations(ByVal operationId As String, ByVal Piva As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow, ByVal data_operazione As Date,
        ...
        If risorse.FindIndex(Function(r) r.classType = ClassType.DettaglioIrrigazione) > -1 Then
            ...
            If Not IsNothing(risorseDettaglioIrrigazione) AndAlso risorseDettaglioIrrigazione.Count > 0 Then
                Dim index = risorseDettaglioIrrigazione.FindIndex(Function(r) Not IsNothing(r.esercizioCDC) AndAlso
                                                                                ...
                                                                                r.esercizioCDC.esercizio.impiantoPK.codice = Dr_Appezzamento_Impianto("Id_Reg") AndAlso
                                                                                r.esercizioCDC.esercizio.codice = Dr_Appezzamento_Impianto("Progetto_Cod"))

                If index > -1 Then

                    Dim irrigation As Irrigation = Nothing

                    Dim risorsaDettaglioIrrigazione As dettagli.DettaglioIrrigazione = risorseDettaglioIrrigazione(index)
    ...
    Private Function MapIrrigation(ByVal operationId As String, ByVal Piva As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow, ByVal data_irrigazione As Date,
        ...
        'TODO Sarà da esportare l'ora per l'irrigazione quando verrà aggiunta da interfaccia
    ...
    Private Function MapChemicalFertilizations(ByVal operationId As String, ByVal operationDesLib As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow, ByVal esercizioCDC As EsercizioCDC, ByVal data_operazione As Date,
        ...
        If risorse.FindIndex(Function(r) r.classType = ClassType.DettaglioFertilizzazione) > -1 Then
            ...
            If Not IsNothing(risorseDettaglioFertilizzazione) AndAlso risorseDettaglioFertilizzazione.Count > 0 Then
                For Each risorsaDettaglioFertilizzazione In risorseDettaglioFertilizzazione
                    ...
                    chemicalFertilizations.Add(MapChemicalFertilization(operationId, operationDesLib, plot, Dr_Appezzamento_Impianto, esercizioCDC, data_operazione, ora_operazione, risorsaDettaglioFertilizzazione, risorsaAcqua, modalitaApplicazione))
                Next
            End If
    ...
    Private Function MapChemicalFertilization(ByVal operationId As String, ByVal operationDesLib As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow, ByVal esercizioCDC As EsercizioCDC,
                                              ByVal data_operazione As Date, ByVal ora_operazione As Date,
        ...
        Dim obj_Qta_Prodotto_X_Impianto = GetProductAndWaterQuantityFromPlot(operationId, risorsaDettaglioFertilizzazione, Dr_Appezzamento_Impianto, esercizioCDC, risorsaAcqua)
    ...
    Private Function MapOrganicFertilizations(ByVal operationId As String, ByVal operationDesLib As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow, ByVal esercizioCDC As EsercizioCDC, ByVal data_operazione As Date,
        ...
        If risorse.FindIndex(Function(r) r.classType = ClassType.DettaglioFertilizzazione) > -1 Then
            ...
            If Not IsNothing(risorseDettaglioFertilizzazione) AndAlso risorseDettaglioFertilizzazione.Count > 0 Then
                For Each risorsaDettaglioFertilizzazione In risorseDettaglioFertilizzazione
                    ...
                    organicFertilizations.Add(MapOrganicFertilization(operationId, operationDesLib, plot, Dr_Appezzamento_Impianto, esercizioCDC, data_operazione, ora_operazione, risorsaDettaglioFertilizzazione, modalitaApplicazione))
                Next
            End If
    ...
    Private Function MapOrganicFertilization(ByVal operationId As String, ByVal operationDesLib As String, ByVal plot As Plot, ByVal Dr_Appezzamento_Impianto As DataRow, ByVal esercizioCDC As EsercizioCDC, ByVal data_operazione As Date,
        ...
        Dim obj_Qta_Prodotto_X_Impianto = GetProductAndWaterQuantityFromPlot(operationId, risorsaDettaglioFertilizzazione, Dr_Appezzamento_Impianto, esercizioCDC, Nothing)
    ...
    Private Function MapWarehouse(ByVal Piva As String, ByRef row As DataRow, ByVal Data_Inizio As Date, ByVal Data_Fine As Date, ByRef objParametri_Server As AgronicaCoreParametri, ByRef objParametri_Utenti As AgronicaCoreParametri) As Warehouse
        ...
        Dim description As String = row("Fabbricato_des")
    ...
    Private Sub MapWarehouseStocks(ByVal Piva As String, ByVal SaCod As Integer, ByVal FabbricatoCod As Integer, ByVal Data_Inizio As Date, ByVal Data_Fine As Date, ByRef warehouse As Warehouse, ByRef objParametri_Server As AgronicaCoreParametri, ByRef objParametri_Utenti As AgronicaCoreParametri)
        ...
        Dim data_giacenza_fine As Date = Data_Fine.AddDays(-1)

        Dim dtGiacenzaInizialeFormulati = objGiacenze.SchedaGiacenzeMagazzino(data_giacenza_inizio, Piva, SaCod, FabbricatoCod, FORMULATI, 0, 0, 0, 0, 0, 0, LOTTO_NONDEFINITO, False, "", "", "", "", "", "", "", "", "", "", "", "", objParametri_Server, objParametri_Utenti).DefaultView.ToTable(True, {"Pro_Cod", "Udm_Cod", "Giacenza"})
        Dim dtGiacenzaFinaleFormulati = objGiacenze.SchedaGiacenzeMagazzino(data_giacenza_fine, Piva, SaCod, FabbricatoCod, FORMULATI, 0, 0, 0, 0, 0, 0, LOTTO_NONDEFINITO, False, "", "", "", "", "", "", "", "", "", "", "", "", objParametri_Server, objParametri_Utenti).DefaultView.ToTable(True, {"Pro_Cod", "Udm_Cod", "Giacenza"})

        Dim dtGiacenzaInizialeFertilizzanti = objGiacenze.SchedaGiacenzeMagazzino(data_giacenza_inizio, Piva, SaCod, FabbricatoCod, FERTILIZZANTI, 0, 0, 0, 0, 0, 0, LOTTO_NONDEFINITO, False, "", "", "", "", "", "", "", "", "", "", "", "", objParametri_Server, objParametri_Utenti).DefaultView.ToTable(True, {"Pro_Cod", "Udm_Cod", "Giacenza"})
        Dim dtGiacenzaFinaleFertilizzanti = objGiacenze.SchedaGiacenzeMagazzino(data_giacenza_fine, Piva, SaCod, FabbricatoCod, FERTILIZZANTI, 0, 0, 0, 0, 0, 0, LOTTO_NONDEFINITO, False, "", "", "", "", "", "", "", "", "", "", "", "", objParametri_Server, objParametri_Utenti).DefaultView.ToTable(True, {"Pro_Cod", "Udm_Cod", "Giacenza"})

        Dim ProCod As Integer
        Dim UdmCod_Inizio As Integer
        Dim UdmCod_Fine As Integer
        Dim Giacenza_Inizio As Decimal
        Dim Giacenza_Fine As Decimal
        Dim drGiacenzaInizio As DataRow()

        For Each row In dtGiacenzaFinaleFormulati.Rows
            ProCod = row("Pro_Cod")
            UdmCod_Fine = row("Udm_Cod")
            Giacenza_Fine = row("Giacenza")
            drGiacenzaInizio = dtGiacenzaInizialeFormulati.Select("Pro_Cod = " & ProCod)
            UdmCod_Inizio = If(drGiacenzaInizio.Length > 0, drGiacenzaInizio(0)("Udm_Cod"), UdmCod_Fine)
            Giacenza_Inizio = If(drGiacenzaInizio.Length > 0, drGiacenzaInizio(0)("Giacenza"), 0)

            MapWarehouseStock(FORMULATI, ProCod, Data_Inizio, Data_Fine, UdmCod_Inizio, UdmCod_Fine, Giacenza_Inizio, Giacenza_Fine, warehouse, objParametri_Server)
        Next

        For Each row In dtGiacenzaFinaleFertilizzanti.Rows
            ProCod = row("Pro_Cod")
            UdmCod_Fine = row("Udm_Cod")
            Giacenza_Fine = row("Giacenza")
            drGiacenzaInizio = dtGiacenzaInizialeFertilizzanti.Select("Pro_Cod = " & ProCod)
            UdmCod_Inizio = If(drGiacenzaInizio.Length > 0, drGiacenzaInizio(0)("Udm_Cod"), UdmCod_Fine)
            Giacenza_Inizio = If(drGiacenzaInizio.Length > 0, drGiacenzaInizio(0)("Giacenza"), 0)

            MapWarehouseStock(FERTILIZZANTI, ProCod, Data_Inizio, Data_Fine, UdmCod_Inizio, UdmCod_Fine, Giacenza_Inizio, Giacenza_Fine, warehouse, objParametri_Server)
        Next
    ...
    Private Function GetProductMeasureUnit(ByVal unitaDiMisura As AgronicaCoreModelsSTD.metaschema.UnitaDiMisura) As String
        ...
        If IsNothing(DT_UdM) Then
            ...
            DT_UdM = objMetaschemaDAL.Leggi(0, Sistema_Cod_Agea, "", 0, AGRODATAINIZIO, AGRODATAFINE, "", "", ObjParametri_Server)
        End If
    ...
    Private Function GetProductAndWaterQuantityFromPlot(ByVal operationId As String, ByVal dettaglio As Risorsa, ByVal Dr_Appezzamento_Impianto As DataRow,
                                                        ByVal esercizioCDC As EsercizioCDC, ByVal risorsaAcqua As RisorsaAcqua) As Obj_Qta_Prodotto
        ...
        If Not IsNothing(dettaglio) Then
            ...
            Else
                ...
                If Not IsNothing(esercizioCDC) Then
                    actionArea = esercizioCDC.superficieTrattata
                End If
