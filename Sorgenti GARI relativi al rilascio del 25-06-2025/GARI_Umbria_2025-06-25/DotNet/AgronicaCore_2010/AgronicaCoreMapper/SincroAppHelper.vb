...
Public Class SincroAppHelper
    ...
    Public Function ImportaRilievo(ByRef unid As String, Optional ByRef riferimento As String = "", Optional ByVal visita As Boolean = False) As Boolean
        ...
        For Each row As DataRow In dtRilievi.Rows
            ...
            For Each rilievo In listaRilievi
                ...
                id_agenda = objAgendaScrivi.Scrivi(agenda, objParametri_Server, documentoPrevisionale:=True)
    ...
    Public Function ImportaAttivita(Optional ByVal guid As String = "", Optional ByRef riferimento As String = "", Optional ByVal importAgenda As String = "", Optional ByVal eseguiVerifiche As Boolean = False, Optional ByVal tipo As enum_Dati_App = Nothing) As Boolean
            ...
            Using scopeImportazione As New TransactionScope(TransactionScopeOption.Required, opzioniTransazione)
                ...
                If risultatoImportazione AndAlso riferimento <> "" AndAlso Not String.IsNullOrEmpty(importAgenda) Then
                    ...
                    For Each row As DataRow In dtRicette.Rows
                        ...
                        If Not objRicettaxAgenda.Scrivi(row.Item("Ricetta_Cod"), row.Item("Ricetta_Operazione_Cod"), Id_Agenda, CostantiPersonalizzate.AGRODATAINIZIO, CostantiPersonalizzate.AGRODATAFINE, objParametri_Server) Then
                        ...
                        If Id_Agenda <> 0 Then
                            ...
                            Dim esitoFinale As Boolean = xLettura.ImportaCDGDaTabelleAPP(Piva, Id_Agenda, msgFinale, objParametri_Server)
                            If esitoFinale = False Then
    ...
    Public Function CancellaAttivita(ByVal guid As String) As Boolean
        ...
        For Each row As DataRow In dtAttivita.Rows
            ...
            Try
                ...
                If cancellaAgenda AndAlso Not IsDBNull(row.Item("Id_Agenda")) AndAlso row.Item("Id_Agenda") <> 0 Then
                    ...
                    If GestisciCancellazione(objParametriAgenda, objParametri_Server, messaggio, True, objParametri_Utenti:=objParametri_Utenti) Then
    ...
    Public Function ImportaAttivitaCdG(Optional ByVal guid As String = "", Optional ByRef riferimento As String = "") As Boolean
        ...
        Dim msgFinale As New StringBuilder
        Dim esito = objCDGAPP.ImportaCDGDaTabelleAPP("", 0, msgFinale, objParametri_Server, guid, riferimento)
        messaggioErrore = msgFinale.ToString
        Return esito
    End Function
                    ...
                    If dettaglio.campioni IsNot Nothing Then
                        ...
                        For Each campione In dettaglio.campioni
                            Dim pdc_campione As New AgronicaCorePianidiCampionamentoBiz.PDC_Campioni With {
                                ...
                                .Descrizione_PuntoDiPrelievo = campione.descrizione,
                                .Codice_Campione = campione.codice_campione,
        ...
        Try
            ...
            For Each row As DataRow In dtAttivita.Rows
                ...
                Try
                    ...
                    r = RicetteOpW.Update_APP_Ricetta_Operazione_ID(riferimento, row("ID"), objParametri_Server)
    ...
    Public Function CancellaAgenda(ByVal guid As String) As Boolean
        ...
        For Each row As DataRow In dtAttivita.Rows
            ...
            Try
                ...
                If Not IsDBNull(row.Item("Id_Agenda")) AndAlso row.Item("Id_Agenda") <> 0 Then
                    ...
                    esito = GestisciCancellazione(objParametriAgenda, objParametri_Server, messaggio, True, objParametri_Utenti:=objParametri_Utenti)
    ...
    Public Function SincronizzaDatiApp(ByRef messaggioRitorno As String, Optional ByVal tipoDati As String = "", Optional ByVal documenti As Boolean = False, Optional ByVal agenda As String = "", Optional logDebug As Boolean = False, Optional azienda As String = "") As Boolean
        ...
        If datiDaImportare = 0 AndAlso tipi.Length = 1 AndAlso tipi.Contains(enum_Dati_App.AttivitaCdG) Then
            Dim msgFinale As New StringBuilder
            Dim xLettura As New AgronicaCoreContabBIZ.CDG_APP
            Dim esito = xLettura.ImportaCDGDaTabelleAPP(azienda, 0, log, objParametri_Server)
    ...
    Public Function SincroDatiApp(ByRef unid As String,
                                  ...
                                  Optional ByRef erroreImport As String = "") As Boolean
        ...
        ElseIf Not String.IsNullOrEmpty(unid) AndAlso riferimento = "" Then
            riferimento = objAppHelper.Leggi_Riferimento_APP(unid, tipo, objParametri_Server)
        End If

        ' scrivo dati app
        If Not cancellaDatiApp AndAlso cancellato Then
            esito = objAppHelper.Aggiorna_Dati_APP(unid, tipo, "", riferimento, cancellato, importazione, objParametri_Server, userAgent)
        Else
            esito = objAppHelper.Scrivi_Dati_APP(unid, tipo, dati, riferimento, cancellato, objParametri_Server, piva, codice, userAgent)
        End If

        ' importa dati app
        If esito AndAlso importazione Then
            Dim rifDocumento As String = ""
            esito = ImportaDatiApp(unid, tipo, riferimento, aggiornamento, cancellato, agenda, posizione, rifDocumento)
            ' importa documenti allegati
    ...
    Public Function ImportaDatiApp(ByRef unid As String,
                                   ...
                                   Optional ByVal agenda As String = "",
                                   Optional ByVal posizione As String = "",
                                   Optional ByRef rifDocumento As String = "") As Boolean
        ...
        Try
            ...
            If esito AndAlso Not cancellato Then
                Select Case tipo
                    ...
                    Case enum_Dati_App.Movimenti
                        esito = ImportaMovimenti(unid, riferimento)
                    Case enum_Dati_App.Acquisti
                        esito = ImportaAcquisti(unid, riferimento)
                    Case enum_Dati_App.Documenti
                        esito = ImportaDocumenti(unid, riferimento)
                    Case enum_Dati_App.Visite
                        Dim rilievo As String = ""
                        If agenda = "NG" Then
                            esito = ImportaRilievo(unid, rilievo, True)
                        Else
                    ...
                    Case enum_Dati_App.Rilievi
                        If agenda = "NG" Then
                            esito = ImportaRilievo(unid, riferimento)
                        Else
                            esito = ImportaAgenda(unid, riferimento, posizione)
                        End If
                    Case enum_Dati_App.Attivita, enum_Dati_App.Ricette
                        esito = ImportaAttivita(unid, riferimento, agenda)
                    Case enum_Dati_App.AttivitaCdG
                        esito = ImportaAttivitaCdG(unid, riferimento)
                    Case Else
                        If DictOrigineAttivitaToImport.ContainsKey(tipo) Then
                            esito = ImportaAttivita(unid, riferimento, agenda, tipo:=tipo)
                        End If
                End Select
            End If
    ...
    Public Function ImportaDocumenti(ByRef unid As String, Optional ByRef riferimento As String = "") As Boolean
        ...
        Dim esito As Boolean = True
        Dim objDocumenti As New Alert_W
    ...
    Public Function ImportaMovimenti(ByRef unid As String, Optional ByRef riferimento As String = "") As Boolean
        ...
        For Each row As DataRow In dtMovimenti.Rows
            ...
            id_agenda = objAgendaHelper.Scrivi(agenda, objParametri_Server, documentoPrevisionale:=True)
    ...
    Public Function ImportaAcquisti(ByRef unid As String, Optional ByRef riferimento As String = "") As Boolean
        ...
        For Each row As DataRow In dtAcquisti.Rows
            ...
            id_agenda = objAgendaHelper.Scrivi(agenda, objParametri_Server, flagUsaOraReale:=True, documentoPrevisionale:=True)
    ...
    Public Sub ScriviDocumento(ByVal piva As String, ByVal documento As AgronicaCoreModelsSTD.documenti.Documento, ByVal unid As String, Optional ByVal cancellazione As Boolean = False)
        ...
        If DocumentoPerScarico.Data_Scadenza < CostantiPersonalizzate.AGRODATAINIZIO Then
            DocumentoPerScarico.Data_Scadenza = CostantiPersonalizzate.AGRODATAFINE
        End If
    ...
    Public Sub ScriviDocumentoPerImport(ByVal piva As String, ByVal documento As DocumentoAllegatoPerImport, ByVal unid As String, ByRef errorMessage As String)
        ...
        Try
            ...
            Try
                ...
                Else
                    DocumentoPerScarico.Data_Scadenza = documento.Data_Scadenza
                    If documento.Data_Scadenza < CostantiPersonalizzate.AGRODATAINIZIO Then
                        DocumentoPerScarico.Data_Scadenza = CostantiPersonalizzate.AGRODATAFINE
                    End If
                End If
