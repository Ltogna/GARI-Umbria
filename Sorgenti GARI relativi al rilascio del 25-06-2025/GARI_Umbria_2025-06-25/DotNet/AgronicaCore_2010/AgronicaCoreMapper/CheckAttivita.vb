...
Public Class CheckAttivita
    ...
    Public Function ControlloInserimentoDoseProdotto(input As Controllo_Inserimento_Dose_Prodotto, objParametri_Super_Server As AgronicaCoreParametri,
        ...
        Dim Blocca_Inserimento = objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_DEFAULT_BLOCCO_INSERIMENTO_NONCONFORME, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)
...
#End Region
    ...
    Private Sub CheckGiacenzaMagazzino(input As Controllo_Inserimento_Dose_Prodotto,
        ...
        If input.Magazzino IsNot Nothing Then
            ...
            Dim Piva_fabbricato = input.Magazzino.primaryKey.centroAziendalePK.partitaIva
            Dim Sa_Cod_fabbricato = input.Magazzino.primaryKey.centroAziendalePK.codice
            Dim Fabbricato_Cod = input.Magazzino.primaryKey.codice
            Dim descrizioneMagazzino As String = input.Magazzino.descrizione
            ...
            Dim codiceProdotto As Integer = risorsaProdotto.prodotto.codice
            Dim descrizioneProdotto As String = risorsaProdotto.prodotto.descrizione
            Dim elemCod As Integer = risorsaProdotto.prodotto.elemCod

            Dim QtaTot As Decimal = input.quantitaTotale
            ...
            End If
            Dim udmBase = STD_Utility.getUdmBasefromUdmIndicata(input.unitadiMisura, QtaTot, objParametri_Server)
            ...
            If Not IsNothing(input.pua) AndAlso input.pua.codice > 0 AndAlso
                ...
                If QtaTot > Math.Round(qta_in_data, 4) Then
                    listaErroriTemp.Add(generaErroreGias(ErroreGias_Severity.WarningBloccante, "", String.Format(My.Resources.AgronicaCoreMapper.QuantitaDichiaratoPUANonSuffScarico, descrizioneProdotto, CDate(input.Data).ToShortDateString, Math.Round(qta_in_data, 4), udmBase.descrizione), ""))
                End If
                ...
                If QtaTot > Math.Round(qta_in_data, 4) Then
                    listaErroriTemp.Add(generaErroreGias(ErroreGias_Severity.WarningBloccante, "", String.Format(My.Resources.AgronicaCoreMapper.GiacenzaDichiaratoPUANonSuffScarico, descrizioneProdotto, Math.Round(qta_in_data, 4), udmBase.descrizione), ""))
                End If
            ...
            If QtaTot > Math.Round(qta_in_data, 4) Then
                listaErroriTemp.Add(generaErroreGias(ErroreGias_Severity.WarningBloccante, "", String.Format(My.Resources.AgronicaCoreMapper.QuantitaNonSuffScarico, descrizioneProdotto, descrizioneMagazzino, CDate(input.Data).ToShortDateString, Math.Round(qta_in_data, 4), udmBase.descrizione), ""))
            End If
            ...
            If QtaTot > Math.Round(qta_in_data, 4) Then
                listaErroriTemp.Add(generaErroreGias(ErroreGias_Severity.WarningBloccante, "", String.Format(My.Resources.AgronicaCoreMapper.GiacenzaNonSuffScarico, descrizioneProdotto, descrizioneMagazzino, Math.Round(qta_in_data, 4), udmBase.descrizione), ""))
            End If
            ...
            If listaErroriTemp.Count > 0 Then
                ...
                If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_SE_SUPERA_GIACENZE, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) <> "0" Then
                    bloccaGiacenze_Utente = True
                End If

                Dim objImpreseImpostazioni As New AgronicaCoreAnagrafeDAL.Imprese_Impostazioni_R

                Dim isBloccante As Boolean = False
                ...
                Else
                    If infoOperazione.IsTrattamento Then
                        ...
                        Dim GestioneGiacenze_Formulati = CInt(objImpreseImpostazioni.LeggiScalareMulticentroAziendaSuperUser_ElemCod(Piva_fabbricato, Nothing, enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_CATEGORIAMAGAZZINO_GIACENZE, CostantiPersonalizzate.FORMULATI, enum_Gestione_Giacenze.TuttiProdotti, objParametri_Utenti, objParametri_Server))
                        If GestioneGiacenze_Formulati = enum_Gestione_Giacenze.SoloPresenti Then
                            isBloccante = True
                        End If
                    Else
                        If infoOperazione.IsFertilizzazione Then
                            'DT: bisognerebbe prendere i centri dei fabbricati, non quelli degli impianti.
                            'ma si è valutato di fermarsi a livello di super user o azienda, quindi per il momento è sufficiente passare la PIVA (del fabbricato)
                            Dim GestioneGiacenze_Fertilizzanti = CInt(objImpreseImpostazioni.LeggiScalareMulticentroAziendaSuperUser_ElemCod(Piva_fabbricato, Nothing, enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_CATEGORIAMAGAZZINO_GIACENZE, CostantiPersonalizzate.FERTILIZZANTI, enum_Gestione_Giacenze.TuttiProdotti, objParametri_Utenti, objParametri_Server))
                            If GestioneGiacenze_Fertilizzanti = enum_Gestione_Giacenze.SoloPresenti Then
                                isBloccante = True
                            End If
                        Else
                            If infoOperazione.IsSemina Then
                                'DT: bisognerebbe prendere i centri dei fabbricati, non quelli degli impianti.
                                'ma si è valutato di fermarsi a livello di super user o azienda, quindi per il momento è sufficiente passare la PIVA (del fabbricato) 
                                Dim GestioneGiacenze_Sementi = CInt(objImpreseImpostazioni.LeggiScalareMulticentroAziendaSuperUser_ElemCod(Piva_fabbricato, Nothing, enum_Impostazioni_Utenti.UTENTE_COD_FILTRO_CATEGORIAMAGAZZINO_GIACENZE, CostantiPersonalizzate.SEMENTI, enum_Gestione_Giacenze.TuttiProdotti, objParametri_Utenti, objParametri_Server))
                                If GestioneGiacenze_Sementi = enum_Gestione_Giacenze.SoloPresenti Then
                                    isBloccante = True
                                End If
                            Else
                                isBloccante = False
    ...
    Private Sub CheckCarenza(input As Controllo_Inserimento_Dose_Prodotto, ByRef ListaErroriTemp As List(Of ErroreGias), objParametri_Utenti As AgronicaCoreParametri)
        ...
        If input.dettaglioTrattamento.tempoCarenza > 0 AndAlso input.disciplinare IsNot Nothing AndAlso input.disciplinare.codice <> enum_Disciplinare_Operazione.NessunDpiNessunaEtichetta Then
            ...
            If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_CARENZA_NON_RISPETTATA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
                Blocca_Carenza = True
            End If
    ...
    Private Sub CheckBloccoFioritura(input As Controllo_Inserimento_Dose_Prodotto, ByRef ListaErroriTemp As List(Of ErroreGias), objParametri_Utenti As AgronicaCoreParametri, objParametri_Server As AgronicaCoreParametri, objParametri_Super_Server As AgronicaCoreParametri)
        ...
        If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_FINO_FIORITURA_ETICHETTA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Private Sub CheckBuffer(input As Controllo_Inserimento_Dose_Prodotto, ByRef ListaErroriTemp As List(Of ErroreGias), objParametri_Utenti As AgronicaCoreParametri)

        Dim objUtentiImpostazioni As New AgronicaCoreUtentiDAL.Utenti_Impostazioni_Read

        Dim Blocca_BufferZone = False
        If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_BUFFERZONE_ETICHETTA_SALVATAGGIO, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Private Sub CheckMixPolverulento(input As Controllo_Inserimento_Dose_Prodotto, gridDosiManaged As List(Of RowGridDosi), Blocca_Inserimento As String, ByRef ListaErroriTemp As List(Of ErroreGias), objParametri_Utenti As AgronicaCoreParametri)
        ...
        If InfoOperazione.IsTrattamento AndAlso InfoOperazione.controlloPolverulenti Then
            ...
            If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_MIX_POLVERULENTI_NONPOLVERULENTI, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Private Sub CheckDoseMassima(doseEtichetta As DoseEtichetta, udm As UnitaDiMisura, DoseConsentitaDiserbo As Decimal, doseHa As Decimal, doseHl As Decimal, doseQ As Decimal, Blocca_Inserimento As String, ByRef errori As Dictionary(Of TipoErrore, List(Of String)), objParametri_Utenti As AgronicaCoreParametri)
        ...
        If doseEtichetta.DoseMax <> 0 Then
            ...
            If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_DOSEMASSIMA_ETICHETTA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Private Sub CheckDoseMinima(doseEtichetta As DoseEtichetta, udm As UnitaDiMisura, doseHa As Decimal, doseHl As Decimal, doseQ As Decimal, Blocca_Inserimento As String, ByRef errori As Dictionary(Of TipoErrore, List(Of String)), objParametri_Utenti As AgronicaCoreParametri)
        ...
        If doseEtichetta.DoseMin <> 0 Then
            ...
            If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_DOSEMINIMA_ETICHETTA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Private Sub CheckNumeroMaxTrattamenti(doseEtichetta As DoseEtichetta, input As Controllo_Inserimento_Dose_Prodotto, Blocca_Inserimento As String, ByRef errori As Dictionary(Of TipoErrore, List(Of String)), objParametri_Utenti As AgronicaCoreParametri, objParametri_Server As AgronicaCoreParametri)
        ...
        If doseEtichetta.Limite > 0 Then
            ...
            Dim objUtentiImpostazioni As New AgronicaCoreUtentiDAL.Utenti_Impostazioni_Read
            If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_NUMERO_MASSIMO_TRATTAMENTI_ETICHETTA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Private Sub CheckIntervalloTrattamenti(doseEtichetta As DoseEtichetta, input As Controllo_Inserimento_Dose_Prodotto, Blocca_Inserimento As String, ByRef errori As Dictionary(Of TipoErrore, List(Of String)), objParametri_Utenti As AgronicaCoreParametri, objParametri_Server As AgronicaCoreParametri)
        ...
        If doseEtichetta.IntervalloTrattamenti_Min > 0 Then
            ...
            If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_INTERVALLOTRATTAMENTI_ETICHETTA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Private Sub CheckAcqua(doseEtichetta As DoseEtichetta,
        ...
        If input.risorsaAcqua IsNot Nothing AndAlso doseEtichetta.UdmAcqua IsNot Nothing Then
            ...
            If Acqua_Max <> 0 Then
                ...
                If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_ACQUAMASSIMA_ETICHETTA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
            ...
            If Acqua_Min <> 0 Then
                ...
                If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_ACQUAMINIMA_ETICHETTA, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) = "1" Then
    ...
    Public Sub CheckMassimali(ConsideraProdottoInInserimento As Boolean,
        ...
        Dim Tipo_PuaRegolamento As Integer = input.disciplinare.regolamentoConcimazione.tipo

        Dim Moltiplicatore As Decimal = 0
        Dim Udm_Cod_Trasformato As Integer = 0

        Dim N_Operazione As Decimal = 0
        Dim P_Operazione As Decimal = 0
        Dim K_Operazione As Decimal = 0
        ...
        Dim N_Massimo, N_Massimo_Org, P_Massimo, K_Massimo, Mg_Massimo As Decimal
        Dim N_Distribuito, P_Distribuito, K_Distribuito, Mg_Distribuito, Cu_Distribuito As Decimal

        Dim strRet As String = ""
        Dim strErr_N As String = ""
        Dim strErr_P As String = ""
        Dim strErr_K As String = ""
        Dim strErr_Cu As String = ""
        Dim strErr_N_Intervento As String = ""

        Dim strErr_N_Ricetta As String = ""
        Dim strErr_P_Ricetta As String = ""
        Dim strErr_K_Ricetta As String = ""
        Dim strErr_Cu_Ricetta As String = ""
        ...
        For i = 0 To input.row_grid_impianti.Count - 1
            ...
            objApporti.Leggi_Macroelementi_Impostati(input.row_grid_impianti(i).PIVA,
                                                    input.row_grid_impianti(i).SA_COD,
                                                    input.row_grid_impianti(i).APPEZZA,
                                                    input.row_grid_impianti(i).ID_REG,
                                                    input.row_grid_impianti(i).Progetto_Cod,
                                                    N_Massimo,
                                                    N_Massimo_Org,
                                                    P_Massimo,
                                                    K_Massimo,
            ...
            If N_Max_Intervento <> 0 Then
                If (N_Operazione - N_Max_Intervento) > 0.001 Then
                    strErr_N_Intervento &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaPerInterventoNonRispettato, input.row_grid_impianti(i).APP_NOME, N_Operazione, N_Max_Intervento)
            ...
            objDett.Leggi_Macroelementi_Distribuiti(N_Distribuito,
                                                        P_Distribuito,
                                                        K_Distribuito,
                                                        Mg_Distribuito,
                                                        Cu_Distribuito,
                                                        0, 0, 0, 0, 0,
                                                        input.row_grid_impianti(i).PIVA,
                                                        input.row_grid_impianti(i).SA_COD,
                                                        input.row_grid_impianti(i).APPEZZA,
                                                        input.row_grid_impianti(i).ID_REG,
                                                        input.row_grid_impianti(i).Progetto_Cod,
                                                        input.row_grid_impianti(i).Validita_Inizio_Distinta,
                                                        input.row_grid_impianti(i).Validita_Fine_Distinta,
                                                        input.id_agenda,
                                                        objParametri_Server,
                                                        input.raccoglitore_cod)
            ...
            dtLavorazioni = ObjContab.LeggiLavorazioni_Da_Principi_Attivi(input.row_grid_impianti(i).PIVA,
                                                                        input.row_grid_impianti(i).SA_COD,
                                                                        PaRameici_str,
                                                                        input.row_grid_impianti(i).APPEZZA,
                                                                        input.row_grid_impianti(i).ID_REG,
            ...
            Cu_Distribuito += Qta_Pa_Tot
            'End If


            If N_Massimo <> -1 Then
                If ((N_Distribuito + N_Operazione) - N_Massimo) > 0.001 And N_Operazione > 0 Then
                    strErr_N &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, (N_Distribuito + N_Operazione), N_Massimo) + NEWLINE
                    nImpiantiViolati_N_Max_Intervento += 1
                End If
                If input.N_Utile > 0 Then
                    Qta_Ha_Distribuibile_Prodotto = IIf(Qta_Ha_Distribuibile_Prodotto > ((N_Massimo - N_Distribuito - N_Operazione) / (input.N_Utile / 100)), Format((N_Massimo - N_Distribuito - N_Operazione) / (input.N_Utile / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto)
                End If
            End If
            If P_Massimo <> -1 Then
                If ((P_Distribuito + P_Operazione) - P_Massimo) > 0.001 And P_Operazione > 0 Then
                    strErr_P &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, (P_Distribuito + P_Operazione), P_Massimo) + NEWLINE
                End If
                If input.P > 0 Then
                    Qta_Ha_Distribuibile_Prodotto = IIf(Qta_Ha_Distribuibile_Prodotto > ((P_Massimo - P_Distribuito - P_Operazione) / (input.P / 100)), Format((P_Massimo - P_Distribuito - P_Operazione) / (input.P / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto)
                End If
            End If
            If K_Massimo <> -1 Then
                If ((K_Distribuito + K_Operazione) - K_Massimo) > 0.001 And K_Operazione > 0 Then
                    strErr_K &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, (K_Distribuito + K_Operazione), K_Massimo) + NEWLINE
                End If
                If input.K > 0 Then
                    Qta_Ha_Distribuibile_Prodotto = IIf(Qta_Ha_Distribuibile_Prodotto > ((K_Massimo - K_Distribuito - K_Operazione) / (input.K / 100)), Format((K_Massimo - K_Distribuito - K_Operazione) / (input.K / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto)
                End If
            End If
            If Cu_Massimo <> -1 Then
                If ((Cu_Distribuito + Cu_Operazione) - Cu_Massimo) > 0.001 And Cu_Operazione > 0 Then
                    strErr_Cu &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, Math.Round((Cu_Distribuito + Cu_Operazione), 3), Cu_Massimo) + NEWLINE
                End If
                If input.Cu > 0 Then
                    Qta_Ha_Distribuibile_Prodotto = IIf(Qta_Ha_Distribuibile_Prodotto > ((Cu_Massimo - Cu_Distribuito - Cu_Operazione) / (input.Cu / 100)), Format((Cu_Massimo - Cu_Distribuito - Cu_Operazione) / (input.Cu / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto)
                End If
            End If

            If (N_Massimo <> -1 And N_Operazione > 0 And (((N_Distribuito + N_Operazione) - N_Massimo) > 0.001)) Or
                (P_Massimo <> -1 And P_Operazione > 0 And (((P_Distribuito + P_Operazione) - P_Massimo) > 0.001)) Or
                (K_Massimo <> -1 And K_Operazione > 0 And (((K_Distribuito + K_Operazione) - K_Massimo) > 0.001)) Or
                (Cu_Massimo <> -1 And Cu_Operazione > 0 And (((Cu_Distribuito + Cu_Operazione) - Cu_Massimo) > 0.001)) Then
                nImpiantiViolati += 1
            End If

            If input.tipoAttivita = AgronicaCoreModelsSTD.attivita.Attivita.Tipo_Attivita.Ricetta AndAlso input.statoAttivita = AgronicaCoreModelsSTD.attivita.Attivita.Stati.Da_Eseguire Then
                ...
                objDettRic.Leggi_Macroelementi_Distribuiti(N_Distribuito_Ricetta,
                                                            ...
                                                            input.ricetta_operazione_cod,
                                                            objParametri_Server,
                                                            Raccoglitore_Cod_Escluso:=input.raccoglitore_cod)

                If N_Massimo <> -1 Then
                    If ((N_Distribuito_Ricetta + N_Operazione) - N_Massimo) > 0.001 And N_Operazione > 0 Then
                        strErr_N_Ricetta &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, (N_Distribuito_Ricetta + N_Operazione), N_Massimo)
                    End If
                    If input.N_Utile > 0 Then
                        Qta_Ha_Distribuibile_Prodotto_Ricette = IIf(Qta_Ha_Distribuibile_Prodotto_Ricette > ((N_Massimo - N_Distribuito_Ricetta - N_Operazione) / (input.N_Utile / 100)), Format((N_Massimo - N_Distribuito_Ricetta - N_Operazione) / (input.N_Utile / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto_Ricette)
                    End If
                End If
                If P_Massimo <> -1 Then
                    If ((P_Distribuito_Ricetta + P_Operazione) - P_Massimo) > 0.001 And P_Operazione > 0 Then
                        strErr_P_Ricetta &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, (P_Distribuito_Ricetta + P_Operazione), P_Massimo)
                    End If
                    If input.P > 0 Then
                        Qta_Ha_Distribuibile_Prodotto_Ricette = IIf(Qta_Ha_Distribuibile_Prodotto_Ricette > ((P_Massimo - P_Distribuito_Ricetta - P_Operazione) / (input.P / 100)), Format((P_Massimo - P_Distribuito_Ricetta - P_Operazione) / (input.P / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto_Ricette)
                    End If
                End If
                If K_Massimo <> -1 Then
                    If ((K_Distribuito_Ricetta + K_Operazione) - K_Massimo) > 0.001 And K_Operazione > 0 Then
                        strErr_K_Ricetta &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, (K_Distribuito_Ricetta + K_Operazione), K_Massimo)
                    End If
                    If input.K > 0 Then
                        Qta_Ha_Distribuibile_Prodotto_Ricette = IIf(Qta_Ha_Distribuibile_Prodotto_Ricette > ((K_Massimo - K_Distribuito_Ricetta - K_Operazione) / (input.K / 100)), Format((K_Massimo - K_Distribuito_Ricetta - K_Operazione) / (input.K / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto_Ricette)
                    End If
                End If
                If Cu_Massimo <> -1 Then
                    If ((Cu_Distribuito_Ricetta + Cu_Operazione) - Cu_Massimo) > 0.001 And Cu_Operazione > 0 Then
                        strErr_Cu_Ricetta &= String.Format(My.Resources.AgronicaCoreMapper.LimiteKgHaMassimoNonRispettato, input.row_grid_impianti(i).APP_NOME, (Cu_Distribuito_Ricetta + Cu_Operazione), Cu_Massimo)
                    End If
                    If input.Cu > 0 Then
                        Qta_Ha_Distribuibile_Prodotto_Ricette = IIf(Qta_Ha_Distribuibile_Prodotto_Ricette > ((Cu_Massimo - Cu_Distribuito_Ricetta - Cu_Operazione) / (input.Cu / 100)), Format((Cu_Massimo - Cu_Distribuito_Ricetta - Cu_Operazione) / (input.Cu / 100), "###,###.00#"), Qta_Ha_Distribuibile_Prodotto_Ricette)
                    End If
                End If

                If (N_Massimo <> -1 And N_Operazione > 0 And (((N_Distribuito_Ricetta + N_Operazione) - N_Massimo) > 0.001)) Or
                    (P_Massimo <> -1 And P_Operazione > 0 And (((P_Distribuito_Ricetta + P_Operazione) - P_Massimo) > 0.001)) Or
                    (K_Massimo <> -1 And K_Operazione > 0 And (((K_Distribuito_Ricetta + K_Operazione) - K_Massimo) > 0.001)) Or
                    (Cu_Massimo <> -1 And Cu_Operazione > 0 And (((Cu_Distribuito_Ricetta + Cu_Operazione) - Cu_Massimo) > 0.001)) Then
        ...
        If strErr_N <> "" Then
            strErr_N = String.Format(My.Resources.AgronicaCoreMapper.QuantitaAzotoInEccesso, NEWLINE + strErr_N) + NEWLINE
        End If
        If strErr_P <> "" Then
            strErr_P = String.Format(My.Resources.AgronicaCoreMapper.QuantitaFosforoInEccesso, NEWLINE + strErr_P) + NEWLINE
        End If
        If strErr_K <> "" Then
            strErr_K = String.Format(My.Resources.AgronicaCoreMapper.QuantitaPotassioInEccesso, NEWLINE + strErr_K) + NEWLINE
        End If
        If strErr_Cu <> "" Then
            strErr_Cu = String.Format(My.Resources.AgronicaCoreMapper.QuantitaRameInEccesso, NEWLINE + strErr_Cu)
        End If
        ...
        If nImpiantiViolati > 0 Then
            strRet &= strErr_N & strErr_P & strErr_K & strErr_Cu
        End If
        ...
        If input.tipoAttivita = AgronicaCoreModelsSTD.attivita.Attivita.Tipo_Attivita.Ricetta AndAlso input.statoAttivita = AgronicaCoreModelsSTD.attivita.Attivita.Stati.Da_Eseguire Then
            ...
            If strErr_N_Ricetta <> "" Then
                strErr_N_Ricetta = String.Format(My.Resources.AgronicaCoreMapper.QuantitaAzotoInEccessoRicetta, strErr_N_Ricetta)
            End If
            If strErr_P_Ricetta <> "" Then
                strErr_P_Ricetta = String.Format(My.Resources.AgronicaCoreMapper.QuantitaFosforoInEccessoRicetta, strErr_P_Ricetta)
            End If
            If strErr_K_Ricetta <> "" Then
                strErr_K_Ricetta = String.Format(My.Resources.AgronicaCoreMapper.QuantitaPotassioInEccessoRicetta, strErr_K_Ricetta)
            End If
            If strErr_Cu_Ricetta <> "" Then
                strErr_Cu_Ricetta = String.Format(My.Resources.AgronicaCoreMapper.QuantitaRameInEccessoRicetta, strErr_Cu_Ricetta)
            End If

            If nImpiantiViolati_Ricetta > 0 Then
                strRet &= strErr_N_Ricetta & strErr_P_Ricetta & strErr_K_Ricetta & strErr_Cu_Ricetta
            End If
        ...
        If strRet <> String.Empty Then
            ...
            Dim UTENTE_COD_BLOCCA_SE_SUPERA_LIMITIMAS = False
            Dim objUtentiImpostazioni As New AgronicaCoreUtentiDAL.Utenti_Impostazioni_Read
            If (objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_SE_SUPERA_LIMITIMAS, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti)) <> "0" Then
                UTENTE_COD_BLOCCA_SE_SUPERA_LIMITIMAS = True
            End If

            If UTENTE_COD_BLOCCA_SE_SUPERA_LIMITIMAS Then
