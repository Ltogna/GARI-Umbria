                ...
                For Each dettaglioMagazzino In dettagliMagazzino
                    ...
                    If InfoOperazione.IsTrattamento Then
                        ...
                        Else
                            ...
                            LeggiDettaglioTrattamento(pivaRicettaOperazione, dettCorrente, dettaglioCampagna, listaDettaglioTecnico.FirstOrDefault, dettaglioMagazzino, destinazioneMagazzino, verbose, objParametri_Server, superficieTrattataTotale, acquaTotale, ricetta_operazione.Lav_Cod, attivita)
...
#Region "Dati che non arrivano da APP"
        ...
        DettaglioDaLeggere.dosiEtichetta = DosiEtichetta_from_Stringhe(Dettagli.DoseEtichetta, Dettagli.DoseEtichetta_Value, verbose, objParametri_Server)
        DettaglioDaLeggere.principiAttivi = GetPrincipiAttivi(Dettagli.PrincipiAttivi, Dettagli.PrincipiAttiviPesi, Dettagli.PrincipiAttiviPercAbb)
        DettaglioDaLeggere.bufferzone = GetBufferZone(Dettagli.Buffer)
        DettaglioDaLeggere.dettaglioProdotto = GetDettaglioProdotto(Dettagli.Extra_Str)
        DettaglioDaLeggere.tempoCarenza = If(Dettagli.TempoCarenza IsNot Nothing, Dettagli.TempoCarenza, 0)
        DettaglioDaLeggere.polverulento = If(Dettagli.Polverulento IsNot Nothing, Dettagli.Polverulento, 0)
...
#End Region
    ...
    Private Sub LeggiRisorsaProdotto(ByVal piva As String, ByVal DettaglioDaLeggere As RisorsaProdotto, ByVal Dettagli As Ricette_Dettagli, ByVal DettaglioMagazzino As Ricette_Dettagli, ByVal DestinazioneMagazzino As Ricette_Destinazioni, ByVal superficieTrattataTotale As Decimal, ByVal acquaTotale As Decimal, ByVal attivita As AgronicaCoreModelsSTD.attivita.Attivita)
        ...
        If IsNothing(DettaglioMagazzino) Then
            DettaglioDaLeggere.MagazziniMovimentazioni = New List(Of RilevamentoDiMagazzino)
            Exit Sub
        End If

        Dim qtaTrasformata As Decimal = GetDosePerMagazzino(DettaglioMagazzino.Qta, DettaglioDaLeggere.unitaDiMisuraIndicata.codice)
        ...
        If DestinazioneMagazzino.Id_Reg <> 0 Then
            ...
            Dim rilevamentoMagazzino = New RilevamentoDiMagazzino With {
                ...
                .udm = New UnitaDiMisura(DettaglioDaLeggere.unitaDiMisura.codice) With {
                    .descrizione = DettaglioDaLeggere.unitaDiMisura.descrizione
                },
                .Qta = DettaglioMagazzino.Qta,
                .Prodotto = New Prodotto(DettaglioDaLeggere.prodotto.codice) With {
                    .elemCod = DettaglioMagazzino.Elem_Cod,
                    .descrizione = DettaglioDaLeggere.prodotto.descrizione,
                    .unitaDiMisura = New UnitaDiMisura(DettaglioDaLeggere.unitaDiMisura.codice) With {
                        .descrizione = DettaglioDaLeggere.unitaDiMisura.descrizione
                    }
                },
                .TipoRilevamento = RilevamentoDiMagazzino.RilevamentoMagazzinoTipo.giacenza,
                .Descrizione = DettaglioDaLeggere.prodotto.descrizione & "[" + DestinazioneMagazzino.Id_Reg.ToString & "]",
            ...
            If DestinazioneMagazzino.MagazzinoEsterno_Cod <> "" Then
                ...
                If magazziniEsterniQta IsNot Nothing AndAlso magazziniEsterniQta.Count > 0 Then
                    For Each magazzinoEsternoQta In magazziniEsterniQta
                        ...
                        Dim qtaTrasformataMagazzinoEsterno As Decimal = GetDosePerMagazzino(magazzinoEsternoQta.Item2, DettaglioDaLeggere.unitaDiMisuraIndicata.codice)
                        ...
                        Else
                            ...
                            Dim attivitaScarico As AgronicaCoreModelsSTD.attivita.Attivita = CreaAttivitaScaricoFittizia(DettaglioDaLeggere, rilevamentoMagazzinoMagazzinoEsterno, magazzinoEsternoQta, attivita)

                            rilevamentoMagazzinoMagazzinoEsterno.registrazioniCollegate.Add(attivitaScarico)
                        End If

                        DettaglioDaLeggere.MagazziniMovimentazioni.Add(rilevamentoMagazzinoMagazzinoEsterno)
                    ...
                    If qtaMagazzinoInterno > 0 Then
                        ...
                        Dim qtaTrasformataMagazzinoInterno As Decimal = GetDosePerMagazzino(qtaMagazzinoInterno, DettaglioDaLeggere.unitaDiMisuraIndicata.codice)
                        ...
                        DettaglioDaLeggere.MagazziniMovimentazioni.Add(rilevamentoMagazzino)
                    End If
                End If
            Else
                DettaglioDaLeggere.MagazziniMovimentazioni.Add(rilevamentoMagazzino)
    ...
    Private Function CreaAttivitaScaricoFittizia(ByVal risorsaProdotto As RisorsaProdotto, ByVal rilevamentoMagazzino As RilevamentoDiMagazzino, ByVal magazziniEsterni As Tuple(Of Fabbricato, Decimal, enum_MagazzinoEsterno_Tipo), ByVal attivita As AgronicaCoreModelsSTD.attivita.Attivita) As AgronicaCoreModelsSTD.attivita.Attivita
        ...
        risorsaRegistrazioneScarico.prodotto = risorsaProdotto.prodotto
...
#Region "Controlli"
    Public Shared Function controlloMagazziniRicettexApp_Menu(listaRicette As List(Of AgronicaCoreDTOStd.InData.Agenda.APP_Ricette_Operazioni),
        ...
        listErroriGias = componiErroreFinale_controlloMagazziniRicettexApp(True, listErrori, listErroriRicetteMultiAttivita, ricetteToRemove.Count)
