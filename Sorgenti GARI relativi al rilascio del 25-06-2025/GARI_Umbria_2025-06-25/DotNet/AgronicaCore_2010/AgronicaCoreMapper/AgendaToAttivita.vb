            ...
            For Each movimentoDettaglioOperazione In MovimentoOperazione.Movimenti_Dettagli
                ...
                If InfoOperazione.IsRaccolta Then
                    ...
                    If dettCorrente.MagazziniMovimentazioni IsNot Nothing Then
                        found = attivita.risorse.Where(Function(r) r.classType =
                                                           ClassType.DettaglioRaccolta).ToList.Find(Function(r As RisorsaProdotto) (r.prodotto.codice = dettCorrente.prodotto.codice AndAlso
                        ...
                        End If
                    Else
                        found = attivita.risorse.Where(Function(r) r.classType = ClassType.DettaglioRaccolta).ToList.Find(Function(r As RisorsaProdotto) (r.prodotto.codice = dettCorrente.prodotto.codice AndAlso r.MagazziniMovimentazioni Is Nothing))
                    End If
...
#Region "Funzioni di creazione oggetti del nuovo modello"
    ...
    Private Sub CreaNote(attivita As Attivita, agenda As Operazione_Agenda, verbose As Boolean, objParametri_Server As AgronicaCoreParametri)
        ...
        For Each nota In agenda.Note
            ...
            Dim notaItem = noteList.Find(Function(p) p.codice = nota.Nota_Cod)
    ...
    Private Function CreaRisorsaAcqua(attivita As Attivita, agenda As Operazione_Agenda, movimentoOperazione As Movimento, superficieTrattataTotale As Decimal) As Decimal
        ...
        Select Case agenda.Lav_Cod
            ...
            Case LAVCOD_CONCIMAZIONE_FOGLIARE, LAVCOD_FERTIRRIGAZIONE, LAVCOD_TRATTAMENTO_ANTIBUTTERATURA,
                ...
                Dim Movimento_Dettaglio_Tecnico_Acqua As Movimento_Dettaglio_Tecnico = movimentoOperazione.Movimenti_Dettagli_Tecnici.FindAll(Function(c) (c.Id_Mov_Det = 0 AndAlso c.dett_cod = 0)).FirstOrDefault
    ...
    Private Sub CreaRisorseCausale(attivita As Attivita, agenda As Operazione_Agenda, movimentoOperazione As Movimento)
        ...
        Select Case agenda.Lav_Cod
            ...
            Case LAVCOD_RILIEVO_INDICI_RESE_RACCOLTA
                ...
                'DT: si assume che ci possano essere più causali
                Dim Movimenti_Dettagli_Tecnico_RisorsaCausale As List(Of Movimento_Dettaglio_Tecnico) = movimentoOperazione.Movimenti_Dettagli_Tecnici.FindAll(Function(c) (c.Id_Mov_Det = 0 AndAlso c.dett_cod <> 0))

                If Movimenti_Dettagli_Tecnico_RisorsaCausale IsNot Nothing AndAlso Movimenti_Dettagli_Tecnico_RisorsaCausale.Count > 0 Then

                    For Each mdCausale In Movimenti_Dettagli_Tecnico_RisorsaCausale

                        Dim risorsaCausale = New RisorsaCausale
                        risorsaCausale.id = mdCausale.dett_cod

                        attivita.risorse.Add(risorsaCausale)
        ...
        For Each movimentoDettaglio In movimentoOperazione.Movimenti_Dettagli
            For Each movimentoDestinazione In movimentoDettaglio.Movimenti_Destinazioni
                ...
                If Not impiantiDict.Contains(movimentoDestinazione.Appezza) Then
                    ...
                    Dim esercizioCDC As EsercizioCDC = CreaEsercizioCdC(attivita, infoOperazione, movimentoDestinazione)
    ...
    Private Function CreaEsercizioCdC(attivita As Attivita, infoOperazione As InfoOperazione, movimentoDestinazione As Movimento_Destinazione) As EsercizioCDC

        Dim esercizioCDC As EsercizioCDC = Nothing

        If movimentoDestinazione.Piva <> "" AndAlso movimentoDestinazione.Sa_Cod > 0 AndAlso movimentoDestinazione.Appezza > 0 AndAlso movimentoDestinazione.Id_Destinazione > 0 Then

            Dim appezzamentoPK = New Appezzamento.PK(movimentoDestinazione.Appezza, attivita.centroAziendale.primaryKey)
            Dim impiantoPK = New Impianto.PK(movimentoDestinazione.Id_Destinazione, appezzamentoPK)

            Dim esercizio = New Esercizio(movimentoDestinazione.Progetto_Cod, "") With {
                .impiantoPK = impiantoPK
            }

            esercizioCDC = New EsercizioCDC With {
                .superficieTrattata = movimentoDestinazione.Qta2,
                .esercizio = esercizio
            }

            If infoOperazione.IsTrattamento OrElse infoOperazione.IsFertilizzazione Then
                esercizioCDC.superficieRiduzioneBufferZone = movimentoDestinazione.Sup_Riduzione_BufferZone
                esercizioCDC.percentualeRiduzioneDeriva = movimentoDestinazione.Perc_Riduzione_Deriva
            End If
    ...
    Private Function CreaDettaglioFertilizzazione(agenda As Operazione_Agenda, attivita As Attivita, infoOperazione As InfoOperazione, MovimentoDettaglioOperazione As Movimento_Dettaglio, MovimentoScarico As Movimento, superficieTrattataTotale As Decimal, acquaTotale As Decimal,
        ...
        If dettCorrente IsNot Nothing Then
            If MovimentoDettaglioOperazione.Movimenti_Dettagli_Tecnici IsNot Nothing AndAlso MovimentoDettaglioOperazione.Movimenti_Dettagli_Tecnici.Count > 0 Then
    ...
    Private Function CreaDettagliRilievo(attivita As Attivita, infoOperazione As InfoOperazione, agenda As Operazione_Agenda, MovimentoDettaglioOperazione As Movimento_Dettaglio) As List(Of dettagli.DettaglioRilievo)
        ...
        If MovimentoDettaglioOperazione.Movimenti_Dettagli_Tecnici IsNot Nothing AndAlso MovimentoDettaglioOperazione.Movimenti_Dettagli_Tecnici.Count > 0 Then
            For Each Movimento_Dettaglio_Tecnico In MovimentoDettaglioOperazione.Movimenti_Dettagli_Tecnici
                ...
                If MovimentoDettaglioOperazione.Movimenti_Destinazioni IsNot Nothing AndAlso MovimentoDettaglioOperazione.Movimenti_Destinazioni.Count > 0 Then
            ...
            For Each Movimento_Dettaglio_Tecnico In MovimentoDettaglioOperazione.Movimenti_Dettagli_Tecnici
                ...
                If MovimentoDettaglioOperazione.Movimenti_Destinazioni IsNot Nothing AndAlso MovimentoDettaglioOperazione.Movimenti_Destinazioni.Count > 0 Then
                    For Each Movimento_Dettaglio_Destinazione In MovimentoDettaglioOperazione.Movimenti_Destinazioni
                        ...
                        dettCorrente.esercizioCDC = CreaEsercizioCdC(attivita, infoOperazione, Movimento_Dettaglio_Destinazione)
    ...
    Private Sub CreaRisorsaProdotto(agenda As Operazione_Agenda, attivita As Attivita, infoOperazione As InfoOperazione, ByRef Dettaglio As RisorsaProdotto, MovimentoDettaglioOperazione As Movimento_Dettaglio, MovimentoMagazzino As Movimento, superficieTrattataTotale As Decimal, acquaTotale As Decimal,
        ...
        If infoOperazione.IsRaccolta Then
            If MovimentoDettaglioOperazione.Movimenti_Destinazioni IsNot Nothing AndAlso MovimentoDettaglioOperazione.Movimenti_Destinazioni.Count > 0 Then
                ...
                For Each Movimento_Dettaglio_Destinazione In MovimentoDettaglioOperazione.Movimenti_Destinazioni
                    ...
                    quantitaSuImpianto.esercizioCDC = CreaEsercizioCdC(attivita, infoOperazione, Movimento_Dettaglio_Destinazione)
        ...
        CreaRilevamentiMagazzino(Dettaglio, infoOperazione, MovimentoMagazzino, MovimentoDettaglioOperazione, superficieTrattataTotale, acquaTotale, agenda.Agenda_Riferimenti, objParametri_Super_Server, objParametri_Server, objParametri_Utenti)
    ...
    Private Sub CreaRisorsaRegistrazione(agenda As Operazione_Agenda, infoOperazione As InfoOperazione, ByRef Dettaglio As RisorsaRegistrazione, MovimentoDettaglioOperazione As Movimento_Dettaglio, MovimentoMagazzino As Movimento,
        ...
        If ProdottoCod <> 0 Then
            ...
            CreaRilevamentiMagazzino(Dettaglio, infoOperazione, MovimentoMagazzino, MovimentoDettaglioOperazione, 0, 0, agenda.Agenda_Riferimenti, objParametri_Super_Server, objParametri_Server, objParametri_Utenti)
    ...
    Public Sub CreaRilevamentiMagazzino(ByRef Dettaglio As Risorsa, infoOperazione As InfoOperazione, MovimentoMagazzino As Movimento, MovimentoDettaglioOperazione As Movimento_Dettaglio, superficieTrattataTotale As Decimal, acquaTotale As Decimal, ByVal MovimentiRiferimenti As List(Of Movimento_Dettaglio_Riferimento),
                                        ByVal objParametri_Super_Server As AgronicaCoreParametri, ByVal objParametri_Server As AgronicaCoreParametri, ByVal objParametri_Utenti As AgronicaCoreParametri)

        If MovimentoMagazzino IsNot Nothing AndAlso MovimentoMagazzino.Movimenti_Dettagli IsNot Nothing AndAlso MovimentoMagazzino.Movimenti_Dettagli.Count > 0 Then
            ...
            Dim movimentiDettaglioMagazzino
            ...
            If infoOperazione.IsCarico OrElse infoOperazione.IsScarico Then
                ...
                movimentiDettaglioMagazzino = (From d In MovimentoMagazzino.Movimenti_Dettagli
                                               Where Utility.Controlla_Se_Stesso_Prodotto(d.Elem_Cod, d.Pro_Cod, d.Mat_Cod, d.Lotto, 0,
                                                                                              MovimentoDettaglioOperazione.Elem_Cod, MovimentoDettaglioOperazione.Pro_Cod,
                                                                                                MovimentoDettaglioOperazione.Mat_Cod, MovimentoDettaglioOperazione.Lotto, 0) AndAlso
                                                       d.Qta = MovimentoDettaglioOperazione.Qta And
                                                   d.Lotto = MovimentoDettaglioOperazione.Lotto
                                               Select d)
            Else
                movimentiDettaglioMagazzino = (From d In MovimentoMagazzino.Movimenti_Dettagli
                                               Where Utility.Controlla_Se_Stesso_Prodotto(d.Elem_Cod, d.Pro_Cod, d.Mat_Cod, d.Lotto, 0,
                                                                                              MovimentoDettaglioOperazione.Elem_Cod, MovimentoDettaglioOperazione.Pro_Cod,
                                                                                                MovimentoDettaglioOperazione.Mat_Cod, MovimentoDettaglioOperazione.Lotto, 0)
                                               Select d)
            End If

            If Not IsNothing(MovimentiRiferimenti) AndAlso MovimentiRiferimenti.Count > 0 Then

                Dim MovimentiRiferimento_Carico_Scarico = (From m_r In MovimentiRiferimenti
                                                           Where m_r.Piva = MovimentoDettaglioOperazione.Piva AndAlso
                                                            m_r.Sa_Cod = MovimentoDettaglioOperazione.Sa_Cod AndAlso
                                                            m_r.Id_Agenda = MovimentoDettaglioOperazione.Id_Agenda AndAlso
                ...
                If Not IsNothing(MovimentiRiferimento_Carico_Scarico) AndAlso MovimentiRiferimento_Carico_Scarico.Count > 0 Then
            ...
            For Each movimentoDettaglioMagazzino In movimentiDettaglioMagazzino
                ...
                If movimentoDettaglioMagazzino IsNot Nothing AndAlso movimentoDettaglioMagazzino.Movimenti_Destinazioni.Count > 0 Then
                    Dim xDestinazione = movimentoDettaglioMagazzino.Movimenti_Destinazioni(0)

                    Dim qtaTrasformata As Decimal = 0

                    Dim dettaglioRisorsa
                    ...
                    If xDestinazione.Id_Destinazione <> 0 Then
                        Dim rilevamentoMagazzino As New RilevamentoDiMagazzino With {
                                    ...
                                    .udm = New UnitaDiMisura(dettaglioRisorsa.unitaDiMisura.codice),
                                    .Qta = movimentoDettaglioMagazzino.Qta,
                                    .Prodotto = New Prodotto(dettaglioRisorsa.prodotto.codice, movimentoDettaglioMagazzino.Elem_Cod) With {
                                        .unitaDiMisura = New UnitaDiMisura(dettaglioRisorsa.unitaDiMisura.codice)
                                    },
                        ...
                        If infoOperazione.IsRaccolta Then
                            For Each q In QsI
                                ...
                                If movimentoDettaglioMagazzino.Cod_Progetto = 0 Then
                                    ...
                                    quantitaSuImpianto.Qta = If(MovimentoDettaglioOperazione.Qta <> 0,
                                    quantitaSuImpianto.Qta * movimentoDettaglioMagazzino.Qta / MovimentoDettaglioOperazione.Qta,
                                    0
                                )
            ...
            For Each Agenda_Carico_Scarico As Operazione_Agenda In List_Agenda_Carico_Scarico_Copy
                ...
                If Not IsNothing(Agenda_Carico_Scarico) AndAlso Agenda_Carico_Scarico.Id_Agenda > 0 Then
                    ...
                    If Not IsNothing(Agenda_Carico_Scarico.Movimenti) AndAlso Agenda_Carico_Scarico.Movimenti.Count > 0 Then
                        ...
                        If Not IsNothing(movimento_Carico_Scarico) AndAlso movimento_Carico_Scarico.Count = 1 AndAlso Not IsNothing(movimento_Carico_Scarico(0).Movimenti_Dettagli) AndAlso movimento_Carico_Scarico(0).Movimenti_Dettagli.Count > 0 Then
                            ...
                            For Each movimento_dettaglio_Carico_Scarico As Movimento_Dettaglio In movimento_Carico_Scarico(0).Movimenti_Dettagli
                                ...
                                If movimentoDettaglioMagazzino.Qta = movimento_dettaglio_Carico_Scarico.Qta Then
                                    If movimento_Carico_Scarico(0).Cau_Mov = CAU_CARICO Then
                                        ...
                                        Mov_Destinazione_Carico = (From m_d In movimento_dettaglio_Carico_Scarico.Movimenti_Destinazioni
                                    ...
                                    ElseIf movimento_Carico_Scarico(0).Cau_Mov = CAU_SCARICO Then
                                        ...
                                        Mov_Destinazione_Scarico = movimento_dettaglio_Carico_Scarico.Movimenti_Destinazioni
                            ...
                            If Not IsNothing(List_Key) AndAlso List_Key.Count > 0 Then
                                Agenda_Carico_Scarico.Movimenti = (From mov In Agenda_Carico_Scarico.Movimenti
                                                                   Where List_Key.FindIndex(Function(key) key.Item1 = mov.Piva AndAlso mov.Sa_Cod = 0 AndAlso
                                                                            key.Item3 = mov.Id_Agenda AndAlso key.Item4 = mov.Id_Mov) > -1
                                                                   Select mov).ToList()

                                For Each mov In Agenda_Carico_Scarico.Movimenti
                                    mov.Movimenti_Dettagli = (From mov_det In mov.Movimenti_Dettagli
                                                              Where List_Key.FindIndex(Function(key) key.Item1 = mov_det.Piva AndAlso key.Item2 = mov_det.Sa_Cod AndAlso
                                                                            key.Item3 = mov_det.Id_Agenda AndAlso key.Item4 = mov_det.Id_Mov AndAlso key.Item5 = mov_det.Id_Mov_Det) > -1
                                                              Select mov_det).ToList()
                                Next

                            End If

                            If Not IsNothing(Mov_Destinazione_Carico) AndAlso Mov_Destinazione_Carico.Count > 0 Then
                                ...
                                If Not IsNothing(movimenti_dettaglio_Carico_x_Prodotto_Lotto) AndAlso movimenti_dettaglio_Carico_x_Prodotto_Lotto.Count > 0 Then
                            ...
                            If Not IsNothing(Mov_Destinazione_Scarico) AndAlso Mov_Destinazione_Scarico.Count > 0 Then
                                ...
                                If Not IsNothing(movimenti_dettaglio_Scarico_x_Prodotto_Lotto) AndAlso movimenti_dettaglio_Scarico_x_Prodotto_Lotto.Count > 0 Then
    ...
    Private Function CreaConsiglio_Irrigazione(ByVal Movimento_Dettaglio_Tecnico As Movimento_Dettaglio_Tecnico) As Consiglio_Irrigazione
        ...
        Dim consiglioIrrigazione As New Consiglio_Irrigazione(IIf(IsDBNull(Movimento_Dettaglio_Tecnico.Extra_Int), 0, Movimento_Dettaglio_Tecnico.Extra_Int), "") With {
                            .qtaAcqua = IIf(IsDBNull(Movimento_Dettaglio_Tecnico.ExtraStr) OrElse Not IsNumeric(Movimento_Dettaglio_Tecnico.ExtraStr), 0, CDec(Movimento_Dettaglio_Tecnico.ExtraStr)),
...
#Region "Funzioni private di estrazione di parti del vecchio modello"
    ...
    Private Function GetPrimoProdottoDaTrattare(agenda As Operazione_Agenda, infoOperazione As InfoOperazione) As Movimento_Dettaglio
        ...
        If MovimentoOperazione IsNot Nothing AndAlso MovimentoOperazione.Movimenti_Dettagli IsNot Nothing AndAlso
            MovimentoOperazione.Movimenti_Dettagli.Count > 0 Then
    ...
    Private Function GetMovimentoFromCausale(agenda As Operazione_Agenda, causale As String) As Movimento
        Dim Movimento As Movimento = agenda.Movimenti.FindAll(Function(c) (c.Cau_Mov = causale)).FirstOrDefault
        Return Movimento
    End Function
#End Region
