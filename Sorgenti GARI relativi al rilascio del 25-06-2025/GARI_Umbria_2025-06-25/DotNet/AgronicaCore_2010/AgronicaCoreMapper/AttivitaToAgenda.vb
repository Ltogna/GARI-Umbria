...
Public Class AttivitaToAgenda
    ...
    Public Function MappaAttivitaToAgenda(attivita As Attivita,
        ...
        '----- MOVIMENTI DETTAGLI 
        Dim Movimenti_Dettaglio As List(Of Movimento_Dettaglio) = CreaMovimentiDettaglio(attivita, Agenda, InfoOperazione, SuperficieTrattataTotale, acquaTotale, objParametri_Super_Server, objParametri_Server, objParametri_Utenti)
    ...
    Public Function SplitByImpianto(ByRef attivitaList As List(Of Attivita), objParametri_Server As AgronicaCoreParametri, objParametri_Utenti As AgronicaCoreParametri) As Boolean
        ...
        Dim creaUnaOperazionePerOgniImpianto As Integer = CInt(objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.SUPERUSER_COD_RICETTE_CREA_UNA_OPERAZIONE_PER_OGNI_IMPIANTO, Username_1Utente_o_2SuperUser:=2, "0", objParametri_Utenti))
...
#Region "Funzioni private"
            ...
            Else
                ...
                If Not (eseguiSoloVerificheConformita) Then
                    ...
                    If attivita.tipo = Tipo_Attivita.QuadernoDiCampagna Then
                        ...
                        '   SCRITTURA AGENDA
                        '-----------------------------------
                        Id_Agenda = scriviAgenda(agenda, attivita, parametriAggiuntiviList, agendeToDelete, lista_mdRif, objParametri_Server, objParametri_Utenti)
    ...
    Private Sub ScriviAttivitaToAgenda_SeminaConFrazionamento(attivita As AgronicaCoreModelsSTD.attivita.Attivita,
        ...
        If Not (eseguiSoloVerificheConformita) Then
            ...
            For Each AttivitaxDettaglioSemina In listAttivitaxDettaglioSemina
                ...
                Dim Id_Agenda = scriviAgenda(agendaxDettaglioSemina, AttivitaxDettaglioSemina, parametriAggiuntiviList, agendeToDelete:=Nothing, agenda_Rif:=Nothing, objParametri_Server, objParametri_Utenti)
    ...
    Private Sub ScriviAttivitaToAgenda_SeminaConAggiornamentoAnagrafica(attivita As AgronicaCoreModelsSTD.attivita.Attivita,
        ...
        If Not (eseguiSoloVerificheConformita) Then
            ...
            idAgenda_to_return = scriviAgenda(agenda, attivita, parametriAggiuntiviList, agendeToDelete:=Nothing, agenda_Rif:=Nothing, objParametri_Server, objParametri_Utenti)
    ...
    Private Shared Function scriviAgenda(agenda As Operazione_Agenda,
        ...
        If attivita.tipo = Tipo_Attivita.QuadernoDiCampagna AndAlso Not IsNothing(attivita.risorse) AndAlso attivita.risorse.Count > 0 Then
            ...
            If Not IsNothing(RisorseConRegistrazioniContabili) AndAlso RisorseConRegistrazioniContabili.Count > 0 Then
                CreaOperazioniContabili(agenda, attivita.job, RisorseConRegistrazioniContabili.ToList(), agenda_Rif, objParametri_Server, objParametri_Utenti)
            End If
    ...
    Private Function CreaMovimentoDettaglioTecnico(attivita As Attivita, agenda As Operazione_Agenda, risorsa As risorse.Risorsa, infoOperazione As InfoOperazione, objParametri_Super_Server As AgronicaCoreParametri, objParametri_Server As AgronicaCoreParametri, objParametri_Utenti As AgronicaCoreParametri) As Movimento_Dettaglio_Tecnico
        ...
        If infoOperazione.IsTrattamento Then
            ...
            If attivita.job.primaryKey.codice = LAVCOD_CONFUSIONE_DISORIENTAMENTO_SESSUALE Then
    ...
    Private Function CreaMovimentiDettaglio(attivita As Attivita, agenda As Operazione_Agenda, infoOperazione As InfoOperazione, superficieTrattataTotale As Decimal, acquaTotale As Decimal, objParametri_Super_Server As AgronicaCoreParametri, objParametri_Server As AgronicaCoreParametri, objParametri_Utenti As AgronicaCoreParametri) As List(Of Movimento_Dettaglio)
            ...
            For Each risorsaProdotto In risorseProdotto
                ...
                If risorsaProdotto.MagazziniMovimentazioni IsNot Nothing AndAlso risorsaProdotto.MagazziniMovimentazioni.Count > 0 Then
                    For Each movimentoMagazzino In risorsaProdotto.MagazziniMovimentazioni
                        ...
                        updateQtaLottoMovimentoDettaglio(movimentoDettaglio, GetDosePerMagazzino(movimentoMagazzino.Qta, risorsaProdotto.unitaDiMisuraIndicata.codice), movimentoMagazzino.doseHlIndicata, movimentoMagazzino.doseHaIndicata, superficieTrattataTotale, acquaTotale, movimentoMagazzino.Lotto, infoOperazione)
                ...
                Else 'Non ho movimentazioni di magazzino
                    ...
                    updateQtaLottoMovimentoDettaglio(movimentoDettaglio, risorsaProdotto.quantitaTotaleReale, risorsaProdotto.doseHlReale, risorsaProdotto.doseHaReale, superficieTrattataTotale, acquaTotale, lotto:="", infoOperazione)
        ...
        If risorseIrrigazione IsNot Nothing AndAlso risorseIrrigazione.Count > 0 Then
            For Each dettaglioIrrigazione In risorseIrrigazione
                ...
                Movimento_Dettaglio_Irrigazione.Movimenti_Dettagli_Tecnici = New List(Of Movimento_Dettaglio_Tecnico)
                Dim Movimento_Dettaglio_Tecnico = CreaMovimentoDettaglioTecnico(attivita, agenda, dettaglioIrrigazione, infoOperazione, objParametri_Super_Server, objParametri_Server, objParametri_Utenti)
                If Movimento_Dettaglio_Tecnico IsNot Nothing Then
                    Movimento_Dettaglio_Irrigazione.Movimenti_Dettagli_Tecnici.Add(Movimento_Dettaglio_Tecnico)
                End If
            Next
        End If
        ...
        For Each movimentoDettaglio In Movimenti_Dettagli
            ...
            If superficieTrattataTotale > 0 Then
                ...
                If infoOperazione.IsRaccolta Then
                    ...
                    Else
                        movimentoDettaglio.Movimenti_Destinazioni = CreaMovimentiDestinazione(attivita, agenda, infoOperazione, DoseHA_Trasformata, superficieTrattataTotale)
                    End If
    ...
    Private Function CreaMovimentiDettaglioRilievo(attivita As Attivita, agenda As Operazione_Agenda, infoOperazione As InfoOperazione, objParametri_Super_Server As AgronicaCoreParametri, objParametri_Server As AgronicaCoreParametri, objParametri_Utenti As AgronicaCoreParametri) As List(Of Movimento_Dettaglio)
        ...
        If risorseRilievo IsNot Nothing AndAlso risorseRilievo.Count > 0 Then
            ...
            For Each dettaglioRilievo In risorseRilievo
                ...
                Select Case agenda.Lav_Cod
                    Case LAVCOD_RILIEVO_AVVERSITA_CAMPO
                        ...
                        rilievoKey = av_cod & "-" & av_gru & "-" & udm_cod
            ...
            Select Case agenda.Lav_Cod
                Case LAVCOD_RILIEVO_AVVERSITA_CAMPO, LAVCOD_RILIEVO_ERBE_INFESTANTI
                    ...
                    For Each dettaglioRilievo In risorseRilievo
                        ...
                        If av_cod <> 0 OrElse av_gru <> 0 Then
                            ...
                            Dim avKey As String = av_cod & "-" & av_gru & "-" & udm_cod
                            ...
                            If Not dictAvversita.ContainsKey(avKey) Then
                                ...
                                Dim Movimento_Dettaglio_Tecnico = CreaMovimentoDettaglioTecnico(attivita, agenda, dettaglioRilievo, infoOperazione, objParametri_Super_Server, objParametri_Server, objParametri_Utenti)
                                If Movimento_Dettaglio_Tecnico IsNot Nothing Then
    ...
    Private Sub updateQtaLottoMovimentoDettaglio(ByRef movimentoDettaglio As Movimento_Dettaglio, qtaTotale As Decimal, doseHl As Decimal, doseHa As Decimal, superficieTrattataTotale As Decimal, acquaTotale As Decimal, lotto As String, infoOperazione As InfoOperazione)
    ...
    Private Sub checkCompatibilita(agenda As Operazione_Agenda, attivita As Attivita, risorsaProdotto As risorse.RisorsaProdotto, movimentoDettaglio As Movimento_Dettaglio, InfoOperazione As InfoOperazione, objParametri_Super_Server As AgronicaCoreParametri, objParametri_Server As AgronicaCoreParametri, objParametri_Utenti As AgronicaCoreParametri)
        ...
        End If
        Dim Movimento_Dettaglio_Tecnico_Current = CreaMovimentoDettaglioTecnico(attivita, agenda, risorsaProdotto, InfoOperazione, objParametri_Super_Server, objParametri_Server, objParametri_Utenti)
        If InfoOperazione.IsTrattamento Then
    ...
    Private Function CreaMovimentoDettaglio(risorsaProdotto As risorse.RisorsaProdotto, movimentoMagazzino As RilevamentoDiMagazzino, attivita As Attivita, agenda As Operazione_Agenda, infoOperazione As InfoOperazione, SuperficieTrattataTotale As Decimal, ByVal objParametri_Super_Server As AgronicaCoreParametri, ByVal objParametri_Server As AgronicaCoreParametri, ByVal objParametri_Utenti As AgronicaCoreParametri) As Movimento_Dettaglio
        ...
        If infoOperazione.IsIrrigazione OrElse infoOperazione.Elem_Cod = TRAPPOLE Then
            With Movimento_Dettaglio_Prodotto
                .Pendente = 3
                .Anno = 1900
            End With
        ...
        Dim Movimento_Dettaglio_Tecnico = CreaMovimentoDettaglioTecnico(attivita, agenda, risorsaProdotto, infoOperazione, objParametri_Super_Server, objParametri_Server, objParametri_Utenti)
        If Movimento_Dettaglio_Tecnico IsNot Nothing Then
    ...
    Private Function CreaMovimentiDettaglioScarico(attivita As Attivita, agenda As Operazione_Agenda, infoOperazione As InfoOperazione, objParametri_Server As AgronicaCoreParametri) As List(Of Movimento_Dettaglio)
        ...
        'DT: chiave: prodotto,lotto,piva,sa_cod,fabbricato_cod
        Dim dictScarico As New Dictionary(Of (Integer, String, String, Integer, Integer), (movimentoDettaglio As Movimento_Dettaglio, movimentoDestinazione As Movimento_Destinazione))
...
movimentoMagazzino.Magazzino IsNot Nothing AndAlso
                        ...
                        If Not dictScarico.ContainsKey((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)) Then
                            ...
                            If infoOperazione.Elem_Cod = TRAPPOLE Then
                                With Movimento_Dettaglio_Scarico
                            ...
                            dictScarico.Add((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod), (Movimento_Dettaglio_Scarico, New Movimento_Destinazione))

                        End If

                        dictScarico((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)).movimentoDettaglio.Qta += movimentoMagazzino.Qta
                        ...
                        If movimentoMagazzino.Agenzia IsNot Nothing Then
                            ...
                            Dim magazzinoesterno_cod_list = dictScarico((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)).movimentoDestinazione.MagazzinoEsterno_Cod
                            Dim magazzinoesterno_des_list = dictScarico((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)).movimentoDestinazione.MagazzinoEsterno_Des
                            Dim magazzinoesterno_dettagli_list = dictScarico((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)).movimentoDestinazione.MagazzinoEsterno_Dettagli
                            ...
                            dictScarico((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)).movimentoDestinazione.MagazzinoEsterno_Cod = magazzinoesterno_cod_list
                            dictScarico((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)).movimentoDestinazione.MagazzinoEsterno_Des = magazzinoesterno_des_list
                            dictScarico((codiceProdotto, lotto.ToUpper, piva, sa_cod, fabbricato_cod)).movimentoDestinazione.MagazzinoEsterno_Dettagli = magazzinoesterno_dettagli_list
                        End If
        ...
        For Each itemDistinctScarico In dictScarico
            ...
            With itemDistinctScarico.Value.movimentoDestinazione
                ...
                .Appezza = 0
                .Id_Destinazione = itemDistinctScarico.Key.Item5
                .Tipo = MAGAZZINO
    ...
    Private Function CreaMovimentiDestinazione(attivita As Attivita, agenda As Operazione_Agenda, infoOperazione As InfoOperazione, Dose As Decimal, SuperficieTrattataTotale As Decimal) As List(Of Movimento_Destinazione)
...
#Region "Controlli"
    Public Sub CheckMagazzino_ListaAttivita(lista_Attivita As List(Of AgronicaCoreModelsSTD.attivita.Attivita),
        ...
        Try
            ...
            '-----------------------------------
            If esisteAlmenoUnaMovimentazione(lista_Attivita) Then
    ...
    Private Shared Sub CheckGiacenze(mostraWarning_CheckMagazzino As Boolean,
        ...
        Dim objUtentiImpostazioni As New AgronicaCoreUtentiDAL.Utenti_Impostazioni_Read
        Dim BloccoGiacenze As Integer = CInt(objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_SE_SUPERA_GIACENZE, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti))
        ...
        For Each dic In dic_AgendexElemCod
            ...
            Dim Elem_Cod As Integer = dic.Key
                    ...
                    If _movimento.Cau_Mov = CAU_SCARICO Then
                        For Each _dettaglio In _movimento.Movimenti_Dettagli
                            For Each _destinazione In _dettaglio.Movimenti_Destinazioni
                                ...
                                Dim DescrizioneProdotto As String = Utility_Agenda.GetDescrizioneProdotto(Elem_Cod, Mat_Cod, Pro_Cod, _agenda.Piva, objParametri_Server)
    ...
    Public Sub CheckDPI_listaAttivita(lista_Attivita As List(Of Attivita),
...
#End Region
    ...
    Public Shared Sub CheckDPI(agenda As Operazione_Agenda,
        ...
        Try
            ...
            If infoOperazione.IsFertilizzazione Then
                getSeverityDaValoreImpostazione_0Warning_1WarningBloccante(severity, enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_SE_SUPERA_LIMITIMAS, objParametri_Utenti)
            End If
    ...
    Public Shared Function CheckAttivita(attivita As Attivita,
        ...
        ValoreImpostazione = CInt(objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_SE_SENZA_MAGAZZINO, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti))
        ...
        ValoreImpostazione = CInt(objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_DEFAULT_BLOCCO_INSERIMENTO_NOTE, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti))
        If ValoreImpostazione = 1 Then
        ...
        ValoreImpostazione = CInt(objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_DEFAULT_BLOCCO_INSERIMENTO_MACCHINE, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti))
        If ValoreImpostazione = 1 Then
        ...
        ValoreImpostazione = CInt(objUtentiImpostazioni.LeggiConDefault(enum_Impostazioni_Utenti.UTENTE_COD_DEFAULT_BLOCCO_INSERIMENTO_OPERATORE, Username_1Utente_o_2SuperUser:=1, "0", objParametri_Utenti))
        If ValoreImpostazione = 1 Then
        ...
        '-----------------------------------
        If InfoOperazione.IsRaccolta AndAlso parametriAggiuntiviList IsNot Nothing AndAlso mostraWarning_CheckListaAttivita Then
    ...
    Public Shared Function checkMagazziniRicettexApp_daAttivita(listaAttivita As List(Of Attivita),
        ...
        listErroriGias = componiErroreFinale_controlloMagazziniRicettexApp(False, listErrori, listErroriRicetteMultiAttivita, listErrorixRicetta.Count)
...
Ricetta_Operazione_Cod As Integer,
    ...
    Private Shared Sub CreaOperazioniContabili(ByVal agenda As Operazione_Agenda,
        ...
        Try
            ...
            ScritturaOperazioniScarico = Crea_Operazioni_Carico_Scarico(agenda, Job_Op_Campagna, List_Id_Agenda_Scarico_Old, ListAttivitaScarico, objParametri_Server, objParametri_Utenti)

            ScritturaOperazioniCarico = Crea_Operazioni_Carico_Scarico(agenda, Job_Op_Campagna, List_Id_Agenda_Carico_Old, ListAttivitaCarico, objParametri_Server, objParametri_Utenti)
        ...
        Try
            ...
            For Each attivita As Attivita In ListAttivita_Carico_Scarico
                ...
                If Not IsNothing(Registrazioni) AndAlso Registrazioni.Count > 0 Then
                    xRisp = Crea_Operazione_Carico_Scarico(Agenda_Op_Campagna, Registrazioni, Lav_Cod, Job_Op_Campagna, Cau_Mov, Id_Agenda, objParametri_Server, objParametri_Utenti)
    ...
    Private Shared Function Crea_Operazione_Carico_Scarico(ByVal Agenda_Op_Campagna As Operazione_Agenda,
        ...
        Try
            ...
            Dim objContabTestata As Contabilita_Output = objContabilitaHelper_Dettaglio.ScriviTestataCaricoScaricoAziendaAgricola(Agenda_Op_Campagna, Piva_Magazzino, Id_Agenda, ScaricoCaricoSemplice, Cau_Mov, 1, DescrizioneAggiuntiva)
            ...
            If objContabTestata.Risultato Then
                ...
                For Each RegistrazioneScarico As RisorsaRegistrazione In Registrazioni
                    ...
                    For Each magazzinoMovimentazione As RilevamentoDiMagazzino In RegistrazioneScarico.MagazziniMovimentazioni
                        ...
                        Dim contabilita_riga As Contabilita_Riga = Crea_Contabilita_Riga(Piva_Magazzino, magazzinoMovimentazione, magazzinoMovimentazione.Qta, magazzinoMovimentazione.udm, objContabTestata)
    ...
    Private Shared Function Crea_Contabilita_Riga(ByVal piva As String, ByVal magazzinoMovimentazione As RilevamentoDiMagazzino, ByVal Qta As Decimal, ByVal UdM As UnitaDiMisura, ByVal objContabTestata As Contabilita_Output,
        ...
        Try
            ...
            contabilita_riga = New Contabilita_Riga With {
                ...
                .OrdineDet = 0,
                .P2O5 = 0,
                .Pendente = enum_Pendenza.MovPendente,
                .piva = piva,
                .Prezzo = 0,
