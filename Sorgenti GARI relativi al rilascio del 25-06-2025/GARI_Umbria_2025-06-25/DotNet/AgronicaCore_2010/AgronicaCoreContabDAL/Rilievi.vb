...
Public Class Rilievi_R
    ...
    Public Function getRilieviQueryString(ByRef objParametri_Utenti As AgronicaCoreDataProvider.AgronicaCoreParametri) As StringBuilder
        ...
        strSql.AppendLine("                 WHEN Mov_Destinazioni.Qta = 1 THEN 'Y' ")
        strSql.AppendLine("                 WHEN Mov_Destinazioni.Qta = 0 THEN 'N' ")
        strSql.AppendLine("                 ELSE CAST(Mov_Destinazioni.Qta AS varchar) ")
        strSql.AppendLine("             END ")
        strSql.AppendLine("         ELSE ")
        strSql.AppendLine("             CASE WHEN Agenda.Lav_Cod = " & LAVCOD_RILIEVO_INDICI_MATURITA & " ")
        strSql.AppendLine("                     THEN ISNULL(IM_A.Anag_des, Mov_Destinazioni.Qta) ")
        strSql.AppendLine("                  WHEN Agenda.Lav_Cod = " & LAVCOD_RILIEVO_AVVERSITA_CAMPO & " ")
        strSql.AppendLine("                     THEN ISNULL(anag.Anag_des, Mov_Destinazioni.Qta) ")
        strSql.AppendLine("                  ELSE CAST(Mov_Destinazioni.Qta AS varchar) ")
        strSql.AppendLine("             END ")
