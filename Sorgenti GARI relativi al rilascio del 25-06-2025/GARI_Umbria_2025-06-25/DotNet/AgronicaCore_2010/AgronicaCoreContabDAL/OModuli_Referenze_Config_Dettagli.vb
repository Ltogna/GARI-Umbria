        ...
        Using GiasContext As New Gias_DeveloperServer_Entities(EFConnString)
            ...
            Dim ParamQual =
             ...
             Select New With {
                 ...
                 .Tipo = parametri_qualitativi.Tipo,
...
#End Region
    ...
    Private Function ImpostaRigheCancellate(tutteleRigheArray As JArray,
        ...
        For Each obj As JObject In righeArray
            ...
            messaggioErrore = VerificaRigaCancellazioneValida(dettaglio, tutteleRigheArray, objParametri)
    ...
    Private Function VerificaRigaCancellazioneValida(dettaglio As OModuli_Referenze_Config_Dettagli,
                                                     tutteleRigheArray As JArray,
                                                     ByRef objParametri As AgronicaCoreParametri) As String
        Dim messaggio As String = String.Empty
        ' Verificare se il parametro è referenziato da altri parametri
        Dim testata = tutteleRigheArray _
            .Where(Function(x) x("Id_Testata").ToString().Equals(dettaglio.Id_Testata.ToString())) _
            .GroupBy(Function(key) key("Id_Testata").ToString(),
                         Function(gr) gr,
                         Function(k, g) New With
                         {
                 ...
                 .FirstOrDefault()
        If IsNothing(testata) Then
            Throw New Exception($"Errore imprevisto - Non è stata trovara la configurazione della testata con id: {dettaglio.Id_Testata}")
        Else
            Dim parametroReferenziato = testata.Parametri _
                .Where(Function(x) Not String.IsNullOrEmpty(x.Tabella_Key_Rif) AndAlso
                    x.Tabella_Key_Rif.ToString().Equals(dettaglio.Tabella_Key, StringComparison.OrdinalIgnoreCase)) _
                .Select(Function(x) x.Tabella_Key) _
                .ToList()

            If parametroReferenziato.Any() Then
