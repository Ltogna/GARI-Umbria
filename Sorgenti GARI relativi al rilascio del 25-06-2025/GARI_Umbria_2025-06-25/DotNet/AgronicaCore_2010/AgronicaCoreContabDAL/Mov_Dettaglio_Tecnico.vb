...
#Region "Trappole - Inneschi"
    ...
    Function Trappole_107122_X_ImpiantiInfluenza(ByVal PIVA As String,
        ...
        Try
            ...
            StrSQL.Append(" Mov_Dettaglio_Tecnico.Piva AS Azienda , ")
            StrSQL.Append(" Mov_Dettaglio_Tecnico.Sa_Cod AS Centro_Aziendale , ")
            StrSQL.Append(" Mov_Dettaglio_Tecnico.Trap_Num AS Numero_Trappola , ")
            StrSQL.Append(" Mov_Dettaglio_Tecnico.Freatimetro AS Numero_Personalizzato , ")
            StrSQL.Append(" Mov_Dettaglio_Tecnico.Sigla_AV AS Sigla_Avversita , ")
            StrSQL.Append(" Mov_Dettaglio_Tecnico.Av_Cod AS Codice_Avversita , ")
            StrSQL.Append(" Mov_Dettaglio_Tecnico.Ditta_Cod AS Codice_Ditta , ")

            StrSQL.Append(" Imp_Install.Appezza           AS Imp_Install_Appezza , ")
            StrSQL.Append(" Imp_Install.Id_Destinazione   AS Imp_Install_Id_Destinazione  , ")
            StrSQL.Append(" Imp_Install.Qta               AS Imp_Install_Qta , ")

            StrSQL.Append(" Imp_Influ.Appezza           AS Imp_Influ_Appezza , ")
            StrSQL.Append(" Imp_Influ.Id_Destinazione   AS Imp_Influ_Id_Destinazione  , ")
            StrSQL.Append(" Imp_Influ.Qta               AS Imp_Influ_Qta , ")

            '(19/10/2017) aggiunti per evitare letture dopo
            StrSQL.Append(" App.app_nome , ")
            StrSQL.Append(" isnull(Cam.Campo_Des,'') as campo_des , ")
            StrSQL.Append(" ISNULL((SELECT TOP 1 Appezzamento_Codici.val_cod FROM Appezzamento_Codici  " & vbCrLf)
            StrSQL.Append(" where (Appezzamento_Codici.PIVA = App.PIVA) " & vbCrLf)
            StrSQL.Append(" and (Appezzamento_Codici.sa_cod = App.sa_cod) " & vbCrLf)
            StrSQL.Append(" and (Appezzamento_Codici.appezza = App.appezza) " & vbCrLf)
            StrSQL.Append(" and (Appezzamento_Codici.id_cod = 1104) ), '') AS App_Nome_Breve " & vbCrLf)

            If EstraiLotto Then
                StrSQL.Append(", ISNULL(imp_p.Progetto_Nome,'') AS Lotto " & vbCrLf)
            End If

            '------------Superflui-----------
            'StrSQL.Append(" Agenda.* , ")
            'StrSQL.Append(" Movimenti.* , ")
            'StrSQL.Append(" Movimenti_dettagli.* , ")
            'StrSQL.Append(" Mov_Dettaglio_Tecnico.*  ")
            '-----------------------

            StrSQL.Append(" FROM  Agenda  ")

            StrSQL.Append("     INNER JOIN [Movimenti] ")
            StrSQL.Append("                 ON    Agenda.Piva  = Movimenti.Piva ")
            StrSQL.Append("                 AND   Agenda.Sa_Cod = Movimenti.Sa_Cod ")
            StrSQL.Append("                 AND   Agenda.Id_Agenda =  Movimenti.Id_Agenda ")

            StrSQL.Append("     INNER JOIN [Movimenti_dettagli] ")
            StrSQL.Append("                 ON    Movimenti.Piva  = Movimenti_dettagli.Piva ")
            StrSQL.Append("                 AND   Movimenti.Sa_Cod = Movimenti_dettagli.Sa_Cod ")
            StrSQL.Append("                 AND   Movimenti.Id_Agenda =  Movimenti_dettagli.Id_Agenda ")
            StrSQL.Append("                 AND   Movimenti.Id_Mov =  Movimenti_dettagli.Id_Mov ")


            StrSQL.Append("     INNER JOIN [Mov_Dettaglio_Tecnico] ")
            StrSQL.Append("                 ON    Movimenti_dettagli.Piva  = Mov_Dettaglio_Tecnico.Piva ")
            StrSQL.Append("                 AND   Movimenti_dettagli.Sa_Cod = Mov_Dettaglio_Tecnico.Sa_Cod ")
            StrSQL.Append("                 AND   Movimenti_dettagli.Id_Agenda =  Mov_Dettaglio_Tecnico.Id_Agenda ")
            StrSQL.Append("                 AND   Movimenti_dettagli.Id_Mov =  Mov_Dettaglio_Tecnico.Id_Mov ")
            StrSQL.Append("                 AND   Movimenti_dettagli.Id_Mov_Det =  Mov_Dettaglio_Tecnico.Id_Mov_Det ")
            ...
            StrSQL.Append("     INNER JOIN [Mov_Destinazioni] AS Imp_Install ")
            StrSQL.Append("              ON   Mov_Dettaglio_Tecnico.Piva = Imp_Install.Piva  ")
            StrSQL.Append("              AND  Mov_Dettaglio_Tecnico.Sa_Cod = Imp_Install.Sa_Cod  ")
            StrSQL.Append("              AND  Mov_Dettaglio_Tecnico.Id_Agenda = Imp_Install.Id_Agenda  ")
            StrSQL.Append("              AND  Mov_Dettaglio_Tecnico.Id_Mov = Imp_Install.Id_Mov  ")
            StrSQL.Append("              AND  Mov_Dettaglio_Tecnico.Id_Mov_Det = Imp_Install.Id_Mov_Det  ")
            '--------------------------------
            '--------------------------------
            ...
            StrSQL.Append("     INNER JOIN [Mov_Destinazioni] AS Imp_Influ ")
            StrSQL.Append("              ON   Mov_Dettaglio_Tecnico.Piva = Imp_Influ.Piva  ")
            StrSQL.Append("              AND  Mov_Dettaglio_Tecnico.Sa_Cod = Imp_Influ.Sa_Cod  ")
            StrSQL.Append("              AND  Mov_Dettaglio_Tecnico.Id_Agenda = Imp_Influ.Id_Agenda  ")
            StrSQL.Append("              AND  Mov_Dettaglio_Tecnico.Id_Mov = Imp_Influ.Id_Mov  ")
            '--------------------------------
            '--------------------------------

            '(19/10/2017) aggiunti per evitare letture dopo
            StrSQL.Append("     INNER JOIN [Appezzamento] AS App ")
            StrSQL.Append("              ON   App.Piva = Imp_Influ.Piva  ")
            StrSQL.Append("              AND  App.Sa_Cod = Imp_Influ.Sa_Cod  ")
            StrSQL.Append("              AND  App.appezza = Imp_Influ.appezza  ")
            StrSQL.Append("     LEFT JOIN [Campi] AS Cam ")
            StrSQL.Append("              ON   App.Piva = Cam.Piva  ")
            StrSQL.Append("              AND  App.Sa_Cod = Cam.Sa_Cod  ")
            StrSQL.Append("              AND  App.campo_cod = Cam.campo_cod  ")

            If LeftJoin_RegImpianti_ImpreseProgetti Then
                StrSQL.Append("     LEFT JOIN [Reg_Impianti] AS reg_imp ")
                StrSQL.Append("              ON   App.Piva = reg_imp.Piva  ")
                StrSQL.Append("              AND  App.Sa_Cod = reg_imp.Sa_Cod  ")
                StrSQL.Append("              AND  App.appezza = reg_imp.appezza  ")
                StrSQL.Append("     LEFT JOIN [Imprese_Progetti] AS imp_p ")
                StrSQL.Append("              ON   reg_imp.Piva = imp_p.Piva  ")
                StrSQL.Append("              AND  reg_imp.Sa_Cod = imp_p.Sa_Cod  ")
                StrSQL.Append("              AND  reg_imp.appezza = imp_p.appezza  ")
                StrSQL.Append("              AND  reg_imp.id_reg = imp_p.id_reg  ")
            End If
            ...
            StrSQL.Append(" WHERE   Agenda.Lav_Cod IN (107, 122) ")
            ...
            If xOrderBy <> "" Then
                StrSQL.Append(" ORDER BY " & Agro_SQL_Save_xOrderBy(xOrderBy, objParametri))
            Else
                StrSQL.Append(" ORDER BY Azienda,Centro_Aziendale, Numero_Trappola, Imp_Install_Appezza, Imp_Influ_Appezza ")
            End If
            '--------------------------------------------------------------------------
