...
Public Class AccessoMultiQuery : Inherits DataProvider
    ...
    Public Function MLT_SelectFiltrataConTabellaTemporanea_2013(ByVal Query1_TempTableCreazione As String,
        ...
        Dim queryParametrizzata As Boolean = False
        ...
        If DataProviderFactory.Instance.TipoProvider = TipiEnumerativi.enum_DataProvidersType.SqlDataProvider Then
            StringaSql = Query1_TempTableCreazione
            queryParametrizzata = Parametrizza(StringaSql, parameters)
            xComando1.CommandText = StringaSql
            If queryParametrizzata Then
                For Each p As SqlParameter In parameters
                    xComando1.Parameters.Add(p)
                Next
            End If
        Else
            xComando1.CommandText = Query1_TempTableCreazione
        ...
        If DataProviderFactory.Instance.TipoProvider = TipiEnumerativi.enum_DataProvidersType.SqlDataProvider Then
            ...
            StringaSql = Query2_TempTableIndice
            queryParametrizzata = Parametrizza(StringaSql, parameters)
            xComando2.CommandText = StringaSql
            If queryParametrizzata Then
                For Each p As SqlParameter In parameters
                    xComando2.Parameters.Add(p)
                Next
            End If
        ...
        If DataProviderFactory.Instance.TipoProvider = TipiEnumerativi.enum_DataProvidersType.SqlDataProvider Then
            ...
            StringaSql = Query3_TempTableFill
            queryParametrizzata = Parametrizza(StringaSql, parameters)
            xComando3.CommandText = StringaSql
            If queryParametrizzata Then
                For Each p As SqlParameter In parameters
                    xComando3.Parameters.Add(p)
                Next
            End If
        ...
        If DataProviderFactory.Instance.TipoProvider = TipiEnumerativi.enum_DataProvidersType.SqlDataProvider Then
            ...
            StringaSql = Query4_TempTableJoin
            queryParametrizzata = Parametrizza(StringaSql, parameters)
            xComando4.CommandText = StringaSql
            If queryParametrizzata Then
                For Each p As SqlParameter In parameters
                    xComando4.Parameters.Add(p)
                Next
            End If
