    ...
    Public Module Operazione_Agenda_Utility
            ...
            Select Case CInt(objParametriAgenda.Lav_Cod)
                ...
                Case Else
                    ...
                    If Not GestisciCancellazione(objParametriAgenda,
                                                 ...
                                                 lista_IdAgenda_Cancellati:=lista_IdAgenda_Cancellati) Then
                        Return False
                    End If
        ...
        Public Function GestisciCancellazione(ByRef objParametriAgenda As ParametriAgenda_Temp.ParametriAgenda,
            ...
            If DT_Agenda.Rows.Count > 0 Then
                ...
                If permessiGruppiMerce = False Then
                    Throw New Exception("Il documento contiene dettagli non visualizzabili per il gruppo merce a loro associato, non è possibile continuare con la cancellazione")
                End If
                ...
                If CancellaOperazioneSingolaNoCorrelate = False Then
                    ...
                    'Elimino le Agende con lo stesso Raccoglitore_Cod ed elimino anche le Agende collegate con i Riferimenti (Carichi,Scarichi,DDT Emessi,DDT Ricevuti)
                    ...
                    End If
                        ...
                        For Each Agenda_ToDelete In list_Agenda_ToDelete
                            ...
                            If ControllaOperazione(matrice_delete, objParametriAgenda, objParametri_Server, messaggio, IgnoraAvvisoWarning, "",
                                                    Id_Mov, Id_Mov_Det, Modalita_Protetta, Bypass_Delete_Exceptions, objParametri_Utenti:=objParametri_Utenti) Then

                                Dim rval As Boolean = OperazioneAgenda_Cancella(matrice_delete, objParametriAgenda, objParametri_Server, False, idServizio, origine)
                                If rval = False Then
                                    Return rval
                                End If
                ...
                Else
                    ...
                    If CancellaOperazioneSingolaNoCorrelate = True Then
                        ...
                        Dim rval As Boolean = OperazioneAgenda_Cancella(matrice_delete, objParametriAgenda, objParametri_Server, CancellaOperazioneSingolaNoCorrelate, idServizio, origine)
                        Return rval

                    ElseIf ControllaOperazione(matrice_delete, objParametriAgenda, objParametri_Server, messaggio, IgnoraAvvisoWarning, "",
                                               Id_Mov, Id_Mov_Det, Modalita_Protetta, Bypass_Delete_Exceptions, Preserva_Costi,, Preserva_GHG, objParametri_Utenti:=objParametri_Utenti) Then
            ...
            If Not IsNothing(DT_Rif) AndAlso DT_Rif.Rows.Count > 0 Then
                For Each Agenda_rif In DT_Rif.Rows
                    ...
                    If Agenda_rif("Lav_Cod_Rif") = LAVCOD_CARICO OrElse Agenda_rif("Lav_Cod_Rif") = LAVCOD_SCARICO OrElse
                        Agenda_rif("Lav_Cod_Rif") = LAVCOD_BOLLA_EMESSA OrElse Agenda_rif("Lav_Cod_Rif") = LAVCOD_BOLLA_RICEVUTA Then
        ...
        Public Function ControllaOperazione(ByRef matrice_delete(,) As String,
                ...
                Case enum_TipoOperazioneDB.Cancellazione, enum_TipoOperazioneDB.Modifica
                    ...
                    If Not SportelloAperto OrElse (objParametriAgenda.Data < dataMin OrElse objParametriAgenda.Data > dataMax) AndAlso AziendaInVerifica = False Then
                        ...
                        Select Case objParametriAgenda.Tipo_Operazione
                            ...
                            Case enum_TipoOperazioneDB.Cancellazione
                                Throw New Exception(AgronicaCoreModelloRes.SportelloChiusoImpossibileEliminareOperazione)

                            Case enum_TipoOperazioneDB.Modifica
                                Throw New Exception(AgronicaCoreModelloRes.SportelloChiusoImpossibileModificareOperazione)
                    ...
                    ElseIf AziendaInVerifica Then
                        ...
                        Select Case objParametriAgenda.Tipo_Operazione
                            ...
                            Case enum_TipoOperazioneDB.Cancellazione
                                Throw New Exception(AgronicaCoreModelloRes.AziendaInVerificaImpossibileEliminareOperazione)

                            Case enum_TipoOperazioneDB.Modifica
                                Throw New Exception(AgronicaCoreModelloRes.AziendaInVerificaImpossibileModificareOperazione)
            ...
            If DT_SDI.Rows.Count > 0 Then
                ...
                Select Case objParametriAgenda.Tipo_Operazione
                    ...
                    Case enum_TipoOperazioneDB.Cancellazione
                        ...
                        Throw New Exception(ReplaceMessaggio(msg, 0, strMessaggio_Info))
            ...
            If {LAVCOD_NASCITA_ANIMALI, LAVCOD_INCREMENTO_CONSISTENZE_ZOO,
                LAVCOD_ACQUISTO_ANIMALI}.Contains(objParametriAgenda.Lav_Cod) AndAlso
                objParametriAgenda.Tipo_Operazione = "3" Then
                Dim objZooBiz As New AgronicaCoreAnagrafeBIZ.Zoo


                Dim listAnimaliMovimentati = objZooBiz.AnimaliMovimentati_Next_List(objParametriAgenda.Piva, objParametriAgenda.Id_Agenda, objParametri_Server)

                If listAnimaliMovimentati.Count > 0 Then
                    Throw New Exception("Non è possibile eliminare l'operazione, i seguenti capi sono stati movimenti: " & String.Join(",", listAnimaliMovimentati))
                End If
                ...
                For i As Integer = 0 To UBound(matrice_delete, 2)
                    ...
                    If i = 0 AndAlso CLng(matrice_delete(enum_Matrice_Delete.Id_Mov_Det, i)) > 0 Then
                        ...
                        If objDettaglioScrivi.Cancella(piva:=CStr(matrice_delete(enum_Matrice_Delete.Piva, i)),
                            ...
                            Log_Errori &= Gias.ImpossibileEliminareOperazione & vbCrLf
