        ...
        Try
            ...
            If IsOrdine(lavCod) = False Then
                ...
                If dtPrecedente Is Nothing Then
                    Throw New Exception("Impossibile ricavare il documento precedente")
                ...
                Else
                    ...
                    If dataMovimento < dataDocPrec Then
                        msgError = String.Format("ERRORE: il documento con numero precedente {0} è stato emesso in data {1} " &
                                                 "che è successiva al documento che si vuole salvare con numero {2} e data {3}. " & "<br>" &
                                                 "Cambiare il numero oppure la data del documento.",
                                                 numDocShowPrec, dataDocPrec.ToShortDateString(),
                                                 thisNumDoc, dataMovimento.ToShortDateString())
                    Else
                ...
                If dtSuccessivo Is Nothing Then
                    Throw New Exception("Impossibile ricavare il documento successivo")
                ...
                Else
                    ...
                    If dataDocSucc < dataMovimento Then
                        msgError = String.Format("ERRORE: il documento con numero successivo {0} è stato emesso in data {1} " &
                                                 "che è antecedente al documento che si vuole salvare con numero {2} e data {3}. " & "<br>" &
                                                 "Cambiare il numero oppure la data del documento.",
        ...
        Try
            ...
            If originalDocNumero = 0 Then
                ...
                If isDocEmesso = False Then
                    Throw New Exception("Questo è un documento ricevuto e non è stato impostato un numero documento.")
                End If
                ...
                msgErrorCoerenzaDate = VerificaCoerenzaDateNumDoc(newNumDoc,
                                                                  docNumeroSin, docNumeroDes,
            ...
            Select Case Tipo_Operazione
                ...
                Case enum_TipoOperazioneDB.Cancellazione
                    ...
                    res = Operazione_Agenda_Utility.GestisciCancellazione(objParametriAgenda, objParametriServer,
                                                                          ...
                                                                          idServizio:=idServizio, origine:=origine)
        ...
        Try
            ...
            Select Case Opzione
                ...
                Case 0
                    ...
                    Data_Scadenza = DateAdd("d", Giorni_Scadenza, Data_Documento)

                Case 1 'Pagamento Fine Mese

                    'Nel caso i giorni siano multiplo di 30 viene inteso in mesi
                    If Giorni_Scadenza > 0 And Giorni_Scadenza Mod 30 = 0 Then

                        Data_Scadenza = DateAdd("M", CInt(Giorni_Scadenza / 30), Data_Documento)
                    ...
                    Mese = Month(Data_Scadenza)

                    Select Case Mese
                        Case 1 : Giorno = 31
                        Case 2

                            If Year(Data_Scadenza) Mod 4 = 0 Then
                                Giorno = 29
                            Else
                                Giorno = 28
                            End If
                    ...
                    Data_Scadenza = CDate(Giorno & "/" & Mese & "/" & Year(Data_Scadenza))
                ...
                Case 2 'Pagamento Data Fine Mese
                    ...
                    'Aggiungo i giorni
                    Data_Scadenza = DateAdd("d", Giorni_Scadenza, Data_Scadenza)
            ...
            objPagamento.Percentuale = 100 'gestione dinamica sulla percentuale
            objPagamento.Importo = Importo_Pagamento

            objPagamento.ChkDataScadenza_Manuale = 0

            objPagamento.Data_Pagamento = AGRODATAFINE
            objPagamento.Note = ""
