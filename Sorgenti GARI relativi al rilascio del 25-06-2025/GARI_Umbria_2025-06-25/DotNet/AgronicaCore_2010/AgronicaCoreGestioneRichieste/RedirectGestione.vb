...
#Region "Vecchie Chiamate Pagine da spostare e uniformare nella regione sotto"
...
#Region "Nuove Chiamate Pagine"


    '--------------------------------------------------------------------------------------------------------
    '------------------------AgronicaDomandaIrrigua--------------------------------------------------------------------------
    '--------------------------------------------------------------------------------------------------------
    ...
    Public Shared Function ApriPopUp_Sito_AgronicaDomandaIrrigua_PassandoDirettamente_ParametriDomandaIrrigua(
                                       ByVal SitoOrigine As Enum_SiteRedirector,
                                       ByRef ParametriDomandaIrrigua As ParametriDomandaIrrigua) As String
    ...
    '--------------------------------------------------------------------------------------------------------
    '------------------------AgronicaAuditSicurezzaGlobalCoop--------------------------------------------------------------------------
    '--------------------------------------------------------------------------------------------------------
    ...
    Public Shared Function ApriPopUp_Sito_AgronicaAuditSicurezzaGlobalCoop_PassandoDirettamente_Parametri(
                                 ...
                                 ByVal LinkAgronicaAgenda2010 As String) As String
    ...
    '--------------------------------------------------------------------------------------------------------
    '------------------------AgronicaAuditSicurezzaGlobalCoop END--------------------------------------------------------------------------
    '--------------------------------------------------------------------------------------------------------
    ...
    Private Shared Function Prepara_ParametriAnalisi_2010_In_Sessione(
        ...
        Dim ParametriAnalisi_2010_2010 As New AgronicaCoreGestioneRichieste.ParametriAnalisi_2010
    ...
    Private Shared Function Prepara_ParametriAgronicaStampe_In_Sessione(ByVal ReportSelezionato As enum_CodificaStampe,
        ...
        Else
            ...
            For i = 0 To strReport.Length - 1
                If Trim(strReport(i)) <> "" AndAlso IsNumeric(Trim(strReport(i))) Then
                    If CInt(Trim(strReport(i))) = CInt(ReportSelezionato) Then
                        permessoReportStampe2010 = True
                        Exit For
                    End If
                End If
        ...
        If (objWebConfig.Stampe2010 = "False") Or
            permessoReportStampe2010 = False Then
        ...
        Else
            Dim objAgronicaStampe_2010 As New AgronicaCoreGestioneRichieste.ParametriAgronicaStampe_2010
    ...
    Public Shared Function Genera_ParametriAgronicaStampe_Da_Session_SuStileXmlParametri_for_SiteRedirector(
        ...
        Select Case ReportSelezionato
            ...
            Case enum_CodificaStampe.GestioneAllegati
                ...
                Dim objd As New AgronicaCoreDataProvider.Albero
                objd.ChiaveAlbero_Codifica(xChiave, enum_TipoNodo.Impresa, Piva)
    ...
    Private Shared Function SalvaParametriPassaggioSuDB_e_Allega_QueryString(ByVal IndirizzoSito As String, ByVal SitoOrigine As Int32,
        ...
        If Not IsNothing(HttpContext.Current.Session("ASG_objParametri_Super_Server")) Then
            ...
            If SitoDestinazione <> Enum_SiteRedirector.GiasNG Then
                ...
                End If
            Else
                Dim objParametri_Super_Server = New AgronicaCoreDataProvider.AgronicaCoreParametri(HttpContext.Current.Session("ASG_objParametri_Super_Server"))
                ...
                'Dim objParametri_server_String As String = AgronicaCoreDataProvider.Utility.convertOBJparametritoString(objParametri_Server)
                'Dim objParametri_utenti_String As String = AgronicaCoreDataProvider.Utility.convertOBJparametritoString(objParametri_Utenti)
                'Dim objparametri_super_server_string As String = AgronicaCoreDataProvider.Utility.convertOBJparametritoString(objParametri_Super_Server)
                ...
                Dim LinkCoreAPI = objConfSiti.Leggi_Valore(0, "GiasOnline_Core_API", "", "", objParametri_Server)
                If LinkCoreAPI = "" Then
                    LinkCoreAPI = objConfSiti.Leggi_Valore(0, "GiasOnline_Core_API", "", "", objParametri_Super_Server)
                End If

                Dim LinkNetCore = objConfSiti.Leggi_Valore(0, "GiasOnline_NetCore_API", "", "", objParametri_Server)
                If LinkNetCore = "" Then
                    LinkNetCore = objConfSiti.Leggi_Valore(0, "GiasOnline_NetCore_API", "", "", objParametri_Super_Server)
                End If
                ...
                'Dim objparametri_super_server_string_compressed = AgroZip.CompressioneBase64PerJS(1, objparametri_super_server_string, Text.Encoding.UTF8)
                'Dim objParametri_server_string_compressed = AgroZip.CompressioneBase64PerJS(1, objParametri_server_String, Text.Encoding.UTF8)
                'Dim objParametri_utenti_string_compressed = AgroZip.CompressioneBase64PerJS(1, objParametri_utenti_String, Text.Encoding.UTF8)

                Dim connessioni As New AgronicaCoreVarieDAL.ConnessioneWS

                'indirizzoCompleto = IndirizzoSito &
                '    "?objss=" & objparametri_super_server_string &
                '    "&objs=" & objParametri_server_String &
                '    "&obju=" & objParametri_utenti_String &
                '    "&unid=" & UnID &
                '    "&lcore=" & LinkCoreWSB64 &
                '    "&lapi=" & LinkCoreAPIB64 &
    ...
    Private Shared Function SalvaParams_ReturnUrl_Indirizzo(ByVal IndirizzoSito As String,
        ...
        If SitoDestinazione <> Enum_SiteRedirector.GiasNG Then
            UnID = Stringa_Codifica(UnID,
                                    AgroKey_EncoderDecoder,
                                    objParametri_Server)
        End If

        cn = Stringa_Codifica(cn,
                              AgroKey_EncoderDecoder,
                              objParametri_Server)
        ...
        If Not IsNothing(objParametri_SuperServer) Then
            ...
            End If
            StrConSup = Stringa_Codifica(StrConSup,
                                         AgroKey_EncoderDecoder,
                                         objParametri_Server)
            ...
            Else
                ...
                Dim objParametri_Server_String As String = AgronicaCoreDataProvider.Utility.convertOBJparametritoString(objParametri_Server)
                Dim objParametri_Utenti_String As String = AgronicaCoreDataProvider.Utility.convertOBJparametritoString(objParametri_Utenti)
                Dim objparametri_SuperServer_String As String = AgronicaCoreDataProvider.Utility.convertOBJparametritoString(objParametri_SuperServer)
    ...
    Public Shared Function ControllaNumeroImprese_per_SiteRedirector(
        ...
        Dim SitoTitolo As String = ""
        Dim Flag_SelezionaImpresa As Boolean = False
        Dim i As Integer
        ...
        Select Case Num_Imprese
            ...
            Case Else
                ...
                ''Controllo se l'utente ha un filtro sulle imprese
                'Dim objut As New AgronicaCoreUtentiDAL.Utenti_Profili_Read
                'UtenteFiltro = objut.Leggi_FiltroUtenteSQL(objParametri_Utenti.UtenteUsername, _
                '                                        5, _
                '                                        "", "", objParametri_Utenti)
                ...
                If UtenteFiltro <> "" Then
                    ...
                    Dim dtImprese As DataTable = objutil.CreaDTFiltrone(
                                          objParametri_Server,
                                          UtenteFiltro,
                    ...
                    If dtImprese.Rows.Count <> 0 Then
                        ...
                        'l'utente ha una sola impresa nella visibilità
