...
Public Class SchedePersonalizzate
    ...
    Public Function Esporta_GiasToSap(ByVal objC As Object,
        ...
        Try
            ...
            StrSQL.Append(" FROM          Reg_impianti_codici AS R2")
            StrSQL.Append(" WHERE      id_cod = 1071 AND R1.piva = R2.piva AND R1.sa_cod = R2.sa_cod AND R1.appezza = R2.appezza AND R1.id_reg = R2.id_reg),'') ")
            StrSQL.Append(" AS PSe, ")
            StrSQL.Append(" CAC_Codifica_Cultivar.Cultivar_Coltiva AS Varieta, Cultivar.Cul_Des AS DesVarieta, R1.Cul_Cod, ")
            StrSQL.Append(" ISNULL((SELECT     TOP 1 val_cod ")
            StrSQL.Append(" FROM          Reg_impianti_codici AS R2 ")
            StrSQL.Append(" WHERE      id_cod = 1074 AND R1.piva = R2.piva AND R1.sa_cod = R2.sa_cod AND R1.appezza = R2.appezza AND R1.id_reg = R2.id_reg),'') ")
            StrSQL.Append(" AS Coop, ")
            StrSQL.Append(" Imprese.Piva AS PIvaSocio, Imprese.rag_soc AS RSSocio, ISNULL(Imprese_Codici.val_cod,'') AS Socio, ")
            StrSQL.Append(" Agenda.Validita_Inizio AS DataSemina, Appezzamento.SUP_APP, R1.Sup_Imp AS Sup, ")
            StrSQL.Append(" Mov_Destinazioni.Qta AS QtaSeme, UnitaMisura.UDM_SIM AS UdmSeme, Materie_Prime.Cod_Articolo AS Lotto, R1.SA_COD, R1.APPEZZA, R1.ID_REG, Centri_Aziendali.sa_nome ")

            StrSQL.Append(" FROM  GerarchiaImprese INNER JOIN ")
            StrSQL.Append(" Imprese_Codici INNER JOIN ")
            StrSQL.Append(" Imprese ON Imprese_Codici.PIVA = Imprese.PIVA INNER JOIN ")
            StrSQL.Append(" Centri_Aziendali INNER JOIN ")
            StrSQL.Append(" Appezzamento ON Centri_Aziendali.PIVA = Appezzamento.PIVA AND ")
            StrSQL.Append(" Centri_Aziendali.sa_cod = Appezzamento.SA_COD INNER JOIN ")
            StrSQL.Append(" Reg_Impianti R1 ON Appezzamento.PIVA = R1.PIVA AND Appezzamento.SA_COD = R1.SA_COD AND ")
            StrSQL.Append(" Appezzamento.APPEZZA = R1.APPEZZA INNER JOIN ")
            StrSQL.Append(" Cultivar ON R1.CUL_COD = Cultivar.Cul_Cod INNER JOIN ")
            StrSQL.Append(" SpecieVegetali ON Cultivar.Veg_Cod = SpecieVegetali.Veg_Cod INNER JOIN ")
            StrSQL.Append(" Movimenti_dettagli INNER JOIN ")
            ...
            StrSQL.Append(" Movimenti_dettagli.Id_Mov_Det = Mov_Destinazioni.Id_Mov_Det AND ")
            StrSQL.Append(" Movimenti_dettagli.Id_Mov = Mov_Destinazioni.Id_Mov INNER JOIN ")
            StrSQL.Append(" Movimenti ON Movimenti_dettagli.PIVA = Movimenti.PIVA AND Movimenti_dettagli.Sa_Cod = Movimenti.Sa_Cod AND ")
            StrSQL.Append(" Movimenti_dettagli.Id_Agenda = Movimenti.Id_Agenda AND Movimenti_dettagli.Id_Mov = Movimenti.Id_Mov INNER JOIN ")
            StrSQL.Append(" Agenda ON Movimenti.PIVA = Agenda.PIVA AND Movimenti.Sa_Cod = Agenda.Sa_Cod AND ")
            StrSQL.Append(" Movimenti.Id_Agenda = Agenda.Id_Agenda INNER JOIN ")
            StrSQL.Append(" UnitaMisura ON Movimenti_dettagli.Udm_Cod = UnitaMisura.UDM_COD ON R1.PIVA = Mov_Destinazioni.Piva AND ")
            StrSQL.Append(" R1.SA_COD = Mov_Destinazioni.Sa_Cod AND R1.APPEZZA = Mov_Destinazioni.Appezza AND R1.ID_REG = Mov_Destinazioni.Id_Destinazione ON ")
            StrSQL.Append(" Imprese.PIVA = Centri_Aziendali.PIVA ON GerarchiaImprese.Figlio = Imprese.PIVA LEFT OUTER JOIN ")
            StrSQL.Append(" Imprese Imprese_1 ON GerarchiaImprese.Padre = Imprese_1.PIVA LEFT OUTER JOIN ")
            StrSQL.Append(" Materie_Prime ON Movimenti_dettagli.Elem_Cod = Materie_Prime.Elem_Cod AND ")
            StrSQL.Append(" Movimenti_dettagli.Mat_Cod = Materie_Prime.Mat_Cod LEFT OUTER JOIN ")
            StrSQL.Append(" CAC_Codifica_Cultivar ON R1.CUL_COD = CAC_Codifica_Cultivar.Cultivar_Gias ")

            StrSQL.Append(" WHERE     (Agenda.Lav_Cod = 2 OR Agenda.Lav_Cod = 71) ")
            StrSQL.Append(" AND Movimenti.Cau_Mov = '2300' ")
            StrSQL.Append(" AND Imprese_Codici.id_cod = 1033 ")

            If objSV("Testo") <> "" Then
                StrSQL.Append(" AND SpecieVegetali.Veg_Cod = " & objSV("Valore"))
            End If
            ...
            If strVarieta <> "" Then
                ...
                StrSQL.Append(" AND R1.Cul_Cod IN (" & Agro_SQL_Save_Clausola_IN(strVarieta) & ")")
            End If

            If objDi <> "" Then
                If objDf <> "" Then
                    'Caso Intervallo/Giorno
                    StrSQL.Append(" AND R1.Validita_Inizio >= " & Agro_SQL_SaveDate(CDate(objDi)))
                    StrSQL.Append(" AND R1.Validita_Inizio <= " & Agro_SQL_SaveDate(CDate(objDf)))
                Else
                    'Caso Anno
                    StrSQL.Append(" AND R1.Validita_Inizio >= " & Agro_SQL_SaveDate(CDate("01/01/" & objDi)))
                    StrSQL.Append(" AND R1.Validita_Inizio <= " & Agro_SQL_SaveDate(CDate("31/12/" & objDi)))
                End If
            End If
