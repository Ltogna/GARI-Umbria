                ...
                Try
                    ...
                    If Not IsNothing(contattoGias) Then
                        ...
                        If IsNothing(contattoDemetra.ruolo) OrElse contattoDemetra.ruolo.Count = 0 Then
                            blocked = True
                            If retrial Then
                                Update_Log_Invio_Chiamate(IDChiamata, pacchettoDaInviare, TipoOperazione, ESITO_BLK, "", timeStamp)
                            Else
                                Chiama_Scrivi_Log_Invio_Contatti(pacchettoDaInviare, Piva, Cod_Contatto, anagrafica.codice_esterno, TipoOperazione, ESITO_BLK, String.Format("{0}{1}", BLK_MESSAGE_PREFIX, "Nessun Ruolo Esportabile"), timeStamp)
                            End If
                        End If
                        ...
                        If Not blocked Then
                            ...
                            If Not objDemetraBIZ_util.CallEndpoint(_parametriExtra.apikey, _parametriExtra.DemetraBaseUrl, pacchettoDaInviare, errorMessage) Then
                    ...
                    Else
                        ...
                        ' ATTENZIONE - Le cancellazioni vengono comunque processate ma possono essere di 2 tipi
                        ' 1) se si trattava della cancellazione effettiva di un contatto e ci troviamo qui è perchè non trovo il contatto
                        ' 2) potrebbe trassarsi della cancellazione di un patentino che però viene processata comunque in quanto il contatto esiste ancora su Gias

                        ' Non trovato contatto in Gias (Significa che qualcuno lo ha eliminato dopo averlo letto come da inviare
                        Throw New Exception(String.Format("Errore nella lettura del contatto: Piva {0}, Cod_Contatto {1}", Piva, Cod_Contatto))
