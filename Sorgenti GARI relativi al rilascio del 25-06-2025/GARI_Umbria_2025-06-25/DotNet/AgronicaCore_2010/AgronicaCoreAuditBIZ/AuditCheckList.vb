...
Public Class AuditCheckList
    ...
    Public Function LeggiAuditDocumenti(ByVal Tipo As Integer,
        ...
        If Piva <> "" AndAlso (Riferimento_Cod <> "" OrElse Tipo = 15 OrElse Tipo = 16 OrElse Tipo = 17) Then
            ...
            Dim tipologie = auditLeggi.LeggiTipologieRiferimento(If(Tipo = 15 OrElse Tipo = 16, "", objParametri.PivaSuperUser), Riferimento_Cod, TipoDocumento_Cod, "", "", objParametri)
    ...
    Public Function AggiornaAuditDocumenti(Audit_Tipo As Integer,
        ...
        For Each codice In codici
            If Left(codice.Tipo, 3) = "doc" Then
                ...
                If Audit_Tipo = 13 And Regolamento_Cod = 3 And codice.Sezione_Cod = 1 And Rintracciabilita = 0 Then
                    bBypass = True
                End If
                '===========================================================================================================
        ...
        If Not String.IsNullOrEmpty(riferimento_cod) Then
            ...
            Select Case tipo
                ...
                Case enum_TipoEntita.Contatto
                    ...
                    'Dim dtContatti = objContatti.Leggi(piva, cod_contatto, 0, enum_Rapporti_Contabili_Standard.Fornitore,
                    '                                   False, False, 0, 0, False, 0, -99, 0, "", True, 0, 0, 0, 0, 0,
                    '                                   AGRODATAINIZIO, AGRODATAFINE, False, "", "", objParametri_Server)
    ...
    Public Function LeggiAuditJson(Audit_Cod As Integer,
        ...
        If Audit_Tipo = 12 OrElse Audit_Tipo = 13 OrElse Audit_Tipo = 14 OrElse Audit_Tipo = 15 OrElse Audit_Tipo = 16 OrElse Audit_Tipo = 17 OrElse Audit_Tipo = 20 OrElse Audit_Tipo = 21 Then
            Dim codici As List(Of AuditCodiciModel) = auditAgronica.LeggiCodici(Audit_Tipo, Regolamento_Cod, 0, 0, "")
            ddCodici = codici.ToDictionary(Function(x) x.Punto_Numero, Function(x) x.Tipo)
            For Each codice In codici
                If Left(codice.Tipo, 3) = "doc" Then
        ...
        Dim auditRisposteLeggi As New Audit_Risposte_R
        Dim dtRisposte As DataTable = auditRisposteLeggi.Leggi(0, Audit_Tipo, Regolamento_Cod, 0, Data_Inizio, Data_Fine, "", "", objParametri_Server)
            ...
            If Audit_Tipo <> 4 Then
                ...
                If auditRiferimento AndAlso Not IsDBNull(audit.Item("Riferimento_Cod")) Then
                    ...
                    If Audit_Tipo <> 17 Then
                        ...
                        If Audit_Tipo = 14 AndAlso riferimento IsNot Nothing Then
                ...
                Dim registrazioni = LeggiRegistrazioni(Audit_Tipo, audit.Item("Audit_Cod"), objAudit, dtRisposte, ddCodici, num_registrazioni, completa, cod_documenti)
    ...
    Public Function LeggiRegistrazioni(Audit_Tipo As Integer, Audit_Cod As Integer,
                                  ByRef objAudit As JObject, ByRef dtRisposte As DataTable, ByRef ddCodici As Dictionary(Of String, String),
                                  ByRef num_registrazioni As Integer, ByRef completa As String, ByRef cod_documenti As String) As String
        ...
        For Each risposta In dtRisposte.Select("Audit_Cod=" & Audit_Cod)
            If Audit_Tipo = 14 Then
                Dim tipo = ddCodici(risposta.Item("Punto_Numero"))
                ...
                objAudit.Add("Risposta_" & risposta.Item("Punto_Numero"), valore)
            ElseIf Audit_Tipo = 12 OrElse Audit_Tipo = 13 OrElse Audit_Tipo = 15 OrElse Audit_Tipo = 16 OrElse Audit_Tipo = 17 OrElse Audit_Tipo = 20 OrElse Audit_Tipo = 21 Then
                Dim tipo = ""
                If ddCodici.ContainsKey(risposta.Item("Punto_Numero")) Then
                    tipo = ddCodici(risposta.Item("Punto_Numero"))
                End If
                Dim valore = CStr(risposta.Item("Valore"))
                If tipo = "ue" Then
                    Select Case valore
                        Case "0"
                            valore = "Non Compliant"
                        Case "1"
                            valore = "Partially Compliant"
                            num_registrazioni += 1
                        Case "2"
                            valore = "Compliant"
                            num_registrazioni += 2
                ...
                Next
                objAudit.Add("Risposta_" & risposta.Item("Punto_Numero"), valore)
            ElseIf Audit_Tipo = 11 Then
                If risposta.Item("Punto_Numero") = "999999" Then
                    completa = If(risposta.Item("Valore") = "1", "SI", "NO")
                Else
                    objAudit.Add("Risposta_" & risposta.Item("Punto_Numero"), If(risposta.Item("Valore") = "1", 1, 0))
                    registrazioni.Append(If(registrazioni.Length > 0, ", ", "") & risposta.Item("Punto_Numero"))
                    num_registrazioni += 1
                End If
            End If
        Next
    ...
    Public Function ScriviAuditRisposte(ByVal Audit_Tipo As Integer,
                                        ...
                                        Optional ByRef objParametri_Utenti As AgronicaCoreParametri = Nothing) As Integer
        ...
        If (Audit_Tipo = 12 OrElse Audit_Tipo = 13 OrElse Audit_Tipo = 20) AndAlso Workflow_Cod > 0 Then
            Stato = 0
        End If
        ...
        Try
            ...
            If auditScrivi.scrivi(Audit_Cod, Audit_Tipo, 0, Stato, Regolamento_Cod, Piva, Note, Campionato, Data, Data, Date.Now, Date.Now, objParametri.UsernameOperazione, objParametri.UsernameOperazione, objParametri, Riferimento_Cod, Profilo_Cod, Rintracciabilita, ExtraInfo, Pratica_Cod) Then
                ...
                If Audit_Tipo = 12 OrElse Audit_Tipo = 13 OrElse Audit_Tipo = 15 OrElse Audit_Tipo = 16 OrElse Audit_Tipo = 17 OrElse Audit_Tipo = 20 Then
                    Dim auditAgronica As New AuditAgronicaWS(objParametri)
                    Dim codici As List(Of AuditCodiciModel) = auditAgronica.LeggiCodici(Audit_Tipo, Regolamento_Cod, 0, 0, Data)
                    AggiornaAuditDocumenti(Audit_Tipo, Regolamento_Cod, Audit_Cod, Piva, Riferimento_Cod, Data, codici, objParametri, Rintracciabilita)
                End If
