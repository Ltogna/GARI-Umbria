...
#Region "Non Usata"
    ...
    Public Function Leggi_UtentiUpload(ByVal piva As String,
        ...
        Try
            ...
            If piva <> "" Then
                StrSQL.Append(" AND Allegati_Documenti_Piva = '" & piva & "' ")
            End If
...
#End Region
    ...
    Private Sub Componi_Leggi_Elenco(ByVal FiltroVisibilitaUtentiDaUtilizzare As Integer,
        ...
        If Inizio_Upload <> "" And IsDate(Inizio_Upload) Then
            StrSQL.AppendLine(" AND CAST(Allegati_Documenti.Data_Upload AS DATETIME) >= CAST( '" & (Inizio_Upload) & "' AS DATETIME) ")
        End If

        If Fine_Upload <> "" And IsDate(Fine_Upload) Then
            StrSQL.AppendLine(" AND CAST(Allegati_Documenti.Data_Upload AS DATETIME) <= CAST( '" & (Fine_Upload) & "' AS DATETIME) ")
        End If
                        ...
                        If UBound(Arrayp) > 0 Then
                            For i = 0 To UBound(Arrayp) - 1
                                ...
                                If Trim(Arrayk(1)) <> "" Then
                                    StrSQL.AppendLine(" AND Alert_Entita.Id_Alert_Entita In (Select Id_Alert_Entita From Alert_EntitaxIndici Where Id_Indice = " & Arrayk(0) & " And (Elenco_Val = '" & Agro_SQL_SaveText(Arrayk(1)) & "' Or Upper(Valore_Des) = '" & UCase(Arrayk(1)) & "'))")
                                End If
    ...
    Public Function Leggi_x_QDC(ByVal Piva As String,
        ...
        Try
            ...
            StrSQL.Append(" And alert_entita.id_alert_entita in (select id_alert_entita from alert_entitaxindici where id_indice = " & Id_Indice_Veg_Cod & " And Elenco_Val = '" & Veg_Cod & "') ")
            StrSQL.Append(" And alert_entita.id_alert_entita in (select id_alert_entita from alert_entitaxindici where  id_indice = " & Id_Indice_Anno & " And Valore_Des = '" & Anno & "') ")
    ...
    Public Function HasAttachment(
        ...
        Try
            ...
            End If
            If idAgenda <> 0 Then
                StrSQL.AppendLine(" AND Alert_Entita.TipoEntita_Cod = 14 AND Alert_Entita.ID_Agenda = " & Agro_SQL_SaveNum(idAgenda) & " ")
                StrSQL.AppendLine(" AND Alert_Area.ID_Area IN (11) ")
            End If
            If macCod <> 0 Then
                StrSQL.AppendLine(" AND Alert_Area.ID_Area IN (2) ")
                StrSQL.AppendLine(" AND Alert_Entita.TipoEntita_Cod = 7 AND Alert_Entita.Mac_Cod = " & Agro_SQL_SaveNum(macCod))
            End If
