...
Public Class Fitofarmaci_Leggi
    ...
    Public Sub Verifica_Udm_Formulato(ByVal Fr_Cod As Integer,
        ...
        If Fr_Cod <> 0 And Udm = enum_UnitaMisura.Numero Then
            ...
            Dim Classificazioni_Confusione_Disorientamento_Sessuale As New List(Of Integer) From {614, 615}
            Dim Classificazioni_Installazione_Trappole_Catture_Massa As New List(Of Integer) From {602, 613, 617}
    ...
    Public Function Risultati_Formulato_Info_DS(
        ...
        If Tipo <> "" AndAlso IsNumeric(Tipo) = True Then
            StrSQL.Append(" ,CASE")
            StrSQL.Append("     WHEN FormulatixClassificazioni.CLASS_COD IN (602,613,617) THEN 'True'")
        ...
        Filtro_Formulato_Classificazione(Tipo, StrSQL)
        ...
        If IsNumeric(Tipo) = True Then
            ...
            Select Case CInt(Tipo)
                ...
                Case enum_TipoFormulato.InstallazioneTrappoleCattureMassa
                    ...
                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN  (602,613,617) " & vbCrLf)
    ...
    Private Sub Filtro_Formulato_Classificazione(ByVal Tipo As String, ByRef StrSQL As Text.StringBuilder)

        If IsNumeric(Tipo) = True Then

            Dim Classificazioni_Tutti_Formulati As New List(Of Integer) From {101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 300, 1000, 610, 611, 200, 201, 202, 203, 400, 401, 402, 403, 404, 405, 407, 408, 409, 411, 412, 413, 414, 415, 418, 608, 603, 607, 500, 501, 502, 503, 504, 505, 506, 606, 614, 615, 602, 613, 617}

            Dim Classificazioni_Trattamento_Antiparassitario As New List(Of Integer) From {101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 300, 1000, 610, 611, 606}

            Dim Classificazioni_Diserbo As New List(Of Integer) From {200, 201, 202, 203}

            Dim Classificazioni_Trattamenti_Fitoregolatori As New List(Of Integer) From {400, 401, 402, 403, 404, 405, 407, 408, 409, 411, 412, 413, 414, 415, 418}

            Dim Classificazioni_Coadiuvanti_Bagnanti_Antischiuma As New List(Of Integer) From {500, 501, 502, 503, 504, 505, 506}

            Dim Classificazioni_Concia As New List(Of Integer) From {608}

            Dim Classificazioni_Disseccamento As New List(Of Integer) From {603}

            Dim Classificazioni_Geodisinfestazione As New List(Of Integer) From {607}

            Dim Classificazioni_Trattamento_Antiparassitario_Concianti As New List(Of Integer) From {101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 300, 1000, 610, 611, 608, 606}

            Dim Classificazioni_Diserbo_Disseccanti As New List(Of Integer) From {200, 201, 202, 203, 603}

            Dim Classificazioni_Trattamento_Antiparassitario_Geodisinfestanti As New List(Of Integer) From {101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 300, 1000, 610, 611, 607, 606}
            ...
            Select Case CInt(Tipo)
                ...
                Case enum_TipoFormulato.Tutti
                    ...
                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Tutti_Formulati) & ")")

                    '=================================================================

                Case enum_TipoFormulato.Antiparassitari

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Trattamento_Antiparassitario) & ")")

                    '=================================================================

                Case enum_TipoFormulato.Diserbanti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Diserbo) & ")")

                    '=================================================================

                Case enum_TipoFormulato.Fitoregolatori

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Trattamenti_Fitoregolatori) & ")")


                Case enum_TipoFormulato.Coadiuvanti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Coadiuvanti_Bagnanti_Antischiuma) & ")")

                Case enum_TipoFormulato.Concianti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Concia) & ")")

                Case enum_TipoFormulato.Disseccanti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Disseccamento) & ")")

                Case enum_TipoFormulato.Geodisinfestanti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Geodisinfestazione) & ")")

                Case enum_TipoFormulato.Antiparassitari_Concianti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Trattamento_Antiparassitario_Concianti) & ")")

                Case enum_TipoFormulato.Diserbanti_Disseccanti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Diserbo_Disseccanti) & ")")

                Case enum_TipoFormulato.Antiparassitari_Geodisinfestanti

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Classificazioni_Trattamento_Antiparassitario_Geodisinfestanti) & ")")

                Case enum_TipoFormulato.ConfusioneDisorientamentoSessuale

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN  (" & String.Join(",", Classificazioni_Confusione_Disorientamento_Sessuale) & ") ")

                Case enum_TipoFormulato.InstallazioneTrappoleCattureMassa

                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN  (" & String.Join(",", Classificazioni_Installazione_Trappole_Catture_Massa) & ") ")
                ...
                Case -enum_TipoFormulato.InstallazioneTrappoleCattureMassa
                    ...
                    StrSQL.Append(" And  FormulatixClassificazioni.CLASS_COD IN (" & String.Join(",", Formulati_Senza_Trappole) & ")")
