...
Public Class FileManager : Implements IFIleManager
    ...
    Private myZip As Ionic.Zip.ZipFile = New Ionic.Zip.ZipFile With {
    ...
    Public Function CreaFileZip(zipFileName As String, xmlFilePath As String) As String Implements IFIleManager.CreaFileZip
        ...
        Try
            Dim zipFullPathName = Path.Combine(
                OttieniPercorso(FatturaElettronicaPath.ZipCicloAttivo), zipFileName)

            Dim zip = New Ionic.Zip.ZipFile With {
                .CompressionLevel = Ionic.Zlib.CompressionLevel.BestCompression
            }
            Using zip
                zip.AddFile(xmlFilePath, "")
                zip.Save(zipFullPathName)
            End Using
    ...
    Public Function aggiungiFileAZip(filePath As String, Optional directoryNelZip As String = "")

        If File.Exists(filePath) Then
            myZip.AddFile(filePath, directoryNelZip)
        End If
    End Function
    Public Function aggiungiPiuFileAZip(filePaths As List(Of String), Optional directoryNelZip As String = "")
    ...
    End Function
    Public Function salvaFileZip(zipFileName As String)
        Dim destFolder = Path.GetDirectoryName(zipFileName)
        If Not Directory.Exists(destFolder) Then
            Directory.CreateDirectory(destFolder)
        End If
