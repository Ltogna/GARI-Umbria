...
Public Class Tipologie
    ...
    Public Sub Scrivi_Tipologie(ByRef Tipologie As TipologiaUtente(), params As ObjParams)
        ...
        For Each Profilo In Tipologie
            ...
            Else
                Profilo.codice = getCodiceNuovaTipologia(params.ObjParametri_Utenti, params.ObjParametri_Server)
    ...
    Public Sub Copia_TipologiePermessi(TipologiaRiferimento As TipologiaUtente, Altra As TipologiaUtente, objParametri As AgronicaCoreParametri)
        ...
        Dim Permessi_DT = objTipologiaxPermessi_R.Leggi(TipologiaRiferimento.codice,
                                                        enumSelezioneVariabile.Selezione_TabellaCompleta,
                                                        "", "", objParametri)
        Dim enu = Permessi_DT.Rows.GetEnumerator
        While enu.MoveNext
            objTipologiaxPermessi_W.Scrivi(
                Altra.codice,
                enu.Current.item("Id_Servizio"),
                enu.Current.item("Id_Attivita"),
                enu.Current.item("Id_Operazione"),
                enu.Current.item("Validita_Inizio"),
                enu.Current.item("Validita_Fine"),
                objParametri
            )
        End While
    ...
    Public Function Carica_Permessi_Gerarchia(
        ...
        Dim permessiCurrUt = utentiPermessi.Leggi_conPermessi(enum_Id_Servizio.GiasOnline, Id_Attivita:=-1,
                                                          enumSelezioneVariabile.Selezione_TabellaDatiMinimi,
                                                          " Utenti_Dettagli.UserName LIKE '" & objParametri_Utenti.UtenteUsername & "' ",
                                                          String.Empty, objParametri_Utenti).Select.
    ...
    Private Function extractFromGerarchia(ByVal gerarchia As List(Of Utente_Permesso_Gerarchia), ByVal rows As IEnumerable(Of DataRow)) As List(Of Utente_Permesso_Gerarchia)
        Return gerarchia.AsParallel.Where(Function(p) rows.Any(Function(row) row("Id_Attivita") = p.Attivita_Cod)).
             Select(Function(p) New With {
                 .permesso = p,
                 .row = rows.First(Function(row) row("Id_Attivita") = p.Attivita_Cod)
             }).Select(Function(p)
                           Dim disabled = 1
                           Select Case p.row("Tipo")
                               Case 1
                                   p.permesso.Id_Operazione = enum_TipoOperazioneDB.Lettura
                               Case 2, 3
                                   p.permesso.Id_Operazione = enum_TipoOperazioneDB.Modifica
                               Case Else
                                   p.permesso.Id_Operazione = disabled
                           End Select
                           Return p.permesso
                       End Function).ToList
    End Function
...
#Region "Funzioni Private"
    ...
    Private Function ControllaNoUtentiCollegati(Tipologia_Cod As Integer,
    ...
    ''' <summary>
    ''' Calcola il prossimo codice da usare per la scrittura di una tipologia utente.
    ''' Se viene passato anche objParametri_Server, sincronizza la tabella Sequenze_Tabelle.
    ''' </summary>
    Private Function getCodiceNuovaTipologia(objParametri_Utenti As AgronicaCoreParametri, Optional objParametri_Server As AgronicaCoreParametri = Nothing) As Integer
        Dim objNuovoId As New AgronicaCoreDataProvider.Agro_Sequenze
        Dim upper = 2000000000
        Dim maxCod = Carica_Tipologie(objParametri_Utenti).
            Select(Function(t) t.codice).
            Where(Function(cod) cod >= 0).
            DefaultIfEmpty(0).
            Max()
        If maxCod = upper Then
            Throw New InvalidOperationException("Max number of profiles reached.")
        End If
        If IsNothing(objParametri_Server) Then
            Return maxCod + 1
        End If
        Dim daSequenzaTabelle = objNuovoId.NuovoId_Tabella("Utenti_Tipologie", 0, upper, objParametri_Server)
        While daSequenzaTabelle <= maxCod
            daSequenzaTabelle = objNuovoId.NuovoId_Tabella("Utenti_Tipologie", 0, upper, objParametri_Server)
        End While
        Return daSequenzaTabelle
    End Function
