...
Public Class Utenti
            ...
            If Sql_Permessi <> "" Then
                ...
                If Not dataUltimoAggiornamentoPermessi.HasValue OrElse dataUltimoAggiornamentoPermessi.Value < dataModificaUtentiProfili Then
                    ...
                    Try
                        ...
                        If Sql_Permessi.Contains(pivaFittizia) Then
                            'objUtentiVisibilita_W.ScriviVisibilitaNulla(utente_Username, objParametri_server)
    ...
    Public Function Aggiorna_Licenze(ByVal Operazione As Integer,
    ...
    Public Function Aggiorna_Licenze(ByVal righeInseriteGrid_Licenze As String,
    ...
    Public Function Aggiorna_Superuser(ByVal Operazione As Integer,
                                       ByVal username As String,
                ...
                Select Case Operazione
                    ...
                    Case 1 'SALVATAGGIO
                        ...
                        If Dummy = 0 Then
                            ...
                            bOk = Utenti_Dettagli_W.Scrivi(username,
                                                            cognome,
                                                            nome,
                                                            via,
                                                            numero,
                    ...
                    Case 2 'MODIFICA
                        ...
                        If Dummy = 0 Then
                            ...
                            bOk = Utenti_Dettagli_W.Modifica(username,
                                                             cognome,
                                                             nome,
                                                             via,
                                                             numero,
    ...
    Public Function Aggiorna_Visibilita_Area(ByVal righeInserite As String,
                                             ByVal righeModificate As String,
                                             ByVal righeCancellate As String,
                                             ByRef objParametriUtenti As AgronicaCoreParametri) As Boolean
        ...
        Dim UtenteAbilitato_VisibilitaCompleta = objPermessi.Controlla_Permessi_Utente(objParametriUtenti.UtenteUsername,
        ...
        Try
            ...
            If inOK Then
                ...
                For Each objRiga As JObject In Jinsert
                    ...
                    If (CStr(objRiga.Item("Piva_Azienda")) <> "") Then
                        ...
                        r = String.Format("INSERT INTO Utenti_Visibilita (Gruppo, UserName, Area, Piva_Azienda, Username_Creazione, Username_Modifica) VALUES ({0}, '{1}', 0, '{2}', '{3}', '{3}')",
                                          CInt(objRiga.Item("Gruppo_Cod")).ToString,
                                          CStr(objRiga.Item("UserName")),
                                          CStr(objRiga.Item("Piva_Azienda")),
                                          objParametriUtenti.UsernameOperazione
                                          )

                    Else

                        If UtenteAbilitato_VisibilitaCompleta Then

                            r = String.Format("INSERT INTO Utenti_Visibilita (Gruppo, UserName, Area, Piva_Azienda, Username_Creazione, Username_Modifica, Visibilita_Completa) VALUES ({0}, '{1}', 0, '{2}', '{3}', '{3}', 1)",
                                          CInt(objRiga.Item("Gruppo_Cod")).ToString,
                                          CStr(objRiga.Item("UserName")),
                                          CStr(objRiga.Item("Piva_Azienda")),
                                          objParametriUtenti.UsernameOperazione
            ...
            If righeCancellate <> "" And righeCancellate <> "[]" Then
                ...
                For Each objRiga As JObject In Jdelete
                    ...
                    r = "DELETE FROM Utenti_Visibilita WHERE Gruppo = " + CInt(IIf(IsNothing(objRiga.Item("Gruppo_Cod")), 0, objRiga.Item("Gruppo_Cod"))).ToString + " AND UserName like '" + CStr(objRiga.Item("UserName")) + "' AND Piva_Azienda like '" + CStr(objRiga.Item("Piva_Azienda")) + "' "
    ...
    Private Function LoadUserData(ByRef Dt_Utenti As DataTable, params As ObjParams) As DataTable
        ...
        Dim sql2017 = False
        If VersioneSqlServer_anno(params.ObjParametri_Utenti) >= 2017 Then
        ...
        For i As Integer = 0 To Dt_Utenti.Rows.Count - 1
            ...
            If finestraTempDic.ContainsKey(username) Then
                Dt_Utenti.Rows(i).Item("FinestraTemporaleInizio") = finestraTempDic(username).FinestraTemporale.inizio
                Dt_Utenti.Rows(i).Item("FinestraTemporaleFine") = finestraTempDic(username).FinestraTemporale.fine
            Else
    ...
    Private Function recuperaDatiAccessoSpid(FiltroUtenti As String, objParametri_Utenti As AgronicaCoreParametri) As Dictionary(Of String, UtenteAccessoSPID)
        ...
        accessiSpid = objUtenti.Leggi(
            ...
            .ObbligaLoginSPID = If(IsDBNull(row("Flag_Accesso_SPID")) OrElse row("Flag_Accesso_SPID") = 0, False, True),
            .UserName = row("UserName")
        }).ToDictionary(Function(row) row.UserName)
        objCF.Leggi(UserName:=String.Empty, CodFisc:=String.Empty, FiltroUtenti, xOrderBy:=String.Empty, objParametri_Utenti).
            Select.AsParallel.
            Where(Function(row) accessiSpid.ContainsKey(row("UserName"))).
            ForAll(Sub(row) accessiSpid(row("UserName")).CfLogin = row("CodFisc"))
        Return accessiSpid
    End Function
    ...
    Private Function LeggiDatiFinestraTemporale(obj_Server As AgronicaCoreParametri) As Dictionary(Of String, IUtenteFinestraTemp)
        ...
        objFinestraTemp_R.Leggi_Da_Gias_Server(xFiltroAggiuntivo:=String.Empty, xOrderBy:=String.Empty, obj_Server).
            ...
            )).ToList.ForEach(Sub(user) dict.Add(user.UserName, user))
        Return dict
    End Function
    ...
    Private Function ControllaDati_Utente(ByVal User As AgronicaCoreModelsSTD.profilazione.Utente,
        ...
        If User.Azienda_Persona = "I" OrElse User.Azienda_Persona = "Impresa" Then
            ...
            User.Cognome = ""
            User.Nome = ""
            ControllaImpresa(User.PIVA, Operazione, errore.messaggio, objParametri_Utenti)
        ElseIf User.Azienda_Persona = "P" OrElse User.Azienda_Persona = "Persona" Then
            User.Azienda_Persona = "P"
            ControllaPersona(User.Nome, User.CodFisc, Operazione, errore.messaggio, objParametri_Utenti)
        End If
    ...
    Private Sub ControllaEmail(ByVal Utente As AgronicaCoreModelsSTD.profilazione.Utente, ByVal Operazione As enum_TipoOperazioneDB,
        ...
        Else
            ...
            If (users.Rows.Count > 1) OrElse (users.Rows.Count = 1 AndAlso users.Rows(0)("UserName") <> Utente.UserName) Then
    ...
    Private Sub ControllaPersona(ByVal Nome As String,
                                      ByVal CF As String,
                                      ByVal Operazione As enum_TipoOperazioneDB,
                                      ByRef Err As String,
        ...
        '----- Verifica formato CF
        If Not VerificaEspressioneRegolare(CF, "", enum_EspressioniRegolari.RegExp_CodiceFiscale) Then
    ...
    Private Sub ControllaImpresa(ByVal PIVA As String,
                                      ByVal Operazione As enum_TipoOperazioneDB,
                                      ByRef Err As String,
                                      objParametri_Utenti As AgronicaCoreDataProvider.AgronicaCoreParametri)
...
#Region "Permessi"
    ...
    Public Sub ModificaValiditaPermessi(utenti As List(Of UtentePermessi), objUtenti As AgronicaCoreParametri)
        '----- APRO CONNESSIONE AL DATABASE
    ...
    Public Sub DisattivaPermessiUtenti(usernames As IEnumerable(Of String), objParametri_Utenti As AgronicaCoreParametri)
...
Public Class Widgets
    ...
    Public Function LeggiWidgets(ByVal UserName As String,
                            ...
                            Optional ByVal Abilitato As Boolean? = Nothing) As List(Of Widget)
    ...
    Public Function CancellaWidgets(ByVal userName As String,
    ...
    Public Function ScriviWidgetUtente(ByVal Widget As Widget,
                                         ByRef objParametri As AgronicaCoreDataProvider.AgronicaCoreParametri) As String
