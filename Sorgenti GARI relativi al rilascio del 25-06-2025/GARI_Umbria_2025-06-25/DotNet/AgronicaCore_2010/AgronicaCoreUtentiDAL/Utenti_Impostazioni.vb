...
Public Class Utenti_Impostazioni_Read
    ...
    ''' <summary>
    ''' Restituisce l'annata agraria partende dalla data impostata nel parametro data riferimento
    ''' </summary>
    ...
    Public Function Leggi_Impostazione_APP(ByVal Impostazione_Cod As Integer, ByRef impostazioni As DataTable, ByRef objParametri_Utenti As AgronicaCoreDataProvider.AgronicaCoreParametri) As Boolean
        Dim dt As DataTable
        dt = Leggi(Impostazione_Cod, 1, enumSelezioneVariabile.Selezione_TabellaCompleta, "", "", objParametri_Utenti)
        If dt.Rows.Count = 0 Then
            dt = Leggi(Impostazione_Cod, 2, enumSelezioneVariabile.Selezione_TabellaCompleta, "", "", objParametri_Utenti)
        End If
    ...
    ''' </summary>
    Public Function LeggiImpostazioneScalare(impostazione As Integer, username As String, objParametri_Utenti As AgronicaCoreDataProvider.AgronicaCoreParametri) As IEnumerable(Of DataRow)
        Dim nomeRoutine As String = "AgronicaCoreUtentiDAL.Utenti_Impostazioni_Read.Leggi2()"
        Dim strSql As New StringBuilder With {.Length = 0}
        Dim dt As DataTable
        Dim profile = GetTipologia(username, objParametri_Utenti)
        Dim userFilter = {username, profile, objParametri_Utenti.SuperUserUsername}.
        ...
        Try
            ...
            If dt.Select("UserName = '" & username & "'").Count > 0 Then
                Return dt.Select("UserName = '" & username & "'")
            ElseIf dt.Select("UserName = '" & profile & "'").Count > 0 Then
    ...
    Public Function LeggiScalare(username As String, impostazioni As IEnumerable(Of Integer), objParametri As AgronicaCoreDataProvider.AgronicaCoreParametri) As IEnumerable(Of Utente_Impostazioni)
        Dim nomeRoutine As String = "AgronicaCoreUtentiDAL.Utenti_Impostazioni_Read.LeggiScalare()"
        Dim strSql As New StringBuilder With {.Length = 0}
        ...
        Try
            ...
            Dim values = dt.Select.
                ...
                Select(Function(gr)
                           If gr.FirstOrDefault(Function(row) CompareStringInvariant(row("username"), username)) IsNot Nothing Then
                               Return gr.FirstOrDefault(Function(row) CompareStringInvariant(row("username"), username))
                           ElseIf gr.FirstOrDefault(Function(row) CompareStringInvariant(row("username"), tipologia)) IsNot Nothing Then
                               Return gr.FirstOrDefault(Function(row) CompareStringInvariant(row("username"), tipologia))
                           ElseIf gr.FirstOrDefault(Function(row) CompareStringInvariant(row("username"), objParametri.SuperUserUsername)) IsNot Nothing Then
                               Return gr.FirstOrDefault(Function(row) CompareStringInvariant(row("username"), objParametri.SuperUserUsername))
                           Else
