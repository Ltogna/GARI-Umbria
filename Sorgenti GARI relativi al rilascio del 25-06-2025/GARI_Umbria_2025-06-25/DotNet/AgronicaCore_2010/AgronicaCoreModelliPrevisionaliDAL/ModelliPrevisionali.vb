    ...
    Public Function Leggi(ByVal Mod_Cod As Integer,
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(PivaSuperuser) Then
                Stb.AppendLine("AND (op_aut.Piva_Superuser IS NULL OR op_aut.Piva_Superuser = '" & PivaSuperuser & "')")
            End If
        ...
        Try
            ...
            Select Case Tipo_Sorgente
                ...
                Case enum_Meteo_Tiposorgente.RetiPartner, enum_Meteo_Tiposorgente.Aziendali
                    ...
                    sql.AppendLine("		WHERE PIVA_Superuser = '" & PivaSuperuser & "' AND PIVA = '" & Agro_Sql_SaveText(Piva) & "' ")
                    sql.AppendLine()
                    sql.AppendLine("		UNION ")
                    sql.AppendLine()
                    sql.AppendLine("		SELECT 1 AS Ordine, Id_Stazione, DescrizioneAggiuntiva, Proprietario ")
                    sql.AppendLine("		FROM MeteoNT_VisibilitaStazioni ")
                    sql.AppendLine("		WHERE PIVA_Superuser = '" & PivaSuperuser & "' AND PIVA = '*' ")
            ...
            sql.AppendLine("    AND (OpAut.Piva_Superuser IS NULL OR OpAut.Piva_Superuser= '" & PivaSuperuser & "') ")
