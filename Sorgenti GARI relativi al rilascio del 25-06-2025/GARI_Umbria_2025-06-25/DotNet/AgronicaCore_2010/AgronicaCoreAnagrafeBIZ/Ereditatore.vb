...
Public Class Ereditatore
    ...
    Public Function editResaPrevista(
        ...
        Dim exceptions As New List(Of DatiPrevisionaliNoDataFoundException)
        ...
        Try
            Using scope As New TransactionScope()
                ...
                For Each es As JObject In esercizi
                    'il try-catch innestato nel for-each serve a garantire la modifica di ogni impianto anche se vengono generate eccezioni gestite
                    Try
                        Dim piva As String = CStr(es("PIVA"))
                        Dim sa_cod As Integer = If(es("sa_cod").Type = JTokenType.Null, 0, CInt(es("sa_cod")))
                        Dim appezza As Integer = If(es("Appezza").Type = JTokenType.Null, 0, CInt(es("Appezza")))
                        Dim id_reg As Integer = If(es("id_Reg").Type = JTokenType.Null, 0, CInt(es("id_Reg")))
                        Dim progettoCod As Integer = If(es("Progetto_Cod").Type = JTokenType.Null, 0, CInt(es("Progetto_Cod")))
                        Dim validitaInizio As Date = If(es("validita_inizio").Type = JTokenType.Null, AGRODATAINIZIO, CDate(es("validita_inizio")))
                        Dim validitaFine As Date = If(es("validita_fine").Type = JTokenType.Null, AGRODATAFINE, CDate(es("validita_fine")))

                        Dim esercizio = (
                            From e In GiasContext.Imprese_Progetti
                            Where e.Piva = piva _
                                And e.Sa_Cod = sa_cod _
                                And e.Appezza = appezza _
                                And e.Id_Reg = id_reg _
                                And e.Progetto_Cod = progettoCod _
                                And e.Validita_Inizio <= validitaFine _
                                And e.Validita_Fine >= validitaInizio
                                ).FirstOrDefault

                        Dim resa = datiPrevisionaliObj.readDatiPrevisionaliForImpianto(piva, sa_cod, appezza, id_reg, objParametriServer, objParametriUtenti, objParametriSuperServer, progettoCod)

                        esercizio.Produzione_Prevista = convertUoM(resa.valore, enum_UnitaMisura.KG__HA, resa.udm.codice)
                        campoModificato = "Resa newValue = [" & resa.valore.ToString() & "]"

                        esercizio.Username_Modifica = objParametriServer.UsernameOperazione
                        esercizio.Data_Modifica = Date.Now
                        ...
                        If campoModificato <> "" Then
                            'Scrittura tabella Agronica_Log_Anagrafe
                            log = objLogAnagrafeW.CreaLogAnagrafeEF(
                        enum_TipoEntita_Des.Progetti,
                        CStr(esercizio.Piva),
                        CStr(esercizio.Progetto_Cod),
                        CStr(esercizio.Sa_Cod),
                        CStr(esercizio.Appezza),
        ...
        If exceptions.Count > 0 Then
            Dim ex = New DatiPrevisionaliNoDataFoundAggregateException(exceptions.ToList)
            Dim species = ex.GetExceptionSpecies()
            Throw New GiasException(
            String.Concat(
            "Nessuna resa trovata per le specie: ",
            DatiPrevisionaliColtureRead.GetSpecieDescriptions(
            species,
            objParametriServer,
            GiasContext
            ).Aggregate(
            Function(ByVal str1, ByVal str2)
                Return str1 & ", " & str2
            End Function)
                        )
                        )
...
#Region "NEW - Massivo"
    ...
    Private Function PopolaDatiAnagrafici(parametri_modificati As JToken,
        ...
        If DT.Rows.Count > 0 Then
            ...
            Dim listaEserciziPostFiltroData = dicEsercizi.Where(Function(x) x.Value.ModificaEsercizioPostFiltroValidita = False).Select(Function(x) x.Key).ToList()
            ...
            If dicEsercizi.Count > 0 Then
                Dim listaEserciziChiusiNonModificabili = dicEsercizi.Where(Function(x) x.Value.EsercizioChiuso = True).Select(Function(x) x.Key).ToList()
        ...
        Try
            ...
            Select Case parametro
                ...
                Case Enum_ParametriModificaMultiplaPianoColturale.IMP_ImpIrrigazione
                    If valore_parametri_modificati("IMP_COD") IsNot Nothing Then
