...
Public Class Reg_Impianto_R
    ...
    Public Sub LeggiImpianti_QdC(ByRef Dt As DataTable,
        ...
        Dim dtUtenti_Impostazioni = objImpost.Leggi2(2, objParametri_Server.SuperUserUsername, 0, "", "",
                                                    objParametri_Utenti)

        If dtUtenti_Impostazioni.Rows.Count > 0 Then

            Dim dr As DataRow()

            dr = dtUtenti_Impostazioni.Select("Impostazione_Cod = " & enum_Impostazioni_Utenti.SuperUser_KPIN_BlockName)

            If Not IsNothing(dr) AndAlso dr.Length > 0 AndAlso dr(0).Item("Impostazione_Valore_1") = "1" Then
                visualizza_Kpin_BlockName = True
            End If

            dr = dtUtenti_Impostazioni.Select("Impostazione_Cod = " & enum_Impostazioni_Utenti.SuperUser_Visualizza_Codici_Anagrafici)

            If Not IsNothing(dr) AndAlso dr.Length > 0 AndAlso dr(0).Item("Impostazione_Valore_1") = "1" Then
                visualizza_codici_imp_app_prj = True
            End If

            dr = dtUtenti_Impostazioni.Select("Impostazione_Cod = " & enum_Impostazioni_Utenti.SUPERUSER_COD_PERMETTI_OPERAZIONI_SU_IMPIANTI_BLOCCATI)

            If Not IsNothing(dr) AndAlso dr.Length > 0 AndAlso dr(0).Item("Impostazione_Valore_1") = "1" Then
                leggiAncheBloccati = True
            End If

            dr = dtUtenti_Impostazioni.Select("Impostazione_Cod = " & enum_Impostazioni_Utenti.SUPERUSER_COD_PERSON_RILIEVO_FASI_FENOLOGICHE)
        ...
        Dt.Columns.Add(New DataColumn("CarenzaStr", GetType(String)))
        Dt.Columns.Add(New DataColumn("DataCarenza", GetType(Date)) With {.DefaultValue = AGRODATAFINE})
