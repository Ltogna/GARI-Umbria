...
namespace AgronicaCoreAPI.Controllers
{
    ...
    public class LoginController : BaseController
    {
        ...
        private Token SaveToken(CoreWS_Utenti_R payload, LoginModel login)
        {
        ...
        private CoreWS_Utenti_R AuthenticateUser(LoginModel login)
        {
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel login)
        {
            ...
            try
            {
                ...
                if (payload != null)
                {
                    ...
                    var authCookieUpdated = await autenticazioneService.AggiornaUtentiTokenJWTAsync(token.idToken, campi, UtilityAgronica.convertStringtoOBJparametri(objP_super_server, _securitySettings));
        ...
        public async Task<IActionResult> Refresh()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(tok_str))
                {
                    ...
                    var objPSuperServer = new AgronicaCoreParametri()
                    {
                    ...
                    var campi = new AggiornaUtentiTokenJWTCampi(authCookie, tokenString, scadenzaToken);
                    var authCookieUpdated = await autenticazioneService.AggiornaUtentiTokenJWTAsync(newTokenID, campi, UtilityAgronica.convertStringtoOBJparametri(objP_super_server, _securitySettings));

                    logInfo("Qui 7 tokenString:" + tokenString);
        ...
        public async Task<IActionResult> BackgroundAuthentication([FromBody] KeyAuth key)
        {
            ...
            try
            {
                ...
                var res = new CoreWSController(hc, decrypt.corewsbaseurl, "", Request).LeggiParametriConnessioneDaAccessToken(new LoginAccessTokenModel() { access_token = decrypt.access_token });
                ...
                if (payload != null)
                {
                    ...
                    var autenticazioneService = _serviceProvider.GetRequiredService<IAutenticazioneService>();
                    var campi = new AggiornaUtentiTokenJWTCampi(authCookie, tokenString, scadenzaToken);
                    var authCookieUpdated = await autenticazioneService.AggiornaUtentiTokenJWTAsync(token.idToken, campi, UtilityAgronica.convertStringtoOBJparametri(payload.objP_super_server, _securitySettings));
