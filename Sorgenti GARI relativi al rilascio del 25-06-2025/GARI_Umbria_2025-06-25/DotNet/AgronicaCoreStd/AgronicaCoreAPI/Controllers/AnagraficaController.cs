...
namespace AgronicaCoreAPI.Controllers
{
    public class AnagraficaController : BaseController
    {
        ...
        public ObjectResult GetProdottiOnline(string piva, int categoria, string filtro, string specie, string codici, string stato, [FromHeader] string Authorization)
        {
            if (!isAuthorized(Authorization)) return StatusCode(StatusCodes.Status401Unauthorized, null);
            ...
            {
                var adapter = new ModelloAdapter();
                var request = new CoreWS_ProdottiSenzaGiacenza(objP_super_server, objP_server, objP_utenti, piva, categoria, filtro, specie, codici, stato);
