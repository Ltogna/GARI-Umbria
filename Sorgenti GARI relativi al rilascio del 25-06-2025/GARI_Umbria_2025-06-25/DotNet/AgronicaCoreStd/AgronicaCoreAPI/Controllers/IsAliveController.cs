    ...
    public class IsAliveController : BaseController
    {
        ...
        public async Task<IActionResult> IsAlive([FromBody] CheckIsAliveIN checkIsAlive, [FromHeader] string Authorization)
        {
            ...
            try {
                ...
                if (!checkIsAlive.sites.Equals(null) && checkIsAlive.sites.Length > 0) {
                    ...
                    if (!resultSites.RispostaOK) { throw new Exception(result.Errore); }
        ...
        private List<ReachableSiteOUT> CheckReachableSites(List<ReachableSiteIN> sitesToCheck, bool setVersion, bool setUrl)
        {
            ...
            sitesToCheck.ForEach(site => {
                if (site.value.Equals("KO") || site.value.Equals("")) {
                    ...
                    try {
                        switch (site.key.ToLower()) {
                            case "giasng":
                                sitesChecked.Add(GiasNGRequest(site.key, site.value, setUrl));
                                break;
                            case "giasbase":
                                sitesChecked.Add(GiasBaseRequest(site.key, site.value, setUrl));
                                break;
                            case "netcoreapi":
                                sitesChecked.Add(NetCoreApiRequest(hc, site.key, site.value, setVersion, setUrl));
                                break;
                            default:
                                sitesChecked.Add(GenericRequest(hc, site.key, site.value, setVersion, setUrl));
                                break;
                        }
                    }
                    catch (Exception ex) {
                        sitesChecked.Add(new ReachableSiteOUT(site.key, setUrl ? site.value : null, "Unreachable: " + ex.Message, setVersion ? "" : null,  false));
                    }
                }
            });
