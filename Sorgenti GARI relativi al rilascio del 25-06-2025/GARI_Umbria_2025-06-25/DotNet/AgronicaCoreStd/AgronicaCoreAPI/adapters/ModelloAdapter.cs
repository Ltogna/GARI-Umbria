...
using System.Linq;
using static AgronicaCoreAPI.models.DatiAziendaEntity;
...
namespace AgronicaCoreAPI.adapters
{
    public class ModelloAdapter
    {
        ...
        public List<LavorazioneAttivitaCDGEntity> leggiLavorazioniAttivitaCDG(string dati) {
            ...
            return (from x in objects where x.Lav_Cod > 0
        ...
        public PianoColturaleEntity leggiPianoColturale(string dati)
        {
            ...
            pianoColturale.destinazioniUso = (from x in objects where x.veg_cod == 0
                select new DestinazioneUsoEntity()
                {
                    codice = x.id_cod,
            ...
            pianoColturale.specie = (from x in objects where x.veg_cod != 0
                select new SpecieEntity()
                {
                    codice = x.veg_cod,
            ...
            pianoColturale.varieta = (from x in objects where x.veg_cod != 0
                select new VarietaEntity()
                {
                    codice = x.cul_cod,
            ...
            pianoColturale.finalita = (from x in objects where x.veg_cod != 0
                select new GruppoFinalitaEntity()
                {
                    codice = x.grfi_cod,
        ...
        public void leggiProdotti(string dati, DatiAziendaEntity datiAzienda)
        {
            ...
            datiAzienda.prodotti.AddRange((
                from x in prodotti select new ProdottoEntity()
                {
