...
Public Class SchedaAziendale
    ...
    Private Sub Carica_DSSchedaAziendale(ByRef DSSchedaAziendale As DS_SchedaAziendale)
        ...
        '------------------------------------------------
        stbQ.AppendLine(" ( ")
        stbQ.AppendLine(" SELECT DISTINCT 0 as copia, Reg_Impianti.PIVA AS Piva, Reg_Impianti.SA_COD AS Sa_Cod, 0 as Campo_Cod, Reg_Impianti.Appezza as Appezza, Reg_Impianti.Id_Reg as Id_Reg,")
        ...
        stbQ.AppendLine(" ISNULL(SpecieVegetali.Veg_Cod, 0) AS veg_cod, ISNULL(SpecieVegetali.Veg_Des, 'Terreno Nudo') AS veg_des, SpecieVegetali.Gru_Cod as gru_cod, ISNULL(Cultivar.Cul_cod, 0) AS cul_cod, ISNULL(Cultivar.Cul_Des, '') AS cul_des, ")
        ...
        stbQ.AppendLine(" ( ")
        stbQ.AppendLine(" SELECT DISTINCT  0 as copia, CampiXParticelle.PIVA as Piva, CampiXParticelle.SA_COD as Sa_Cod, CampiXParticelle.Campo_Cod as Campo_Cod, 0 as Appezza, 0 as Id_Reg,")
        stbQ.AppendLine(" ISTAT.LOCALITA, ISTAT.COMUNI_PROV, ISTAT.CAP, ImpreseXParticelle.TitoloPossesso, ' ' AS TitoloPossessoDesc, ImpreseXParticelle.Validita_Inizio AS Inizio_Part, ImpreseXParticelle.Validita_Fine AS Fine_Part, ")
        stbQ.AppendLine(" ParticelleCatastali.Part_Cod, ParticelleCatastali.PROV, ParticelleCatastali.COM, ParticelleCatastali.SEZIONE, ")
        stbQ.AppendLine(" ParticelleCatastali.FOGLIO, ParticelleCatastali.NUMERO, ParticelleCatastali.SUBALTERNO, ")
        stbQ.AppendLine(" ParticelleCatastali.ETTARI AS ETTARI_Sup_Cat, ParticelleCatastali.[ARE] AS ARE_Sup_Cat, ParticelleCatastali.CENTIARE AS CENTIARE_Sup_Cat, ")
        ...
        stbQ.AppendLine(" INNER JOIN Portinnesti ON Portinnesti.Port_Cod = Reg_Impianti.PORT_COD ")
        stbQ.AppendLine(" LEFT JOIN Imprese_Progetti ON Reg_Impianti.PIVA = Imprese_Progetti.PIVA AND Reg_Impianti.SA_COD = Imprese_Progetti.Sa_Cod AND Reg_Impianti.APPEZZA = Imprese_Progetti.APPEZZA AND Reg_Impianti.ID_REG = Imprese_Progetti.ID_REG ")
        stbQ.AppendLine(" LEFT JOIN FasiCicloColturale_Anagrafiche ON Imprese_Progetti.Stato_Impianto = FasiCicloColturale_Anagrafiche.Fase_Cod ")
    ...
    Private Sub Carica_DSSuperficieSpeciexParticelle(ByRef DSSuperficieSpeciexParticelle As DS_SuperficieSpeciexParticelle)
        ...
        stbQ.AppendLine("            INNER JOIN ImpreseXParticelle AS ixp ON 	")
        stbQ.AppendLine("                ixp.PIVA = ca.PIVA 	")
        stbQ.AppendLine("            AND ixp.sa_cod = ca.SA_COD    	")
        stbQ.AppendLine("            LEFT JOIN AppezzamentiXParticelle axp ON 	")
        stbQ.AppendLine("                axp.PIVA = ixp.PIVA 	")
        ...
        stbQ.AppendLine("            LEFT JOIN Reg_Impianti ri ON 	")
        stbQ.AppendLine("                axp.PIVA = ri.PIVA 	")
        stbQ.AppendLine("            AND axp.SA_COD = ri.SA_COD 	")
        stbQ.AppendLine("            AND axp.APPEZZA = ri.APPEZZA  	")
        stbQ.AppendLine("            LEFT JOIN Cultivar cv ON 	")
        stbQ.AppendLine("                ri.CUL_COD = cv.Cul_Cod    	")
        stbQ.AppendLine("            LEFT JOIN SpecieVegetali sv ON 	")
        stbQ.AppendLine("                cv.Veg_Cod = sv.Veg_Cod    	")
        stbQ.AppendLine("            LEFT JOIN ISTAT i ON i.PROV = axp.PROV AND i.COM = axp.COM    	")
        stbQ.AppendLine("            LEFT    	")
        stbQ.AppendLine("                 JOIN ParticelleCatastali pc ON pc.PROV = ixp.PROV    	")
        ...
        stbQ.AppendLine("            	LEFT JOIN ISTAT ipc ON ipc.PROV = pc.PROV AND ipc.COM = pc.COM    	")
        stbQ.AppendLine("            LEFT JOIN Reg_Impianti_Codici ric ON 	")
        stbQ.AppendLine("                ric.PIVA = ri.PIVA 	")
        ...
        stbQ.AppendLine("             FROM Cte_Catasto Catasto  LEFT 	")
        stbQ.AppendLine("             JOIN ParticelleCatastali pc ON pc.PROV = Catasto.PROV 	")
