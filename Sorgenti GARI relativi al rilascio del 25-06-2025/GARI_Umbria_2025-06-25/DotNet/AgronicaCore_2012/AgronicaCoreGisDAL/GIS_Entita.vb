...
Public Class GIS_Entita_R
    ...
    Private Shared Sub LeggiGetQuery_infoEstese_AnalisiPeriodicitaAgenda(ByVal Stb As System.Text.StringBuilder)
        ...
        Stb.Append("                '^Data Operazione§ ' + convert(varchar(50), A1.DataOperazione, 103 ) + '|' +  STUFF(  " & vbCrLf)
        Stb.Append("                    CAST(( " & vbCrLf)
        Stb.Append("                    SELECT 'Giorni trascorsi§  ' +  replace( cast(ggUltimaOperazione as varchar(100)), ',', '.') + '|' as [text()] " & vbCrLf)
        Stb.Append("                    FROM  ( " & vbCrLf)
    ...
    Public Shared Function LeggiGetQuery_DecidiFiltro_Utenti_Visibilita_Appoggio(ByVal ObjParametri_server As AgronicaCoreParametri) As Boolean
            ...
            Case enum_Gis_LayerElementiGrafici_std.CATASTO,
                ...
                If includiJoinTabelleCatasto Then
                    ...
                    If Not String.IsNullOrEmpty(wktBoundaySTIntersects) Then
                        ...
                        stb.AppendLine(" ) p  ")
                        stb.AppendLine("     inner Join(select '" & piva & "' as piva, " & sa_Cod & " as sa_cod) fintaImpresa ")
                        stb.AppendLine(" On 1=1 ")
                        stb.AppendLine(" ) pp")
    ...
    Private Sub LeggiGetQueryExportSHP(
        ByVal PivaSuperUser As String,
        ByVal CODICE_FISCALE_TECNICO As String,
        ByVal Piva As String,
        ByVal Sa_Cod As Int32,
        ...
        stb.AppendLine("     Where ri.CODICE_FISCALE_TECNICO = '" & CODICE_FISCALE_TECNICO & "'  ")
        stb.AppendLine("     and ri.piva = '" & Piva & "'  ")
    ...
    Private Sub LeggiGetQuery(
        ...
        If isXml Then
            ...
            If Not chiamataXAggiornaElencoTipologie Then
                ...
                If Not cfgAlbero Is Nothing Then
                    ...
                    If cfgAlbero.TipologiaLayer_Cod = enum_TipologiaLayer.AnalisiPeriodicitaAgenda And test_Ottimizzazione_1 Then
                        ...
                        'Stb0.Append(" " & vbCrLf)
                        'Stb0.Append("     Select op.Piva, op.Sa_Cod, op.Appezza, op.id_reg, op.id_mov_det, op.LAV_COD, '^Data Operazione§ ' + convert(varchar(50), op.DataOperazione, 103 ) + '|' +  'Giorni trascorsi§  ' +  replace( cast(op.ggUltimaOperazione as varchar(100)), ',', '.') + '|' as InfoEstese " & vbCrLf)
                        ...
                        Stb0.Append("Select Piva, Sa_Cod, Appezza, id_reg, id_mov_det, LAV_COD, '^Data Operazione§ ' + convert(varchar(50), DataOperazione, 103 ) + '|' +  'Giorni trascorsi§  ' +  replace( cast(ggUltimaOperazione as varchar(100)), ',', '.') + '|' as InfoEstese " & vbCrLf)
                        Stb0.Append("from ( " & vbCrLf)
                    ...
                    ElseIf cfgAlbero.TipologiaLayer_Cod = enum_TipologiaLayer.Cultivar Then
                        ...
                        Stb.Append(",isNull( " & vbCrLf)
                        Stb.Append("(select '^Data Raccolta§ ' + convert(varchar(50), DataRaccolta, 103 ) from @TableRaccolta tr where tr.piva = entita.piva and tr.sa_cod = entita.sa_cod and tr.appezza = entita.appezza and tr.id_imp = entita.id_imp) " & vbCrLf)
                        Stb.Append(", '') as '@info_estese' " & vbCrLf)
                    ...
                    ElseIf ({enum_TipologiaLayer.RilieviVegetoProduttivi, enum_TipologiaLayer.Avversita}).Contains(cfgAlbero.TipologiaLayer_Cod) And test_Ottimizzazione_2 Then
                        ...
                        Stb0.Append(" " & vbCrLf)
                        Stb0.Append("Select piva, sa_cod, appezza, id_reg, id_agenda, ricetta_operazione_cod, '^Data Operazione§ ' + convert(varchar(50), DataOperazione, 103 ) + '|' + descr , DataOperazione " & vbCrLf)
                        Stb0.Append("from ( " & vbCrLf)
            ...
            If Not chiamataXAggiornaElencoTipologie Then
                ...
                Stb.Append("    , ( " & vbCrLf)
                Stb.Append("         SELECT " & vbCrLf)
                Stb.Append("              dbo.fGetKeyValue(geodata.ElementoGrafico_DES,'AppldRate','|','§') as AppIdRate " & vbCrLf)
                ...
                Else
                    Stb.Append("              case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as inserimento " & vbCrLf)
                    Stb.Append("            , case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as modifica " & vbCrLf)
                    Stb.Append("            , case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as cancellazione " & vbCrLf)
                    Stb.Append("            , case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as informazioni " & vbCrLf)
                End If
            ...
            If Piva <> "" Then
                ...
                If isXml Then
                    ...
                    If Sementieri_Sportello_Configurazione_cod <> -1 Then
                        ...
                        Stb.Append("    ( " & vbCrLf)
                        Stb.Append("        ( " & vbCrLf)
                        Stb.Append("            coalesce(filtroSementi.Visibilita_Impianti, 1) = 0 and coalesce(reg1.Codice_Fiscale_Tecnico, ent.Codice_Fiscale_Tecnico) =  '" & PivaPadre & "' OR '" & PivaPadre & "' = '" & PivaSuperUser & "'" & vbCrLf)
                        Stb.Append("        ) " & vbCrLf)
                        Stb.Append("        OR coalesce(filtroSementi.Visibilita_Impianti, 1) = 1 " & vbCrLf)
                        Stb.Append("    ) " & vbCrLf)
    ...
    Public Function getProprietaImpianto(
        ...
        Try
            ...
            Stb.AppendLine("   where r.piva = '" & Piva & "'")
        ...
        Try
            ...
            Select Case gruppo.ToUpper
                ...
                Case "CENTRO"
                    ...
                    Stb.AppendLine(" where c.piva = '" & piva & "'")
    ...
    Public Function Leggi_Entita_Cod(
        ...
        Try
            ...
            Stb.AppendLine("    inner join  gis_elementiGrafici g ")
            Stb.AppendLine("    on  e.entita_cod = g.entita_cod ")
            Stb.AppendLine(" where e.piva = '" & piva & "' ")
            Stb.AppendLine(" and e.Sa_Cod =  " & sa_cod)
    ...
    Public Function TipoEntita_Cod_Leggi_Da_Impianto(
        ...
        Try
            ...
            Stb.AppendLine(" Where ent.piva = '" & piva & "'")
...
#Region "Nuove chiamate GIS"
    ...
    Private Function LeggiElencoEntita_GetQuery(
        ...
        Dim xLeggiVersioneSql As New DataProvider
        Dim versioneSql As Integer = xLeggiVersioneSql.VersioneSqlServer_anno(objParametri_Server)
    ...
    Private Sub LeggiElencoEntita_GetQuery_Select(
            ...
            If cfgAlbero IsNot Nothing Then
                ...
                Select Case cfgAlbero.TipologiaLayer_Cod
                    ...
                    Case enum_TipologiaLayer.Cultivar
                        StrSQL.AppendLine("      , isNull( '^Data Raccolta§ ' + convert(varchar(50), tr.DataRaccolta, 103 ) , '' ) as '@info_estese'  ")
        ...
        If Not chiamataXAggiornaElencoTipologie Then
            ...
            'Dettaglio
            StrSQL.AppendLine("  , dbo.fGetKeyValue(geodata.ElementoGrafico_DES,'AppldRate','|','§') as AppIdRate ")
            ...
            Else
                StrSQL.AppendLine("  , case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as inserimento ")
                StrSQL.AppendLine("  , case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as modifica ")
                StrSQL.AppendLine("  , case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as cancellazione ")
                StrSQL.AppendLine("  , case when coalesce(filtroSementi.PermessoModifiche, 'False') = 'False' then 'False' else case when '" & Codice_Fiscale_Tecnico & "' = '" & PivaSuperUser & "' then 'False' else case when cast(coalesce(reg1.codice_Fiscale_tecnico, ent.codice_fiscale_tecnico) as varchar(1000)) = '" & Agro_SQL_SaveText(Codice_Fiscale_Tecnico) & "' then 'True' else 'False' end end end as informazioni ")
            End If
    ...
    Private Sub LeggiElencoEntita_GetQuery_SelectLayer(
        ...
        If Not cfgAlbero Is Nothing AndAlso (
            ...
            StrSQL.AppendLine("         Else ")
            StrSQL.AppendLine("             Case when charindex('isStop§ 1', geodata.ElementoGrafico_DES , 0)>1 then ")
    ...
    Private Sub SQL_CTE_ElencoEntita(ByVal piva As String,
        ...
        StrSQL.AppendLine("   , Part_Cod int ")
        StrSQL.AppendLine("   , ETICHETTA nvarchar(300)   COLLATE SQL_Latin1_General_CP850_CI_AS ")
        StrSQL.AppendLine("   , id_cod int ")
        StrSQL.AppendLine("   , Rilievo_Cod int ")
        StrSQL.AppendLine("   , Area_Cod int ")
    ...
    Private Sub Internal_SQL_GisEntitaConFiltroTemporaleGetQuery_SelectPart(AgendaRilieviParteGis As Boolean, AgendaRilieviAvversita As Boolean, LayerElementiGrafici_Cod As enum_Gis_LayerElementiGrafici_std, includiJoinTabelleCatasto As Boolean, StrSQL As StringBuilder, TabellaAliasDateValidita As String, etichetta As String, ByVal TipoEntitaCodRiproiettata As enum_GIS2012_TipoEntita, ByVal id_cod As Integer)
        ...
        Select Case TipoEntitaCodRiproiettata
            ...
            Case enum_GIS2012_TipoEntita.Indici_Rischio_Produttivita
                ...
                StrSQL.AppendLine(", 'Anno§ ' + CAST(YEAR(iai.Validita_Fine) as nvarchar) + '|Produttività (q/ha)§ ' + CAST(iai.Produttivita as nvarchar) + '|PLV (Produzione Lorda Vendibile) (Euro/ha)§ ' + CAST(iai.PLV as nvarchar) + '|Impresa§ ' + REPLACE(REPLACE(impxi.rag_soc, '§', '_'), '|', '_') + '|' as Testo")
            Case enum_GIS2012_TipoEntita.Indice_Erosione

                StrSQL.AppendLine(", 'Anno§ ' + CAST(YEAR(iai.Validita_Fine) as nvarchar) + '|Indice di erosione (t/(ha*yr))§ ' + CAST(iai.IndiceErosione as nvarchar) + '|Impresa§ ' + REPLACE(REPLACE(impxi.rag_soc, '§', '_'), '|', '_') + '|' as Testo")
            Case enum_GIS2012_TipoEntita.Indice_CO2

                StrSQL.AppendLine(", 'Anno§ ' + CAST(YEAR(iai.Validita_Fine) as nvarchar) + '|Indice di CO2 (t/ha)§ ' + CAST(iai.IndiceCO2 as nvarchar) + '|Impresa§ ' + REPLACE(REPLACE(impxi.rag_soc, '§', '_'), '|', '_') + '|' as Testo")
            Case enum_GIS2012_TipoEntita.Indice_Rischio_Meteo

                StrSQL.AppendLine(", 'Anno§ ' + CAST(YEAR(iai.Validita_Fine) as nvarchar) + 
                                     '|Indice di rischio meteo aggregato (%)§ ' + CAST(iai.IndiceRischioMeteoAggregato as nvarchar) + 
                                     '|Indice di rischio meteo allagamento (%)§ ' + CAST(iai.IndiceRischioAllagamento as nvarchar) + 
                                     '|Indice di rischio meteo grandine (%)§ ' + CAST(iai.IndiceRischioGrandine as nvarchar) + 
                                     '|Indice di rischio meteo siccità (%)§ ' + CAST(iai.IndiceRischioSiccita as nvarchar) + 
                                     '|Indice di rischio meteo vento forte (%)§ ' + CAST(iai.IndiceRischioVentoForte as nvarchar) + 
                                     '|Indice di rischio meteo gelata (%)§ ' + CAST(iai.IndiceRischioGelata as nvarchar) + 
        ...
        If Not FiltroAggiuntivo_EscludeAltriFiltri Then
            ...
            If Piva <> "" Then
                If Sementieri_Sportello_Configurazione_cod <> -1 Then
                    ...
                    StrSQL.AppendLine("    ( ")
                    StrSQL.AppendLine("        ( ")
                    StrSQL.AppendLine("            coalesce(filtroSementi.Visibilita_Impianti, 1) = 0 and coalesce(reg1.Codice_Fiscale_Tecnico, ent.Codice_Fiscale_Tecnico) =  '" & PivaPadre & "' OR '" & PivaPadre & "' = '" & PivaSuperUser & "'")
                    StrSQL.AppendLine("        ) ")
                    StrSQL.AppendLine("        OR coalesce(filtroSementi.Visibilita_Impianti, 1) = 1 ")
                    StrSQL.AppendLine("    ) ")
                End If
    ...
    Private Sub internal_SQL_InsertSelect_Operazioni(ByRef StrSQL As StringBuilder)
        ...
        StrSQL.AppendLine("			, '^Data Operazione§ ' + convert(varchar(50), DataOperazione, 103 ) + '|' +  'Giorni trascorsi§  ' +  replace( cast(ggUltimaOperazione as varchar(100)), ',', '.') + '|' as InfoEstese ")
        StrSQL.AppendLine("		from ( ")
    ...
    Private Sub internal_SQL_InsertSelect_TableOp(ByRef StrSQL As StringBuilder)
        ...
        StrSQL.AppendLine("          '^Data Operazione§ ' + convert(varchar(50), DataOperazione, 103 ) + '|' + descr, ")
        StrSQL.AppendLine("          DataOperazione, 0 as av_cod ")
        StrSQL.AppendLine("   from ( ")
        StrSQL.AppendLine("         select piva, sa_cod, appezza, id_reg, id_agenda, ricetta_operazione_cod, ")
        StrSQL.AppendLine("                Dataoperazione, STRING_AGG(UDM_DES + '§  ' + replace( cast(qta as varchar(100)), ',', '.'), '|') as descr ")
        StrSQL.AppendLine("         from ( ")
        StrSQL.AppendLine("               select '' as piva, 0 as sa_cod, 0 as appezza, 0 as id_reg, op.id_agenda, op.Ricetta_Operazione_Cod, ")
        StrSQL.AppendLine("                      mav.ordine,  op.Validita_Inizio as DataOperazione, ")
        StrSQL.AppendLine("                      ISNULL(mav.Av_Des_Vol collate Latin1_General_CI_AS, '') + ' ' + ISNULL(mav.UDM_DES, '') AS UDM_DES, ")
        ...
        StrSQL.AppendLine("			               ) op ")
        StrSQL.AppendLine("			               on ( ( op.av_cod = mav.av_cod And op.veg_cod = mav.veg_cod And op.lav_cod = " & CostantiPersonalizzate.LAVCOD_RILIEVO_AVVERSITA_CAMPO & ") or ")
        StrSQL.AppendLine("			                    ( op.FF_Classe = mav.av_cod And op.lav_cod in ( " & CostantiPersonalizzate.LAVCOD_RILIEVO_INDICI_MATURITA & " , " & CostantiPersonalizzate.LAVCOD_RILIEVO_INDICI_RESE_RACCOLTA & " ) ) ) ")
        StrSQL.AppendLine("			              and op.udm_cod = mav.udm_cod And op.lav_cod = mav.lav_cod ")
        ...
        StrSQL.AppendLine("              ) tmp1 ")
        StrSQL.AppendLine("         group by piva, sa_cod, appezza, id_reg, id_agenda, ordine, ricetta_operazione_cod, dataoperazione ")
    ...
    Private Sub internal_SQL_InsertSelect_TableOpAvv(
        ...
        StrSQL.AppendLine("   insert into @TableOp ")
        StrSQL.AppendLine("   select piva, sa_cod, appezza, id_reg, id_agenda, ricetta_operazione_cod, ")
        StrSQL.AppendLine("          '^Avversità§ ' + cast(av_cod as varchar) + '|' + descr, ")
...
#End Region
    ...
    Public Function LeggiXDocumentExportSHP_List(
        ...
        Try
            ...
            Else
                ...
                If applicaFiltroTabellaUtentiVisibilitaAppoggio Then
                    ...
                    StrSQL.AppendLine("    On appv.username = '" & Agro_SQL_SaveText(objParametri.UtenteUsername) & "'")
...
Public Class GIS_Entita_W
    ...
    Private Function AggregaPoligoni_verifica_getAggregateFunction(ByRef objParametri As AgronicaCoreDataProvider.AgronicaCoreParametri) As String
        ...
        stb.Length = 0
        stb.Append(" select @@version " & vbCrLf)
        '--------------------------------------------------------------------------
        ...
        If Not dbV.Contains("SQL Server 2008") Then
