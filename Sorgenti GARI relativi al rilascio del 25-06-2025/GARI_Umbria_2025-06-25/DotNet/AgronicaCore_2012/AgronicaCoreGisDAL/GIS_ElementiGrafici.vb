    ...
    Private Sub Leggi_creaSQL(ByVal PivaSuperUser As String,
        ...
        Select Case xSelezioneVariabile
            ...
            Case enumSelezioneVariabile.Selezione_TabellaDatiMinimi
                ...
                StrSQL.AppendLine(" From GIS_ElementiGrafici")
                StrSQL.AppendLine(" WHERE Entita_Cod = " & Agro_SQL_SaveNum(Entita_Cod) & " ")
            ...
            Case enumSelezioneVariabile.Selezione_TabellaCompleta
                ...
                If Not isXML Then
                    ...
                    If numeroRigheDaLeggere > 0 Then
                        StrSQL.Append(String.Format(" TOP {0} ", numeroRigheDaLeggere))
                    End If

                    StrSQL.Append(String.Format(" {0} ", colonneDaLeggere))
                ...
                If IsDaConvertireGml(FormatoGraficoConvertito) Then
                    ...
                    StrSQL.Append(" Poligono_GeoEntity.AsGml() as geodata " & vbCrLf)

                End If

                If IsDaConvertireWkt(FormatoGraficoConvertito) Then

                    StrSQL.Append(" Poligono_GeoEntity.STAsText() as geodata " & vbCrLf)

                End If

                StrSQL.Append(" FROM    GIS_ElementiGrafici Entita ")

                StrSQL.Append(" WHERE   Entita.Validita_inizio < " & Agro_SQL_SaveDate(objParametri.FinestraTemporaleFine) & " ")
                StrSQL.Append(" AND     Entita.Validita_Fine > " & Agro_SQL_SaveDate(objParametri.FinestraTemporaleInizio) & " ")

                StrSQL.Append(" AND Entita.PivaSuperUser = '" & Agro_SQL_SaveText(PivaSuperUser) & "' ")

                If ElementoGrafico_Cod <> 0 Then
                    StrSQL.Append(" AND Entita.ElementoGrafico_Cod = " & Agro_SQL_SaveNum(ElementoGrafico_Cod) & " ")
                End If

                If Entita_Cod <> 0 Then
                    StrSQL.Append(" AND Entita.Entita_Cod = " & Agro_SQL_SaveNum(Entita_Cod) & " ")
                End If

                If LayerElementiGrafici_Cod <> 0 Then
                    StrSQL.Append(" AND Entita.LayerElementiGrafici_Cod = " & Agro_SQL_SaveNum(LayerElementiGrafici_Cod) & " ")
                End If
                ...
                Select Case objParametri.FlagVisibilita
                    Case enumVisibilita.Visibilita_SoloNonCancellati
                        StrSQL.Append(" AND   Entita.Inviato >=0 ")
                    Case enumVisibilita.Visibilita_SoloCancellati
                        StrSQL.Append(" AND   Entita.Inviato =-1 ")
                ...
                If isXML Then
                    StrSQL.Append(" for xml auto, root('DatiEntita'), elements")
                End If
    ...
    Public Function Leggi_X_IF(listaChiavi As List(Of mykey), ByRef objParametri As AgronicaCoreDataProvider.AgronicaCoreParametri) As DataTable
        ...
        Try
            ...
            For Each k In listaChiavi
                sql.AppendLine(sep & "('" & k.piva & "', " & k.sa_cod.ToString & ", " & k.appezza.ToString & ", " & k.extKey.ToString & ") ")
                sep = ","
            Next
