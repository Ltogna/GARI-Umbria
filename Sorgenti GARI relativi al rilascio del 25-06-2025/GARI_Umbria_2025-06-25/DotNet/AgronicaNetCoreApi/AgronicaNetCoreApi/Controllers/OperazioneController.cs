    ...
    public class OperazioneController : BaseController
    {
        ...
        public async Task<IActionResult> GetOperazioni([FromBody] LeggiOperazioni_IN leggiOperazioni)
        {
            ...
            try
            {
                ...
                var objParametriServer = ExtractObjParametriServerAndSetCulture(auth);
                var operazioni = await _operazioniService.Operazioni_GestioneFiltroUtente_LeggiAsync(leggiOperazioni,objParametriServer,
                                                                                                 UtilityAgronica.convertStringtoOBJparametri(auth.objP.objP_utenti, _securitySettings));
        ...
        public async Task<IActionResult> SaveOperazioneCausale([FromBody] OperazioneCausale_In body)
        {
            ...
            try
            {
                ...
                var objParametriServer = ExtractObjParametriServerAndSetCulture(auth);
                var result = await _operazioneCausaleService.OperazioneCausale_ScriviModificaAsync(body, objParametriServer,
                                                                                                         UtilityAgronica.convertStringtoOBJparametri(auth.objP.objP_utenti, _securitySettings));
        ...
        public async Task<IActionResult> LeggiTrattamentiPerAvversita([FromQuery] string Piva, int Sa_Cod, int APPEZZA, int ID_REG, int Av_Cod, int Intervallo)
        {
            ...
            try
            {
                ...
                var result = await _trattamentiService.LeggiTrattamentiPerAvversitaAsync(Piva, Sa_Cod, APPEZZA, ID_REG, Av_Cod, Intervallo, UtilityAgronica.convertStringtoOBJparametri(auth.objP.objP_server, _securitySettings));
