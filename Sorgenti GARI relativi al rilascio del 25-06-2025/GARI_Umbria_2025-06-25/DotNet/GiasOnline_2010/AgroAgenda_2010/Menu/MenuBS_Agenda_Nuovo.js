...
function KendoOperazioni_inizializza(divKendoOperazioni) {
    ...
    if ($("input[name$='hf_UtenteAbilitatoGestioneVisualizaAllegato']").val() == "True") {
        strBottoni = strBottoni + "<span class='fa fa-file-text-o fa-2x info_elem' title='Gestione Documenti' onclick=ApriKendoWindowRicercaDocumenti(this.closest('tr'),this.closest('.k-grid'))></span>";
    }


    if ($("input[name$='hf_UtenteAbilitatoGestioneNuovoAllegato']").val() == "True") {
        strBottoni = strBottoni + "<span class='fa fa-paperclip fa-2x info_elem' title='Aggiungi nuovo allegato'  onclick=ApriKendoWindowAggiungiNuovoAllegato(this.closest('tr'),this.closest('.k-grid')) ></span>";
    }
