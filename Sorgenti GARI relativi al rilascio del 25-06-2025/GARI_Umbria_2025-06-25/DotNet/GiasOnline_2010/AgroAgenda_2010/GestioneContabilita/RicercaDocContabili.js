    ...
    if (type !== DocContab_TipoRicerca_Conferimenti) {
        ...
        colonneKendoGrid.unshift({
            command: [
                ...
                },
                {
                    ...
                    visible: function (dataItem) {
                        return dataItem.Blocco_Flag === false && workflowPermessoModCanc(dataItem.Permesso_Modifica_Workflow);
                    }
                },
                {
                    template: templateEliminaDocumento,
                    visible: function (dataItem) {
                        return dataItem.Blocco_Flag === false && workflowPermessoModCanc(dataItem.Permesso_Cancella_Workflow);
                    }
                },
                {
                    template: templateSbloccaDocumento,
                    visible: function (dataItem) {
                        return dataItem.Blocco_Flag === true;
                    }
                },
                {
                    template: templateBloccaDocumento,
                    visible: function (dataItem) {
                        return dataItem.Blocco_Flag === false;
                    }
                },
                {
        ...
        colonneKendoGrid.unshift({
            command: [
                ...
                {
                    template: templateModificaDocumento,
                    visible: function (dataItem) { return dataItem.Blocco_Flag === false && workflowPermessoModCanc(dataItem.Permesso_Modifica_Workflow) && (dataItem.Tipo_Accettazione != -1 || utenteAbilitatoPomodoro); }
                },
                {
                    template: templateEliminaDocumento,
                    visible: function (dataItem) { return dataItem.Blocco_Flag === false && workflowPermessoModCanc(dataItem.Permesso_Cancella_Workflow) && (dataItem.Tipo_Accettazione != -1 || utenteAbilitatoPomodoro); }
                },
                {
                    template: templateSbloccaDocumento,
                    visible: function (dataItem) { return dataItem.Blocco_Flag === true; }
                },
                {
                    template: templateBloccaDocumento,
    ...
    switch (IDControllo)
    {
        ...
        case "tab_testata_griglia_report_vendite":
            ...
            colonneKendoGrid = [
                ...
                { field: "Blocco_Flag", title: TraduzioneMultiResx(resxObj, "Bloccata", "Bloccata"), template: '<input type="checkbox" #= Blocco_Flag ? \'checked="checked"\' : "" # class="k-checkbox" disabled/>', attributes: { style: "text-align:center;" }, width: 95 },
    ...
    if (type !== DocContab_TipoRicerca_Conferimenti) {
        ...
        colonneKendoGrid.unshift({
            command: [
                ...
                {
                    template: templateModificaDocumento,
                    visible: function (dataItem) { return dataItem.Blocco_Flag === false && workflowPermessoModCanc(dataItem.Permesso_Modifica_Workflow); }
                },
                {
                    template: templateEliminaDocumento,
                    visible: function (dataItem) { return dataItem.Blocco_Flag === false && workflowPermessoModCanc(dataItem.Permesso_Cancella_Workflow); }
                },
                {
                    template: templateVisualizzaDocumento
                },
...
function RichiamaPassaggioStato() {
    ...
    if (docSelezionati.length > 0) {
        ...
        for (let i = 0; i < docSelezionati.length; i++) {
            ...
            if (docSelezionati[i].Blocco_Flag === true) {
                docBloccato = true;
                break;
            }
