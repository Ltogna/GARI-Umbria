...
Public Class DocumentoContabileGenerico
    ...
    Private Sub XML_Leggi_MovimentoDettaglio(ByRef XmlDoc As XmlDocument,
        ...
        If str_Movimento_Dettaglio <> "" Then
            ...
            If Not IsNothing(XML_MovimentoDettaglio) Then
                ...
                'Dr.Item(str_COL_PIVA_BASE) = CStr(XML_MovimentoDettaglio.GetAttribute("piva"))
                Piva = CStr(XML_MovimentoDettaglio.GetAttribute("piva"))

                'Dr.Item(str_COL_SACOD_BASE) = CInt(XML_MovimentoDettaglio.GetAttribute("sa_cod"))
                Sa_Cod = CInt(XML_MovimentoDettaglio.GetAttribute("sa_cod"))

                'Dr.Item(str_COL_ID_AGENDA_BASE) = CInt(XML_MovimentoDettaglio.GetAttribute("id_agenda"))
                Id_Agenda = CInt(XML_MovimentoDettaglio.GetAttribute("id_agenda"))

                'Dr.Item(str_COL_ID_MOV_BASE) = CInt(XML_MovimentoDettaglio.GetAttribute("id_mov"))
                Id_Mov = CInt(XML_MovimentoDettaglio.GetAttribute("id_mov"))

                'Dr.Item(str_COL_ID_MOV_DET_BASE) = CInt(XML_MovimentoDettaglio.GetAttribute("id_mov_det"))
                ...
                If Not IsNothing(XML_MovimentoDestinazione) Then
                    'Dr.Item(str_COL_IDDEST_BASE) = CInt(XML_MovimentoDestinazione.GetAttribute("id_destinazione"))
                    ...
                    End If
                Else
                    'Dr.Item(str_COL_IDDEST_BASE) = 0
                    Id_Destinazione = 0
                    'Dr.Item(str_COL_DEST_BASE) = ""
                    Destinazione = ""
                End If
    ...
    Private Sub Leggi_MovimentoDettaglio(ByVal Movimento_Dettaglio As Movimento_Dettaglio,
        ...
        If Not IsNothing(Movimento_Dettaglio) Then
            ...
            Sa_Cod = Movimento_Dettaglio.Sa_Cod 'CInt(XML_MovimentoDettaglio.GetAttribute("sa_cod"))
            Id_Agenda = Movimento_Dettaglio.Id_Agenda ' CInt(XML_MovimentoDettaglio.GetAttribute("id_agenda"))
            Id_Mov = Movimento_Dettaglio.Id_Mov ' CInt(XML_MovimentoDettaglio.GetAttribute("id_mov"))
            Id_Mov_Det = Movimento_Dettaglio.Id_Mov_Det 'CInt(XML_MovimentoDettaglio.GetAttribute("id_mov_det"))
            ...
            x_Mov_Det_Des = Movimento_Dettaglio.Mov_Det_Des 'XML_MovimentoDettaglio.GetAttribute("mov_det_des")

            'chiave del prodotto
            x_Elem_Cod = Movimento_Dettaglio.Elem_Cod ' XML_MovimentoDettaglio.GetAttribute("elem_cod")
            x_Pro_Cod = Movimento_Dettaglio.Pro_Cod 'XML_MovimentoDettaglio.GetAttribute("pro_cod")
            x_Mat_Cod = Movimento_Dettaglio.Mat_Cod 'XML_MovimentoDettaglio.GetAttribute("mat_cod")
            x_Cod_Progetto = Movimento_Dettaglio.Cod_Progetto 'XML_MovimentoDettaglio.GetAttribute("cod_progetto")
            x_Fase_Cod = Movimento_Dettaglio.Fase_Cod 'XML_MovimentoDettaglio.GetAttribute("fase_cod")
            x_Lotto = Movimento_Dettaglio.Lotto 'XML_MovimentoDettaglio.GetAttribute("lotto")
            x_Cal_Cod = Movimento_Dettaglio.Cal_Cod 'XML_MovimentoDettaglio.GetAttribute("cal_cod")
            x_Udm_Cod = Movimento_Dettaglio.Udm_Cod 'XML_MovimentoDettaglio.GetAttribute("udm_cod")
            ...
            x_Udm_Cod_Extra = Movimento_Dettaglio.Udm_Cod_Extra ' XML_MovimentoDettaglio.GetAttribute("udm_cod_extra")
            x_Qta_Extra = Movimento_Dettaglio.Qta_Extra 'XML_MovimentoDettaglio.GetAttribute("qta_extra")
            x_Prezzo_Effettivo = Movimento_Dettaglio.Prezzo_Effettivo 'XML_MovimentoDettaglio.GetAttribute("prezzo_effettivo")

            'serve per ricavare x_Descrizione
            x_Descrizione = Ricava_Tipo_Descrizione(Piva, Data_Operazione)

            x_Quantita = Movimento_Dettaglio.Qta ' XML_MovimentoDettaglio.GetAttribute("qta")

            x_Prezzo_Unitario = Movimento_Dettaglio.Prezzo_Unitario ' CDbl(XML_MovimentoDettaglio.GetAttribute("prezzo_unitario"))
            x_Prezzo_Unitario_Netto = Movimento_Dettaglio.Prezzo_Unitario_Netto ' CDbl(XML_MovimentoDettaglio.GetAttribute("prezzo_unitario_netto"))

            x_Sconto_Perc = Movimento_Dettaglio.Sconto ' XML_MovimentoDettaglio.GetAttribute("sconto")
            x_Sconto = ((x_Prezzo_Unitario * x_Sconto_Perc) / 100) * x_Quantita
            ...
            x_Cod_IVA = Movimento_Dettaglio.Cod_Iva 'XML_MovimentoDettaglio.GetAttribute("cod_iva")
            x_Aliquota = objIVA.Aliquota_from_CodIVA(x_Cod_IVA, "", objParametri_Server)
            x_IVA = objContabHLP.Leggi_IVA_PositivaNegativa(QS_LavCod, Movimento_Dettaglio.Iva)

            x_Anno = Movimento_Dettaglio.Anno ' XML_MovimentoDettaglio.GetAttribute("anno")
            x_Ric_Cod = Movimento_Dettaglio.Ric_Cod 'XML_MovimentoDettaglio.GetAttribute("ric_cod")

            x_Cod_Conto = Movimento_Dettaglio.Cod_Conto ' XML_MovimentoDettaglio.GetAttribute("cod_conto")
            If x_Cod_Conto = 0 Then
                x_Conto = ""
            Else
                'da form prodotto
            ...
            If Not IsNothing(Movimento_Dettaglio.Movimenti_Destinazioni) AndAlso Movimento_Dettaglio.Movimenti_Destinazioni.Count > 0 Then
                ...
                Id_Destinazione = Movimento_Dettaglio.Movimenti_Destinazioni(0).Id_Destinazione ' CInt(XML_MovimentoDestinazione.GetAttribute("id_destinazione"))
            ...
            If Flag_DDTAllegato Then
                ...
                If Not IsNothing(Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti) AndAlso Not Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti.Count > 0 Then
                    ...
                    Piva_DDT = Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti(0).Piva_Rif '.GetAttribute("piva_rif")
                    Sa_Cod_DDT = Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti(0).Sa_Cod_Rif 'XML_MovRiferimento2.GetAttribute("sa_cod_rif")
                    Id_Agenda_DDT = Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti(0).Id_Agenda_Rif 'XML_MovRiferimento2.GetAttribute("id_agenda_rif")
                    Id_Mov_DDT = Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti(0).Id_Mov_Rif 'XML_MovRiferimento2.GetAttribute("id_mov_rif")
                    Id_Mov_Det_DDT = Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti(0).Id_Mov_Det_Rif ' XML_MovRiferimento2.GetAttribute("id_mov_det_rif")
                    Lav_Cod_DDT = Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti(0).Lav_Cod_Rif 'XML_MovRiferimento2.GetAttribute("lav_cod_rif")
                ...
                If Not IsNothing(Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti) AndAlso Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti.Count > 0 Then
                    ...
                    Select Case Lav_Cod_DDT
                        ...
                        Case LAVCOD_CARICO, LAVCOD_SCARICO 'movimenti di magazzino
                            ...
                            'Dr.Item(str_COL_PIVA_RIF) = CStr(XML_MovRiferimento2.GetAttribute("piva"))
                            Piva_Rif = Movimento_Dettaglio.Movimenti_Dettagli_Riferimenti(0).Piva 'CStr(XML_MovRiferimento2.GetAttribute("piva"))
...
#End Region
    ...
    Private Function CreaOggettoAgenda() As Operazione_Agenda
        ...
        '----- Calcolo i valori di BaseCode e TopCode
        '------------------------------------------------
        Call Calcola_BaseCode_TopCode(BaseCode, TopCode,
                                      Session("ASG_ProgressivoGIAS"))
