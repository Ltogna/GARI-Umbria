...
function ImpostaOpzioniVisibilitaOperazione(impJson, lavCod) {
    ...
    switch (lavCod) {
        ...
        case enum_LavCod.Fattura_Emessa:
...
function DisabilitaAltreTab() {
    ...
    $(".tab-pane.active").find(".btn").attr("disabled", true);
    $(".tab-pane.active").find(".k-button").attr("disabled", true);
    $(".tab-pane.active").find('[role="button"]').attr("disabled", true);
    $(".tab-pane.active").find('[role="tab"]').attr("disabled", true);

    // Eccezione, questi bottoni devono essere sempre attivi:
    $("#btn_cedente_cessionario_2_raccolte").attr("disabled", false);
    $("#btn_cedente_cessionario_2_compliant_iscc").attr("disabled", false);

    //$(".tab-pane.active").find("input").attr("readonly", true);
    //$(".tab-pane.active").find("input").attr("disabled", true);
}
...
function RiabilitaAltreTab() {
    ...
    $(".tab-pane.active").find(".btn").attr("disabled", false);
    $(".tab-pane.active").find(".k-button").attr("disabled", false);
    $(".tab-pane.active").find('[role="button"]').attr("disabled", false);
    $(".tab-pane.active").find('[role="tab"]').attr("disabled", false);

    //$(".tab-pane.active").find("input").attr("readonly", false);
    //$(".tab-pane.active").find("input").attr("disabled", false);
}
...
function AbilitaModificaDatiMinimiTestata(flagAbilita) {
    ...
    if (isRaccolteXConferimentiAttive() && raccolteXConferimenti_dsSelezionate.length > 0) {
        ...
        if (contattiAcc4ConGerarchia === true) {
            ...
            let kddlContattoCoop2Nome = KendoDDL(ddlContattoCoop1Nome);
