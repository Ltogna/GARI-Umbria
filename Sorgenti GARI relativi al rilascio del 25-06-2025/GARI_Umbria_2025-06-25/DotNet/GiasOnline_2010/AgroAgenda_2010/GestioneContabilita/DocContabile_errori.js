...
function EvidenziaErroriIntestazione() {
    ...
    let listElementsError = [];
    if (yearVersioneKendo >= 2022) {
        listElementsError = $("#intestazione").find(".k-invalid").children().not("span,button").filter("[id]");
    } else {
        listElementsError = $("#intestazione").find(".k-invalid").not("span");
    }
...
function EvidenziaErroriTabTestata() {
    ...
    let listElementsError = [];
    if (yearVersioneKendo >= 2022) {
        listElementsError = $("#tabTestataDoc").find(".k-invalid").children().not("span,button").filter("[id]");
    } else {
        listElementsError = $("#tabTestataDoc").find(".k-invalid").not("span");
    }
