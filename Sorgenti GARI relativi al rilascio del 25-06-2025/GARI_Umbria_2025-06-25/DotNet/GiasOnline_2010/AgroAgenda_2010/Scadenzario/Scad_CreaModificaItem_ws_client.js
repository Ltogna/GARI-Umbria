...
function RiempiDdlMacchina(options) {
    ...
    var parametri = kendo.stringify({ "objP_server": objP_server, "piva": piva, "mac_cod": parseInt($(cMac_Cod).val()) });

    ajaxAgronicaSync(pathCoreWS + "Anagrafica/Macchine.asmx/Leggi_Macchine_Per_Piva",
        parametri,
        false,
        function (risposta) {
            risp = JSON.parse(risposta.RispostaStringa);
