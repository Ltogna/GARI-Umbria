    ...
    ajaxAgronicaSync(indirizzohttp + "/Leggi_Specie",
        ...
        function (risposta) {
            ...
            if (trovata && index_specie_non_valorizzata!==-1) {
                ...
                risp[index_specie_non_valorizzata].veg_des = "--- Specie non valorizzata";
                ...
                let specie_non_valorizzata = {
                    "veg_cod": 0,
                    "veg_des": "--- Specie non valorizzata"
                };
...
function Carica_prodotto_UC_ddlCodIvaCompensazione(options) {
    ...
    for (var i = 1; i < risp.length; i++) {
        ElencoIvaComp[i] = { Cod: risp[i].Cod, Descrizione: risp[i].Codice + " - " + risp[i].Descrizione + "- Iva Ord " + risp[i].Aliquota_Ordinaria + " % - Iva Comp " + risp[i].Aliquota_Compensativa + " %" };
    }
...
function Carica_ddl_set(options) {
    let ElencoSet = [];
    //i18N Datasource Set da tradurre
    ElencoSet.push({ peso_set_cod: 0, peso_set_des: "Peso Nominale Non Modificabile" });
    ElencoSet.push({ peso_set_cod: 1, peso_set_des: "Peso Nominale Modificabile" });
    ElencoSet.push({ peso_set_cod: 2, peso_set_des: "Peso Modificabile e Consenti Sfuso" });
...
function Carica_ddl_peso(options) {
    ...
    let elem_cod = parseInt($(Controls.xElem_Cod).val());
     ...
     //i18N Datasource Peso da tradurre
    if (elem_cod === categorieProdotti.BENI_CONFEZIONAMENTO_ANIMALI || elem_cod === categorieProdotti.BENI_CONFEZIONAMENTO_VEGETALI) {
        ElencoPeso.push({ tipo_peso_cod: enum_TipoPeso.Peso_Netto, tipo_peso_des: "Capacità:" });
    }
    else {
        ElencoPeso.push({ tipo_peso_cod: enum_TipoPeso.Peso_Netto, tipo_peso_des: "Peso Netto Nominale Kg:" });
        ElencoPeso.push({ tipo_peso_cod: -1, tipo_peso_des: "Capacità Nominale Litri:" });
    }
...
function Carica_ddl_Confezione_Base(options) {
    let ElencoConfezioneBase = [];
     ...
     //i18N Confezione Base da tradurre
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_NESSUNO, base_des: "Nessun Automatismo" });
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_CONFEZIONI_FF, base_des: "Confezione" });
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_CONTENITORI_FF, base_des: "Contenitore" });
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_IMBALLAGGI_FF, base_des: "Imballaggio" });
...
function Carica_Tipo_Default(options) {
    ...
    ajaxAgronicaSync(indirizzohttpPaginaProdotto + "/Leggi_XddlTipo_Default",
        ...
        function (risposta) {
            let risp = JSON.parse(risposta.RispostaStringa);
            var objVuoto = { "Codice_Generazione": 0, "Descrizione": "Altro" };
            risp.unshift(objVuoto);
            options.success(risp);
        },
        null);
...
function RiempiMultiseSpecieDettagliBeniConf(options) {
    ...
    if (elencoSpecie !== null && elencoSpecie !== undefined) {
        ...
        if (index !== null) {
            //I18N da tradurre "Tutte le specie Vegetali"
            //Aggiungo nelle multiselect per il caso vuoto il messaggio "Tutte le specie Vegetali"
            elencoSpecie[index].Veg_Des = "Tutte le Specie Vegetali";
        }
...
function RiempiMultiseVarietaDettagliBeniConf(options) {
    ...
    if (elencoVarieta !== null && elencoVarieta !== undefined) {
        ...
        if (index !== null) {
            //I18N da tradurre "Tutte le Varietà Coulturali"
            //Aggiungo nelle multiselect per il caso vuoto il messaggio "Tutte le Varietà Colturali"
            elencoVarieta[index].Cul_Des = "Tutte le Varietà Colturali";
        }
...
function Carica_ddlProduzionePropria(options) {
    ...
    //i18N Produzione Propria da tradurre
    ElencoProduzioni.push({ Produzione_Cod: enum_Produzione_Cod.Prodotto_Commercializzato, Produzione_Des: "Prodotto Commercializzato" });
    ElencoProduzioni.push({ Produzione_Cod: enum_Produzione_Cod.Produzione_Propria, Produzione_Des: "Produzione Propria" });
...
function Carica_ddlPesoEgalizzato(options) {
    ...
    ElencoPesi.push({ Peso_Cod: 1, Peso_Des: "Sì" });
    ElencoPesi.push({ Peso_Cod: 0, Peso_Des: "No"});
                    ...
                    if (cal_cod !== 0) {
                        ...
                        if (mat_cod_ref !== parseInt(Get_KendoDDLValue("ddl_prodotto_UC_Prodottobase"))) {
                            if (Prodotto_Movimentato === true) {
                                ...
                                let Msg_Errore = "Il Prodotto Base di Riferimento non può essere cambiato perchè il prodotto risulta già movimentato";
    ...
    if (controlliPrimaDelSubmitOk) {
        ajaxAgronicaSync(indirizzohttpPaginaProdotto + "/Submit",
            ...
            function (risposta) {
                ...
                //i18N Prodotto salvato correttamente! da tradurre
                
                kendo.alert(indirizzohttpPaginaProdotto);
                kendo.alert("Prodotto salvato correttamente!");
...
function Cancella_Prodotto_Effettivo(rigaDaCancellare,materiaprima) {
    ...
    ajaxAgronica("../Anagrafica/Prodotto_Edit.aspx/Cancella_Prodotto",
        ...
        function (risposta) {
            ...
            if (materiaprima === true) {
                kendo.alert("Prodotto cancellato correttamente");
            }
            else {
                kendo.alert("'Storico Prezzi' e 'Dati Contabilità' cancellati correttamente");
            }
