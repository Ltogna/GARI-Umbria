...
function kendoProdotto_inizializza(divKendo, keys) {
    ...
    var templateModificaProdotto = "<span class='fa fa-pencil-square-o fa-2x edit_elem' title='Modifica Prodotto' onclick=modificaProdotto(this.closest('tr'),this.closest('.k-grid'))></span>";

    var templateEliminaProdotto = "<span class='fa fa-trash-o fa-2x del_elem' title='Elimina Prodotto' onclick=eliminaProdotto(this.closest('tr'),this.closest('.k-grid'))></span>";

    var templateVisualizzaProdotto = "<span class='fa fa-info fa-2x info_elem' title='Visualizza Prodotto' onclick=infoProdotto(this.closest('tr'),this.closest('.k-grid'),0)></span>";

    var templateDuplicaProdotto = "<span class='fa fa-files-o fa-2x copia_elem' title='Duplica Prodotto' onclick=duplicaProdotto(this.closest('tr'),this.closest('.k-grid'),0)></span>";
...
function caricaGrigliaProdotto(keys) {
    ...
    if (FiltroMinCaratteriPerRicerca() &&
        ...
        kendo.alert("Inserire almeno tre caratteri della Descrizione/Cod. Articolo del Prodotto.");
    }
    else {
...
async function eliminaProdotto(tr_elem, grid_elem) {
    ...
    if (permessoModifica) {
        ...
        if (mat_prima === true) {
            ...
            if (movimentato === true) {
                kendo.alert("Il Prodotto è già movimentato non è possibile cancellarlo");
            } else {
                ...
                if (prod.CancellaOk === false) {
                    ...
                    if (prodotto !== null && prodotto !== undefined && prodotto !== "") {
                        ...
                        if (parseInt(prodotto.Flag_Importato) === 1) {
                            kendo.alert("Il Prodotto è stato importato non è possibile cancellarlo");
                            ...
                            if (mess !== "") {
                                msgTextDialog = "Si desidera davvero eliminare il prodotto aziendale dall'archivio anagrafico? <br> <strong>Verrano eliminati anche:<br>" + mess + "</strong>";
                            }
                            else {
                                msgTextDialog = "Si desidera davvero eliminare il prodotto aziendale dall'archivio anagrafico?";
                            }
                        }
                    }
                    else {
                        kendo.alert("Errore non è possibile completare la cancellazione");
                        mostra_dialog_cancella = false;
                    }
                }
            }
            ...
            mostra_dialog_cancella = true;
            msgTextDialog = "Si desidera davvero eliminare lo 'Storico Prezzi' , i 'Dati Contabilità' e i 'Dati Vendita Dettaglio' associati al prodotto aziendale?";
        }
        ...
        if (mostra_dialog_cancella) {
            ...
            //I18N kendo Conferma Cancellazione confirm da tradurre
        ...
        kendo.alert("Non si dispongono dei permessi per poter Cancellare il Prodotto");
    }
...
function Prodotto_Edit_UC_ddl_prodotto_UC_categ_prod_change(e) {
    ...
    if ($("#ddl_prodotto_UC_categ_prod").attr("last_selected_categ_prod") !== undefined &&
        ...
        var msgText = "Si desidera davvero cambiare la Categoria Prodotto?(Verranno persi i valori inseriti per la Categoria '" + $("#ddl_prodotto_UC_categ_prod").attr("last_selected_categ_prod_des") + "')";
        ...
        $("#id_dialog_change_categ_prod").kendoDialog({
            title: "Conferma Cambio Categoria Prodotto",
            closable: false,
            modal: {
                preventScroll: true
            },
...
function Prodotto_Edit_UC_ddl_prodotto_UC_Regolamento_Load() {
    //i18N Datasource Regolamenti da tradurre
    let Regolamenti = [{ Reg_DES: "Convenzionale (Reg. Nessuno)", Reg_COD: 1 }, { Reg_DES: "Biologico (Reg.CE 834/07 (Ex.Reg.CE 2092/91))", Reg_COD: 4 }];
...
function Ddl_prodotto_UC_mat_prima_priorita_cdg_Read(options) {
    options.success([
        { Val: 0, Des: "Prodotto principale" },
        { Val: 1, Des: "Prodotto secondario" }
    ]);
}
        ...
        if (is_alias === false) {
            ...
            if (grid_Alias !== undefined && grid_Alias !== null && grid_Alias !== "") {
                ...
                if (ds_grid_Alias.length > 0) {
                    ...
                    kendo.alert("Non è possibile cambiare la Visibilità perché ci sono delle Descrizioni Alternative associate al Prodotto.");
            ...
            if (Prodotto_Edit_UC_Alias_Associato_A_Materia_Prima === true) {
                ...
                kendo.alert("Non è possibile cambiare la Visibilità perché il Prodotto per Descrizioni Alternative è già stato associato a dei Prodotti.");
...
function PopolaGrigliaStoricoPrezzi(IDControllo, Disabilita_Griglia) {
    ...
    var colonneKendoGrid = [
        {
            field: "Validita_Inizio", title: "Validità Inizio", format: "{0:dd/MM/yyyy}"
        },
        {
            field: "Validita_Fine", title: "Validità Fine", format: "{0:dd/MM/yyyy}"
        },
        {
            field: "Udm_Des", title: "Unità di Misura", editor: unita_misura_DropDownEditor
        },
        {
            field: "Prezzo_Unitario", title: "Prezzo", editor: numberEditor2decimals, format: "{0:n2}"
        }
    ];
...
function SubmitProdottiXGrigliaStoricoPrezzi(options) {
    ...
    if (nrErr > 0) {
        if (nrErr === 1)
            //i18N Esiste una riga con dati non completi nella griglia Storico Prezzi da tradurre
            MessaggioErrore_Bootstrap("Esiste una riga con dati non completi nella griglia Storico Prezzi.", "DIV_Messaggi");
        else
            //i18N Esistono righe con dati non completi nella griglia Storico Prezzi da tradurre
            MessaggioErrore_Bootstrap("Esistono " + nrErr + " righe con dati non completi nella griglia Storico Prezzi.", "DIV_Messaggi");
...
function PopolaGrigliaCalibri(IDControllo) {
    ...
    var colonneKendoGrid = [
        {
            field: "Cal_Des", title: "Calibri", filterable: { multi: true, search: true }
        }
    ];
    var parametriPerLettura = null;
...
function PopolaGrigliaIndici(IDControllo) {
    ...
    var colonneKendoGrid = [
        {
            field: "IND_MAT_DES", title: "Indici di Maturità", filterable: { multi: true, search: true }
        }
    ];
...
function PopolaGrigliaTraduzioni(IDControllo, Disabilita_Griglia) {
    ...
    var colonneKendoGrid = [
        {
            field: "Nome", title: "Lingua", editor: Lingua_DropDownEditor
        },
        {
            field: "Mat_Des", title: "Traduzione"
        },
        {
            field: "DATA_AGG", title: "Data Aggiunta", format: "{0:dd/MM/yyyy}"
        }
    ];
...
function SubmitProdottiXTraduzioni(options) {
    ...
    if (nrErr > 0) {
        if (nrErr == 1)
            //i18N Esiste una riga con dati non completi nella griglia Traduzioni da tradurre
            MessaggioErrore_Bootstrap("Esiste una riga con dati non completi nella griglia Traduzioni.", "DIV_Messaggi");
        else
            //i18N Esistono righe con dati non completi nella griglia Traduzioni da tradurre
            MessaggioErrore_Bootstrap("Esistono " + nrErr + " righe con dati non completi nella griglia Traduzioni.", "DIV_Messaggi");
        ...
        if ($(xDatiImmagineCaricata).val() !== "") {
            ...
            if (rilevatoVirus !== "") {
                ...
                if (TipidiImmagineCaricabili.includes(datiImmagine.Estensione.toLowerCase()) === false) {
                    let Msg_Errore = "Estensione dell'Immagine caricata non supportata.<br> Le estensioni supportate sono: <strong>" + TipidiImmagineCaricabili.join(",") + "</strong>.";
...
function prodotto_Edit_UC_ValidaxSubmit(nuovo, esci) {
    ...
    if (tipoOperazione === enum_tipoOperazione.Scrittura && impGruppoMerceControllo == true) {
        ...
        if (kddlGruppoMerce === undefined || (kddlGruppoMerce !== undefined && kddlGruppoMerce.dataSource.data().length > 1 && parseInt(kddlGruppoMerce.value()) === 0)) {
            ...
            content.push("Non è stato inserito il gruppo merce");
    ...
    switch (elem_cod) {
        ...
        case categorieProdotti.BENI_CONFEZIONAMENTO_VEGETALI:
            ...
            if (tabStripAperti.includes("a_tab_prodotto_UC_configurazione") &&
                ...
                if ((Mostra_chkconfezione === true || Impostazione_utenteTabConfigurazione.Mostra_ChkContenitore === true ||
                    ...
                    mostraDialogConferma = true;
                    content.push("Se non viene selezionato 'Impostazione Pesi', perderai i dati di 'Dettagli Beni Di Confezionamento'");
                    listaAzioniConferma.push(function (e) {
        ...
        case categorieProdotti.TRASFORMATI_VEGETALI:
            ...
            if (elem_cod === categorieProdotti.TRASFORMATI_VEGETALI && Modulo_FF === true && Is_OMNI === false &&
                ...
                mostraDialogConferma = true;
                content.push("Non è stato scelto il Prodotto Base di Riferimento per il Trasformato Vegetale");
                //listaAzioniConferma.push(function (e) {});
            }
            break;
    }
    ...
    if (!mostraDialogConferma) {
        ...
        //I18N kendo confirm da tradurre
        let strContent = "";
        if (content.length === 1) {
            strContent = "È stato rilevato il seguente messaggio"
        }
        else {
            strContent = "Sono stati rilevati i seguenti messaggi"
        }

        strContent += "<br/><br/>- " + content.join("<br/>-") + "<br/><br/>Procedere comunque al salvataggio?"
...
function Configuratore_Maschera_MateriaPrima(Elem_Cod_CategScelta) {
    ...
    switch (Elem_Cod_CategScelta) {
        case categorieProdotti.ALTRE_RISORSE:
            ...
            $('label[for="chk_prodotto_UC_Udm_Cod_Extra"]').html("Prodotto Confezionato");
            break;
        case categorieProdotti.MATERIE_PRIME_VEGETALI:
            $("#switch_prodotto_UC_componi_descrizione").hide();
            $("#cb_prodotto_UC_componi_descrizione").data("kendoSwitch").enable(false);
        ...
        case categorieProdotti.MANGIMI:
            ...
            if (Elem_Cod_CategScelta === categorieProdotti.SEMILAVORATI_PRODUZIONE_ANIMALE ||
                ...
                $("#lbl_prodotto_UC_specie_anim").html("Specie Animale *:");

                if (Elem_Cod_CategScelta === categorieProdotti.SEMILAVORATI_PRODUZIONE_ANIMALE) {
                    $("#lbl_prodotto_UC_indi_produt").html("");
                    $("#lbl_prodotto_UC_indi_produt").html("Indirizzo Produttivo *:");

                    $("#lbl_prodotto_UC_razza_anim").html("");
                    $("#lbl_prodotto_UC_razza_anim").html("Razza *:");
                }
            ...
            if (categorieProdotti.BENI_CONFEZIONAMENTO_ANIMALI === Elem_Cod_CategScelta ||
                ...
                if (categorieProdotti.BENI_CONFEZIONAMENTO_ANIMALI === Elem_Cod_CategScelta) {
                    $(".bene_conf").show();
                    //i18N Impostazione Pesi da tradurre
                    $('label[for="chk_prodotto_UC_Udm_Cod_Extra"]').html("Impostazione Pesi");
        ...
        case categorieProdotti.SEMENTI:
            ...
            $("#lbl_prodotto_UC_sementi_materiale").html("Sementi e materiale vivaisti *:");

            $("#lbl_prodotto_UC_specie_veg").html("");
            $("#lbl_prodotto_UC_specie_veg").html("Specie Vegetale *:");

            $("#lbl_prodotto_UC_varieta").html("");
            $("#lbl_prodotto_UC_varieta").html("Varietà Colturale *:");
            //############################################
        ...
        case categorieProdotti.SEMILAVORATI_PRODUZIONE_VEGETALE:
            ...
            if (categorieProdotti.BENI_CONFEZIONAMENTO_VEGETALI === Elem_Cod_CategScelta) {
                ...
                $("#switch_prodotto_UC_componi_descrizione").hide();
                $("#cb_prodotto_UC_componi_descrizione").data("kendoSwitch").enable(false);
                //i18NImpostazione Pesi da tradurre
                $('label[for="chk_prodotto_UC_Udm_Cod_Extra"]').html("Impostazione Pesi");
            }
            else {
                $(".bene_conf").hide();
            ...
            if (paramQual_FF_filtrospevar[ipar].Tipo === 3 ||
                ...
                if (parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 5 || parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 8) {
                    ...
                    if (parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 5) {
                        descr_label = "Numero di confezioni per contenitore";
                    } else if (parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 8) {
                        descr_label = "Numero di contenitori per imballo";
                    }
...
function Prodotto_Edit_UC_Genera_Descrizione_change(e) {
    ...
    if (switch_descrizione.check() === true) {
        if ($("#txt_prodotto_UC_Descrizione").val() !== "" && $("#txt_prodotto_UC_Descrizione").val() !== " ") {
            ...
            //I18N kendo Conferma Generazione Descrizione da tradurre
            $("#id_dialog_genera_descr").kendoDialog({
                title: "Conferma Generazione Descrizione",
                closable: false,
                modal: {
                    preventScroll: true
                },
                content: "Si desidera davvero comporre in automatico la Descrizione del prodotto in base ai dati compilati?</br>( Verrà sovrascritta la Descrizione inserita )",
                actions: [{
                    text: 'No',
                    action: function (e) {
...
function Prodotto_Importato(Flag_Importato) {
    ...
    $("#lbl_prodotto_UC_cod_prod").html("Codice Prodotto *:");
        ...
        //abilito textbox codice esterno e disabilito codice articolo che viene generato
            ...
            if (elem_cod === categorieProdotti.TRASFORMATI_VEGETALI) {
                ...
                if (risp === true) {
                    ...
                    $("#lbl_prodotto_UC_cod_est").html("Codice Esterno *:");
                }
            }
...
$("#btn_prodotto_UC_rimuovi_immagine").on("click", function () {
    ...
    $("#prodotto_UC_lblImage").text("Immagine:");
...
function PopolaGrigliaAlias(IDControllo, Disabilita_Griglia) {
    ...
    var colonneKendoGrid = [
        {
            field: "Mat_Des_Alias", title: "Prodotto", editor: Alias_DropDownEditor, filterable: { multi: true, search: true }
        },
        {
            field: "Filtro_Contatti_Des_String", title: "Contatti", editor: Contatti_MultiSelectEditor, filterable: { multi: true, search: true }
        }
    ];
...
function SubmitProdottiXAlias(options) {
    ...
    if (nrErr > 0) {
        if (nrErr === 1)
            //i18N Esiste una riga con dati non completi nella griglia Alias da tradurre
            MessaggioErrore_Bootstrap("Esiste una riga con dati non completi nella griglia delle Descrizioni Alternative.", "DIV_Messaggi");
        else
            //i18N Esistono righe con dati non completi nella griglia Alias da tradurre
            MessaggioErrore_Bootstrap("Esistono " + nrErr + " righe con dati non completi nella griglia delle Descrizioni Alternative.", "DIV_Messaggi");
