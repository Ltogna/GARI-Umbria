...
#Region "EventiControlli"
    ...
    Private Sub caricaUdm()
        ...
        Dim obj As New AgronicaCoreUtility.CaricaListControl
        obj.Udm_Optimize(CmbUdm, 0, "",
                                     0,
                                     0,
                                     CAU_CARICO,
                                     Elem_Cod,
    ...
    Private Sub generaTabellaTaccoltaManuale()
        ...
        If Veg_Cod_Val = "" Or Veg_Cod_Val = "0" Or Veg_Cod_Val = "-1" Or Not IsNumeric(Split(Veg_Cod_Val, "/")(0)) Then
    ...
    Private Sub CreoTabellaRilievoDaImpianti(ByVal ListaImpianti As List(Of AgronicaCoreModello.ParametriAgenda_Temp.Impianto))
        ...
        For i = 0 To ListaImpianti.Count - 1
            ...
            If Not DtImp Is Nothing AndAlso DtImp.Rows.Count > 0 Then
                ...
                If Not DrImp Is Nothing AndAlso DrImp.Length > 0 Then
    ...
    Private Sub distribuisci(ByVal SupPiante As Boolean)
        If Not IsNumeric(QtaTot.Text) Or (IsNumeric(QtaTot.Text) AndAlso CDbl(QtaTot.Text) < 0) Then
        ...
        For i = 0 To GridView_Rilievo.Rows.Count - 1
            ...
            If i < GridView_Rilievo.Rows.Count - 1 Then
                dose = New AgronicaCoreDataProvider.Agro_Math().RoundNumber_2Decimali(Totale / Suptot * Sup)
                count = count + dose
            Else
                dose = Totale - count
            End If
...
#Region "Creazione oggetto Agenda"
    ...
    Private Function CreaOggettoAgenda(ByVal ListaImp As List(Of AgronicaCoreModello.ParametriAgenda_Temp.Impianto), ByVal Sa_Cod As Integer, ByRef messaggio_errore As String, ByRef ListaOpAgendaCollegate As List(Of Operazione_Agenda)) As Operazione_Agenda
        ...
        If Tipo_Raccolta <> enum_RACCOLTA_TIPO.Fast And ComboMagazzinoDestinazione.ddl_Magazzini.SelectedValue <> "0" Then
...
#Region "Salvataggio"
    ...
    Private Sub SalvaTutto(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If Not IsNothing(Session("UTENTE_COD_BLOCCA_SE_SENZA_MAGAZZINO")) AndAlso Session("UTENTE_COD_BLOCCA_SE_SENZA_MAGAZZINO") = True Then
            If ComboMagazzinoDestinazione.ddl_Magazzini.SelectedValue = "0" Or ComboMagazzinoDestinazione.ddl_Magazzini.SelectedValue = "" Then
    ...
    Private Function SalvaOperazioneAgenda(ByRef messaggio_errore As String, ByRef messaggio_alert As String) As Boolean
        ...
        If ComboLavorazione.SelectedValue <> "0" AndAlso ComboLavorazione.SelectedValue <> "" Then
            ...
            Select Case Operazione
                Case 1
                    If ComboUDS.SelectedValue = "" Or ComboUDS.SelectedValue = "0" Or ComboCentroCura.SelectedValue = "" Or ComboCentroCura.SelectedValue = "0" Then
        ...
        If Tipo_Raccolta <> enum_RACCOLTA_TIPO.Fast AndAlso ComboMagazzinoDestinazione.ddl_Magazzini.SelectedValue <> "0" AndAlso (Not IsNumeric(QtaTot.Text) Or (IsNumeric(QtaTot.Text) AndAlso CDbl(QtaTot.Text) <= 0)) Then
        ...
        'recupero impianti dalla gridview
        If Not IsNumeric(QtaTot.Text) Or (IsNumeric(QtaTot.Text) AndAlso CDbl(QtaTot.Text) <> totale) Then
                ...
                Select Case rbl_Chiusura.SelectedValue
                    ...
                    Case "1", "2", "3"
                        ...
                        Else
                            ...
                            Select Case rbl_Chiusura.SelectedValue
                                Case "1"
                                    If Chk_NuovoEsercizio.Checked = True Then
                                    ...
                                    End If
                                Case "2"
                                    If Chk_NuovoImpianto.Checked = True Then
    ...
    Private Function GeneraProdotto(Veg_Cod As Integer, Veg_Des As String, ListaCul_Cod As List(Of Integer),
        ...
        'GENERO PRODOTTO SE NON INDICATO (per ora solo sulla specie, varietà altre)
        If Elem_Cod <> TRASFORMATI_VEGETALI And Elem_Cod <> SEMILAVORATI_VEGETALI Then
            Throw New Exception
        End If
...
#End Region
    ...
    Private Function Apertura_Progetto(ByVal Piva As String, ByVal Sa_Cod As Integer,
        ...
        Dim DT, DT_CapPri, DT_Codici As DataTable
        ...
        Dim Validita_inizio_prog As Date
        Dim Validita_fine_prog As Date
    ...
    Private Shared Function componiNoteLog(rbl_Chiusura As RadioButtonList, Chk_NuovoEsercizio As CheckBox, Chk_NuovoImpianto As CheckBox) As String
        ...
        If Chk_NuovoEsercizio.Checked = True AndAlso rbl_Chiusura.SelectedValue = "1" Then
