...
#Region "Caricamenti"
    ...
    Private Shared Function TrovaRichiestePrimaDel2025(piva As String, richiesta_cod As Integer, check As Boolean, ByRef Programmazione_Cod As Integer,
        ...
        If (DtMacro.Rows.Count > 0 And (check Or dtRichieste.Rows.Count = 0)) Then
    ...
    Public Shared Function Trova_Richieste_Dettaglio_Post2025(ByVal piva As String,
        ...
        Try
            ...
            Dt.Columns.Add(New DataColumn("Qualita_Cod", GetType(String)))
            Dt.Columns.Add(New DataColumn("Occupazione_Cod", GetType(String)))
            'se macrouso_UMA_Cod = 0 allora significa che voglio tutti i dettagli di tutti i macrousim quindi aggiungo altre info
    ...
    <WebMethod(EnableSession:=True)>
    Public Shared Function Verifica_Superficie_Appezzamenti(ByVal piva As String,
                                                            ByVal richiesta_cod As Integer,
        ...
        Try
            ...
            Dim umaRichiestaXRegImpianti As New UMA_RichiesteXReg_Impianti_R
            Dim dtRichiestaXImpianti = umaRichiestaXRegImpianti.Leggi_Richiesta_X_Reg_Impianti(piva, richiesta_cod, "", 0, rendicontazione_terzista, enumSelezioneVariabile.Selezione_JoinDescrizioni, objParametri_Server, filtroSuLogOpAnagraficheSuccessive:=True)
            ...
            If dtRichiestaXImpianti.Rows.Count > 0 Then
                ...
                Dim umaRichiesta As New UMA_Richieste_R
                Dim objAnalisi As New AgronicaCoreAnagrafeBIZ.Analisi_Modello_R
                Dim strFiltroColtureAppezzamenti As String
                Dim dtColture As DataTable

                Dim lsMacrousi = (From row1 In dtRichiestaXImpianti Select row1.Field(Of String)("Gruppo_Colturale_UMA")).Distinct().ToList()
                strFiltroColtureAppezzamenti = "macrouso_UMA_Cod in ('" & String.Join("', '", lsMacrousi) & "') "
                ...
                For Each row In dtRichieste.Rows
                    ...
                    drColture = dtColture.Select("macrouso_UMA_Cod = '" & Gruppo_Colturale_Cod & "'")
                    ...
                    If totSup <> Totale_Superficie_UMA Or
                       totSupA <> Zona_Pendenza_A_UMA Or
                       totSupB <> Zona_Pendenza_B_UMA Or
                       totSupNormale <> Zona_Tessitura_Normale_UMA Or
                       totSupMedia <> Zona_Tessitura_Media_UMA Or
                       totSupTenace <> Zona_Tessitura_Tenace_UMA Then

                        tessituraAggiornata =
                            Zona_Tessitura_Normale_UMA <> row.Item("Zona_Tessitura_Normale_UMA_Edit") Or
                            Zona_Tessitura_Media_UMA <> row.Item("Zona_Tessitura_Media_UMA_Edit") Or
    ...
    Public Shared Function Correggi_Superfici_Appezzamenti(ByVal piva As String,
        ...
        Try
            ...
            If Not IsNothing(dtAppezzamentiData) And dtAppezzamentiData.Rows.Count > 0 Then
    ...
    Public Shared Function Leggi_Appezzamenti_Programmazione(ByVal piva As String, ByVal programmazione_cod As Integer, ByVal richiesta_Cod As Integer) As RispostaStandard
        ...
        Try
            ...
            If programmazione_cod >= 0 Then
                ...
                If Not DT_Appezzamenti Is Nothing Then
    ...
    Public Shared Function Cerca_Dettagli_Azienda(piva As String,
        ...
        Try
            ...
            If testata IsNot Nothing AndAlso testata.Rows.Count > 0 Then
                ...
                permessoAcqua = IIf(IsDBNull(riga("Permesso_acqua")), 0, riga("Permesso_acqua"))
                notePermessoAcqua = IIf(IsDBNull(riga("Note_permesso_acqua")), "", riga("Note_permesso_acqua"))

                If integrativa And Tipo_Richiesta = enum_UMA_TipoRichiesta.Conto_Proprio Then
            ...
            d0("Anticipazioni_Gasolio") = Anticipazioni_Gasolio
            d0("Anticipazioni_Benzina") = Anticipazioni_Benzina
            d0("Anticipazioni_Gasolio_Serra") = Anticipazioni_Gasolio_Serra
    ...
    Public Shared Function Aggiorna_PermessiAcqua(ByVal piva As String,
        ...
        Try
            ...
            If dtLav.Rows.Count > 0 Then
                If permessoAcqua <= 0 And Not isIntegrativaAcqua Then
                    r.RispostaStringa = "Totale permesso acqua non indicato o non valido"
                    r.RispostaOK = False
                ElseIf notePermessoAcqua = "" And Not isIntegrativaAcqua Then
        ...
        Try
            ...
            If (dt.Rows.Count > 0) Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    dtresRow.Item("Lavorazioni_Alt") = IIf(IsDBNull(dtresRow.Item("Lavorazioni_Alt")), "", dtresRow.Item("Lavorazioni_Alt") + " , ") + row.Item("Lavorazione_UMA_Alt") + " "
                ...
                For Each elem In arrUpd
                    ...
                    Dim uma_r = (From u In
                                     GiasContext.UMA_Richieste Where
                                                    u.Piva = piva And
                                                    u.Richiesta_Cod = Richiesta_Cod And
                                                    u.Gruppo_Colturale_UMA = Macrouso_UMA_Cod And
...
"OR (a.macrouso_UMA_Cod IS NULL AND b.macrouso_UMA_Cod IS NULL AND c.macrouso_UMA_Cod IS NULL AND d.macrouso_UMA_Cod  = " & Richiesta_Ins.Gruppo_Colturale_UMA & ")) ", objParametri_Server)
                ...
                Dim RichiesteDB = (From rd In GiasContext.UMA_Richieste Where rd.Richiesta_Cod = Richiesta_Cod And rd.Piva = piva)
            ...
            ElseIf Programmazione_Cod = CodiceFascicoloAnticipi Then
                ...
                If macrousiList.Count > 0 AndAlso Avanzamento = 1 Then
                    ...
                    Dim dtRichiesteConEccedenzeAnticipo = leggiLavorazioni.Leggi_Lavorazioni(Piva,
                                                                                             ...
                                                                                             enum_WAnagraficaStati.Verifica_Intermedia_Completata_Con_Successo,
                                                                                             objParametri_Server)
                    If Not IsNothing(dtRichiesteConEccedenzeAnticipo) And dtRichiesteConEccedenzeAnticipo.Rows.Count = 0 Then
            ...
            ElseIf Programmazione_Cod = CodiceFascicoloTrasferimenti Then
                ...
                If macrousiList.Count > 0 AndAlso Not IsNothing(CUAA) Then
                    ...
                    Dim dtTrasferimento = leggiVendite.LeggiCarburante("F1999998826",
                                                                       ...
                                                                       objParametri_Server)
                    If Not IsNothing(dtTrasferimento) And dtTrasferimento.Rows.Count = 0 Then
