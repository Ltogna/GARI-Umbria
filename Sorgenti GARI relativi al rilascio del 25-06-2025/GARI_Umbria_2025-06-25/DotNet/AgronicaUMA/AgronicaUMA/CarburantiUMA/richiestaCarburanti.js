...
function App_onDataBoundImpianti(e) {
    ...
    VerificaESegnalaSuperificiAppezzamenti(e);
    coloraRighe_Impianti("#tab_griglia_dettagliImpianti", e);
}
...
function VerificaESegnalaSuperificiAppezzamenti(e) {

    if ($("#stato_pratica_cod").val() == In_Compilazione.toString() && !superficiAppezzamentiVerificate) {

        VerificaSuperficieAppezzamenti();
        superficiAppezzamentiVerificate = true;
        ...
        if (anomalieSuperficiAppezzamenti != null && anomalieSuperficiAppezzamenti.length > 0) {
            ...
            for (var i = 0; i < anomalieSuperficiAppezzamenti.length; i++) {                
                ...
                warningMsg = warningMsg + "Gruppo Colturale " + rigaAnomalia["Gruppo_Colturale_UMA_Des"] + ": "
...
async function CorreggiSuperficiAppezzamenti_Click() {
    ...
    if (res) {
        ...
        if (QS_Type == -1) {
            await LeggiRichiesteTerzista($('#piva')[0].value);
...
async function detailInitGrigliaDettagliLavorazioni(e) {
    ...
    if (QS_Type == 0 /*&& QS_Avanzamento == 1*/) {
        if (lav_incrociati[id_macrouso] == undefined) {
            lav_incrociati[id_macrouso] = await controlloIncrociato($('#piva')[0].value, Programmazione_Cod, id_macrouso, $('#anno')[0].value);
        }
    }
...
async function AggiornaDati() {
    ...
    if (await controlliVari()) {
        ...
        await aggiornaTestata($('#piva')[0].value, richiesta_cod, TxtRimanenza_Gasolio.value(), TxtRimanenza_Benzina.value(), TxtRimanenza_Gasolio_Serra.value());

        if (QS_Type == 0) {
            await aggiornaPermessiAcqua($('#piva')[0].value, richiesta_cod, $("#permessoAcqua").val(), $("#notePermessoAcqua").val())
        }
        ...
        if (QS_Type == 0) {
            ...
            await AggiornaRichieste_UMA($('#piva')[0].value, richiesta_cod, JSON.stringify(data));
            ...
            for (var i = 0; i < tabelleAperte.length; i++) {
                ...
                if (modificheFatte) {
                    await ws_Inserisci_Lavorazioni($('#piva')[0].value,
                        richiesta_cod,
                        Programmazione_Cod,
                        Macrouso_UMA_Cod,
                        created,
...
async function salvaGestioneRimanenzeTestata() {
    ...
    SubmitTrasferiti();
    SubmitRestituzioni();
    let res = await aggiornaTestataGestRima($('#piva')[0].value, richiesta_cod, inconferma);
    if ($("#stato_pratica_cod").val() === In_Compilazione.toString()) {
        if (res.Item1 === true) {
...
function RecuperoAnticipazioni() {
    if (windowdettagliAnt == undefined) {
        MostraAnticipazioniTerzisti($('#piva')[0].value, $('#anno')[0].value)
...
function GrigliaTrasferitiCampiKendoModel() {
    return {
        ...
        Piva: { editable: true, type: "string", defaultValue: $("#piva").val() },
...
function GrigliaRestituzioniCampiKendoModel() {
    return {
        ...
        Piva: { editable: false, type: "string", defaultValue: $("#piva").val() },
    ...
    $('#iframe_analisi_terreno').kendoWindow({
        ...
        close: function () {
            ...
            setTimeout(function () {
                ...
                superficiAppezzamentiVerificate = false;
                $("#tab_griglia_dettagliImpianti").data("kendoGrid").refresh()
            }, 200);
        }
    }).data('kendoWindow').center().maximize();
