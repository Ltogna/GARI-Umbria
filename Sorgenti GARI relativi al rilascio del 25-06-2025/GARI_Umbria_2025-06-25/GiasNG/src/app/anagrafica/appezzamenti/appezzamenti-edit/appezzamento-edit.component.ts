...
import { catchError, from, interval, lastValueFrom, Subject, Subscription, take, takeUntil, tap } from 'rxjs';
...
export class AppezzamentoEditComponent implements OnInit, OnDestroy {
  ...
  protected lista_Campi: Array<{ codice: number; descrizione: string }>;
  ...
  constructor(
    ...
    this.presaveForm.controls['supNuova'].valueChanges.pipe(takeUntil(this.signal$)).subscribe(v => {
      this.updateSuperficieGIS();
    });

    this.presaveForm.controls['associa'].valueChanges.pipe(takeUntil(this.signal$)).subscribe(v => {
      if (!this.createFrom)
        this.updateSuperficieGIS();
    });
  }
  ...
  ngOnInit() {
    ...
    ).subscribe(async newAppezza => {
      this.appezzamento = newAppezza;

      if (!!this.appezzamento) {
        this.updateFormFromAppezza(this.appezzamento);
        ...
        this.AppezzamentoEditForm.controls['metodo_Produzione'].valueChanges.pipe(
          takeUntil(this.signal$)
        ).subscribe((val: MetodoProduzione) => {
          if (val.codice == 3) {
            ...
            if (mostraDialog && !this.saving) {
              ...
              obsResp.pipe(take(1)).subscribe(val => {
                if ((<any>val).primary) {
                  for (let i = 0; i < (<FormArray>this.AppezzamentoEditForm.controls['impianti']).length; i++) {
                    let impianto: FormGroup = (<FormArray>this.AppezzamentoEditForm.controls['impianti']).at(i) as FormGroup;
                    for (let j = 0; j < (<FormArray>impianto.controls['esercizi']).length; j++) {
    ...
    if (this.objParametriAgenda.TipoOperazioneDB == Enum_DBTypeOperation.Write) {
      ...
      this.AppezzamentoEditForm.get('primaryKey').get('centroAziendalePK').get("codice").valueChanges.pipe(
        ...
        this.campiService.LeggiCampi({
          ...
          this.lista_Campi = val.map(el => {
            return { codice: el.primaryKey.codice, descrizione: el.descrizione };
          });
          this.lista_Campi.splice(0, 0, { codice: 0, descrizione: '' });
      ...
      this.AppezzamentoEditForm.controls['validita'].valueChanges.pipe(
        ...
        if (this.objParametriAgenda.TipoOperazioneDB == Enum_DBTypeOperation.Write &&
          (<FormArray>this.AppezzamentoEditForm.controls['impianti']).length == 1) {
    ...
    this.AppezzamentoEditForm.get('primaryKey').get('centroAziendalePK').valueChanges.pipe(takeUntil(this.signal$)).subscribe(val => {
  ...
  private handleQueryParams(): void {
    this.route.queryParams.pipe(takeUntil(this.signal$))
      .subscribe(params => {
        ...
        if (params.cfgSementi != undefined && SementieriParametrizzazione.isInstanceOfSementieri(JSON.parse(params.cfgSementi)))
          this.sharedDataService.setCfgSementi(new SementieriParametrizzazione(JSON.parse(params.cfgSementi)));
      });
  }
  ...
  private aggiornaValidita(formControl: string, val: any): void {
    if (this.objParametriAgendaService.getObjParamValue().TipoOperazioneDB == Enum_DBTypeOperation.Write) {
      let appezzamento: Appezzamento = <Appezzamento>this.AppezzamentoEditForm.getRawValue();

      if (appezzamento.impianti.length == 1 && appezzamento.impianti[0].esercizi.length == 1) {
        let formImpianto = <FormGroup>(<FormArray>this.AppezzamentoEditForm.controls['impianti']).controls['0'];

        if (!(<FormGroup>formImpianto.controls['validita']).controls[formControl].touched) {
          (<FormGroup>formImpianto.controls['validita']).controls[formControl].setValue(val);
          let formEsercizio = <FormGroup>(<FormArray>formImpianto.controls['esercizi']).controls['0'];

          if (!(<FormGroup>formEsercizio.controls['validita']).controls[formControl].touched) {
            (<FormGroup>formEsercizio.controls['validita']).controls[formControl].setValue(val);
          }
        }
      }
    }
  ...
  private updateFormFromAppezza(appezzamento: Appezzamento): void {
    // --------------------------------- dati per GIS ---------------------------------
    ...
    if (appezzamento.impianti as Impianto[]) {
      this.updateAppezzamentoImpiantiForm(appezzamento);
    }
    ...
    (<FormGroup>this.AppezzamentoEditForm.controls['validita']).controls['inizio'].valueChanges.pipe(
      takeUntil(this.signal$),
      tap((val) => {
        this.aggiornaValidita('inizio', val);
      })
    ).subscribe();

    (<FormGroup>this.AppezzamentoEditForm.controls['validita']).controls['fine'].valueChanges.pipe(
      takeUntil(this.signal$),
      tap((val) => {
        this.aggiornaValidita('fine', val);
      })
    ).subscribe();
  }
  ...
  private gestioneRedirect(appezzamento: Appezzamento) {
    if (this.inFrame) {
      ...
      let newUrl = '';
      segments.forEach(segment => { newUrl = newUrl.concat('/' + segment) })
  ...
  private async Salva() {
    ...
    if (this.AppezzamentoEditForm.valid) {
      ...
      if (!sovrapposizione) {
        return this.appezzamentiService.ScriviAppezzamento_Obs(appezzamento, true, false).toPromise();
      }
    } else {
  ...
  private updateAppezzamentoImpiantiForm(appezzamento: Appezzamento): void {
  ...
  private updateFormImpianto(appezzamento: Appezzamento, impianto: Impianto, idx: number): void {
