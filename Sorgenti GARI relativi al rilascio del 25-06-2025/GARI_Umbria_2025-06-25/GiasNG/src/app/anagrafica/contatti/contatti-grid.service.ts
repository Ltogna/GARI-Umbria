...
export class ContattiHttpService extends AbstractGridConfigService<ContattiServerResult>{
  ...
  GridContattiColumns: KendoGridColumn[] = [
    ...
    new KendoGridColumn(
      { field: 'Settore_Des', title: this.translocoService.translate('Settore') },
      { resizable: true, editable: false, width: 120 }
    ),
    new KendoGridColumn(
  ...
  handleCustomizations(): void {
      let ImpostazioneImpedisciEliminazione = this.permessiUtenteService.getImpostazione_Utente_poi_SuperUser(enum_Impostazioni_Utenti.SUPERUSER_IMPEDISCI_ELIMINAZIONE_CONTATTI_E_RISORSE_UMANE)
  ...
  read(): Observable<ContattiServerResult> {
    ...
    return this.contattiService.leggi(objAgenda).pipe(
      catchError((err) => {
      ...
      map((result => {
        ...
        this.GridContattiServerResult = new ContattiServerResult((<any>result).kendo_rows, this.GridContattiColumns, this.GridContattiModel);
