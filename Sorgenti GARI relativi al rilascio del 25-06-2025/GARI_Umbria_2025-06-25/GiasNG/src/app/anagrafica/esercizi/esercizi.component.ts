...
import { Subscription, Subject, takeUntil, tap, take, catchError, of, Observable, forkJoin, zip, switchMap, map } from 'rxjs';
...
import { AgriculturalPlot } from './special-edit/agricultural-plots/multi-edit-plots.model';
import { AgriculturalExerciceContributeLinkComponent } from './special-edit/exercices/agricultural-exercice-contribute-link/agricultural-exercice-contribute-link.component';
...
export class EserciziComponent implements OnInit, OnDestroy {
  @ViewChild('axpMultiLinkMsgContent') axpMultiLinkMsgContent: TemplateRef<any>;
  ...
  ngOnInit() {
    this.permessoEdit = this.permessiUtenteService.getPermesso(enum_Security_Attivita.Anagrafica_Impianto, 2);
  ...
  canEditSlopeOrWeaving(): boolean {
    return this.canEditWeaving() || this.canEditSlope();
  }
  ...
  specialEditBtnTitle(): string {
    let title: string = '';
    if (this.canEditWeaving() && this.canEditSlope()) {
      title = 'EditWeavingOrSlope';
    } else if (this.canEditSlope()) {
      title = 'EditSlope';
    } else if (this.canEditWeaving()) {
      title = 'EditWeaving';
    }
  ...
  onLinkExerciceToContribute(): void {
    ...
    const dialog: DialogRef = this.giasDialogService.dialogMessageRef(
      ...
      AgriculturalExerciceContributeLinkComponent,
      [],
      window.innerWidth * 0.9,
      window.innerHeight
    );
  ...
  private linkAgriculturalPlotsToMachine(plots: AgriculturalPlot[], chosenMachine: object, validity: IntervalloTemporale): void {
  ...
  private removePlotsMachinesLinks(plots: AgriculturalPlot[], chosenMachine: object): void {
  ...
  private preventConfirmPlotsMachinesLink(ev: DialogBooleanResult | DialogAction, dialog?: DialogRef) {
    if (ev['returnObj']?.confirm && !ev['returnObj']?.unlink) {
      ...
      const plots: AgriculturalPlot[] = (dialog.content.instance as AgriculturalPlotMachineLinkComponent).chosenAgriculturalPlots;
  ...
  private checkPlotsMultiAssociation(plots: AgriculturalPlot[], classCode: string = ''): Observable<boolean> {
    ...
    return zip(agriculturalPlots$, actualLinks$).pipe(
      switchMap(([pp, ll]) => {
        ...
        if (ll.some(l => l.length > 0)) {
          ...
          resp = dialog.result.pipe(map<DialogResult, boolean>(resp => resp['returnObj']));
        } else {
          resp = of(true);
        }
