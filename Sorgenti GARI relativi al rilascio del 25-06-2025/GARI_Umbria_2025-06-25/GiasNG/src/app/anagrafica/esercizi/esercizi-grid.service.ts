...
import {IndirizzoAssociato} from 'app/Model/anagrafiche/IndirizzoAssociato';
...
export class EserciziHttpService extends AbstractGridConfigService<EserciziServerResult> {
  ...
  private eserciziModel: KendoEserciziModel = {
    ...
    Campo_Cod: new ModelEntry(CELL_TYPES.DROPDOWNLIST, true),
  ...
  constructor(
    ...
    this.copiaSpostaService.getRicaricaImpianti().pipe(map(r => {
      if (r) {
  ...
  private handleCustomizations(): void {
    ...
    this.gridPublicService.openCommands.GiasSubscribe(dataItem => {
      if (!dataItem) {
        return;
      }
      if (dataItem['Bloccato']) {
  ...
  private handleDropdowns(): void {
    let col: KendoGridColumn = undefined;
    let data: any[] = [];

    this.handleDdlConfig('SA_COD', 'descrizione', 'codice', 'sa_nome', this.caricaCentri.bind(this));
    this.handleDdlConfig('veg_cod', 'descrizione', 'codice', 'veg_des', this.caricaSpecieVegetali.bind(this));
    this.handleDdlConfig('cul_cod', 'descrizione', 'codice', 'cul_des', this.caricaCultivar.bind(this));
    this.handleDdlConfig('Destinazione_Uso_Cod', 'descrizione', 'codice', 'Destinazione_Uso_Des', this.caricaDestinazioniUso.bind(this));
    this.handleDdlConfig('GRFI_COD', 'descrizione', 'codice', 'Grfi_Des', this.caricaFinalita.bind(this));
    this.handleDdlConfig('Campo_Cod', 'descrizione', 'codice', 'Campo_Des', this.caricaCampi.bind(this));
    this.handleDdlConfig('cop_cod', 'descrizione', 'codice', 'Cop_Des', this.caricaCopertura.bind(this));
    this.handleDdlConfig('port_cod', 'descrizione', 'codice', 'port_des', this.caricaPortinnesto.bind(this));
    this.handleDdlConfig('foral_cod', 'descrizione', 'codice', 'foral_des', this.caricaFormaAllevamento.bind(this));
    this.handleDdlConfig('GRVA_Cod_VEG', 'descrizione', 'codice', 'grva_des', this.caricaGruppoVarietale.bind(this));
    this.handleDdlConfig('RegolamentoDisciplinare_Cod', 'descrizione', 'codice', 'RegolamentoDisciplinare_Des', this.caricaVincoli.bind(this));
    this.handleDdlConfig('Tipologia_Cod', 'descrizione', 'codice', 'Tipologia_Des', this.caricaTipologie.bind(this));
    this.handleDdlConfig('Stato_Impianto', 'descrizione', 'codice', 'fase_des', this.caricaStatoImpianto.bind(this));
    this.handleDdlConfig('Metodo_Produzione_Cod', 'descrizione', 'codice', 'Metodo_Produzione_Des', this.caricaMetodoProduzione.bind(this));
    this.handleDdlConfig('organismo_Referente', 'descrizione', 'codice', 'organismo_Referente_des', this.caricaOrganismoReferente.bind(this));
    this.handleDdlConfig('Mat_Cod', 'descrizione', 'codice', 'Mat_Des', this.caricaTrasformati.bind(this));
    this.handleDdlConfig('Udm_Cod_Alt', 'descrizione', 'codice', 'Udm_Des_Alt', this.caricaUnitaDiMisuraAlt.bind(this));
    this.handleDdlConfig('GruppoRaccolta_Cod', 'descrizione', 'codice', 'GruppoRaccolta_Des', this.caricaGruppiRaccolta.bind(this));
    this.handleDdlConfig('dettSpeciePersonalizzatoCod', 'descrizione', 'codice', 'dettSpeciePersonalizzatoDes', this.loadDettaglioSpeciePersonalizzato.bind(this));
  ...
  private handleDdlConfig(
    ...
    let col: KendoGridColumn = this.eserciziColumns.find(s => s.field === formControlName);
    ...
    if (dataItem.SA_COD != undefined) {
      ...
      return from(this.campiService.LeggiCampi({centro: centro, data: dataItem.Validita_Inizio}, false, '').then(vals => {
        let values = vals.map(el => {
          return {codice: el.primaryKey.codice, descrizione: el.descrizione};
        });
  ...
  private performAppezzamento(actionType: HttpAction, item: any): Observable<any> {
    ...
    switch (actionType) {
      case HttpAction.CREATE:
        return this.createAppezza(item);
  ...
  private prepareAppezzamento(item: any): Appezzamento {
    ...
    newAppezza.impianti = new Array<Impianto>();
    ...
    newAppezza.impianti[0].esercizi = new Array<Esercizio>();
    ...
    newAppezza.validita.inizio = item.Validita_Inizio_Impianto;
    newAppezza.validita.fine = validitySettingInterpretation?.Valore === '0' ? item.Validita_Fine : AGRODATAFINE;
    ...
    impianto.validita.inizio = item.Validita_Inizio_Impianto;
    impianto.validita.fine =
      (validitySettingInterpretation?.Valore === '0' || validitySettingInterpretation?.Valore === '1') ? item.Validita_Fine : AGRODATAFINE;
    ...
    impianto.data_Inizio_Portinnesto = item.Data_Inizio_Portinnesto;
    impianto.data_Inizio_Impianto = item.Data_Inizio_Impianto;
    impianto.data_Inizio_Produzione = item.Data_Inizio_Produzione;
    ...
    esercizio.validita.inizio = item.Validita_Inizio;
    esercizio.validita.fine = item.Validita_Fine;
    esercizio.lotto = item.Progetto_Nome;
    esercizio.descrizione = item.Progetto_Des;
    esercizio.resa_prevista = item.Resa;
    esercizio.data_Semina_Trapianto_Prevista = item.DataTrapiantoSemina;
    esercizio.data_Fioritura_Prevista = item.DataFioritura;
    esercizio.data_Raccolta_Prevista = item.DataRaccolta;
  ...
  private createAppezza(item: any): Observable<any> {
    let newAppezza: Appezzamento = this.prepareAppezzamento(item);
    ...
    return this.caricaVincoli(item).pipe(
      ...
      switchMap((newAppezza) => {
        return this.appezzamentiService.ScriviAppezzamento(newAppezza);
      }),
      tap((resp) => {
  ...
  private updateAppezza(DBAppezza: Appezzamento, item: any, fb: FormGroup): Observable<any> {
    ...
    if (DBimpianto) {
      ...
      DBimpianto.data_Inizio_Portinnesto = item.Data_Inizio_Portinnesto;

      DBimpianto.data_Inizio_Impianto = item.Data_Inizio_Impianto;

      DBimpianto.data_Inizio_Produzione = item.Data_Inizio_Produzione;
      ...
      if (DBEsercizio) {
        ...
        DBEsercizio.data_Semina_Trapianto_Prevista = item.DataTrapiantoSemina;
        DBEsercizio.data_Fioritura_Prevista = item.DataFioritura;
        DBEsercizio.data_Raccolta_Prevista = item.DataRaccolta;
  ...
  private handleImpostazioniSuperUser(): void {
    if (this.permessiUtenteService.getImpostazione_SuperUser(enum_Impostazioni_Utenti.SuperUser_KPIN_BlockName)?.Valore == '1') {
  ...
  private onGridEditAction(event: any) {
    ...
    fb.controls['Campo_Cod'].disable({emitEvent: false});
