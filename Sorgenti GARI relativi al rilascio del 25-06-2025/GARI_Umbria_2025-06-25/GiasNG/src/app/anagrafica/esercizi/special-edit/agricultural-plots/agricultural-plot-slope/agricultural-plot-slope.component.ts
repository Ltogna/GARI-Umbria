...
import {AgriculturalPlotService} from '../agricultural-plot.service';
...
@Component({
  ...
  providers: [AgriculturalPlotService]
})
export class AgriculturalPlotSlopeComponent implements OnInit, OnDestroy{
  @Input() private windowHeight$: ReplaySubject<number> = new ReplaySubject<number>();
  @Input() private checkedRows$: ReplaySubject<KendoGridRow[]> = new ReplaySubject<KendoGridRow[]>();

  protected listViewHeight$: ReplaySubject<number> = new ReplaySubject<number>();
  protected rows$: BehaviorSubject<AgriculturalPlot[]> = new BehaviorSubject<AgriculturalPlot[]>([]);
  ...
  protected loading$ = this.agriculturalPlot.loading$;
  ...
  constructor(
    ...
    private agriculturalPlot: AgriculturalPlotService
  ) {  }
  ...
  ngOnInit(): void {
    ...
    this.checkedRows$.pipe(takeUntilDestroyed(this._destroyRef)).subscribe(crs => {
      this.rows$.next([]);
      this.agriculturalPlot.checkedRows = crs;
      this.agriculturalPlot.setListViewRows();
    });

    this.agriculturalPlot.listViewRows$.pipe(takeUntilDestroyed(this._destroyRef)).subscribe(rr => {
      this.rows$.next([...rr]);
    });
  }
