...
export class SpecialEditComponent implements OnInit{
  @Input() protected windowHeight$: ReplaySubject<number> = new ReplaySubject<number>();
  @Input() protected checkedRows$: ReplaySubject<KendoGridRow[]> = new ReplaySubject<KendoGridRow[]>();
  ...
  constructor(
    ...
    this.selectedPage = this.tabs.find(t => this.isUserAllowed(t.id, 2));
  }
