...
export class MenuProfilazioneGridConfigService extends AbstractGridConfigService<GridProfilazioneServerResult> {
  ...
  private kendoColumns: KendoGridColumn[] = [
    ...
    new KendoGridColumn({
      field: 'Flag_Azienda_Persona',
      title: this.transloco.translate('AziendaPersona')
    }, {editable: true, width: 153, hidden: true}),
    ...
    new KendoGridColumn({field: 'NumeroAccessi', title: this.transloco.translate('NumeroAccessi')}, {
      hidden: true,
      editable: false
    }),
    new KendoGridColumn({
      field: 'UserNameCommerciale',
      title: this.transloco.translate('UserNameCommerciale')
    }, {editable: true, width: 205}), // Ex 'Qualifica'
    new KendoGridColumn({field: 'CodFisc', title: this.transloco.translate('CodiceFiscale')}, {
      editable: true,
      width: 140
    }),
    new KendoGridColumn({field: 'GruppiCod', title: this.transloco.translate('Gruppo')}, {editable: true, width: 140}),
    new KendoGridColumn({field: 'Tipologia_Cod', title: this.transloco.translate('Profilo')}, {
      editable: true,
      width: 140
    }),
    new KendoGridColumn({field: 'Attivo', title: this.transloco.translate('Attivo')}, {editable: false, hidden: true}),
    new KendoGridColumn({field: 'Validita_Inizio', title: this.transloco.translate('DataInizio')}, {
      editable: true,
      hidden: true,
      width: 150,
      date: {placeholder: '...'}
    }),
    new KendoGridColumn({field: 'Validita_Fine', title: this.transloco.translate('DataFine')}, {
      editable: true,
      hidden: true,
      width: 150,
      date: {placeholder: '...'}
    ...
    new KendoGridColumn({
      field: 'FinestraTemporaleInizio',
      title: this.transloco.translate('FinestraTempInizio')
    }, {editable: false, hidden: true, width: 150, date: {placeholder: '...'}}),
    new KendoGridColumn({
      field: 'FinestraTemporaleFine',
      title: this.transloco.translate('FinestraTempFine')
    }, {editable: false, hidden: true, width: 150, date: {placeholder: '...'}}),
    new KendoGridColumn({field: 'Lingua_Cod', title: this.transloco.translate('Lingua')}, {
      editable: true,
      hidden: true,
      width: 110
    }),
    ...
    new KendoGridColumn({field: 'Password', title: this.transloco.translate('Password')}, {
      editable: true,
      hidden: false,
      filterable: false,
      width: 250,
      component: GridPasswordComponent
    }),
    new KendoGridColumn({field: 'Username_Creazione', title: this.transloco.translate('CreatoDa')}, {
      editable: false,
      hidden: false
    }),
    new KendoGridColumn({
      field: 'Username_Modifica',
      title: this.transloco.translate('UltimaModificaDi')
    }, {editable: false, hidden: false}),
    new KendoGridColumn({field: 'Data_Creazione', title: this.transloco.translate('DataCreazione')}, {
      editable: false,
      hidden: false
    }),
    new KendoGridColumn({field: 'Data_Modifica', title: this.transloco.translate('DataModifica')}, {
      editable: false,
      hidden: false
    })
  ];
  ...
  /** Removes the button in the superuser's row */
  private hideRemoveBtn(renderer: any, domElems: NodeList, elems: UtenteFlatModel[]) {
    elems.forEach((dataItem: any, index: number) => {
      let currDomRow = domElems[index];
      if (this.isSuperuser(dataItem)) {
        // display e delete sono proprietà dell'elemento nel dom
        UtilityFunctions.setStyle(renderer, currDomRow, '#btnDelete', 'display', 'none');
      } else {
        UtilityFunctions.setStyle(renderer, currDomRow, '#btnDelete', 'display', 'table-row');
      }
    });
  }
  ...
  private manageColumns() {
    ...
    if (this.kendoRows.some(r => r.Flag_Azienda_Persona === enum_Azienda_Persona.Impresa)) {
      ...
      this.kendoColumns.find(s => s.field === 'CodFisc').title = this.transloco.translate('CodiceFiscale') + ' / ' + this.transloco.translate('PIVA');
    }
  }
  ...
  private scrollToInvalidField(formGroup: FormGroup) {
    ...
    const invalidField = getFirstInvalid();
  ...
  private preventEditFunction(dataItem, col: ColumnComponent) {
    ...
    if (isEditingSelf) {
      ...
      return !(['Email', 'Tel', 'Password', 'Nome', 'Cognome'].includes(col.field));
    }
    ...
    return !isEditingSelf && !this.datashare.userPermissions.canEditAllUsers;
  }
  ...
  private manageColsForBusiness(formgroup: UtenteFlatModel) {
    if (formgroup.Flag_Azienda_Persona === enum_Azienda_Persona.Impresa) {
      ...
      this.kendoColumns.find(c => c.field === 'Nome').title = "Nome";
    }
  }
  ...
  private configureProfilesDDL() {
    ...
    return this.tipologieService.readTipologie().pipe(map((res: TipologiaUtente[]) => {
