...
export class GridVisibilitaEditService extends AbstractGridConfigService<GridProfilazioneServerResult> {
  ...
  private kendoColumns: KendoGridColumn[] = [
    ...
    new KendoGridColumn({ field:'piva', title:this.transloco.translate('PartitaIVA')}, {editable: false}),
  ...
  perform(actionType: HttpAction, items: any): Observable<any> {
    if (actionType === HttpAction.REMOVE) {
      ...
      let haFigli = this.kendoRows.map(r => r.Piva_Padre).some(p => p === items.piva);
      let haFratelli = false;
      let padrePresente = false;
      if (pivaPadre) {
        padrePresente = this.kendoRows.map(r => r.piva).some(p => p.id === pivaPadre);
        haFratelli = this.kendoRows.map(r => r.Piva_Padre).filter(p => p.id === pivaPadre).length > 1;
      }
      if (haFratelli) {
        this.dialog.baseError('_Errore', 'prof.ImpossibileElinareAziendaConFratelliDaVisibilita');
      } else if (padrePresente) {
        ...
        this.dialog.warningThen(
          'prof.EliminazioneAziendaVisibilita',msg,
          true, () => this.removeFormVisibilita(items)
        );
      }
    }
    return null;
  ...
  private removeFormVisibilita(azienda: any) {
    ...
    this.visibilitaService.rimuoviImpreseDaVisibilita(this.visibilitaService.utentiSelezionati, [azienda])
      .pipe(take(1), switchMap(() => this.visibilitaService.leggiVisibilitaUtente(username, false)))
      .subscribe((imprese) => {
        ...
        this.impreseLoaded$.next(this.impreseLoaded$.value);
