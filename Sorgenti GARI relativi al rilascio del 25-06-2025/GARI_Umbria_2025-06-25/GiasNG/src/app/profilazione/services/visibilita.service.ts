...
export class VisibilitaService {
  ...
  public copiaVisibilita(base: string[], template: string) {
    return this.APIService.ajaxAPIPost<any, any>(
      'Profilazione/CopiaVisibilitaUtenti', new CopiaImpostazioniObj(base, template)
    ).pipe(map(R => {
      if (R.RispostaOK) {
   ...
   */
  public leggiVisibilitaUtente(username: string, leggiDatiImprese: boolean): Observable<IUtenteImpresa[]> {
