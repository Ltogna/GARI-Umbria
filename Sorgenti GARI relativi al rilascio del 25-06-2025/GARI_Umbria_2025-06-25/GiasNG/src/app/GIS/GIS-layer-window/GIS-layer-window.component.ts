...
import { DatiLayer, ObjOptionHTML_Out, TipologiaLayer } from 'app/Service/api.service';
import { combineLatest, debounceTime, filter, map, merge, Subject, take, takeUntil, tap } from 'rxjs';
    ...
    constructor(
        ...
        merge(
            ...
            .subscribe(layers => {
                ...
                layers.forEach(layer => {
                    ...
                    this.featuresNumberByLayer.set(layer.id, features.length);
                });
            });
        ...
        .subscribe(bounds => {
            ...
            this.pagedLayersData.forEach(layer => {
                const features = this.featureInformationService.getByLayer(layer.id);
                const positions = features.map(x => this.featureInformationService.getPosition(x.properties.id));
