...
export class MultiAziendaService {
  ...
  private setGeoJsonFilterServiceDistribuzione(
    ...
    let readParams = new GisDataReadParam();
    ...
    const filtroTemporaleAvanzato =
      this.sharedDataService.getFiltroTemporaleAvanzato();
    readParams.filtroTemporale = filtroTemporaleAvanzato.FiltroPeriodo;
    readParams.filtroTemporaleSingolaData =
      filtroTemporaleAvanzato.FiltroSingolaData;
  ...
  private setGeoJsonFilterServicePoligoni(wkt: string, year: string): void {
    ...
    readParams.filtroTemporale = new FiltroTemporale();
    readParams.filtroTemporale.setValiditaData(
      new Date(year + "-01-01T00:00:00"),
      new Date(year + "-12-31T00:00:00")
    );
    readParams.filtroTemporaleSingolaData = new FiltroTemporale();
    readParams.filtroTemporaleSingolaData.setIntervalloTemporaleSingolaData(
      new Date()
    );
    readParams.layerElementiGrafici_cod.push(
      parseInt(enum_LayerElementiGraficiStd.IMPIANTI)
    );
    this.geoJsonFilterService.setGisDataReadParam(readParams);
