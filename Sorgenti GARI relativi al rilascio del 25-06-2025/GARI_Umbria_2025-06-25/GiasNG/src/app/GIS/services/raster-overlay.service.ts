...
import { AuthDispatcherClient, MascheraLayerRaster, RichiestaAbacoType, RichiestaAbacoUrl_In, RichiestaSignedUrl_In, UrlFirmato } from "app/Service/api.service";
...
export class RasterOverlayService {
  ...
  public load(rasterId: number, masks: MascheraLayerRaster[], layerParams: RasterParameterVisualizationLayer[], features: GeoJson_Feature_New<GeoJSONAgroGisProp>[], hasPermissions: boolean, opacity: number = 1): void {
  ...
  private handleVisualizationParameter(layerParam: RasterParameterVisualizationLayer, tiles: Tile[]): Observable<UrlFirmato[]> {
    ...
    return this.authDispatcherClient.authDispatcherRichiediAbacoURL(request)
      .pipe(map(res => {
  ...
  private getImageMapType(rasterId: number, opacity: number, urls: UrlFirmato[]): google.maps.ImageMapType {
    const mapType = new google.maps.ImageMapType({
      ...
      getTileUrl: function (tile, zoom) {
        ...
        return Array.isArray(urls) ? urls.find(url => url.Key == `${zoom}/${tile.x}/${tile.y}`)?.SignedUrl : getLegacyUrl(zoom, tile.x, tile.y);
      },
  ...
  public static clearRasterOverlays(rasterId: number, map: google.maps.Map): void {
