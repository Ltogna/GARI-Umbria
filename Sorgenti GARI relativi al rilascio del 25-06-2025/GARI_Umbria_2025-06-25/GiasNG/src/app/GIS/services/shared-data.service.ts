...
import { TipologiaLayer, CfgAlbero_CfgGisUtente, ConfigurazioniGisGenerali } from 'app/Service/api.service';
import {  FiltroTemporaleAvanzato } from 'app/Model/GIS/FiltroTemporale';
...
export class SharedDataService{
  ...
  private treeViewCheckedKeys: any[] = [];

  private treeViewCheckedKeysSource = new BehaviorSubject(this.treeViewCheckedKeys);

  private lastCheckedKeyFeatureId: any = null;

  private lastCheckedKeyFeatureIdSource = new BehaviorSubject(this.lastCheckedKeyFeatureId);

  private ricaricaAlbero: boolean = false;

  private ricaricaAlberoSource = new BehaviorSubject(this.ricaricaAlbero);

  private visualizzazioneTotale: boolean = false;

  private visualizzazioneTotaleSource = new BehaviorSubject(this.visualizzazioneTotale);

  private resetVisualizzazioneTotale: boolean = false;

  private resetVisualizzazioneTotaleSource = new BehaviorSubject(this.resetVisualizzazioneTotale);
  ...
  public getTrasparenzaLayer(idLayer: string): number {
    ...
    if (datiLayer != null) {
      ...
      // console.log(`[getTrasparenzaLayer] Dati layer ${idLayer} non trovati`);
    }

    let trasparenzaNumero = parseFloat(trasparenzaStringa.replace(',','.'));

    return trasparenzaNumero;
  ...
  public getMostraDescrizioneAssociata(idLayer: string): boolean {
    ...
    if (datiLayer != null) {
      mostraDescrizioneAssociata = (datiLayer.MostraDescrizioneAssociata === "1") ? true : false;
    } else {
      // console.log(`[getMostraDescrizioneAssociata] Dati layer ${idLayer} non trovati`);
    }
  ...
  public getRaggruppaDescrizioneAssociata(idLayer: string): boolean {
    ...
    if (datiLayer != null) {
      ...
      // console.log(`[getRaggruppaDescrizioneAssociata] Dati layer ${idLayer} non trovati`);
    }
  ...
  public getFlagVisibile(idLayer: string): boolean {
    ...
    if (datiLayer != null) {
      ...
      // console.log(`[getFlagVisibile] Dati layer ${idLayer} non trovati`);
    }
  ...
  public getFlagVisibileBoolean(flagVisibile: string): boolean {
    return flagVisibile === "1" ? true : false;
  }
  ...
  public getPermessiLayer(layer: TipologiaLayer): PermessiLayer {
    let permessiLayer = new PermessiLayer;
    // Inizializzazione
    permessiLayer.inserimento = false;
    permessiLayer.modifica = false;
    permessiLayer.cancellazione = false;
    permessiLayer.informazioni = false;
    if (layer) {
      // Inserimento
      if (layer.FlagInserimento === '1') {
        permessiLayer.inserimento = true;
      }
      // Modifica
      if (layer.FlagModifica === '1') {
        permessiLayer.modifica = true;
      }
      // Cancellazione
      if (layer.FlagCancellazione === '1') {
        permessiLayer.cancellazione = true;
      }
      // Informazioni
      if (layer.FlagInformazioni === '1') {
        permessiLayer.informazioni = true;
      }
    }
  ...
  public getResetVisualizzazioneTotale(): boolean {
    return this.resetVisualizzazioneTotaleSource.getValue();
  }
  ...
  public isFeatureAvversita(feature: GeoJson_Feature_New<GeoJSONAgroGisProp>): boolean {
  ...
  public getFeatureScaleByLayerAndTheme(feature: GeoJson_Feature_New<GeoJSONAgroGisProp>, layer: TipologiaLayer, themeName: string): number {
    const nomeTema = this.getNomeTemaTipoLayer(layer.nome, themeName);
    let scale = this.getValoreScalaFeature(feature, nomeTema);
    return scale;
  }

  public getColoreTemaFeature(layerSelezionatoNome: string, temaSelezionatoNome: string, feature: GeoJson_Feature_New<GeoJSONAgroGisProp>, scalaColori?: DatiColoreTema[]): string {
  ...
  public getLimitiTematizzazioneLayer(features: GeoJson_Feature_New<GeoJSONAgroGisProp>[], layerSelezionato: TipologiaLayer, temaSelezionatoNome: string): LimitiValoriTema {
    ...
    features.forEach(feature => {
      ...
      if (layerFeature === layerSelezionato.id) {
        const nomeTema = this.getNomeTemaTipoLayer(layerSelezionato.nome, temaSelezionatoNome);
  ...
  public getValoreScalaFeature(feature: GeoJson_Feature_New<GeoJSONAgroGisProp>, nomeTema: string): number {
    ...
    stringaDatiTematizzazioni.split('|').forEach(stringaNomeValore => {
      ...
      // Elimino il primo carattere se accento circonflesso
      nome = nome.replace(/^\^+/g, '');
  ...
  public getColoreScalaFeature(feature: GeoJson_Feature_New<GeoJSONAgroGisProp>, layerSelezionatoNome: string, temaSelezionatoNome: string, scalaColori: DatiColoreTema[]): string {
