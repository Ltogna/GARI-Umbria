...
import {FeatureService, PermessiFeature} from 'app/GIS/services/feature.service';
import { enum_LayerElementiGraficiStd } from '../../GIS-enum/GIS-layer-elementi-grafici';
import { enum_FeatureGeometryType, enum_FeatureProperty, GISModality } from 'app/GIS/GIS-enum/GIS-feature';
import { SharedDataService } from 'app/GIS/services/shared-data.service';
import {combineLatest, debounceTime, filter, map, merge, of, startWith, Subject, takeUntil, withLatestFrom} from 'rxjs';
...
import { Plot } from 'app/GIS/GIS-attributi/GIS-attributi-muz-grid/GIS-attributi-muz-grid.component';
import { DEFAULT_PLOT, GISAttributiMuzService } from 'app/GIS/GIS-attributi/GIS-attributi-muz-grid/GIS-attributi-muz.service';
...
export class DrawWindowComponent implements OnInit, OnDestroy {
  ...
  private abilitaDisabilitaPulsantiDisegno(
    ...
    switch (true) {
      ...
      case featureSelezionate.length > 0:
        ...
        aggiornaVisibilitaBottoni.strumentoDisegnoAvanzato = aggiornaVisibilitaBottoni.strumentiDisegnoPunti;
        aggiornaVisibilitaBottoni.copiaFeature = bTipologiaLayerStandard;
        aggiornaVisibilitaBottoni.incollaFeature = bTipologiaLayerStandard;

        aggiornaVisibilitaBottoni.scomponiModificaPunti = this.isScomponiAvailable(featureSelezionate);
        aggiornaVisibilitaBottoni.lineeGuidaAB = bPermessiPrecision && layerAppartenenza === enum_LayerElementiGraficiStd.IMPIANTI;
        aggiornaVisibilitaBottoni.datiAgricolturaPrecisone = bPermessiPrecision && layerAppartenenza === enum_LayerElementiGraficiStd.IMPIANTI && ricetteSelected;
        aggiornaVisibilitaBottoni.strumentoUnisci = isLayerMergeable(layerAppartenenza);

        // Modifica
        if (permessiFeature.modifica) {
          aggiornaVisibilitaBottoni.modificaImpianto = true;
          aggiornaVisibilitaBottoni.nuovoImpiantoAppezzamentoSelezionato = (layerAppartenenza === enum_LayerElementiGraficiStd.APPEZZAMENTI);
        } else {
          aggiornaVisibilitaBottoni.modificaImpianto = false;
          aggiornaVisibilitaBottoni.nuovoImpiantoAppezzamentoSelezionato = false;
        }
        // Cancellazione
        aggiornaVisibilitaBottoni.eliminaPoligono = permessiFeature.cancellazione && featureSelezionate.length === 1;
        // Informazioni
      ...
      default:
        ...
        aggiornaVisibilitaBottoni.strumentoUnisci = isThereALayerSelected && layerSelezionato != null && isLayerMergeable(layerSelezionato.id);
        ...
        if ( (permessiLayer.inserimento || this.isLayerMuz(layerSelezionato)) && isThereALayerSelected) {
          aggiornaVisibilitaBottoni.disegnaPoligono = true;
          ...
          const visibileSuLayerNonCampioni: boolean = !isThereALayerSelected && isLayerImpiantiVisible && bPermessiAnagrafica;
    ...
    if (this.featureService.getFeatureDaCopiare() != undefined && tipologiaLayerSelezionato === enum_TipologiaLayer.Entita) {
      aggiornaVisibilitaBottoni.copiaFeature = true;
      aggiornaVisibilitaBottoni.incollaFeature = true;
    }
  ...
  private isScomponiAvailable(featureSelezionate: GeoJson_Feature_New<GeoJSONAgroGisProp>[]): boolean {
  ...
  private handleSementiCase(aggiornaVisibilitaBottoni: VisualizzaBottone, featureSelezionate: GeoJson_Feature_New<GeoJSONAgroGisProp>[]) {
    let cfgS: SementieriParametrizzazione = this.sharedDataService.getCfgSementiAsValue();
    // TODO Salvo: suppongo l'indice della stringa sia 4, vedi mail Vanni del 24-04-2023
    let auth: boolean = cfgS.DatiPassaggio.split('|')[4] == '2' ||
      cfgS.SementiMappaturaLibera == 'True';

    let deleteAuth: boolean = false;
    let modifyAuth: boolean = false;
    let permessiFeature: PermessiFeature = new PermessiFeature();

    if (featureSelezionate.length === 1) {
      permessiFeature = this.featureService.getPermessiFeature(featureSelezionate[0]);
      deleteAuth = permessiFeature.cancellazione;
      modifyAuth = permessiFeature.modifica;
    }
    ...
    aggiornaVisibilitaBottoni.disegnaPoligono = auth;
    aggiornaVisibilitaBottoni.strumentiDisegnoPunti = auth;
    aggiornaVisibilitaBottoni.scomponiModificaPunti = auth;
    aggiornaVisibilitaBottoni.modificaImpianto = (auth && modifyAuth);
    aggiornaVisibilitaBottoni.nuovoImpiantoAppezzamentoSelezionato = auth;
    aggiornaVisibilitaBottoni.copiaFeature = auth;
    aggiornaVisibilitaBottoni.incollaFeature = auth;
    aggiornaVisibilitaBottoni.eliminaPoligono = (auth && deleteAuth);
    aggiornaVisibilitaBottoni.lineeGuidaAB = auth;
    aggiornaVisibilitaBottoni.datiAgricolturaPrecisone = auth;
  }
