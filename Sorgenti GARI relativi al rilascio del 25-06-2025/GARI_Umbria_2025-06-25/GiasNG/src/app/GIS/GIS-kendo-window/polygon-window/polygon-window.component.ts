...
import { GeoJSONAgroGisProp, GeoJson_Geometry_New, FeatureType, GeoJson_New, GeoJSONAgroGisPropTreeNode } from 'app/Model/GIS/GisDataReadRval_New';
...
export class PolygonWindowComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.sharedDataService.datiFeatureConAttributiSource
      ...
      .subscribe(datiFeature => {
        if (datiFeature.DatiCompleti) {
          if (datiFeature.TipoOperazione === enum_TipoOperazioneDB.Scrittura) {
            this.googleMapGeoJsonService.inserisciFeatureConAttributi(datiFeature);
          } else {
            this.googleMapGeoJsonService.modificaFeatureConAttributi(datiFeature);
          }
        }
      });
  ...
  private seCaricaFeatureDaGeoJson(geoJson: GeoJson_New<GeoJSONAgroGisProp>) {
    //--------------------------------------------------------------------------------
  ...
  private sistemazioneGeoJson(geoJson: GeoJson_New<GeoJSONAgroGisProp>) {
  ...
  private saveEntityGlobal(polygon: GiasPolygon, ca?: ChiaveAlbero) {
    ...
    this.polyWindowForm.value.polygonWKT = this.wktService.geoJsonToWKT(this.geoJsonService.polygonToGeoJson(polygon.polygon));
  ...
  private saveAssociazioneEstesa(objPolygon: GiasPolygon, chiaveAlbero: ChiaveAlbero, layer: string) {
    let polygonWKT: string = this.wktService.geoJsonToWKT(this.geoJsonService.polygonToGeoJson(objPolygon.polygon));
