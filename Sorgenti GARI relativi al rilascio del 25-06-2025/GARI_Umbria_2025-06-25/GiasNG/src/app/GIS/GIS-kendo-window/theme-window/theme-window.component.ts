...
import { SalvaColoriLayer2_In, DatiTema, TipologiaLayer, TipologiaTile, GisClient, TipologiaLabel, LayerTilesDescrizione_In, Enum_Tipo_Operazione, ObjOptionHTML_Out, LeggiLayerTilesDescrizione_In } from "app/Service/api.service";
...
export class ThemeWindowComponent implements OnDestroy {
  ...
  tiles$: Observable<TipologiaTile[]>;
  ...
  constructor(
    ...
    this.tiles$ = this.loadTiles.asObservable()
      .pipe(
        ...
        map(([_, layer]) => this.getValidTiles(layer))
      );
  ...
  onChangeTheme(tileSelezionato: TipologiaTile, layer: TipologiaLayer): void {
  ...
  getValidTiles(layer: TipologiaLayer): TipologiaTile[] {
    const tiles = [];
    if (layer == null) {
      return [];
    }
