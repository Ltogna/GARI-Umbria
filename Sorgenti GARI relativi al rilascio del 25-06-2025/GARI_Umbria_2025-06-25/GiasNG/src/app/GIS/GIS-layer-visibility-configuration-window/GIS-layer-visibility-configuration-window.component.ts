...
import { AggiornaElencoTipologie_In, FlagVisibilitaLayer, GisClient, ObjOptionHTML_Out, SalvaFlagVisibilitaLayer_In, SalvaVisibilitaLayer_In, ScriviNuovoLayerPersonalizzato_In, TipologiaLayer, VisibilitaLayer } from 'app/Service/api.service';
import { GiasMessageService } from 'app/Service/gias-message.service';
import { Subject, tap, debounceTime, takeUntil } from 'rxjs';
...
export class GISLayerVisibilityConfigurationWindowComponent implements OnDestroy {
  ...
  newLayerToggle(): void {
    ...
    if (this.saveVisibile && this.newLayerName !== '') {
      this.gisClient.gisScriviNuovoLayerPersonalizzato(
        ...
        } as ScriviNuovoLayerPersonalizzato_In
      ).subscribe({
        next: _ => {
  ...
  private submit(): void {
    const payload: SalvaFlagVisibilitaLayer_In = {
      ...
      DatiVisibilitaLayer: this.layersData.map(x => <FlagVisibilitaLayer>{ ID: x.id, Flag_Visibile: +x.flagvisibile, Flag_Attivo: +x.flagattivo }),
    };
  ...
  private getActiveLayers(layerSelezionato: string): void {
    ...
    this.gisClient
      .gisAggiornaElencoTipologie4(payload)
      .subscribe(result => {
