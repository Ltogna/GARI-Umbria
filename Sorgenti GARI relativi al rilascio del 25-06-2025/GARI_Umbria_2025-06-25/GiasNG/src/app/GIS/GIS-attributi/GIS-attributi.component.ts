...
import { FeatureType, GeoJson_Feature_New, GeoJSONAgroGisProp } from 'app/Model/GIS/GisDataReadRval_New';
import {
  ChiaveAlbero,
...
export class GISAttributiComponent implements OnChanges {
  ...
  private isAgendaVisible(features: GeoJson_Feature_New<GeoJSONAgroGisProp>[]): boolean {
    const isImpianto = this.isFeatureImpianto(features);
    return isImpianto;
  }

  private isDatiVisible(layer: TipologiaLayer): boolean {
    return layer?.id != enum_LayerElementiGraficiStd.CATASTO && layer?.id != enum_LayerElementiGraficiStd.IMPIANTI;
  }
  ...
  private populateTabs(layer: TipologiaLayer, configurations: ConfigurazioneSuLayer[], features: GeoJson_Feature_New<GeoJSONAgroGisProp>[], agendaVisible: boolean): Tabs {
    if (layer == null) {
      return;
    }
  ...
  private isFeatureImpianto(features: GeoJson_Feature_New<GeoJSONAgroGisProp>[]): boolean {
