...
import { FeatureType, GeoJSONAgroGisProp, GeoJson_Feature_New } from 'app/Model/GIS/GisDataReadRval_New';
import { KendoWindowsService, WindowTypes } from 'app/Service';
import { TipologiaLayer, GisClient } from 'app/Service/api.service';
...
export class GISAttributiConfigService extends AbstractGridConfigService<GISAttributiKendoServerResult>{
  ...
  private ultimaFeatureSelezionata: GeoJson_Feature_New<GeoJSONAgroGisProp>;
  ...
  read(options?: any): Observable<GISAttributiKendoServerResult> {
    ...
    if (this.layerTypeSelected != undefined) {
      this.loadingService.set_isLoading({ isLoading: true, message: '', component: this.gridPublicService.gridElRef });
      obs = [
        this.gisClient.gisLeggiElencoStrutturaAttributiLayer(this.layerTypeSelected)
      ];
    }
  ...
  private extractDescrizioneFromFeatures(): void {
    let features: GeoJson_Feature_New<GeoJSONAgroGisProp>[];
    if (
