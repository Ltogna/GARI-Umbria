...
import { GeoJsonService } from 'app/GIS/services/geojson.service';
...
import { FeatureType, GeoJson_Feature_New, GeoJson_Geometry_New, GeoJSONAgroGisProp, GisDataReadRval_New } from 'app/Model/GIS/GisDataReadRval_New';
...
export class GISAttributiEventsService {
  ...
  private addPolygon(items: any): Observable<any> {
    ...
    if (this.polygonValidator.isValid) {
      ...
      if (this.layerService.layerItemSelected[0].id == enum_LayerElementiGraficiStd.Mappe_Prescrizione) {
        return this.addItemToMappePrescrizione(items, d.polygon);
      }

      let geoJson = this.geoJsonService.polygonToGeoJson(d.polygon);
  ...
  private addItemToMappePrescrizione(items: any, polygon: google.maps.Polygon): Observable<RispostaStandard_1OfSalvaEntitaConAttributi_Out> {
    ...
    const newPolygonGeoJson = this.geoJsonService.polygonToGeoJson(polygon);
  ...
  private getMappePrescrizioneProperties(items: any, features: GeoJson_Feature_New<GeoJSONAgroGisProp>[]): GeoJSONAgroGisProp {
    const key1 = 'CAP_N+';
    const key2 = 'Prod#_secc';

    // La chiave può essere sia con '^' che senza
    const entry1 = items['^' + key1] ?? items[key1];
  ...
  private addMarker(items: any): Observable<any> {
    let d = this.drawingService.markers[0];

    let geoJson = this.geoJsonService.markerToGeoJson(d.marker);
    let newEntity = this.prepareEntity(items, this.wktService.geoJsonToWKT(geoJson), d.tipologiaLayer.id);
  ...
  private addPolyline(items: any) {
    ...
    let geoJson = this.geoJsonService.polylineToGeoJson(d.polyline);
    let newEntity = this.prepareEntity(items, this.wktService.geoJsonToWKT(geoJson), d.tipologiaLayer.id);
  ...
  private prepareEntity(items: any, cartografia: string, layer: string): SalvaEntitaConAttributi_In {
    ...
    if (this.isCampionamenti()) {
      ...
      let f = featureSelezionate[featureSelezionate.length - 1];
      ...
      if (f)
        ...
        let node = this.treeGisService.getTreeNodeFromCheckedKey(checkedKeys[0]);
  ...
  public cancelMarker(): void {
    let draw = this.drawingService.markers[0];
    this.drawingService.removeEvent.next(draw);
  }

  public cancelPolyline(): void {
    let draw = this.drawingService.polylines[0];
    this.drawingService.removeEvent.next(draw);
  }
  ...
  public selectFeature(chiave: string) {
  ...
  public readTecnici(p: ReadTecniciParams): Observable<rispostaStandard<GisDataReadRval_New<GeoJSONAgroGisProp>>> {
    if (this.isTecnici()) {
