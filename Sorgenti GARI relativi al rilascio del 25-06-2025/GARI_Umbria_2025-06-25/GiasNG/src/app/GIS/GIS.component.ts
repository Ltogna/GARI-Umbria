...
import {SatelliteService} from './services/satellite-animation.service';
...
export class GISComponent implements OnInit, OnDestroy, AfterContentChecked {
  ...
  constructor(
    ...
    this.featureService
      ...
      .subscribe(features => {
        ...
        this.handleFeaturesEdit(features);
      });
    ...
    if (this.masterService.gmapsApiLoaded == false) {
      this.configurazioneSitiService.currentConfigurazione_Siti
        ...
        .subscribe(cs => {
          ...
          let url = '';
          if (urlWithApiKey !== '') {
            url = urlWithApiKey + ',places,visualization';
          } else {
            url = 'https://maps.googleapis.com/maps/api/js?libraries=drawing,geometry,places,visualization&client=gme-addictive&v=beta';
          }
  ...
  private handleFeaturesEdit(features: GeoJson_Feature_New<GeoJSONAgroGisProp>[]): void {
    ...
    const lastSelected = features[features.length - 1];
    this.googleMapGeoJsonService.handleEditPolygon(lastSelected);
  }
  ...
  private handleIsSementieri(): void {
    this.sharedDataService.setCfgSementi(new SementieriParametrizzazione(JSON.parse(this.objParametriAgendaService.getObjParamValue().QueryStringFiltrino)));
  ...
  private setTimeFilterSementieri(): void {
    ...
    filtroTemporaleAvanzatoDefault.FiltroPeriodo = new FiltroTemporale();
    ...
    filtroTemporaleAvanzatoDefault.FiltroPeriodo.setValiditaData(dataInizioQS, dataFineQS);

    filtroTemporaleAvanzatoDefault.FiltroSingolaData = new FiltroTemporale();
    filtroTemporaleAvanzatoDefault.FiltroSingolaData.setIntervalloTemporaleSingolaData(dataInizioQS);
  ...
  private setDefaultTimeFilter() {
    ...
    filtroTemporaleAvanzatoDefault.FiltroPeriodo = new FiltroTemporale();
    filtroTemporaleAvanzatoDefault.FiltroPeriodo.setValidiDataOggi();

    const dataInizio: Date = filtroTemporaleAvanzatoDefault.FiltroPeriodo.DataInizio;
    filtroTemporaleAvanzatoDefault.FiltroSingolaData = new FiltroTemporale();
    filtroTemporaleAvanzatoDefault.FiltroSingolaData.setIntervalloTemporaleSingolaData(dataInizio);
