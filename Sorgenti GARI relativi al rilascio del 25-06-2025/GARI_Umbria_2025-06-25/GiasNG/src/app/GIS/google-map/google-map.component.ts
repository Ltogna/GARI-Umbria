...
import { catchError, combineLatest, debounceTime, filter, forkJoin, map, of, skip, Subject, Subscription, switchMap, take, takeUntil, tap, throwError } from 'rxjs';
...
export class GoogleMapComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    this.drawWindowOperationService
      ...
      .subscribe(operation => this.setOperation(operation));
  ...
  private handleSelectedFeaturesStyle(): void {
    const selected = this.gisGeometrySelectionService
      .getCurrentSelection()
    ...
    this.googleMap.data.forEach(f => {
      ...
      const chiaveAlberoCompleta = FunzioniComuniService.chiaveAlberoRidottaToBig(feature.properties.chiavealbero) as string;
      if (!selected.find(key => chiaveAlberoCompleta.includes(key))) {
        return;
      }
    ...
    switch (this.operation) {
      ...
      case enum_GISDrawingOperations.delete: {
        if (this.featureService.getFeatureSelezionate().length === 0) {
          ...
          this.giasDialogService.baseError('Elimina feature', 'gis.SelezionarePoligono');
          break;
        }
        ...
        const windowArgs = new WindowArgs(WindowTypes.DeleteLayerWindow, true, "Elimina impianto", altezzaFinestra, 350, undefined, undefined, 200, true, true, true, false, false, false);
      ...
      case enum_GISDrawingOperations.save: {
        ...
        this.editFeatureService.modificaFeature();
        break;
      }
  ...
  private loadPrecisionData(): void {
    ...
    const sub: Subscription = this.gisClient
      ...
      .pipe(
        ...
        filter(res => res != null),
        switchMap(code => forkJoin([of(code), this.gisClient.gisCaricaDatiPrecisionXmlDaAllegati({ Flag_TipoCod: 1, Lista_RicettaOp_Cod: [code] })])),
        switchMap(([code, res]) => res == null ? throwError(() => new Error("gis.DatiAgricolturaPrecisoneErrore")) : of([code, res]))
      )
      .subscribe({
