...
import { GisClient, Gis_Sat_Sentinel_Overlay, MascheraLayerRaster, RispostaStandard_1OfLista_GisSat_SentinelOverlay_Out } from "app/Service/api.service";
...
export class SatelliteLocalDataLoader {
  ...
  public load(): Observable<[Gis_Sat_Sentinel_Overlay[], GeoJson_Feature_New<GeoJSONAgroGisProp>[], Tile[]]> {
    ...
    return combineLatest([
      ...
      .pipe(
        debounceTime(100),
        filter(() => this.gisAnalisiMappeSatellitariWindowService.isActive),
        withLatestFrom(this.gisAnalisiMappeSatellitariWindowService.masks$),
        map(([[features, _], masks]: [[GeoJson_Feature_New<GeoJSONAgroGisProp>[], [number, number]], MascheraLayerRaster[]]) => {
        ...
        map(([res, tiles]: [RispostaStandard_1OfLista_GisSat_SentinelOverlay_Out, Tile[]]) => [res.RispostaStringa.ListaOverlayer.sort((a, b) => a.DataRiferimento.localeCompare(b.DataRiferimento)), tiles]),
        tap(([layers, _]: [Gis_Sat_Sentinel_Overlay[], Tile[]]) => this.gisAnalisiMappeSatellitariWindowService.nextCalendarSatelliteData(layers)),
        map(([layers, tiles]) => {
          ...
          return [layers, this.featureService.getFeatureSelezionate(), tiles];
        }),
      ) as Observable<[Gis_Sat_Sentinel_Overlay[], GeoJson_Feature_New<GeoJSONAgroGisProp>[], Tile[]]>;
  }

  private getIntersectedTiles(tiles: Tile[], feature: GeoJson_Feature_New<GeoJSONAgroGisProp>): Tile[] {
    const result: Tile[] = [];
