...
import { enum_Menu_Agenda_NG_Mode, enum_Security_Attivita } from 'app/Model/TipiEnumerativi';
import { UtilityFunctions } from 'app/Utility/UtilityFunctions';
import { CookieService } from 'ngx-cookie-service';
import { CookiePathDataFilterFrom, CookiePathDataFilterTo } from '../models';
...
export class TuttiTipiConfig extends AbstractGridConfigService<KendoServerResult> {
  ...
  constructor(
    ...
    const impVal = this.impostazioiImprese.getValore_Impostazione_Scalare_Centro_Azienda_SuperUser(
    ...
    this.handleCommands();
    this.gridPublicService.selection.getSelectedValue.GiasSubscribe(s => console.log('SELECTION::', s))
  }
  ...
  private getFilters() {
    ...
    if (filtersService === null) {
      filtersService = this.filters.inizializzaFilterConfig();
    }
    if (!filtersService.Impianti || filtersService.Impianti.length == 0) {
      this.cookieService.set(CookiePathDataFilterFrom, JSON.stringify(filtersService.Da));
      this.cookieService.set(CookiePathDataFilterTo, JSON.stringify(filtersService.A));
    }
    return filters;
  }
  ...
  private setButtonsVisibility() {
    this.gridPublicService.openCommands.pipe(takeUntil(this.signal), filter(attivita => !!attivita))
      .subscribe(async attivita => {
        ...
        let impostazione = this.permessiUtenteService.getImpostazione_SuperUser(enum_Impostazioni_Utenti.SUPERUSER_COD_ALGORITMO_COSTI_ACCESSORI);
        ...
        if (this.permessoRicercaDocumenti) {
          this.bussinessLogic.CheckAttachedDocuments(attivita)
