...
import {map, Observable, of, skip, switchMap, takeUntil} from 'rxjs';
...
export class GridVisiteHttpService extends AbstractGridConfigService<GridVisiteServerResult>{
  ...
  private permessoDocumentaleLista_R: boolean;
  ...
  constructor(injector: Injector,
    ...
    this.permessoDocumentaleLista_R = this.permessiUtenteService.getPermesso(enum_Security_Attivita.Documentale_Lista, 0);
  ...
  read(options?: any): Observable<GridVisiteServerResult> {
    ...
    if (filtersService.OperatoreVisita)
      if (filtersService.OperatoreVisita.Cod_RisUm != -1)
        param.operatore.Username = filtersService.OperatoreVisita.username;

    if (filtersService.AziendeVisita)
      if (filtersService.AziendeVisita.partitaIva != "-1")
        param.azienda = filtersService.AziendeVisita;

    if (filtersService.SpecieVisita) {
      if (filtersService.SpecieVisita.hasOwnProperty("classType")) {
        ...
        if (filtersService.SpecieVisita.codice != -1) {
    ...
    if (filtersService.CentroAziendaleVisita)
      if (filtersService.CentroAziendaleVisita.primaryKey.partitaIva != "-1")
    ...
    if (filtersService.TipoVisita) {
      if (filtersService.TipoVisita.key != -1)
    ...
    return this.visiteService.leggiVisite(param).pipe(map((r:any)  => {
  ...
  setColumnsGridVisite(response): Array<KendoGridColumn>{
    ...
    for (let itemOfColumns of response.kendo_columns) {
      if (itemOfColumns.field != 'Azioni') {
        if (itemOfColumns.field == 'Veg_Des') {
          columns.push(
            new KendoGridColumn({field: itemOfColumns.field, title: itemOfColumns.title}, 
            {resizable:true,editable: false, hidden: itemOfColumns.hidden, filterOrdering: (rows) => {
                ...
                columns.push(
                  new KendoGridColumn({field: itemOfColumns.field, title: itemOfColumns.title}, {resizable:true,editable: false, hidden: itemOfColumns.hidden})
                );
      }
    }
  ...
  private deleteItem(rowsIn: KendoGridRow[] | KendoGridRow, proseguiInCasoDiAlert: boolean): Observable<any> {
    ...
    return this.visiteService.deleteItem(getParams).pipe(switchMap((risposta: RispostaStandard | any) => {
  ...
  private showDialogConfirmDelete(msgParam: string) {
    ...
    return this.visiteService.createDialogWindow(
      '', msg
    );
    ...
    this.gridPublicService.commandEvent.GiasSubscribe(ev => {
      ...
      switch(ev.command.action) {
        ...
        case enum_visiteGridCommands.RICERCA_DOCUMENTI:
          ...
          this.visiteService.ApriKendoWindowRicercaDocumenti((ev.dataItem.Id_Agenda_Rif !== 0) ? ev.dataItem.Id_Agenda_Rif : ev.dataItem.Id_Agenda, ev.dataItem.Piva);
  ...
  private setButtonsVisibility() {
    this.gridPublicService.openCommands.GiasSubscribe(async attivita => {
      ...
      if (attivita) {
        ...
        if ((this.cmdDropDown.cmdList.find(o => o.action === enum_visiteGridCommands.COPIA_VISITA) === undefined) && this.permessoVisite_W){
          this.cmdDropDown.addCommand(new GridCommandItem(
            "visite.CopiaVisita",
        ...
        //Per ora commento i bottoni Vai a Rilievo e Cancella Rilievo
        //Gestione Visibilita Bottone Vai a Rilievo e Cancella Rilievo
        /*if(attivita.Lav_Cod_Rif  && attivita.Lav_Cod_Rif > 0 &&
          attivita.Lav_Des_Rif && attivita.Lav_Des_Rif !== ""){

          if(this.cmdDropDown.cmdList.find(o => o.action === enum_visiteGridCommands.VAI_A_RILIEVO) === undefined){

            let destinazione: string = attivita.Lav_Des_Rif;

            this.cmdDropDown.addCommand(new GridCommandItem(
              "VaiA",
              enum_visiteGridCommands.VAI_A_RILIEVO,
              "faEditFull",
              null,
              true,
          ...
          if(this.cmdDropDown.cmdList.find(o => o.action === enum_visiteGridCommands.CANCELLA_RILIEVO_ASSOCIATO) === undefined){

            this.cmdDropDown.addCommand(new GridCommandItem(
              "visite.CancellaRilievoAssociato",
              enum_visiteGridCommands.CANCELLA_RILIEVO_ASSOCIATO,
              "",
              faTrashAlt
            ));
          }
  ...
  private async deleteRilieviVisita(param: any, proseguiInCasoDiAlert: boolean) {
    ...
    if (dialogResponse['returnObj']) {
      ...
      this.visiteService.deleteRilievoVisita(paramToDelete).subscribe((risposta: RispostaStandard | any) => {
