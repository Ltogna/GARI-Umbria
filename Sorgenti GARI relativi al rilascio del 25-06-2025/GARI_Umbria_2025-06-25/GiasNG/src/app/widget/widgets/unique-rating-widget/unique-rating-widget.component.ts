...
export class UniqueRatingWidgetComponent  implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.widgetsClient
      .widgetsReadAvailableYearsIndiciProduttivita(this.piva)
      .pipe(
        map(res => res.RispostaStringa.length > 0 ? Math.max(...res.RispostaStringa.map(s => +s)) : (new Date().getFullYear() - 1)),
        switchMap(year => this.widgetsClient.widgetsReadWidgetKPI({ piva: this.piva, year: year })),
        catchError(() => of({ RispostaStringa: [] })),
        tap(res => this.data = res.RispostaStringa),
