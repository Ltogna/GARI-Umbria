...
export class ComparisonWidgetService {
  ...
  private nextData(piva: string, years: number[]): void {
    ...
    this.widgetsClient
      .widgetsReadWidgetIndiciProduttivita({
        requestXImpresa: [{
          piva: piva,
      ...
      .subscribe(res => this.dataSource.get(key).next(res.RispostaStringa));
  }
  ...
  public getYears$(piva: string) {
    ...
    this.widgetsClient
      .widgetsReadAvailableYearsIndiciProduttivita(piva)
      .subscribe(res => this.validYearsSource.get(piva).next(res.RispostaStringa));
