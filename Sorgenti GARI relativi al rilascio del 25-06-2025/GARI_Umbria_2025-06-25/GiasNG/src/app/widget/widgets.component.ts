...
export class WidgetsComponent implements AfterViewInit {
  ...
  constructor(
    ...
    this.menuClient
      ...
      .subscribe(user => {
        // console.log("userLoaded", user.RispostaStringa);
  ...
  private getData(): void {
    ...
    this.widgetsClient
      ...
      .subscribe(response => {
        const elements = response.RispostaStringa.map(element => ({ ...element, aspettoReale: JSON.parse(element.Aspetto) } as WidgetData)).filter(element => element.MultiAziendale == this.readSwitchMultiAzienda());
