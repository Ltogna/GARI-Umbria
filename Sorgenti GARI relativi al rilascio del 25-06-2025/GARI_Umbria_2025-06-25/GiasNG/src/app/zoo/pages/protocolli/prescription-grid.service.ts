...
import { AGRODATAFINE, AGRODATAINIZIO, TUTTI_CENTRI_AZIENDALI } from "app/Model/CostantiPersonalizzate";
...
import { ItemSelector } from "app/menu-agenda/components/operations-list/utils";
import { truncate } from "fs/promises";
import { enum_TypeTab_Zootecnia, enumUdMDoseArray } from "app/zoo/models/tipi-enumerativi-zoo";
  ...
  perform(actionType: HttpAction, items: any, oldRow: any): Observable<any> {
    ...
    switch (actionType) {
      case HttpAction.UPDATE:
        ...
        const params: UpdatePrescrizione = {
          Piva: items.Piva,
          SaCod: items.Sa_Cod,
        ...
        return this.prescriptions.prescrizioniUpdatePrescription(params)
        .pipe(
          filter(r => r.RispostaOK),
          map(() => {
            let options = new ZooPrescriptionsFilters(0, 0, AGRODATAINIZIO, AGRODATAFINE, []);
            options.prescriptionType = enum_TipoPrescrizione.Protocollo_Terapeutico;
