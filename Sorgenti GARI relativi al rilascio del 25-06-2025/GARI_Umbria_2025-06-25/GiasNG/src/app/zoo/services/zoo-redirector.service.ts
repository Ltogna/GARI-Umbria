...
export class ZooRedirectorService {
  ...
  public redirectOpenOperationInfo(activity: ZooActivityForRedirect) {
    if (activity.Lav_Cod == enum_LAVCOD.CURE_MEDICAMENTI_ANIMALI) {
      this.redirectActivityToTreatment(activity, false);
    } else {
  ...
  public redirectOpenOperationEdit(activity: ZooActivityForRedirect) {
    if (activity.Lav_Cod == enum_LAVCOD.CURE_MEDICAMENTI_ANIMALI) {
      this.redirectActivityToTreatment(activity, true);
    } else {
  ...
  private redirectActivityToTreatment(activity: ZooActivityForRedirect, isEditing: boolean) {
    forkJoin([
      from(this.gestioneRichieste.gestionePassaggioStessoSito(enum_PagineGiasNG.Pagina_Trattamento_Zoo)),
      this.client.operazioniZooGetAttivitaTrattamentoZooFromAgenda(activity.Piva, activity.Id_Agenda)
