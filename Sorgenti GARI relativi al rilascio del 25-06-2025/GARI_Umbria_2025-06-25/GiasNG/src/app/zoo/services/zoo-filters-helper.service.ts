...
export class ZooFiltersHelperService {
  ...
  private stable: { piva:string, centro:number, stalle:IBaseCodeDescr[] }[] = [];
  ...
  constructor(
    ...
    this.resetFilters$.pipe(tap((val) => {
      if (val) {
   ...
   */
  public loadStables(piva: string, saCod: number): Observable<IBaseCodeDescr[]> {
    ...
    return this.client.operazioniZooGetStalle(piva, saCod)
      .pipe(
        map(r => JSON.parse(r.RispostaStringa)),
        map(stables => stables.map(x => new BaseCodeDescr(x['STA_NUM'], x['STA_DES']))),
        tap((lav) => {
        this.stable.push({
          piva: piva,
          centro: saCod,
