...
import { OPERATIONS_MODEL_4, ZooActivityForRedirect, ZooRedirectorService } from "../../services/zoo-redirector.service";
...
export class ZooOperationsGridConfigService extends AbstractGridConfigService<KendoServerResultImpl> {
  ...
  private _gridColumns: KendoGridColumn[] = [
    new KendoGridColumn({ field: 'Id_Agenda', title: 'ID' }, { editable: false }),
    new KendoGridColumn({ field: 'Data', title: this.transloco.translate('Data') }, { editable: false }),
    new KendoGridColumn({ field: 'lav_des', title: this.transloco.translate('Operazione') }, { editable: false }),
    ...
    new KendoGridColumn({
      field: 'Dettaglio_Tecnico',
      title: this.transloco.translate('DettaglioTecnico')
    }, { editable: false }),
    new KendoGridColumn({ field: 'Prodotti', title: this.transloco.translate('Prodotti') }, { editable: false }),
    new KendoGridColumn({ field: 'Segnalazioni', title: this.transloco.translate('Segnalazioni') }, { editable: false }),
    new KendoGridColumn({
    ...
    this.gridPublicService.commandEvent.pipe(
      ...
      switch (cmd.command.action) {
        case CommandsDropDownEvents.INFO:
          this.zooRedirector.redirectOpenOperationInfo(cmd.dataItem);
          break;
        case CommandsDropDownEvents.FULL_EDIT:
          this.zooRedirector.redirectOpenOperationEdit(cmd.dataItem);
          break;
