...
export class TreeGisService extends BehaviorSubject<TreeNode[]> {
    visualizzaAlbero = true;
    // Filtro albero
    public filterTerm = '';
    // Filtro catasto
    public filtroCatasto: FiltroCatasto = {
        Provincia: '',
        Comune: '',
        Foglio: '',
        Particella: '',
    }
    public filterProvincia = '';
    //
    ...
    /*public loadData_Old(filters: GisTreeFilters, forceLoad?: boolean) {
        ...
        // return this.ajax$.post_legacy(getNodesLink, this.getParams(filters))
        //     .pipe(map((data) => {
        ...
        //         const nodes: TreeNode[] = JSON.parse(data.RispostaStringa);
        //         super.next(nodes);
        //     })).subscribe();
    ...
    private getParams(filters: GisTreeFilters) {
        ...
        // Filtro temporale
        const filtroTemporale = this.sharedDataService.getFiltroTemporaleAvanzato().FiltroPeriodo;
   ...
   */
    ...
    public handleChecked(e: TreeItemLookup, checkedKeys: any[]) {
        let isChecked = (this.getIndexCheckedTreeItem(e.item, checkedKeys)) >= 0;
        // if(isChecked) {
        //     let val: string[] = this.checkedItemIds.value;
        //     val.push(<string>(e.item.dataItem.id));
        //     this.checkedItemIds.next(val);
        // } else {
        //     let val: string[] = this.checkedItemIds.value;
        //     val.splice(val.findIndex(i => i == e.item.dataItem.id), 1);
        //     this.checkedItemIds.next(val);
        // }
    ...
    getIndexCheckedTreeItem(e: TreeItem, checkedKeys: any[]): number {
        ...
        let indexCheckedTreeItem = checkedKeys.findIndex(element => element === indexClicked);
        return indexCheckedTreeItem;
    }
    ...
    toggleVisualizzaAlbero() {
        this.visualizzaAlbero = !this.visualizzaAlbero;
    }

    setVisualizzaAlbero(flagVisualizzaAlbero: boolean) {
        this.visualizzaAlbero = flagVisualizzaAlbero;
    }
