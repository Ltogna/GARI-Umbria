...
import { AggiornaFiltroImpianti_In, ChiaveImpianto_In, GisClient } from 'app/Service/api.service';
...
import { enum_OperatoreFiltroTemporale } from 'app/GIS/GIS-enum/GIS-filtro-temporale';
import moment from 'moment';
import { ParametriFiltroRicercaNG } from 'app/filtro-ricerca/utils';

const DEFAULT_ITEM_POSITION = 0;

export type GisTreeFilters = {
...
export class GisTreeFiltersComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.sharedDataService.getResetVisualizzazioneTotale$()
      ...
      .subscribe(flag => {
        if (this.visualizzazioneTotale) {
          ...
          this.treeGisService.toggleVisualizzaAlbero();
        }
      })
    ...
    this.visualizzazioneTotale = this.sharedDataService.getVisualizzazioneTotale();
  ...
  onChangeVisualizzazioneTotale(matSlideToggle: any) {
    this.applicaVisualizzazioneTotale(matSlideToggle.checked, true);
  }
  ...
  private applicaFiltroAvanzatoTemporale(params: string[]) {
    const advancedFilter: FiltroTemporaleAvanzato = new FiltroTemporaleAvanzato();
    const periodFilter: FiltroTemporale = new FiltroTemporale();
    const singleDateFilter: FiltroTemporale = new FiltroTemporale();
    // For now just the first two dates are supported
    const opStart =
      params[0][0] == "precedente"
        ? enum_OperatoreFiltroTemporale.PrecedenteUguale
        : enum_OperatoreFiltroTemporale.SuccessivoUguale;
    const opEnd =
      params[1][0] == "precedente"
        ? enum_OperatoreFiltroTemporale.PrecedenteUguale
        : enum_OperatoreFiltroTemporale.SuccessivoUguale;
    const startDate = new Date(moment(params[0][1], 'DD/MM/YYYY').format());
    const endDate = new Date(moment(params[1][1], 'DD/MM/YYYY').format());
    periodFilter.setValiditaData(startDate, endDate, opStart, opEnd);
    singleDateFilter.setIntervalloTemporaleSingolaData(startDate);
    advancedFilter.FiltroPeriodo = periodFilter;
    advancedFilter.FiltroSingolaData = singleDateFilter;
