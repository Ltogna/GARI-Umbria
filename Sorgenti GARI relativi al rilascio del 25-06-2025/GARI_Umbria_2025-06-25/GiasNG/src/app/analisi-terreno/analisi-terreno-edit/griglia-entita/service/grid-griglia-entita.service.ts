...
export class GridEntitaHttpService extends AbstractGridConfigService<GridEntitaServerResult> {
    ...
    constructor(injector: Injector,
        ...
        this.selectable.selectable = new SelectableSettings({
            ...
            // mode: (this.reportMultiSelectGridOrogel.includes(paramFiltroRicercaNG.CodificaStampe) || paramFiltroRicercaNG.TipoComportamentoFiltroRicercaNG == Enum_TipoComportamento_FiltroRicerca.SelezionamentoEntita) ? 'multiple' : 'single'
        });
        ...
        // this.analisiFormService.formAnalisiTerreno.get('validita').valueChanges.subscribe(values => {

        //     //pulisco l'array delle entità 
        ...
        //             statusGrid.data.rows = this.arrayFiltersData(this.allRows, validitaInizio, validitaFine);
        //         }
        //     }
    ...
    public selectionChangeFn = (event: SelectionEvent, component: GiasKendoGridComponent) => {
        ...
        if (this.analisiFormService.arrayEntita.length > 0) {
            // this.analisiFormService.formAnalisiTerreno.get('validita')?.get('inizio')?.disable({ emitEvent: false });
            // this.analisiFormService.formAnalisiTerreno.get('validita')?.get('fine')?.disable({ emitEvent: false });
            this.analisiFormService.formAnalisiTerreno.get('validita')?.disable({ emitEvent: false });
        } else {
            // this.analisiFormService.formAnalisiTerreno.get('validita')?.get('inizio')?.enable({ emitEvent: false });
            // this.analisiFormService.formAnalisiTerreno.get('validita')?.get('fine')?.enable({ emitEvent: false });
    ...
    read(options?: any): Observable<GridEntitaServerResult> {
        ...
        // let entitaValue = this.analisiFormService.formAnalisiTerreno.get('entitaCoinvolte').value;
