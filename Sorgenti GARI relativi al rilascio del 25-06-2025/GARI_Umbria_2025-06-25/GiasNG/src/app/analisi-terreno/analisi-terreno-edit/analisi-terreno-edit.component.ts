...
import { Subject, BehaviorSubject, combineLatest, startWith, takeUntil, filter, tap, skip, take, catchError, of } from "rxjs";
...
export class AnalisiTerrenoEditComponent implements OnDestroy {
    ...
    private completeFormSettings(): void {

        this.fromGIS = (this.objParametriAgenda.Pagina_Provenienza == enum_PagineGiasNG.Pagina_GIS);
        ...
        this.infoAzienda = this.objParametriAgenda.RagSoc + ' (' + this.transloco.translate('PartitaIVA') + ': ' + this.objParametriAgenda.Piva + ')';

        this.analisiDataService.LeggiLaboratori().subscribe(r => {
            // r.forEach(element => {
            //     this.listaLaboratori.push({ ragione_Sociale: element.descrizione,
            //                             risorseUmane: [{ codice: element.codice }]});
            // });
            r.unshift({
                ragione_Sociale: '',
        ...
        //stimolo
        // this.analisiFormService.formAnalisiTerreno.get('laboratorio')?.updateValueAndValidity({ emitEvent: true });
    ...
    SalvaEditAnalisiTerreno() {
        // console.log('SalvaEditAnalisiTerreno');
        ...
        // if (analisiTerreno.dettagli)
        //     analisiTerreno.dettagli.splice(1);
        ...
        //CAMPIONI
        // if(analisiTerreno.campioni && analisiTerreno.campioni.length > 0) {
        //     analisiTerreno.campioni.forEach(item => {
        //         if (item.particella) {
        //             let particellaCompleta = this.listaCompletaParticelle.find(itemCompleto => itemCompleto.chiave == item.particella.chiave);
        //             let chiaveParticella = new ParticelleCatastali.PK(particellaCompleta.Prov, particellaCompleta.Com, particellaCompleta.SEZIONE, particellaCompleta.FOGLIO, particellaCompleta.NUMERO, particellaCompleta.SUBALTERNO);
        //             item.particella = new ParticelleCatastali();
        //             item.particella.primaryKey = chiaveParticella;
        //         }
        //     });
        // }
        ...
        const elemFormArr = this.fb.group(
            {
                ...
                riferimento5: [null],
                note: [null],
                dataPrelievo: [null, Validators.required],
                KeyPiva: [null],
                KeySaCod: [0],
                KeyGrafica: [null],
                particella: [null],
