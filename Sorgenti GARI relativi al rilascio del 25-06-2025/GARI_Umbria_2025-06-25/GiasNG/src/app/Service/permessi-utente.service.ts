...
export class PermessiUtenteService implements IUsernameService {
  ...
  getImpostazione_SuperUser(Impostazione_Cod: number): Utente_Impostazioni{

    const impostazione = this.Utente_Permessi.Impostazioni_SuperUser.find((imp) => imp.Impostazione_Cod == Impostazione_Cod);


    return impostazione;
  }

  getImpostazione_Utente_poi_SuperUser(Impostazione_Cod: number): Utente_Impostazioni{

    let impostazione = this.getImpostazione_Utente(Impostazione_Cod);

    if(impostazione!==undefined  && impostazione!==null){
      return impostazione;
    }else{
      impostazione=this.getImpostazione_SuperUser(Impostazione_Cod);
    }
