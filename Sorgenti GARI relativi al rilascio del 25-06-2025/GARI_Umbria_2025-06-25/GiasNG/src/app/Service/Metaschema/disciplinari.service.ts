...
export class DisciplinariService {
  ...
  constructor(private ajaxAgronicaService: AjaxAgronicaService,
    ...
    /*leggi_Old(leggiDisciplinari: LeggiDisciplinare): Observable<Disciplinare[]> {
       let parametri: CoreWS_Generic<object> = new CoreWS_Generic(
          this.masterService.getCoreWSGenericObjP(),
          leggiDisciplinari
      );
      return (this.ajaxAgronicaService.ajaxCoreWSPost<object, Disciplinare[]>(
            this.masterService.link_CoreWS + "/AgronicaCoreDPI/DPI.asmx/CaricaComboDisciplinare_Modello",
             parametri,
             false).pipe(map((risp) => {
    ...
    leggi(leggiDisciplinari: LeggiDisciplinare): Observable<Disciplinare[]> {
        return (this.APIService.ajaxAPIPost<object, Disciplinare[]>(
            "AgronicaCoreDPING/CaricaComboDisciplinareModello",
                leggiDisciplinari,
            false).pipe(map((risp) => {
                return (risp.RispostaStringa);
        })));
    ...
    /*leggiIAF_Old(leggiDisciplinari: LeggiIAF): Observable<ImpegniAggiuntiviFacoltativi[]> {

        if (leggiDisciplinari.specie.codice == 0 || leggiDisciplinari.disciplinare.codice == '0') {
            return of([]);
        }

        if (leggiDisciplinari.disciplinare.codice == '' || leggiDisciplinari.disciplinare.codice == '0') {
            return of([]);
        }
        ...
        return (this.ajaxAgronicaService.ajaxCoreWSPost<object, ImpegniAggiuntiviFacoltativi[]>(
              this.masterService.link_CoreWS + "/AgronicaCoreDPI/DPI.asmx/CaricaComboIAF_Modello",
              parametri,
              false).pipe(map((risp) => {
                  return (risp.RispostaStringa);
              })));
  ...
  leggiIAF(leggiDisciplinari: LeggiIAF): Observable<ImpegniAggiuntiviFacoltativi[]> {
    ...
    return (this.APIService.ajaxAPIPost<object, ImpegniAggiuntiviFacoltativi[]>(
          "AgronicaCoreDPING/CaricaComboIAFModello",
          leggiDisciplinari,
          false).pipe(map((risp) => {
              return (risp.RispostaStringa);
          })));
