...
import { faFileCode, faFileText } from '@fortawesome/free-solid-svg-icons';
...
import { Actions, Dialog_Type, GiasDialogService } from "../../Service/gias-dialog.service";
import {
  Obj_Errore_Gias_QdC
...
export class ExportQdCToAgeaComponent implements OnInit, OnDestroy {
  ...
  form = this.fb.group({
    ...
    impresa: new FormControl<Impresa | null>(null),
  });
  ...
  parseMessage(item: HubAgeaResult): string {
    ...
    if (item.submissionState?.state == SUBMISSION_ERROR) {
      return item.submissionState?.message;
    }
  ...
  private readData(anno: number | null, impresa: Impresa | null, state: State): Observable<{ data: ExportQdCToAgeaModel[], total: number }> {
    ...
    return forkJoin([
      this.ageaService.getAllBundleState(payload).pipe(map(x => x.RispostaStringa)),
      count$
    ]).pipe(map(([data, count]: [HubAgeaResult[], number]) => {
      this.oldCount = count;
