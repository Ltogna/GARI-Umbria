...
const INDICE_AVV = enum_LAVCOD.RILIEVO_AVVERSITA_CAMPO.toString();
...
export class RilieviComponent implements OnInit, DoCheck, OnDestroy {
  ...
  private get isRilievoAvversita(): boolean {
  ...
  ngOnInit(): void {
    ...
    this.Subs.add(
      this.qdcservice.TestataForm.get("Disciplinare")?.valueChanges.subscribe((d: DropdownListDisciplinare) => {
        ...
        if (this.ddl.id === INDICE_AVV && this.selectedOp) {
  ...
  onSubmit() {
    ...
    if (this.isRilievoRese) {
      this.rilieviService.getStimeResa().subscribe(rese => {
        this.addDettagliRilievi(dettaglioRilievoTemplate, rese);
      });
      return;
    }
    this.addDettagliRilievi(dettaglioRilievoTemplate);
  }
  ...
  private addDettagliRilievi(dettaglioRilievoTemplate:DettaglioRilievo, rese?:Map<string,string>) {

    const eserciziCDC =  this.qdcservice.GetEserciziCDCSelezionatiModel();
    ...
    if(eserciziCDC && eserciziCDC.length > 0){
      for(let i = 0; i < eserciziCDC.length; i++){
        ...
        error_found = this.addDettaglioRilievoFG(eserciziCDC[i], dettaglioRilievoTemplate,rese);

        if(error_found)
          break;
      }
    }else{
      error_found = this.addDettaglioRilievoFG(null, dettaglioRilievoTemplate,rese);
    }
  ...
  public ApplyDivClass(): string {
    if (this.qtaType === this.TXT || this.qtaType === this.TXT_AREA || this.qtaType === this.DDL) {
      return "col-12 col-sm-5";
    } else if (this.qtaType === this.BOOLEAN) {
  ...
  private addDettaglioRilievoFG(esercizioCDC: EsercizioCDC,dettaglioRilievoTemplate:DettaglioRilievo, rese?:Map<string,string>): boolean{

    let dettaglioRilievoFG: FormGroup = null;
    ...
    if(esercizioCDC){
      ...
      if (rese) {
        const key = dettaglioRilievoTemplate.esercizioCDC.esercizio.impiantoPK.appezzamentoPK.centroAziendalePK.partitaIva + "|"
          ...
          + dettaglioRilievoTemplate.esercizioCDC.esercizio.codice;
    ...
    dettaglioRilievoFG =  dettaglioRilievoTemplate.toFormGroup();

    if(dettaglioRilievoFG)
      (this.Rilievo.get("DettagliRilievi") as FormArray).push(dettaglioRilievoFG);
