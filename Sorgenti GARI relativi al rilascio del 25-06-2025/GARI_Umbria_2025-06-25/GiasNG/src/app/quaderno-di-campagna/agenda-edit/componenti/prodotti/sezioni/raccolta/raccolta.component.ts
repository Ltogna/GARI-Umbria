...
export class RaccoltaComponent implements OnInit, OnDestroy {
    ...
    private loadSettings() {
        ...
        const GestioneMagazzino = this.ACService.getValore_Impostazione_Scalare_Centro_Azienda_SuperUser_per_Elem_Cod(
        ...
        const FiltroLotti = this.ACService.getValore_Impostazione_Scalare_Centro_Azienda_SuperUser_per_Elem_Cod(
            this.masterService.objP_utenti.PivaSuperUser, 0,
        ...
        if (FiltroLotti.includes(TRASFORMATI_VEGETALI+'_0')) {
            lottoAttivo = false;
        }
        if (GestioneMagazzino.includes(TRASFORMATI_VEGETALI+'_0')) {
            lottoAttivo = false;
            carichiAttivi = false;
        }
