...
export class TrattamentoComponent implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
    ...
    constructor(
        ...
        this.operationChangeSub = this.qdcservice.TestataForm.controls
            ...
            .subscribe(value => { this.checkRilievo(value) });
    ...
    private subscribeToGis(): void {
        ...
        this.GISToolbarService.latValueSubject.next(`${lat >= 0 ? '+' : '-'}${lat.toPrecision(9).toString()}`)
        this.GISToolbarService.lngValueSubject.next(`${lng >= 0 ? '+' : '-'}${lng>=100?lng.toFixed(7).toString():(lng>=10?'0'+lng.toFixed(7).toString():'00'+lng.toFixed(7).toString())}`)
