...
export class QdCTestataService implements OnDestroy{
    ...
    async getArray_UtilizziTerreno(fromSwitch?: boolean){
        ...
        if (!flagVisitaValue
            ...
            const centroAziendale = this.qdcservice.TestataForm.get('Centro_Aziendale').value ?? new CentroAziendale({ codice: objParamsAgenda.Sa_Cod, partitaIva: objParamsAgenda.Piva } as PKCentroAziendale);
            const data = this.qdcservice.TestataForm.get('Data').value;
            const consideraTerrenoNudo = this.qdcservice.getCONSIDERATERRENONUDO();
            this.Array_UtilizziTerreno = await this.impiantiservice.getSpecieQdCA(centroAziendale, data, impresa as any, consideraTerrenoNudo);
        } else {
    ...
    async CambioCentroAziendale(){
        ...
        this.qdcservice.getImpostazioni_QdC_Centro_Azienda();
