...
import {
  ...
  FERTILIZZANTI,
  FORMULATI,
  INSETTI, LAVCOD_ABBATTIMENTOIMPIANTI,
...
import {
  ...
  Parametri_Aggiuntivi_Attivita, Sezione,
...
import {Enum_DBTypeOperation, enum_TipoControllo} from 'gias-ui-kit';
...
import {DettaglioRegistrazione} from "../../../../Model/attivita/dettagli/DettaglioRegistrazione";
...
export class QdCFormToAttivitaService{
    ...
    mapAttivita(o: Lavorazione, Sezione_ProdottoDaMappare: FormGroup, listParametri_Aggiuntivi_Attivita:Parametri_Aggiuntivi_Attivita[],
                fromDdl: boolean = false,array_dosi_for_fromDdl: MultiColumnComboboxDose_Etichetta[] = [], flagVisita: boolean) {
        ...
        attivita.attivitaCollegate = this.qdcservice.TestataForm.get("Attivita_Collegate").getRawValue();
        ...
        attivita.blocco = this.qdcservice.TestataForm.get("Blocco_Attivita").getRawValue();
        ...
        if(GridMacchine && GridMacchine.length > 0 && !flagVisita){
            ...
            GridMacchine.forEach((m: GridMacchinaModel)=>{
                ...
                attivita.risorse.push(newRisorsaMacchina);
            });
        }
    ...
    private getDisciplinareAttivita(ca: CodiciXOperazione): Disciplinare{
        ...
        if(this.qdcservice.Operazioni_Con_Disciplinare(ca.Operazione)){
            ...
            if(+ ca.Operazione.primaryKey.codice === enum_LAVCOD.CONFUSIONE_DISORIENTAMENTO_SESSUALE ||
              + ca.Operazione.primaryKey.codice === enum_LAVCOD.TRATTAMENTO_POST_RACCOLTA ||
              + ca.Operazione.primaryKey.codice === enum_LAVCOD.CONCIA_SEME){
   ...
   */
                ...
                DosiFormulati.forEach(f => {
                    ...
                    if(f.Prodotto && f.Operazione) {
                        ...
                        if(index === -1) {
                            ...
                            newDettaglioTrattamento.avversitaGruppo = <AvversitaGruppo> f.Avversita;
    ...
    private getDettaglioTrattamentoFromDdl(
        ...
        if(Sezione_Formulato) {
            ...
            newDettaglioTrattamento.avversitaGruppo = <AvversitaGruppo> Sezione_Formulato.Avversita;
    ...
    private getRilevamentoDiMagazzino(gridrowDosiProdotti: any): RilevamentoDiMagazzino{
