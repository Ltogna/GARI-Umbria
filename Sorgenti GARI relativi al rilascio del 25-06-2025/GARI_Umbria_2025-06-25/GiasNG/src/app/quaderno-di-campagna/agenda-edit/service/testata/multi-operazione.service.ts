...
import {enum_LAVCOD, enum_PagineGiasNG, enum_SEMINA_TIPO} from 'app/Model/TipiEnumerativi';
...
export class QdCMultiOperazioneService implements OnDestroy{
    ...
    changeOperazioni_QdC(multiElem: MultiSelectFormItem){
        ...
        this.Controlla_Compatibilita_Dati_Con_Nuova_Operazione(multiElem).then(async (listErroriGias : ErroreGias[]) =>{
            ...
            if(Operazioni_Scelte){
                ...
                this.qdcservice.Inizializza_QdC().then(async r=>{
                    if(r){
                        ...
                        if(Operazioni_Scelte.length === 1 && Operazioni_Scelte.findIndex(o=>+ o.primaryKey.codice === enum_LAVCOD.CONFUSIONE_DISORIENTAMENTO_SESSUALE ||
    ...
    async openMultiOperazione(multiEl:  GiiasMultiselectTemplateSComponent) {
        ...
        if(Operazioni_Selezionate && Operazioni_Selezionate.length > 0){
            ...
            if(Operazione_con_Sementi){
                ...
                if(!Opzione_Semina)
                    Opzione_Semina = this.qdcservice.get_Default_Opzione_Semina(+ Operazione_con_Sementi.primaryKey.codice);
            }
