    ...
    perform(actionType: HttpAction, item: any): Observable<any> {
        ...
        if(actionType === HttpAction.UPDATE || actionType === HttpAction.CREATE) {
            ...
            Obs = from(new Promise( (resolve, reject) => {
                ...
                this.gridcontrolliservice.ControllaSeInserireDoseProdotto(DosiProdotti,item,this.prodottiservice.ProdottiForm,tipo_operazione_db, item.DosiProdottiGridrowId).then(errori=>{
                    if(errori.length === 0){
