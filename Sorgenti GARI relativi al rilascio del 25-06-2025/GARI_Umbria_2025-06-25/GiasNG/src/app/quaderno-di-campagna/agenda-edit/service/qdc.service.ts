...
import {Actions, Dialog_Type, GiasDialogService} from 'app/Service/gias-dialog.service';
import {
...
import {KendoGridRow, NumericSettings} from 'gias-kendo-grid';
import {GridDataWithFilter} from 'gias-kendo-grid';
import {DataValidator} from 'gias-ui-kit';
import {MultiSelectValidator} from 'gias-ui-kit';
...
export class QdCService implements OnDestroy {
    ...
    Obj_Default_Numeric_Settings = new NumericSettings({
        defaultValue: 0,
        format: 'n4',
        min: 0,
        step: 0.0001,
    ...
    public digitsInfo_QdC = "1.0-"+this.Obj_Default_Numeric_Settings.decimals;
    ...
    Elenco_Operazioni_Lavorazioni = [enum_LAVCOD.ANDANAMENTO, enum_LAVCOD.ARATURA, enum_LAVCOD.ASPORTAZIONE_ORGANI_INFETTI, enum_LAVCOD.ASSOLCATURA,
        ...
        enum_LAVCOD.STRIGLIATURA, enum_LAVCOD.PIRODISERBO];
    ...
    Elenco_Operazioni_Rilievi = [enum_LAVCOD.DANNI_RACCOLTA,enum_LAVCOD.FASI_FENOLOGICHE,enum_LAVCOD.RILIEVO_AVVERSITA_CAMPO,enum_LAVCOD.RILIEVO_INDICI_RESE_RACCOLTA,
                                                enum_LAVCOD.RILIEVO_INDICI_MATURITA,enum_LAVCOD.RILIEVO_ERBE_INFESTANTI];
    ...
    */
    ...
    private validateRaccolta(control: AbstractControl): ValidationErrors | null {
        ...
        /*if (genLotto === enum_Generazione_Lotto_Raccolta.MANUALE) {
    ...
    public DosiProdotticonRigheSalvate(Sezione_Prodotto: FormGroup,Operazione: Lavorazione): Array<any> {
        ...
        if(DosiProdottiFormArray)
            DosiProdotti = DosiProdottiFormArray.getRawValue().filter(d=>d.Riga_Salvata);
    ...
    getDETTAGLITERRENONUDO(): boolean {
        return this.CONSIDERATERRENONUDO;
    }
    ...
    setImpostazione_Avversita_Prima_Dei_Prodotti(){
        let impostazione;
  ...
  getImpostazioni_QdC() {
    const toNumber = (stringNumber: string) => +stringNumber;
    const equalsOne = (stringNumber: string) => +stringNumber === 1;
    let impostazione;

    this.getImpostazioni_QdC_Centro_Azienda();

    impostazione = this.PermessiUtenteService.getImpostazione_SuperUser(enum_Impostazioni_Utenti.SuperUser_KPIN_BlockName);
    if (impostazione !== undefined && impostazione !== null && impostazione.Valore === '1') {
      this.visualizza_Kpin_BlockName = true;
    }

    impostazione = this.PermessiUtenteService.getImpostazione_SuperUser(enum_Impostazioni_Utenti.SuperUser_Visualizza_Codici_Anagrafici);
    if (impostazione !== undefined && impostazione !== null && impostazione.Valore === '1') {
      this.visualizza_codici_imp_app_prj = true;
    }

    this.Impostazione_Mag = this.getSettingValueAs(enum_Impostazioni_Utenti.UTENTE_GESTIONE_MAGAZZINO_2, false);

    impostazione = this.PermessiUtenteService.getImpostazione_SuperUser(enum_Impostazioni_Utenti.SUPERUSER_COD_PERMETTI_OPERAZIONI_SU_IMPIANTI_BLOCCATI);
    if (impostazione !== undefined && impostazione !== null && impostazione.Valore === '1') {
      this.leggiAncheImpiantiBloccati = true;
    }

    // Recupero le colonne visibili della Grid Impianti
    impostazione = this.PermessiUtenteService.getImpostazione_Utente_poi_SuperUser(enum_Impostazioni_Utenti.UTENTE_COD_ColonneVisibili_TabellaImpiantiAgenda);
    if (impostazione !== undefined && impostazione !== null && impostazione.Valore !== '') {
    ...
    if (this.objParametriAgenda.TipoOperazioneAgenda !== Tipo_Attivita.Ricetta) {
      impostazione = this.PermessiUtenteService.getImpostazione_SuperUser(enum_Impostazioni_Utenti.SUPERUSER_COD_ALGORITMO_COSTI_ACCESSORI);
    ...
    this.Blocca_Soglia = this.getSettingValueAs(enum_Impostazioni_Utenti.UTENTE_COD_LIVELLO_CHK_DPI, false, value => value === "2", false);
    ...
    if (!Operazioni || Operazioni.length === 0) {
      impostazione = this.PermessiUtenteService.getImpostazione_Utente_poi_SuperUser(enum_Impostazioni_Utenti.UTENTE_OPERAZIONI_QDC_PREFERITE);
    ...
    this.Blocca_Raccolta_Carenza_non_Rispettata = this.getSettingValueAs(
      enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_RACCOLTA_CARENZA_NON_RISPETTATA, true, equalsOne, false
    );
    this.Chili_Litri = this.getSettingValueAs(enum_Impostazioni_Utenti.UTENTE_COD_CHILI_LITRI, false, equalsOne, false);
    //Utilizzo Magazzino --> valore: 0=no 1=si
    this.Utente_Cod_Default_Gestione_Magazzino = this.getSettingValueAs(
      enum_Impostazioni_Utenti.UTENTE_COD_DEFAULT_GESTIONE_MAGAZZINO, false, (value) => +value === -1, false
    );
    this.Utente_Cod_Blocca_se_Supera_Giacenze = this.getSettingValueAs(enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_SE_SUPERA_GIACENZE, false, equalsOne, false);
    this.Opzione_Raccolta = this.getSettingValueAs(enum_Impostazioni_Utenti.UTENTE_COD_RACCOLTA_TIPO_NUOVO_IMPIANTO, true, toNumber, this.Opzione_Raccolta);
    this.Opzione_Abbattimento = this.getSettingValueAs(enum_Impostazioni_Utenti.Utente_OpzioneChiusuraAbbattimenti, true, toNumber, this.Opzione_Abbattimento);
    this.Utente_Cod_Blocca_Raccolta_Carenza_non_Rispettata = this.getSettingValueAs(
      enum_Impostazioni_Utenti.UTENTE_COD_BLOCCA_RACCOLTA_CARENZA_NON_RISPETTATA, true, equalsOne, false
    );
  }

  private getSettingValueAs(
    settings: enum_Impostazioni_Utenti,
    scalarRead: boolean = true,
    mapper: (readVal: string) => any = (readVal) => readVal,
    defaultValue: any = ''
  ) {
    let impostazione: Utente_Impostazioni;
    if (scalarRead) {
      impostazione = this.PermessiUtenteService.getImpostazione_Utente_poi_SuperUser(settings);
    } else {
      impostazione = this.PermessiUtenteService.getImpostazione_Utente(settings);
    }
    if (impostazione !== undefined && impostazione !== null && impostazione.Valore !== '') {
    ...
    getImpostazioni_Magazzino_Lotto_Giacenze(Piva:string){
      ...
      valore_impostazione = this.impostazioniaziendecentriService.getValore_Impostazione_Scalare_Centro_Azienda_SuperUser_per_Elem_Cod(Piva, 0,
    ...
    getImpostazioni_QdC_Centro_Azienda() {

        let Piva = this.getImpresa_Model().partitaIva;

        let obj_Impostazioni_Magazzino_Lotto_Giacenze = this.getImpostazioni_Magazzino_Lotto_Giacenze(Piva);
        ...
        if (GridRows) {
            GridRows.forEach(row => {
                if (row.Selected) {
                    ...
                    this.ImpiantiSelezionatiFormArray.push(new FormControl(
                        new GridImpiantoSelezionatoModel(
                            row.PIVA,
                            row.SA_COD,
                            row.APPEZZA,
                            row.ID_REG,
    ...
    ripartizionaQtaTrattata(): any {
        ...
        const attr = this.funzionicomuniservice.roundNumber(supTrattata / supSelezionata,this.Obj_Default_Numeric_Settings.decimals);
        ...
        for (const [idx, dataItem] of filtered.entries()) {
            ...
            if (nSel - 1 !== idx) {
                daInserire = this.funzionicomuniservice.roundNumber((app * attr), this.Obj_Default_Numeric_Settings.decimals);
                somma += daInserire;
            } else {
                daInserire = (+supTrattata.toFixed(this.Obj_Default_Numeric_Settings.decimals) - +somma.toFixed(this.Obj_Default_Numeric_Settings.decimals)).toFixed(this.Obj_Default_Numeric_Settings.decimals);
            }

            dataItem.qta =  this.funzionicomuniservice.roundNumber(+daInserire, this.Obj_Default_Numeric_Settings.decimals);
    ...
    private GetImpiantoSelezionatoModel(row: GridImpiantoSelezionatoModel): Impianto {

        let centro = new CentroAziendale(new CentroAziendale.PK(row.SA_COD, row.PIVA));

        let appezzamentoPK = new Appezzamento.PK(row.APPEZZA, centro.primaryKey);
        ...
        let esercizio: Esercizio = this.GetEsercizioModel(row,centro);
    ...
    private GetEsercizioModel(row: GridImpiantoSelezionatoModel,centro_aziendale: CentroAziendale): Esercizio {

        let esercizio = new Esercizio(row.Progetto_Cod);
    ...
    GetEserciziCDCSelezionatiModel(): EsercizioCDC[] {
        ...
        if (gridImpiantiSelezionati && gridImpiantiSelezionati.length > 0) {
            ...
            gridImpiantiSelezionati.forEach((row: GridImpiantoSelezionatoModel) => {
                ...
                lstEserciziCDC.push(esercizioCDCNew);
            });
        }
                ...
                if (elem_cod > 0) {
                    if (this.Sezioni_ProdottoFormArray.value.findIndex(
                        ...
                        if (elem_cod === SEMENTI) {
                            ...
                            let Default_Opzione_Semina = this.get_Default_Opzione_Semina(+o.primaryKey.codice);

                            if (Operazioni_Scelte.length > 1 &&
                                Default_Opzione_Semina &&
                                (Default_Opzione_Semina.codice === enum_SEMINA_TIPO.Frazionamento_Appezzamenti_perLotto_e_Semina_sui_Singoli_Default ||
                                    Default_Opzione_Semina.codice === enum_SEMINA_TIPO.Semina_e_Modifica_Appezzamenti_Default)) {

                                Default_Opzione_Semina = Elenco_Opzioni_Semina.find(o => o.codice === enum_SEMINA_TIPO.Solo_Semina_Default);
                            }
   ...
   */
    ...
    get_Default_Opzione_Semina(lav_cod: number): BaseCodeDescr {
        ...
        if (this.objParametriAgenda.TipoOperazioneDB === Enum_DBTypeOperation.Write &&
            ...
            if (lav_cod === enum_LAVCOD.SOVESCIO) {

                Opzione_Semina = enum_SEMINA_TIPO.Solo_Semina_Default;

            } else {

                let impostazione = this.PermessiUtenteService.getImpostazione_Utente_poi_SuperUser(enum_Impostazioni_Utenti.UTENTE_COD_SEMINA_TIPO);

                if (impostazione && impostazione.Valore && impostazione.Valore !== "") {
                    const Tipo_Semina_Val = +impostazione.Valore;

                    switch (Tipo_Semina_Val) {
                        ...
                        case enum_SEMINA_TIPO.Solo_Semina_Vincolo:
                            ...
                            break;
                        default:
                            Opzione_Semina = Tipo_Semina_Val;
                            break;
                    }
                } else {
                    Opzione_Semina = enum_SEMINA_TIPO.Solo_Semina_Default;
    ...
    setCodDescrMultiColumnComboboxProdotto(elem_cod: number, multicolumncomboboxprodotto: MultiColumnComboboxTrattamento | MultiColumnComboboxFertilizzazione | MultiColumnComboboxSemina) {
        ...
        switch (elem_cod) {
            case FORMULATI:
            case INSETTI:
                multicolumncomboboxprodotto = this.setDescrizioneConcatenataMultiColumnComboboxTrattamento(multicolumncomboboxprodotto as MultiColumnComboboxTrattamento, elem_cod);
                break;
            case FERTILIZZANTI:
    ...
    private setCodMultiColumnComboboxProdotto(elem_cod: number,multicolumncomboboxprodotto: MultiColumnComboboxTrattamento | MultiColumnComboboxFertilizzazione | MultiColumnComboboxSemina) {
        ...
        if (multicolumncomboboxprodotto.MagazziniMovimentazioni && multicolumncomboboxprodotto.MagazziniMovimentazioni.length === 1) {

            if (multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.centroAziendalePK.partitaIva &&
                multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.centroAziendalePK.partitaIva !== "") {

                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}${multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.centroAziendalePK.partitaIva}`;

            } else {
                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}''`;
            }

            if (multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.centroAziendalePK.codice &&
                multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.centroAziendalePK.codice !== 0) {

                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}${multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.centroAziendalePK.codice}`;

            } else {
                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}0`;
            }

            if (multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.codice &&
                multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.codice !== 0) {

                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}${multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.primaryKey.codice}`;

            } else {
                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}0`;
            }

            if (multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Lotto &&
                multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Lotto !== "") {

                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}${multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Lotto}`;
            } else {
                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}''`;
            }

            if (multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Qta &&
                multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Qta !== 0) {

                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}${multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Qta}`;
            } else {
                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}0`;
            }

            if (multicolumncomboboxprodotto.MagazziniMovimentazioni[0].QtaTot &&
                multicolumncomboboxprodotto.MagazziniMovimentazioni[0].QtaTot !== 0) {

                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}${multicolumncomboboxprodotto.MagazziniMovimentazioni[0].QtaTot}`;
            } else {
                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}0`;
            }

            if (multicolumncomboboxprodotto.MagazziniMovimentazioni[0].udm &&
                multicolumncomboboxprodotto.MagazziniMovimentazioni[0].udm.codice !== 0) {

                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}${multicolumncomboboxprodotto.MagazziniMovimentazioni[0].udm.codice}`;
            } else {
                multicolumncomboboxprodotto.Codice_Concatenato += `${this.getCarattere_Separatore_Codice_Concatenato()}0`;
            }

        } else {
            multicolumncomboboxprodotto.Codice_Concatenato += `''${this.getCarattere_Separatore_Codice_Concatenato()}0${this.getCarattere_Separatore_Codice_Concatenato()}0${this.getCarattere_Separatore_Codice_Concatenato()}''${this.getCarattere_Separatore_Codice_Concatenato()}0${this.getCarattere_Separatore_Codice_Concatenato()}0${this.getCarattere_Separatore_Codice_Concatenato()}0`;
        }
    ...
    showInfoGiacenzaProdotto(multicolumncomboboxprodotto: MultiColumnComboboxTrattamento | MultiColumnComboboxFertilizzazione | MultiColumnComboboxSemina, Categoria_Magazzino: number) {

        let descr = "";

        if (multicolumncomboboxprodotto && multicolumncomboboxprodotto.MagazziniMovimentazioni && multicolumncomboboxprodotto.MagazziniMovimentazioni.length === 1) {

            let udm_simbolo = "";

            if(multicolumncomboboxprodotto.MagazziniMovimentazioni[0].udm){
                udm_simbolo = multicolumncomboboxprodotto.MagazziniMovimentazioni[0].udm.simbolo;
            }

            let magazzino_descr = "";

            if(multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino){
                magazzino_descr = multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Magazzino.descrizione;
            }

            let Qta = this.decimalpipe.transform(multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Qta,this.digitsInfo_QdC,this.locale_id);

            let QtaTot = this.decimalpipe.transform(multicolumncomboboxprodotto.MagazziniMovimentazioni[0].QtaTot,this.digitsInfo_QdC,this.locale_id);
            ...
            if(multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Lotto !== ""){
                descr += " --- [" + this.translocoService.translate("Lotto2")+": "+multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Lotto+"]";
            }

            if(multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Agenzia){
                descr += "<br> <strong> [" + this.translocoService.translate("MagazzinoAgenzia")+ ": "+multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Agenzia.descrizione+"] </strong>";
            }

            if(multicolumncomboboxprodotto.MagazziniMovimentazioni[0].registrazioniCollegate && multicolumncomboboxprodotto.MagazziniMovimentazioni[0].registrazioniCollegate.length > 0){

              const Magazzino_Esterno_con_Lotto = this.getMagazzino_Esterno_con_Lotto(multicolumncomboboxprodotto.MagazziniMovimentazioni);
              ...
              if(Magazzino_Esterno_con_Lotto){
                if(Magazzino_Esterno_con_Lotto.Magazzino_Esterno && Magazzino_Esterno_con_Lotto.Magazzino_Esterno.primaryKey.codice !== 0){
                  ...
                  if(Magazzino_Esterno_con_Lotto.Lotto !== ""){
                    descr += "<strong> --- [" + this.translocoService.translate("Lotto2")+": "+multicolumncomboboxprodotto.MagazziniMovimentazioni[0].Lotto+"] </strong>";
                  }
                }
              }
    ...
    private setDescrizioneConcatenataMultiColumnComboboxTrattamento(multicolumncomboboxtrattamento: MultiColumnComboboxTrattamento, Categoria_Magazzino: number) {
            ...
            if(multicolumncomboboxtrattamento.prodotto && multicolumncomboboxtrattamento.prodotto.codice > 0)
                ...
                if (multicolumncomboboxtrattamento.bufferzone && (multicolumncomboboxtrattamento.bufferzone.massimo > 0 || multicolumncomboboxtrattamento.bufferzone.minimo > 0)){
                    if(multicolumncomboboxtrattamento.bufferzone.minimo > 0 && multicolumncomboboxtrattamento.bufferzone.massimo === 0){
                        multicolumncomboboxtrattamento.Descrizione_Concatenata += " --- ["+this.translocoService.translate("Buffer_Zone")+" "+this.decimalpipe.transform(multicolumncomboboxtrattamento.bufferzone.minimo,this.digitsInfo_QdC,this.locale_id)+" "+this.translocoService.translate("MetriAbbr")+"]";
                    }else{
                        multicolumncomboboxtrattamento.Descrizione_Concatenata += " --- ["+this.translocoService.translate("Buffer_Zone")+" "+this.decimalpipe.transform(multicolumncomboboxtrattamento.bufferzone.minimo,this.digitsInfo_QdC,this.locale_id)+" - "+this.decimalpipe.transform(multicolumncomboboxtrattamento.bufferzone.massimo,this.digitsInfo_QdC,this.locale_id)+" "+this.translocoService.translate("MetriAbbr")+"]";
                    }
                }

                if (multicolumncomboboxtrattamento.tempoCarenza){
                    if(multicolumncomboboxtrattamento.tempoCarenza !== 0)
                        multicolumncomboboxtrattamento.Descrizione_Concatenata += " --- ["+this.translocoService.translate("CarenzaStr")+" "+this.decimalpipe.transform(multicolumncomboboxtrattamento.tempoCarenza,this.digitsInfo_QdC,this.locale_id)+" "+this.translocoService.translate("GiorniAbbr")+"]";

                    if (this.getCentroDiCostoTipo() != Tipo.ProdottoDaTrattare)
                        multicolumncomboboxtrattamento.Descrizione_Concatenata += " --- ["+this.translocoService.translate("qdc.BoundFieldResource16.HeaderText")+" "+this.getDataPrimaRaccoltaUtile(multicolumncomboboxtrattamento.tempoCarenza,this.TestataForm.get("Data").value)+"]";
                }
                ...
                //durataFeromone e scadenzaFeromone valorizzati solo nel lav_Cod CONFUSIONE_DISORIENTAMENTO_SESSUALE
    ...
    private setDescrizioneConcatenataMultiColumnComboboxFertilizzazione(multicolumncomboboxfertilizzazione: MultiColumnComboboxFertilizzazione) {
        ...
        if(multicolumncomboboxfertilizzazione.N !== null){
          multicolumncomboboxfertilizzazione.N_Str = this.decimalpipe.transform(multicolumncomboboxfertilizzazione.N,this.digitsInfo_QdC,this.locale_id);
        ...
        if(multicolumncomboboxfertilizzazione.P !== null){
          multicolumncomboboxfertilizzazione.P_Str = this.decimalpipe.transform(multicolumncomboboxfertilizzazione.P,this.digitsInfo_QdC,this.locale_id);
        ...
        if(multicolumncomboboxfertilizzazione.K !== null){
          multicolumncomboboxfertilizzazione.K_Str = this.decimalpipe.transform(multicolumncomboboxfertilizzazione.K,this.digitsInfo_QdC,this.locale_id);
        ...
        if(multicolumncomboboxfertilizzazione.Cu !== null){
          multicolumncomboboxfertilizzazione.Cu_Str = this.decimalpipe.transform(multicolumncomboboxfertilizzazione.Cu,this.digitsInfo_QdC,this.locale_id);
      ...
      if(multicolumncomboboxfertilizzazione.effluente && multicolumncomboboxfertilizzazione.effluente.carico > 0){
        ...
        multicolumncomboboxfertilizzazione.Dichiarato_nel_PUA_Str = this.decimalpipe.transform(multicolumncomboboxfertilizzazione.effluente.carico,this.digitsInfo_QdC,this.locale_id);
        ...
        multicolumncomboboxfertilizzazione.Dichiarato_nel_PUA_Str += " - " + this.decimalpipe.transform(multicolumncomboboxfertilizzazione.effluente.N,this.digitsInfo_QdC,this.locale_id) + " " + this.translocoService.translate('n');
    ...
    setCodDescrConcatenataMultiColumnComboboxMultiColumnComboboxDose_Etichetta(multicolumncomboboxdose_etichetta: MultiColumnComboboxDose_Etichetta) {
        ...
        if(multicolumncomboboxdose_etichetta.codice === 0){
            ...
            multicolumncomboboxdose_etichetta.Descrizione_Concatenata = this.decimalpipe.transform(multicolumncomboboxdose_etichetta.DoseMin,this.digitsInfo_QdC,this.locale_id) + "-" + this.decimalpipe.transform(multicolumncomboboxdose_etichetta.DoseMax,this.digitsInfo_QdC,this.locale_id);
            ...
            if(multicolumncomboboxdose_etichetta.UdmAcqua && multicolumncomboboxdose_etichetta.AcquaMin && multicolumncomboboxdose_etichetta.AcquaMax){
                ...
                if (AcquaMin !== 0 || AcquaMax !== 0)
                    multicolumncomboboxdose_etichetta.Descrizione_Concatenata += " (Vol.Acqua " + this.decimalpipe.transform(AcquaMin,this.digitsInfo_QdC,this.locale_id) + "-" + this.decimalpipe.transform(AcquaMax,this.digitsInfo_QdC,this.locale_id) + " " + AcquaUdm_Sim + ")";
            }
    ...
    setCodDescrDdlAvversita(ddlavversita: DropdownListAvversita, Lav_Cod: number){
        ...
        return new Promise<DropdownListDisciplinare>(async (resolve, reject) => {
            ...
            if (Veg_Cod > 0) {
                ...
                if (impianti_selezionati && impianti_selezionati.length > 0) {
                                ...
                                for (var d of Array_Disciplinari) {
                                    ...
                                    for (var impianto of impianti_filtrati) {
                                        ...
                                        if (d.codice === impianto.Obj_Disciplinare.codice &&
                                            ...
                                            this.Controlla_GruppoFinalita_Disciplinari(d,impianto.Obj_Disciplinare)) {
                    ...
                    //Se gli Impianti non hanno un disciplinare prendo quello da DpiPredefinitoUtente
                    if (!disciplinare) {
        ...
        if (Impianti_Selezionati.length > 1) {
            ...
            if (Operazione_con_Sementi) {
                ...
                if (!Opzione_Semina &&
                    ...
                    Opzione_Semina = this.get_Default_Opzione_Semina(+Operazione_con_Sementi.primaryKey.codice);
    ...
    gestisci_ErroriGias(listErroriGias: ErroreGias[], cerca_errori_bloccanti: boolean, mostra_errori_bloccanti: boolean,key_msg_conferma: string = "Sidesideraproseguire",dialog_type: number = null,
                        maxHeight:string = "auto",maxWidth:string = "auto"): Promise<Obj_Errore_Gias_QdC> {
        ...
        return new Promise<Obj_Errore_Gias_QdC>((resolve, reject) => {
            ...
            if (list_warning_bloccanti && list_warning_bloccanti.length > 0) {
                ...
                if (list_warning && list_warning.length > 0) {
                    ...
                    let Warning_Action: Array<Actions> = [
    ...
    Imposta_VolumiAcqua(AcquaMax: number, AcquaUdm: UnitaDiMisura, Dose_Desc: string): Promise<boolean> {
        ...
        return new Promise<boolean>(async (resolve, reject) => {
            ...
            if (AcquaMax !== 0) {
                    ...
                    if (obj.perHa_hl === enum_UnitaMisura.Ettaro) {
                        ...
                        if ((AcquaTaraturaHL > 0 && AcquaTaraturaHL > AcquaMaxHL) || AcquaTaraturaHL === 0) {
                            ...
                            if (Acqua_Ha > 0) {
                                ...
                                let AcquaMaxHL_arrotondata = this.funzionicomuniservice.roundNumber(AcquaMaxHL, this.Obj_Default_Numeric_Settings.decimals);
                                ...
                                if (imposta_volumi) {
                                    ...
                                    if (Sup_Trattata > 0) {
                                        this.AcquaForm.patchValue({
                                            Acqua_Tot: this.funzionicomuniservice.roundNumber(AcquaMaxHL * Sup_Trattata, this.Obj_Default_Numeric_Settings.decimals)
                                        });
                                    }
                                }
                            } else {
                                ...
                                this.AcquaForm.patchValue({
                                    Acqua_Ha: this.funzionicomuniservice.roundNumber(AcquaMaxHL, this.Obj_Default_Numeric_Settings.decimals)
                                });
                ...
                let AcquaTaraturaHL_arrotondata = this.funzionicomuniservice.roundNumber(AcquaTaraturaHL, this.Obj_Default_Numeric_Settings.decimals);
                ...
                if (imposta_volumi) {
                    ...
                    if (Sup_Trattata > 0) {
                        this.AcquaForm.patchValue({
                            Acqua_Tot: this.funzionicomuniservice.roundNumber(AcquaTaraturaHL * Sup_Trattata, this.Obj_Default_Numeric_Settings.decimals)
                        });
                    }
                }
            ...
            this.Subs.add(this.gisClient.gisVerificaEsistenzaEntitaPerImpianti(payload).subscribe(resp=>{
                ...
                if(resp.RispostaOK && resp.RispostaStringa && resp.RispostaStringa.NumeroEntita > 0){
                    ...
                    if (latLngCentroAziendale.isValid) {
                        ...
                        objCentraMappa.punto = Object.assign({},latLngCentroAziendale)
    ...
    AggiornaFormArrayGridDosiProdotti(dataItem: any, index: number, Operazione: Lavorazione, tipo_operazione_db: enum_TipoOperazioneDB,DosiProdottiFormArray: FormArray,emitEvent:boolean = true) {
        ...
        if(Operazione){
            ...
            if(tipo_operazione_db === enum_TipoOperazioneDB.Scrittura){
                ...
                const gridDosiProdottiValidator = new GridDosiProdottiValidator(elem_cod,this);
        ...
        if(Operazioni_Scelte && Operazioni_Scelte.length > 0){
            ...
            if((Operazioni_Scelte.findIndex(o=>this.Elenco_Operazioni_Non_Utilizzo.includes(+ o.primaryKey.codice)) >-1 &&
                ...
                if (this.getCentroDiCostoTipo() != Tipo.ProdottoDaTrattare) {
                    ...
                    this.QdCForm.get("Trattamento").get("Superfici").get("Sup_Trattata").setValidators([Validators.required, Validators.min(0.0001)]);
                } else {
    ...
    ControllaSuperficieTrattabileXPercentualeAbbattimento(dataItem:any):ErroreGias[] {
        ...
        if(percAbb > 0 && percAbb < 100 && dataItem.Selected){
            ...
            let supTrattabile = this.funzionicomuniservice.roundNumber(supImpianto * percAbb / 100 , this.Obj_Default_Numeric_Settings.decimals);
    ...
    MostraBtnSendDataToAPP(): boolean {
        return (
            ...
            this.TestataForm.get('Operazioni').getRawValue().find(op => + op.primaryKey.codice === enum_LAVCOD.DISTRIBUZIONE_INSETTI) == undefined
        );
    }
