...
export class GridFiltroRicercaHttpService extends AbstractGridConfigService<GridFiltroRicercaServerResult> implements OnDestroy {
    ...
    constructor(injector: Injector,
        ...
        this.applyViewFiltroJSONObs = this.gridPublicService.applyViewFiltroJSON.pipe(skip(1)).subscribe((view) => {
            this.filtroRicercaFormService.onLoad = (view.FiltroJSON == '');

            let filtroJSON = this.conversionService.ConversionDateInObject(JSON.parse((view.FiltroJSON !== '') ? view.FiltroJSON : JSON.stringify(this.filtroRicercaFormService.InitializeFormFiltroRicerca({ CategoriaEsito: this.filtroRicercaFormService.formFilters.get("CategoriaEsito").value }))));
