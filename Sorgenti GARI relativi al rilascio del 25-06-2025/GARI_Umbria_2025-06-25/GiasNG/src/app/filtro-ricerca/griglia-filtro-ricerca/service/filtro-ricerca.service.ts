...
import { Enum_ColonnaData_FiltroRicerca, Enum_Entita_FiltroRicerca, Enum_FiltroDestinazioneUso_FiltroRicerca, Enum_FiltroOperatoreLogico_FiltroRicerca, Enum_FiltroOperazioniSelezionate_FiltroRicerca, Enum_FiltroPoligoni_FiltroRicerca, Enum_FiltroRipartoCatasto_FiltroRicerca, Enum_TipoConfronto_FiltroRicerca, Enum_TipoMostra_FiltroRicerca, enum_Security_Attivita } from "app/Model/TipiEnumerativi";
...
export class FiltroRicercaService implements OnDestroy {
    ...
    constructor(private fb: FormBuilder,
        ...
        //this.specieObs = this.formFieldsService.metaschemaGetSpecieFiltroUtente(paramSpecie);

        //this.waitResultBeforeFillParametriFiltroRicerca();
    ...
    waitResultBeforeFillParametriFiltroRicerca() {
        ...
        if (/*this.ListaDestinazioniDuso.length == 0 || this.ListaGruppiVegetali.length == 0 || this.ListaUtilizzoTerreno.length == 0 ||*/ this.ListaSpecieCompleta.length == 0) {
    ...
    setFormFieldsForChiamante(paramFiltro) {

        // this.paramFiltroRicercaNG = paramFiltro;
        ...
        //    this.formFilters.get("FiltriAziende").get("Piva").patchValue(paramFiltro.Piva);
        }
        ...
        this.formFilters.patchValue(formObj, { emitEvent: false });
    }
    ...
    InitializeFormFiltroRicerca(vals?: Partial<FormFiltroRicercaConfig>): FormFiltroRicercaConfig {
        return {
            ...
            FiltriPianoColturale: {
                FiltroDestinazioneUso: vals?.FiltriPianoColturale?.FiltroDestinazioneUso ?? (vals?.CategoriaEsito == this.ListeCategorieEsito.find(item => item.idCat == Enum_TipoMostra_FiltroRicerca.PianoColturale) || 
                                                                                             vals?.CategoriaEsito == this.ListeCategorieEsito.find(item => item.idCat == Enum_TipoMostra_FiltroRicerca.PianoColturaleBudget) ||
                                                                                             vals?.CategoriaEsito == this.ListeCategorieEsito.find(item => item.idCat == Enum_TipoMostra_FiltroRicerca.Esercizi) ||
                                                                                             vals?.CategoriaEsito == this.ListeCategorieEsito.find(item => item.idCat == Enum_TipoMostra_FiltroRicerca.Appezzamenti) ||
                                                                                             vals?.CategoriaEsito == this.ListeCategorieEsito.find(item => item.idCat == Enum_TipoMostra_FiltroRicerca.Impianti))? this.ListaFiltroDestinazioniDuso.find(item => item.id == Enum_FiltroDestinazioneUso_FiltroRicerca.EscludiDestinazioniUso) : this.ListaFiltroDestinazioniDuso.find(item => item.id == Enum_FiltroDestinazioneUso_FiltroRicerca.Tutto),
