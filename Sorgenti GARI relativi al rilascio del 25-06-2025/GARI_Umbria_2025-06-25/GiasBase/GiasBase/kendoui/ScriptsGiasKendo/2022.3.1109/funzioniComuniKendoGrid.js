...
function creaKendoGrid(
    ...
    let filterExcludeButton = ":not(.k-grid-cancel-changes):not(.k-grid-add):not(.k-grid-excel):not(.k-grid-Info):not(.k-grid-Modifica):not(.k-grid-Bdn):not(.k-grid-pdf):not(.btnSalvaPersonalizzazioniGriglia):not(.btnCancellaPersonalizzazioniGrigliaKendo):not(.k-grid-Cancella)";
...
function ExportExcelServer(e) {
    ...
    try {
            ...
            let param = kendo.stringify(
                {
                    fileName: fileName,
                    chunk: i + 1,
                    data: kendoEscapeOggetto(righeChunked[i])
                });

            ajaxAgronicaSync(urlBaseCoreWS + indirizzohttp_GridServerExport + "/StoreDataChunk",
                param,
                false,
                function (risposta) {
                },
        ...
        ajaxAgronicaSync(urlBaseCoreWS + indirizzohttp_GridServerExport + "/ExportToExcel",
            param,
            false,
            function (risposta) {
